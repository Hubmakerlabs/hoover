...
import (
	...
	"github.com/Hubmakerlabs/hoover/cmd/blueskyhoover/diskutil"
				...
				if err != nil {
					...
					for _, op := range evt.Ops {
						...
						case repomgr.EvtKindCreateRecord, repomgr.EvtKindUpdateRecord:
							...
							if err != nil {
								e := fmt.Errorf("getting record %s (%s) within seq %d for %s: %w", op.Path, *op.Cid,
									evt.Seq, evt.Repo, err)
								log.Error(e)
								return nil
							}
							...
							if cctx.Bool("authed") {
								...
								if pst.Reply != nil {
									replyUserProfile, err = appbsky.ActorGetProfile(context.TODO(), xrpcc,
										strings.Split(pst.Reply.Parent.Uri, "/")[2])
									if err != nil {
										fmt.Println(err)
									}
								}
							...
							if pst.LexiconTypeID == "app.bsky.feed.post" {
								...
								_, rec, err := rr.GetRecord(ctx,
									like.Subject.Uri[strings.LastIndex(like.Subject.Uri[:strings.LastIndex(like.Subject.Uri,
										"/")], "/")+1:])
								if err != nil {
									log.Error(err)
									return nil
								...
								PrintPost(cctx, pst, likedUserProfile, nil, userProfile,
									like.Subject.Uri[strings.LastIndex(like.Subject.Uri, "/")+1:])
...
func PrintPost(cctx *cli.Context, pst appbsky.FeedPost,

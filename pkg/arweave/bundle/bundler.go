...
import (
	...
	irysResponses "github.com/Hubmakerlabs/hoover/pkg/arweave/utils/bundlr/responses"
	...
	"github.com/Hubmakerlabs/hoover/pkg/arweave/utils/tool"
	"github.com/Hubmakerlabs/hoover/pkg/arweave/utils/turbo"
	turboResponses "github.com/Hubmakerlabs/hoover/pkg/arweave/utils/turbo/responses"
...
func NewBundler(config *config.Config, db *gorm.DB) (self *Bundler) {
	...
	self.Task = task.NewTask(config, "bundler").
		...
		WithWorkerPool(config.Bundler.BundlerNumBundlingWorkers,
			config.Bundler.WorkerPoolQueueSize).
...
func (self *Bundler) upload(dataItem *model.BundleItem,
	item *bundlr.BundleItem) (response []byte, resp *resty.Response, id string,
	err error) {
	...
	case model.BundlingServiceTurbo:
		...
		if len(uploadResponse.Id) == 0 {
			...
			self.Log.WithError(err).WithField("id",
				dataItem.InteractionID).Error("Bad Turbo response")
	...
	case model.BundlingServiceIrys:
		...
		if len(uploadResponse.Id) == 0 {
			...
			self.Log.WithError(err).WithField("id",
				dataItem.InteractionID).Error("Bad Irys response")
	...
	default:
		...
		self.Log.WithError(err).WithField("service",
			dataItem.Service).Error("Unknown bundling service")
	}
	return
}
		...
		self.SubmitToWorker(func() {
			...
			if err != nil {
				...
				if resp != nil && resp.StatusCode() > 399 && resp.StatusCode() < 500 {
					...
					if err != nil {
						self.Log.WithError(err).WithField("id",
							item.InteractionID).Warn("Failed to update bundle item state")
					}
				}
			...
			if len(id) == 0 {
				...
				self.Log.WithError(err).WithField("id",
					item.InteractionID).Warn("Bad bundlr response")
				self.monitor.GetReport().Bundler.Errors.BundrlError.Inc()
				return
			}
			...
			case self.Output <- &Confirmation{
				InteractionID: item.InteractionID,
				BundlerTxID:   id,
				Response: pgtype.JSONB{Bytes: uploadResponse,
...
func (self *Bundler) createDataItem(item *model.BundleItem) (bundleItem *bundlr.BundleItem,
	err error) {
	...
	if n != 32 {
		self.Log.WithError(err).WithField("id",
			item.InteractionID).Warn("Failed to generate anchor, will retry later.")
		return
	}
	if err != nil {
		self.Log.WithError(err).WithField("id",
			item.InteractionID).Warn("Error when generating anchor, will retry later.")
		return
	}
...
func (self *Bundler) createBundle(item *model.BundleItem) (bundleItem *bundlr.BundleItem,
	err error) {
	...
	if err != nil {
		self.Log.WithError(err).WithField("id",
			item.InteractionID).Error("Failed to get interaction data")
		return
	}
...
func (self *Bundler) createNestedBundle(item *model.BundleItem) (bundleItem *bundlr.BundleItem,
	err error) {
	// Parse bundle item
	...
	if err != nil {
		self.Log.WithError(err).WithField("id",
			item.InteractionID).Error("Failed to unmarshal nested bundle item")
		return
	}
	...
	if err != nil {
		self.Log.WithError(err).WithField("id",
			item.InteractionID).Error("Failed to nest bundle")
		return
	}
...
func (self *Bundler) getTags(item *model.BundleItem) (tags bundlr.Tags,
	err error) {
	tags = make(bundlr.Tags, 0, 10)

	tagBytes, err := item.Tags.MarshalJSON()
	if err != nil {
		self.Log.WithError(err).WithField("len", len(tagBytes)).WithField("id",
			item.InteractionID).Error("Failed to get transaction tags")
		return
	}
	...
	if err != nil {
		self.Log.WithError(err).WithField("len", len(tagBytes)).WithField("id",
			item.InteractionID).Error("Failed to unmarshal transaction tags")
		return
	}

...
func NewConfirmer(config *config.Config) (self *Confirmer) {
	...
	self.SinkTask = task.NewSinkTask[*Confirmation](config, "confirmer").
		...
		WithBackoff(config.Bundler.ConfirmerBackoffMaxElapsedTime,
			config.Bundler.ConfirmerBackoffMaxInterval)
...
func (self *Confirmer) save(confirmations []*Confirmation) error {
	...
	self.Log.WithField("len",
		len(confirmations)).Trace("Saving confirmations to DB")
		...
		for _, confirmation := range confirmations {
			err := tx.Table(model.TableBundleItem).
				...
				Updates(model.BundleItem{
					...
					BlockHeight: sql.NullInt64{Int64: currentBlockHeight,
						Valid: true},

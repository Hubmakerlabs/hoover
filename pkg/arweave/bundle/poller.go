...
func NewPoller(config *config.Config) (self *Poller) {
	...
	self.Task = task.NewTask(config, "poller").
		WithRepeatedSubtaskFunc(config.Bundler.PollerInterval,
			self.handleNewTransactions).
		WithRepeatedSubtaskFunc(config.Bundler.PollerInterval,
			self.handleRetrying)
...
func (self *Poller) handleNewTransactions() (repeat bool, err error) {
	ctx, cancel := context.WithTimeout(self.Ctx,
		self.Config.Bundler.PollerTimeout)
	defer cancel()
	...
	if len(bundleItems) > 0 {
		self.Log.WithField("count",
			len(bundleItems)).Debug("Polled new bundle items")
	}
...
func (self *Poller) handleRetrying() (repeat bool, err error) {
	...
	WHERE interaction_id IN (
		...
		LIMIT ?
		FOR UPDATE SKIP LOCKED)
	RETURNING *`, fmt.Sprintf("%d seconds",
			int(self.Config.Bundler.PollerRetryBundleAfter.Seconds())),
			self.Config.Bundler.PollerMaxBatchSize).
		Scan(&bundleItems).
		Error
	...
	if len(bundleItems) > 0 {
		self.Log.WithField("count",
			len(bundleItems)).Trace("Polled bundle items for retrying")
	}

...
func (s *EthRPCTestSuite) registerResponse(result string,
	callback func([]byte)) {
	httpmock.Reset()
	response := fmt.Sprintf(`{"jsonrpc":"2.0", "id":1, "result": %s}`, result)
	httpmock.RegisterResponder("POST", s.rpc.url,
...
func (s *EthRPCTestSuite) SetupSuite() {
	s.rpc = NewEthRPC("http://127.0.0.1:8545",
		WithHttpClient(http.DefaultClient), WithLogger(nil), WithDebug(false))
...
func (s *EthRPCTestSuite) TestEthCoinbase() {
	s.registerResponse(`"0x407d73d8a49eeb85d32cf465507dd71d507100c1"`,
		func(body []byte) {
...
func (s *EthRPCTestSuite) TestEthAccounts() {
	...
	s.Require().Nil(err)
	s.Require().Equal([]string{"0x407d73d8a49eeb85d32cf465507dd71d507100c1"},
		accounts)
}
...
func (s *EthRPCTestSuite) TestEthGetStorageAt() {
	...
	s.registerResponse(`"0x00000000000000000000000000000000000000000000000000000000000004d2"`,
		func(body []byte) {
	...
	s.Require().Nil(err)
	s.Require().Equal("0x00000000000000000000000000000000000000000000000000000000000004d2",
		result)
}
...
func (s *EthRPCTestSuite) TestGetBlock() {
	...
	s.Require().Equal("0x913f938dcb4ff83b2b6b42a0cf6517d438a3ce95174e9342c780fd20c84dfd03",
		block.ParentHash)
	s.Require().Equal("0xefd7ef000d0b78b8", block.Nonce)
	s.Require().Equal("0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
		block.Sha3Uncles)
	s.Require().Equal("0x111", block.LogsBloom)
	s.Require().Equal("0x97849642410701c38f904912238eb78d3aa854e72c5ae39394c7217f4f9474bc",
		block.TransactionsRoot)
	s.Require().Equal("0xab9287d3b8864338892d1d572198933979e39bfcfbde569ea52be15a9691b4c1",
		block.StateRoot)
	...
	s.Require().Equal("0x706f6f6c2e65746866616e732e6f726720284d4e323729",
		block.ExtraData)
	...
	s.Require().Equal([]string{"0xf14cdb8a75de31dcf3da7a3a52c1fffcbaa3d56de9f50f86767fa411c10f4397"},
		block.Uncles)
	...
	s.Require().Equal("0xbc3e37984a619008d75e7f73865247fb420ae5ed2c921599d099ab5f20519396",
		block.ParentHash)
	s.Require().Equal("0x19a48ee424b5088f", block.Nonce)
	s.Require().Equal("0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
		block.Sha3Uncles)
	s.Require().Equal("0x222", block.LogsBloom)
	s.Require().Equal("0x1bcd58c2420d63c5e8ed3182afd33c01737be38a4a8c10a81dfb70b692e8f286",
		block.TransactionsRoot)
	s.Require().Equal("0xbe7e86ee05a5d49ba64b3d9f3f0129bab90308032e42307a1a2ef5c8971c5f5c",
		block.StateRoot)
	...
	s.Require().Equal("0xd58301050b8650617269747986312e31352e31826c69",
		block.ExtraData)
...
func (s *EthRPCTestSuite) TestEthGetTransactionReceipt() {
	...
	s.registerResponse(result, func(body []byte) {
		...
		s.paramsEqual(body,
			`["0x9c17afa5336d3cfd47e2e795520959b92e627e123e538fd4d5d7ece9025a8dce"]`)
	})
	...
	s.Require().Equal("0x11537af16aec572bb72d6d52e2c801dbfc10f42ab6ea849fd8e31b57d7099eea",
		receipt.BlockHash)
	...
	s.Require().Equal("0x55b68780caee96e686eb398371bb679574d4b995614ae94243da4886059a47ee",
		receipt.Root)
...
func (s *EthRPCTestSuite) TestGetTransaction() {
	...
	s.Require().Equal("0x3068bb24a6c65a80eb350b89b2ef2f4d0605f59e5d07fd3467eb76511c4408e7",
		transaction.Hash)
	s.Require().Equal(168, transaction.Nonce)
	s.Require().Equal("0x8b0404b2e5173e7abdbfc98f521d50808486ccaff3cd0a6344e0bb6c7aa8cef0",
		transaction.BlockHash)
	s.Require().Equal(4262381, *transaction.BlockNumber)
	s.Require().Equal(152, *transaction.TransactionIndex)
	s.Require().Equal("0xe3a7ca9d2306b0dc900ea618648bed9ec6cb1106",
		transaction.From)
	s.Require().Equal("0x8d12a197cb00d4747a1fe03395095ce2a5cc6819",
		transaction.To)
	...
	s.Require().Equal("0x32c9f5da77aaacb83fbf3ce61c08f49668add324195e8cba3c9490508e39a3b4",
		transaction.R)
	s.Require().Equal("0x43052bcfef39d0012daccf8d93876e89269aefce78c5dad47c633e3297da153e",
		transaction.S)
...
func (s *EthRPCTestSuite) TestEthNewFilterWithTopics() {
	...
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "eth_newFilter")
		s.paramsEqual(body,
			fmt.Sprintf(`[{"topics": [["%s", "%s"]]}]`, topics[0][0],
				topics[0][1]))
	})
...
func (s *EthRPCTestSuite) TestEthNewFilterWithAddressAndTopics() {
	...
	s.registerResponse(fmt.Sprintf(`"%s"`, result), func(body []byte) {
		s.methodEqual(body, "eth_newFilter")
		s.paramsEqual(body,
			fmt.Sprintf(`[{"address": ["%s"], "topics": [["%s"], ["%s"]]}]`,
				address[0], topics[0][0], topics[1][0]))
	})

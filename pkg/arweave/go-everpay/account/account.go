...
func (a *Account) VerifySig(tx Transaction) (err error) {
	...
	case AccountTypeEVM:
		...
		if err != nil {
			return fmt.Errorf("ecrecover failed, hash:%s, sig:%s, err:%v",
				hexutil.Encode(tx.Hash), tx.Sig, err)
		}

		if addr.String() != a.ID {
			return fmt.Errorf("address not equal, ecAddr:%s, accid:%s",
				addr.String(), a.ID)
		}
		return nil
	...
	case AccountTypeAR:
		...
		if addr != a.ID {
			return fmt.Errorf("address not equal, ownerAddr:%s, accid:%s", addr,
				a.ID)
		}

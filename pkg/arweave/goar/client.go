...
func (c *Client) GetTransactionByID(id string) (tx *types.Transaction,
	err error) {
...
func (c *Client) GetTransactionData(id string, extension ...string) ([]byte,
	error) {
...
func (c *Client) GetTransactionDataStream(id string,
	extension ...string) (*os.File, error) {
	urlPath := fmt.Sprintf("tx/%v/%v", id, "data")
	if extension != nil {
		urlPath = urlPath + "." + extension[0]
	}
...
func (c *Client) GetTransactionDataByGateway(id string) (body []byte,
	err error) {
...
func (c *Client) GetTransactionDataStreamByGateway(id string) (*os.File,
	error) {
	urlPath := fmt.Sprintf("/%v/%v", id, "data")
	body, statusCode, err := c.httpGet(urlPath)
	if err != nil {
...
func (c *Client) GetTransactionPrice(dataSize int,
	target *string) (reward int64, err error) {
...
func (c *Client) SubmitTransaction(tx *types.Transaction) (status string,
	code int, err error) {
...
func (c *Client) SubmitChunks(gc *types.GetChunk) (status string, code int,
	err error) {
...
// Wallet
func (c *Client) GetWalletBalance(address string) (arAmount *big.Float,
	err error) {
...
func (c *Client) GetWalletWinstonBalance(address string) (arAmount *big.Int,
	err error) {
...
func (c *Client) GetBlockByHeight(height int64) (block *types.Block,
	err error) {
...
func (c *Client) httpGet(_path string) (body []byte, statusCode int,
	err error) {
...
func (c *Client) httpPost(_path string, payload []byte) (body []byte,
	...
	resp, err := c.client.Post(u.String(), "application/json",
...
func (c *Client) getTransactionOffset(id string) (*types.TransactionOffset,
	error) {
...
func (c *Client) ConcurrentDownloadChunkData(id string,
	concurrentNum int) ([]byte, error) {
	offsetResponse, err := c.getTransactionOffset(id)
	if err != nil {
		return nil, err
	}
	...
	p, _ := ants.NewPoolWithFunc(concurrentNum, func(i interface{}) {
		...
		if err != nil {
			...
			for count < 2 {
				...
				log.Error("retry getChunkData failed and try again...", "err",
					err, "idx", oss.Idx, "offset", oss.Offset, "retryCount",
					count, "arId", id)
	...
	for i := 0; int64(i)+start < endOffset; {
		...
		if err != nil {
			...
			for count < 2 {
				...
				log.Error("latest two chunks retry getChunkData failed and try again...",
					"err", err, "offset", int64(i)+start, "retryCount", count,
					"arId", id)
				if err != ErrRequestLimit {
					count++
				}
...
func (c *Client) ConcurrentDownloadChunkDataStream(id string,
	concurrentNum int) (dataFile *os.File, err error) {
	offsetResponse, err := c.getTransactionOffset(id)
	if err != nil {
		return nil, err
	}
	...
	p, _ := ants.NewPoolWithFunc(concurrentNum, func(i interface{}) {
		...
		if err != nil {
			...
			for count < 5 {
				...
				log.Warn("retry getChunkData failed and try again...", "err",
					err, "idx", oss.fileOffset/types.MAX_CHUNK_SIZE, "offset",
					oss.chunkOffset, "retryCount", count, "arId", id)
				if err != ErrRequestLimit {
					count++
				}
			}
		...
		if err != nil {
			log.Error("getChunkData failed", "err", err, "arId", id, "idx",
				oss.fileOffset/types.MAX_CHUNK_SIZE, "offset", oss.chunkOffset)
			return
		}
		var n int
		lock.Lock()
	...
	for i, offset := range offsetArr[:len(offsetArr)-2] {
		wg.Add(1)
		if err = p.Invoke(Offset{fileOffset: offset,
			chunkOffset: offset + startOffset}); err != nil {
			log.Error("p.Invoke(i)", "err", err, "i", i)
			return
		}
	}
	...
	for i := 0; int64(i)+start < endOffset; {
		...
		if err != nil {
			err = errors.New(fmt.Sprintf("concurrent get latest two chunks failed,err:%v",
				err))
			return
		}
		n := 0
		n, err = dataFile.Write(chunkData)
		if err != nil || n < len(chunkData) {
			err = fmt.Errorf("write dataFile error writeSize:%d, expectSize:%d",
				n, len(chunkData))
			return
		}
		i += len(chunkData)
...
func (c *Client) DataSyncRecord(endOffset string, intervalsNum int) ([]string,
	error) {
...
func (c *Client) GetBundleItems(bundleInId string,
	itemsIds []string) (items []*types.BundleItem, err error) {

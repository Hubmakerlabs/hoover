...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/arweave/everpay-go/sdk"
...
func TestBundleToArweave4(t *testing.T) {
	...
	data := []byte("aabbcc")
	item01, err := itemSigner01.CreateAndSignItemStream(bytes.NewReader(data),
		"", "", []types.Tag{
...
func TestBundleToArweave(t *testing.T) {
	...
	itemSigner01, err := goar.NewItemSigner(signer01)
	assert.NoError(t, err)
	item01, err := itemSigner01.CreateAndSignItem([]byte("aa bb cc"), "", "",
		[]types.Tag{
			{Name: "Content-Type", Value: "application/txt"},
			{Name: "App-Version", Value: "2.0.0"},
		})
	...
	assert.NoError(t, err)
	item02, err := itemSigner02.CreateAndSignItem([]byte("dd ee ff"), "", "",
		[]types.Tag{
			{Name: "Content-Type", Value: "application/txt"},
		})
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	tx, err := wal.SendBundleTx(context.TODO(), 0, bundle.BundleBinary,
		[]types.Tag{
			{Name: "App", Value: "goar"},
		})
	assert.NoError(t, err)
...
func TestNestedBundleToArweave(t *testing.T) {
	...
	nestedItem, err := itemSigner01.CreateAndSignNestedItem("", "",
		[]types.Tag{}, item01, item02)
	assert.NoError(t, err)
	item03, err := itemSigner02.CreateAndSignItem([]byte("gg hh ii"), "", "",
		[]types.Tag{
			{Name: "Content-Type", Value: "application/txt"},
		})
	assert.NoError(t, err)
...
func TestBundleItemToArseeding(t *testing.T) {
	...
	resp, err := utils.SubmitItemToArSeed(item01, "USDC",
		"https://seed.everpay.io")
	assert.NoError(t, err)
...
func TestBundleItemToBundlr(t *testing.T) {
	...
	resp, err := utils.SubmitItemToBundlr(item01,

...
func CreateTcKeyPair(bitSize, k, l int) (shares tcrsa.KeyShareList,
	meta *tcrsa.KeyMeta, err error) {
	if bitSize > 4096 || bitSize < 512 {
		return nil, nil, errors.New(fmt.Sprintf("bitSize:%d parameter must in [512,4096]",
			bitSize))
	}
	if k <= 0 || l <= 1 || l < k || k < (l/2+1) {
		return nil, nil, errors.New(fmt.Sprintf("k: %d l: %d parameter incorrect; k must > 0, l must > 1, l must >= k, k must >= (l/2+1)",
			k, l))
	}
	...
	if err != nil {
		log.Error("tcrsa newKey", "err", err, "bitSize", bitSize, "k", k, "l",
			l)
		return nil, nil, err
	}
	log.Debug("Create rsa threshold keyPair success", "bitSize", bitSize,
		"spendTime", time.Since(now).String())
...
func NewTcSign(meta *tcrsa.KeyMeta, signData []byte, salt []byte) (*TcSign,
	error) {
	signHashed := sha256.Sum256(signData)

	signDataByPss, err := tcrsa.PreparePssDocumentHash(meta.PublicKey.N.BitLen(),
		signHashed[:], salt, &rsa.PSSOptions{
			SaltLength: 0,
			Hash:       crypto.SHA256,
		})
	if err != nil {
...
func (ts *TcSign) ThresholdSign(signShare *tcrsa.KeyShare) (*tcrsa.SigShare,
	error) {
...
// for server hub
// AssembleSigShares
func (ts *TcSign) AssembleSigShares(signedShares tcrsa.SigShareList) ([]byte,
	error) {
	...
	if err := rsa.VerifyPSS(ts.keyMeta.PublicKey, crypto.SHA256, signHashed[:],

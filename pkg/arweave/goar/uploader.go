...
func newUploader(tt *types.Transaction, client *Client) (*TransactionUploader,
	error) {
	if tt.ID == "" {
...
func CreateUploader(api *Client, upload interface{},
	data []byte) (*TransactionUploader, error) {
	var (
	...
	if id, ok := upload.(string); ok {
		...
		if err != nil {
			log.Error("(&TransactionUploader{Client: api}).FromTransactionId(id)",
				"err", err)
			return nil, err
		}
	} else {
	...
	uploader, err = (&TransactionUploader{Client: api}).FromSerialized(upload.(*SerializedUploader),
		data)
	return uploader, err
}
...
func (tt *TransactionUploader) ConcurrentOnce(ctx context.Context,
	concurrentNum int) error {
	// post tx info
	...
	p, _ := ants.NewPoolWithFunc(concurrentNum, func(i interface{}) {
		...
		var err error
		if tt.DataReader != nil {
			chunk, err = utils.GetChunkStream(*tt.Transaction, idx,
				tt.DataReader)
		} else {
		...
		log.Error("concurrent submitChunk failed", "chunkIdx", idx,
			"statusCode", statusCode, "gatewayErr", body, "httpErr", err)
		// try again
		retryCount := 0
		for {
			select {
			...
			log.Warn("retry submitChunk failed", "retryCount", retryCount,
				"chunkIdx", idx, "statusCode", statusCode, "gatewayErr", body,
				"httpErr", err)
		}
	})
...
func (tt *TransactionUploader) UploadChunk() error {
	...
	if tt.TotalErrors == 100 {
		return errors.New(fmt.Sprintf("Unable to complete upload: %d:%s",
			tt.LastResponseStatus, tt.LastResponseError))
	}

	var delay = 0.0
	if tt.LastResponseError != "" {
		delay = math.Max(float64(tt.LastRequestTimeEnd+types.ERROR_DELAY-time.Now().UnixNano()/1000000),
			float64(types.ERROR_DELAY))
	}
	if delay > 0.0 {
	...
	var err error
	if tt.DataReader != nil {
		chunk, err = utils.GetChunkStream(*tt.Transaction, tt.ChunkIndex,
			tt.DataReader)
	} else {
	...
	_, chunkOk := utils.ValidatePath(tt.Transaction.Chunks.DataRoot, offset, 0,
		dataSize, path)
	if !chunkOk {
		return errors.New(fmt.Sprintf("Unable to validate chunk %d ",
			tt.ChunkIndex))
	}
	// Catch network errors and turn them into objects with status -1 and an error message.
	var gc *types.GetChunk
	if tt.DataReader != nil {
		gc, err = utils.GetChunkStream(*tt.Transaction, tt.ChunkIndex,
			tt.DataReader)
	} else {
	...
	if statusCode == 200 {
		...
		if _, ok := types.FATAL_CHUNK_UPLOAD_ERRORS[body]; ok {
			return errors.New(fmt.Sprintf("Fatal error uploading chunk %d:%v",
				tt.ChunkIndex, body))
		}
	}
	return nil
 ...
 * @param data
 */
func (tt *TransactionUploader) FromSerialized(serialized *SerializedUploader,
	data []byte) (*TransactionUploader, error) {
	if serialized == nil {
 ...
 * @param data
 */
func (tt *TransactionUploader) FromTransactionId(id string) (*SerializedUploader,
	error) {
	tx, err := tt.Client.GetTransactionByID(id)
	if err != nil {
		return nil, errors.New(fmt.Sprintf("Tx %s not found; error: %v", id,
			err))
	}
	transaction := tx
	transaction.Data = ""
...
func (tt *TransactionUploader) uploadTx(withBody bool) error {
	...
	if err != nil || statusCode >= 400 {
		...
		tt.LastResponseStatus = statusCode
		return errors.New(fmt.Sprintf("Unable to upload Transaction: %d, %v, %s",
			statusCode, err, body))
	}

...
func generateBlockDataSegment(b types.Block) []byte {
	...
	list = append(list,
		Base64Encode([]byte(fmt.Sprintf("%v", b.CumulativeDiff))))
...
func generateBlockDataSegmentBase(b types.Block) []byte {
	...
	if b.Height >= height_2_4 {
		...
		if b.Height >= height_2_5 {
			...
			props2 = append(props2,
				Base64Encode([]byte(fmt.Sprintf("%s", RateDividend))))
			props2 = append(props2,
				Base64Encode([]byte(fmt.Sprintf("%s", RateDivisor))))
			props2 = append(props2,
				Base64Encode([]byte(fmt.Sprintf("%s", ScheduledRateDividend))))
			props2 = append(props2,
				Base64Encode([]byte(fmt.Sprintf("%s", ScheduledRateDivisor))))

			props2 = append(props2,
				Base64Encode([]byte(fmt.Sprintf("%s", b.Packing25Threshold))))
			props2 = append(props2, Base64Encode([]byte(fmt.Sprintf("%s",

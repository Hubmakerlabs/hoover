...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/btcsuite/base58"
...
func DecodeBundle(bundleBinary []byte) (*types.Bundle, error) {
	...
	for i := 0; i < itemsNum; i++ {
		...
		if bundleItem.Id != id {
			return nil, fmt.Errorf("bundleItem.Id != id, bundleItem.Id: %s, id: %s",
				bundleItem.Id, id)
		}
...
func NewBundleItemStream(owner string, signatureType int, target, anchor string,
	data io.Reader, tags []types.Tag) (*types.BundleItem, error) {
	return newBundleItem(owner, signatureType, target, anchor, data, tags)
}

func NewBundleItem(owner string, signatureType int, target, anchor string,
	data []byte, tags []types.Tag) (*types.BundleItem, error) {
	return newBundleItem(owner, signatureType, target, anchor, data, tags)
}

func newBundleItem(owner string, signatureType int, target, anchor string,
	data interface{}, tags []types.Tag) (*types.BundleItem, error) {
	if target != "" {
...
func GenerateItemBinaryStream(d *types.BundleItem) (binaryReader io.Reader,
	err error) {
	metaBinary, err := generateItemMetaBinary(d)
	if err != nil {
		return nil, err
...
func SubmitItemToBundlr(item types.BundleItem,
	bundlrUrl string) (*types.BundlrResp, error) {
	...
	// post to bundler
	resp, err := http.DefaultClient.Post(bundlrUrl+"/tx",
		"application/octet-stream", bytes.NewReader(itemBinary))
	if err != nil {
		return nil, err
	}
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("send to bundler request failed; http code: %d",
			resp.StatusCode)
	}
...
func SubmitItemToArSeed(item types.BundleItem,
	currency, arseedUrl string) (*schema.RespOrder, error) {
	itemBinary := item.ItemBinary
	if len(itemBinary) == 0 {
		var err error
		itemBinary, err = GenerateItemBinary(&item)
	...
	resp, err := http.DefaultClient.Post(arseedUrl+"/bundle/tx/"+currency,
		"application/octet-stream", bytes.NewReader(itemBinary))
	if err != nil {
		return nil, err
	}
...
func SubmitItemToMU(item types.BundleItem, muUrl string) ([]byte, error) {
	...
	resp, err := http.DefaultClient.Post(muUrl, "application/octet-stream",
		bytes.NewBuffer(itemBinary))
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()

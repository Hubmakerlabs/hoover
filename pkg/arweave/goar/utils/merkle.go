...
func chunkStreamData(data *os.File) (chunks []types.Chunk, err error) {
	...
	for dataSize-cursor >= types.MAX_CHUNK_SIZE {
		...
		if n < chunkSize || err != nil {
			err = fmt.Errorf("data.Read(chunk) error: chunkSize: %d, readSize: %d",
				chunkSize, n)
			return
		}
		dataHash := sha256.Sum256(chunk)
...
func resolveBranchProofs(node *types.Node, proof []byte,
	depth int) (proofs []*types.Proof) {
	...
	if node.Type == types.BranchNodeType {
		...
		rightProofs := resolveBranchProofs(node.RightChild, partialProof,
			depth+1)
...
// 验证 merkle path
func ValidatePath(id []byte, dest, leftBound, rightBound int,
	path []byte) (*ValidateResult, bool) {
	if rightBound <= 0 {
		return nil, false
	}
	...
	if arrayCompare(id, pathHash) {
		if dest < offset {
			return ValidatePath(left, dest, leftBound,
				int(math.Min(float64(rightBound), float64(offset))), remainder)
		}
		return ValidatePath(right, dest,
			int(math.Max(float64(leftBound), float64(offset))), rightBound,
			remainder)
	}
	return nil, false
}

...
func PrepareChunks(tx *types.Transaction, data interface{},
	dataSize int) error {
...
func GetChunk(tx types.Transaction, idx int, data []byte) (*types.GetChunk,
	error) {
	if tx.Chunks == nil {
...
func GetChunkStream(tx types.Transaction, idx int,
	data *os.File) (*types.GetChunk, error) {
	if tx.Chunks == nil {
		return nil, errors.New("Chunks have not been prepared")
	}
	...
	if n < dataLen || err != nil {
		return nil, fmt.Errorf("getChunkStream failed, err: %v, readByte:%d, dataLen:%d",
			err, n, dataLen)
	}
	return &types.GetChunk{
		DataRoot: tx.DataRoot,
...
func GetSignatureData(tx *types.Transaction) ([]byte, error) {
	...
	case 1:
		...
		for _, tag := range dcTags {
			tags = append(tags,
				ConcatBuffer([]byte(tag.Name), []byte(tag.Value))...)
		}
		data := make([]byte, 0)
	...
	case 2:
		...
		dataList = append(dataList,
			Base64Encode([]byte(fmt.Sprintf("%d", tx.Format))))
	...
	default:
		return nil, errors.New(fmt.Sprintf("Unexpected Transaction format: %d",
			tx.Format))
	}
}

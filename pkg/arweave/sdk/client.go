...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/arweave/schema"
...
func (a *ArSeedCli) SubmitTxConcurrent(ctx context.Context, concurrentNum int,
	arTx types.Transaction) error {
	uploader, err := goar.CreateUploader(a.ACli, &arTx, nil)
	if err != nil {
		return err
	}
...
func (a *ArSeedCli) getTask(taskType, arId string) (schema.Task, error) {
	...
	defer resp.Close()
	if !resp.Ok {
		return schema.Task{}, errors.New(fmt.Sprintf("resp failed: %s",
			resp.String()))
	}
	tk := schema.Task{}
	err = resp.JSON(&tk)
...
func (a *ArSeedCli) SubmitItem(itemBinary []byte, currency string,
	apikey string, needSequence bool) (*schema.RespOrder, error) {
	req := a.SCli.Post()
	if currency != "" {
	...
	defer resp.Close()
	if !resp.Ok {
		return nil, fmt.Errorf("send to bundler request failed; http code: %d, errMsg:%s",
...
func (a *ArSeedCli) SubmitItemStream(itemBinary io.Reader, currency string,
	apikey string, needSequence bool) (*schema.RespOrder, error) {
	req := a.SCli.Post()
	req.Path(fmt.Sprintf("/bundle/tx/%s", currency))
	req.SetHeader("Content-Type", "application/octet-stream")
...
func (a *ArSeedCli) SubmitNativeData(apiKey string, currency string,
	data []byte, contentType string,
	tags map[string]string) (*schema.RespItemId, error) {
	req := a.SCli.Post()
	...
	defer resp.Close()
	if !resp.Ok {
		return nil, fmt.Errorf("resp failed.http code: %d, errMsg:%s",
			resp.StatusCode, resp.String())
	}
...
func (a *ArSeedCli) SubmitNativeDataStream(apiKey string, currency string,
	data io.Reader, contentType string,
	tags map[string]string) (*schema.RespItemId, error) {
	req := a.SCli.Post()
	req.Path(fmt.Sprintf("/bundle/data/%s", currency))
	req.SetHeader("X-API-KEY", apiKey)
...
func (a *ArSeedCli) GetItemMeta(itemId string) (types.BundleItem, error) {
	...
	defer resp.Close()
	if !resp.Ok {
		return types.BundleItem{}, errors.New(fmt.Sprintf("resp failed: %s",
			resp.String()))
	}
...
func (a *ArSeedCli) BundleFee(size int64, currency string) (schema.RespFee,
	error) {
	req := a.SCli.Get()
	...
	defer resp.Close()
	if !resp.Ok {
		return schema.RespFee{}, errors.New(fmt.Sprintf("resp failed: %s",
			resp.String()))
	}
...
func (a *ArSeedCli) GetOrders(addr string, startId int) ([]schema.Order,
	error) {
	req := a.SCli.Get()
...
func (a *ArSeedCli) GetApiKey(addr string) (schema.RespApiKey, error) {
	...
	defer resp.Close()
	if !resp.Ok {
		return schema.RespApiKey{}, errors.New(fmt.Sprintf("resp failed: %s",
			resp.String()))
	}

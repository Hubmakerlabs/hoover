...
import (
	...
	paySdk "github.com/Hubmakerlabs/hoover/pkg/arweave/go-everpay/sdk"
	"github.com/Hubmakerlabs/hoover/pkg/arweave/goar"
	"github.com/Hubmakerlabs/hoover/pkg/arweave/goar/types"
	"github.com/Hubmakerlabs/hoover/pkg/arweave/goar/utils"
	arseedSchema "github.com/Hubmakerlabs/hoover/pkg/arweave/schema"
	"github.com/Hubmakerlabs/hoover/pkg/arweave/sdk/schema"
)
...
func (s *SDK) SendDataAndPay(data []byte, currency string,
	option *schema.OptionItem,
	needSequence bool) (everTx *paySchema.Transaction, itemId string,
	err error) {
...
func (s *SDK) SendDataStreamAndPay(data *os.File, currency string,
	option *schema.OptionItem,
	needSequence bool) (everTx *paySchema.Transaction, itemId string,
	err error) {
...
func (s *SDK) SendData(data []byte, currency string, apikey string,
	option *schema.OptionItem,
	needSequence bool) (order *arseedSchema.RespOrder, err error) {
	bundleItem := types.BundleItem{}
	if option != nil {
		bundleItem, err = s.ItemSigner.CreateAndSignItem(data, option.Target,
			option.Anchor, option.Tags)
	} else {
...
func (s *SDK) SendDataStream(data *os.File, currency string, apikey string,
	option *schema.OptionItem,
	needSequence bool) (order *arseedSchema.RespOrder, err error) {
	bundleItem := types.BundleItem{}
	if option != nil {
		bundleItem, err = s.ItemSigner.CreateAndSignItemStream(data,
			option.Target, option.Anchor, option.Tags)
	} else {
		bundleItem, err = s.ItemSigner.CreateAndSignItemStream(data, "", "",
			nil)
	}
	...
	order, err = s.Cli.SubmitItemStream(binaryReader, currency, apikey,
		needSequence)
	return
}

func (s *SDK) BatchPayOrders(orders []*arseedSchema.RespOrder) (everTxs []*paySchema.Transaction,
	err error) {
	if len(orders) <= 500 {
...
func (s *SDK) PayOrders(orders []*arseedSchema.RespOrder) (everTx *paySchema.Transaction,
	err error) {
	if len(orders) == 0 {
	...
	everTx, err = s.Pay.Transfer(useTag, totalFee, orders[0].Bundler,
		string(dataJs))
	return
}

func (s *SDK) PayApikey(tokenTag string, amount *big.Int) (everHash string,
	err error) {

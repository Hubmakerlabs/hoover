...
func TestSDK_SendDataAndPay_RsaSigner(t *testing.T) {
	...
	tx, itemId, err := sdk.SendDataAndPay(data, "usdt",
		&schema.OptionItem{Tags: tags},
		false) // your account must have enough balance in everpay
	assert.NoError(t, err)
	t.Log("itemId:", itemId)
	t.Log(tx.HexHash())
}
...
func TestSDK_SendData_EccSigner(t *testing.T) {
	...
	t.Log("aaaa")
	ord, err := sdk.SendData(data, "usdc", apikey,
		&schema.OptionItem{Tags: tags},
		false) // your account must have enough balance in everpay
	assert.NoError(t, err)
...
func TestArSeedCli_SubmitNativeData(t *testing.T) {
	...
	currency := "DODO"
	res, err := cli.SubmitNativeData(apiKey, currency, data, "image/jpeg",
		map[string]string{
			"key1": "arseeding test",
...
func TestArSeedCli_SubmitNativeDataStream(t *testing.T) {
	...
	res, err := cli.SubmitNativeDataStream(apiKey, "usdc", data, "image/jpeg",
		map[string]string{
			"key1": "arseeding test",
			"key2": "sandy test bundle native data",
		})
...
func TestSDK_SendDataStreamAndPay(t *testing.T) {
	...
	tx, itemId, err := sdk.SendDataStreamAndPay(data, "usdt",
		&schema.OptionItem{Tags: tags},
		false) // your account must have enough balance in everpay
	assert.NoError(t, err)
	t.Log("itemId:", itemId)

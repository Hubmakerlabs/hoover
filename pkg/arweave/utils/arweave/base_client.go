...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/arweave/utils/logger"
...
func newBaseClient(ctx context.Context,
	config *config.Config) (self *BaseClient) {
	self = new(BaseClient)
	self.config = config
...
func (self *BaseClient) onStatusToError(c *resty.Client,
...
func (self *BaseClient) onForcePeer(c *resty.Client,
	req *resty.Request) (err error) {
...
func (self *BaseClient) onRateLimit(c *resty.Client,
	req *resty.Request) (err error) {
	...
	if limiter == nil {
		...
		limiter, ok = self.limiters[url.Host]
		if !ok {
			limiter = ratelimit.New(self.config.Arweave.LimiterBurstSize,
				ratelimit.Per(self.config.Arweave.LimiterInterval))
...
func (self *BaseClient) onRetryRequest(c *resty.Client,
	...
	endpoint := strings.Clone(strings.TrimPrefix(resp.Request.URL,
		self.config.Arweave.NodeUrl))

	self.log.WithField("peer", peer).WithField("idx", idx).WithField("endpoint",
		endpoint).Trace("Retrying begin")
	...
	for {
		...
		self.log.WithField("peer", peer).WithField("idx",
			idx).WithField("endpoint",
			endpoint).Trace("Retrying request with different peer")
		...
		self.log.WithField("peer", peer).WithField("idx",
			idx).WithField("endpoint",
			endpoint).WithError(err).Warn("Retried request failed")

...
func (self Encoder) WriteSliceAny(bins []any, lenBytes, elemSizeBytes int) {
	var encodeBinList func(bins []any, encoded []byte, n int,
		lenBytes, elemSizeBytes int) []byte
	encodeBinList = func(bins []any, encoded []byte, n int,
		lenBytes, elemSizeBytes int) []byte {
		if len(bins) > 0 {
			...
			return encodeBinList(bins[1:], append(elem, encoded...), n+1,
				lenBytes, elemSizeBytes)
		}
...
func (self Encoder) WriteSliceByte(bins [][]byte, lenBytes, elemSizeBytes int) {
	var encodeBinList func(bins [][]byte, encoded []byte, n int,
		lenBytes, elemSizeBytes int) []byte
	encodeBinList = func(bins [][]byte, encoded []byte, n int,
		lenBytes, elemSizeBytes int) []byte {
		if len(bins) > 0 {
			// Still bins to encode
...
func (self Encoder) WriteBase64StringSlice(bins []Base64String,
	lenBytes, elemSizeBytes int) {
	var encodeBinList func(bins []Base64String, encoded []byte, n int,
		lenBytes, elemSizeBytes int) []byte
	encodeBinList = func(bins []Base64String, encoded []byte, n int,
		lenBytes, elemSizeBytes int) []byte {
		if len(bins) > 0 {
			// Still bins to encode
			elem := self.encodeBin(bins[0], elemSizeBytes)

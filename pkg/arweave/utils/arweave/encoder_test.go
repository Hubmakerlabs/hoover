...
func (s *EncoderTestSuite) TestEncodeBinInt() {
	...
	env.WriteBuffer([]byte{9, 7}, 2)
	env.WriteUint64(1678120795, 1)
	require.Equal(s.T(), []byte{0, 2, 9, 7, 0x04, 0x64, 0x06, 0x17, 0x5B},
		env.Bytes())
}
...
func (s *EncoderTestSuite) TestEncodeBuffer() {
	...
	// fmt.Printf("%v\n", env.Bytes())
	require.Equal(s.T(),
		[]byte{48, 101, 121, 45, 44, 133, 46, 96, 70, 147, 172, 222, 165, 218,
			10, 216, 59, 143, 73, 122, 6, 142, 167, 75, 27, 207, 248, 173, 159,
			168, 85, 40, 182, 168, 227, 178, 255, 145, 209, 100, 50, 82, 119,
			85, 100, 136, 241, 242, 193}, env.Bytes())
...
func (s *EncoderTestSuite) TestRawWriteSize() {
	...
	// 00000000000004D2
	// 00000000000004D2
	require.Equal(s.T(), []byte{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4, 0xd2},
		env.Bytes())
}
...
func (s *EncoderTestSuite) TestRawWriteSizeBigger() {
	...
	require.Equal(s.T(),
		[]byte{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
			...
			0x0, 0x0, 0x0, 0x0, 0x4, 0xd2}, env.Bytes())
}
...
func (s *EncoderTestSuite) TestBigIntBig() {
	...
	v.Int.SetString("115792071036953558077439067295872093892934358766013398190786716226139132215951",
		10)
	env.Write(v, 2)
	require.Equal(s.T(),
		[]byte{0x0, 0x20, 0xff, 0xff, 0xfd, 0x5c, 0xe9, 0x39, 0x3c, 0x84, 0xa8,
			0x39, 0x69, 0xfd, 0x88, 0x8, 0x88, 0xf0, 0xdb, 0xae, 0xdf, 0xc5,
			0xb4, 0x3a, 0x5f, 0xfa, 0xe7, 0x66, 0x56, 0x23, 0xbb, 0x9b, 0x42,
			0x8f}, env.Bytes())

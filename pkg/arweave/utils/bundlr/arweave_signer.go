...
func (self *ArweaveSigner) Sign(data []byte) (signature []byte, err error) {
	...
	return rsa.SignPSS(rand.Reader, self.PrivateKey, crypto.SHA256, hashed[:],
		&rsa.PSSOptions{
...
func (self *ArweaveSigner) Verify(data []byte, signature []byte) (err error) {
	...
	return rsa.VerifyPSS(ownerPublicKey, crypto.SHA256, hashed[:],
		[]byte(signature), &rsa.PSSOptions{
			SaltLength: rsa.PSSSaltLengthAuto,
			Hash:       crypto.SHA256,
		})
}

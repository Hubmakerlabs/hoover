...
func (self *BlockDownloader) WithInitStartHeight(db *gorm.DB,
	component model.SyncedComponent) *BlockDownloader {
	self.Task = self.Task.WithOnBeforeStart(func() (err error) {
...
func (self *BlockDownloader) WithStopHeight(db *gorm.DB, stop uint64,
	component model.SyncedComponent) *BlockDownloader {
...
func (self *BlockDownloader) SetPreviousBlock(previousBlockHeight uint64,
	previousBlockIndepHash arweave.Base64String) {
...
func (self *BlockDownloader) run() error {
	...
	for networkInfo := range self.input {
		...
		self.Log.
			...
			WithField("numNewBlocks",
				uint64(networkInfo.Height)-lastSyncedHeight).
		...
		for height := lastSyncedHeight + 1; height <= uint64(networkInfo.Height) && height <= self.stopBlockHeight; height++ {
			...
			err := task.NewRetry().
				...
				WithOnError(func(err error, isDurationAcceptable bool) error {
					self.Log.WithError(err).WithField("height",
						height).Error("Failed to download block, retrying...")
				...
				Run(func() (err error) {
					blocks, err := self.downloadBlocks(height,
			...
			if err != nil {
				self.Log.WithError(err).WithField("height",
					height).Error("Failed to download block, stop retrying")
				return err
			}
...
func (self *BlockDownloader) downloadBlocks(height uint64,
	lastProcessedBlockHash arweave.Base64String) (out []*arweave.Block,
	err error) {
	var mtx sync.Mutex
	...
	for _, peer := range peers {
		...
		self.SubmitToWorker(func() {
			block, err := self.downloadOneBlock(height, lastProcessedBlockHash,
				peer)
			if err != nil {
				block = nil
			}
...
func (self *BlockDownloader) downloadOneBlock(height uint64,
	lastProcessedBlockHash arweave.Base64String,
	peer string) (block *arweave.Block, err error) {
	...
	if len(lastProcessedBlockHash) > 0 &&
		...
		self.Log.
			...
			WithField("age", resp.Header().Get("Age")).
			WithField("x-trace", resp.Header().Get("X-Trace")).
			WithField("last_processed_block_hash",
				lastProcessedBlockHash.Base64()).
...
func (self *BlockDownloader) vote(blocks []*arweave.Block) (out *arweave.Block,
	err error) {
	minNumberOfVotes := int(math.Round(2.0 * float64(len(blocks)) / 3.0))
	self.Log.WithField("min_number_of_votes",
		minNumberOfVotes).Debug("Voting for the best block")

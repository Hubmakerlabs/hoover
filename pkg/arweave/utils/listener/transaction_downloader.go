...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/arweave/utils/smartweave"
...
func NewTransactionDownloader(config *config.Config) (self *TransactionDownloader) {
	...
	self.Task = task.NewTask(config, "transaction-downloader").
		...
		WithWorkerPool(config.TransactionDownloader.NumWorkers,
			config.TransactionDownloader.WorkerQueueSize).
...
func (self *TransactionDownloader) WithFilterInteractions(isInputInDataEnabled bool) *TransactionDownloader {
	self.filter = func(tx *arweave.Transaction) bool {
		isInteraction, err := smartweave.ValidateInteraction(tx,
			isInputInDataEnabled)
		if err != nil {
			self.Log.WithField("txId",
				tx.ID.Base64()).WithError(err).Warn("neglecting invalid interaction")
			return false
		}
...
func (self *TransactionDownloader) run() error {
	...
	for block := range self.input {
		...
		if err != nil {
			...
			self.Log.WithError(err).WithField("height",
				block.Height).Error("Failed to download transactions in block")
			continue
		}
...
func (self *TransactionDownloader) downloadTransactions(block *arweave.Block) (out []*arweave.Transaction,
	err error) {
	if len(block.Txs) == 0 {
		// Skip
		return
	}
	self.Log.WithField("height", block.Height).WithField("len",
		len(block.Txs)).Debug("Start downloading transactions...")
	defer self.Log.WithField("height",
		block.Height).Debug("...Stopped downloading transactions")
		...
		self.SubmitToWorker(func() {
			...
			err = task.NewRetry().
				...
				WithOnError(func(err error, isDurationAcceptable bool) error {
					self.Log.WithError(err).WithField("txId",
						txId).Warn("Failed to download transaction, retrying after timeout")

					if errors.Is(err,
						context.Canceled) && self.IsStopping.Load() {
						// Stopping
				...
				Run(func() error {
					...
					if err != nil {
						self.Log.WithField("tx",
							txId).Error("Failed to download transaction")
						return err
					}
					...
					if err != nil {
						self.Log.WithField("tx",
							txId).Error("Transaction failed to verify, retry downloading...")
					...
					if self.isGetTransactionData(tx) {
						...
						if err != nil {
							...
							self.Log.WithField("tx",
								txId).Error("Failed to download transaction data, retry downloading...")
						}
					}
			...
			if err != nil {
				...
				self.Log.WithError(err).WithField("txId",
					txId).Error("Failed to download proper transaction, giving up")
				goto end
			}
...
func (self *TransactionDownloader) downloadTransactionData(tx *arweave.Transaction) (arweave.Base64String,
	error) {

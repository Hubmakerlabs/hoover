...
import (
	...
	l "github.com/Hubmakerlabs/hoover/pkg/arweave/utils/logger"
	"github.com/Hubmakerlabs/hoover/pkg/arweave/utils/model/sql_migrations"
...
func Connect(ctx context.Context, dbConfig *config.Database,
	username, password, applicationName string) (self *gorm.DB, err error) {
	...
	if dbConfig.CaCertPath != "" && dbConfig.ClientKeyPath != "" && dbConfig.ClientCertPath != "" {
		...
		dsn += fmt.Sprintf(" sslcert=%s sslkey=%s sslrootcert=%s",
			dbConfig.ClientCertPath, dbConfig.ClientKeyPath,
			dbConfig.CaCertPath)
		...
		dsn += fmt.Sprintf(" sslcert=%s sslkey=%s sslrootcert=%s",
			certFile.Name(), keyFile.Name(), caFile.Name())
	}
...
func NewConnection(ctx context.Context, config *config.Config,
	applicationName string) (self *gorm.DB, err error) {
	err = Migrate(ctx, config)
	if err != nil {
		return
	}

	return Connect(ctx, &config.Database, config.Database.User,
		config.Database.Password, applicationName)
}

func NewReadOnlyConnection(ctx context.Context, config *config.Config,
	applicationName string) (self *gorm.DB, err error) {
	return Connect(ctx, &config.ReadOnlyDatabase, config.Database.User,
		config.Database.Password, applicationName)
}
...
func Migrate(ctx context.Context, config *config.Config) (err error) {
	...
	self, err := Connect(ctx, &config.Database, config.Database.MigrationUser,
		config.Database.MigrationPassword, "migration")
	if err != nil {
		return
	}
...
func ping(ctx context.Context, dbConfig *config.Database,

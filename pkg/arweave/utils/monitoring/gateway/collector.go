...
func NewCollector() *Collector {
	return &Collector{
		...
		// Gateway
		InteractionsReturned: prometheus.NewDesc("interactions_returned", "",
			nil, nil),
...
func (self *Collector) Collect(ch chan<- prometheus.Metric) {
	...
	// Gateway
	ch <- prometheus.MustNewConstMetric(self.InteractionsReturned,
		prometheus.CounterValue,
		float64(self.monitor.Report.Gateway.State.InteractionsReturned.Load()))
	ch <- prometheus.MustNewConstMetric(self.DbError, prometheus.CounterValue,
		float64(self.monitor.Report.Gateway.Errors.DbError.Load()))
}

...
func (self *Collector) Collect(ch chan<- prometheus.Metric) {
	...
	// Interactor
	ch <- prometheus.MustNewConstMetric(self.CheckerDelay,
		prometheus.GaugeValue,
		float64(self.monitor.Report.Interactor.State.CheckerDelay.Load()))
	ch <- prometheus.MustNewConstMetric(self.CheckerDbError,
		prometheus.CounterValue,
		float64(self.monitor.Report.Interactor.Errors.CheckerDbError.Load()))
}

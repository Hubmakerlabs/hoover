...
func ValidateInteraction(tx *arweave.Transaction,
	isInputInDataEnabled bool) (isInteraction bool, err error) {
		...
		case TagInputFormat:
			...
			default:
				err = fmt.Errorf("%s' tag value can only be '%s' or '%s'",
					TagInputFormat, TagInputFormatTagValue,
					TagInputFormatDataValue)
			}
	...
	if jsonError := tool.CheckJSON(input); jsonError != nil {
		err = fmt.Errorf("value of the input is not a valid JSON: %s",
			jsonError.Error())
	}

	if tx.Size() > MaxInteractionDataItemSizeBytes {
		err = fmt.Errorf("the size of the interaction exceeds the limit: %d bytes",
	...
	if !hasContractTag {
		return true, fmt.Errorf("interaction should have a tag '%s'",
			TagContractTxId)
	}

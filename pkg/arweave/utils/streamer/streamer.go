...
func (self *Streamer) connect() (err error) {
	...
	if self.Config.Database.ClientCert != "" && self.Config.Database.ClientKey != "" && self.Config.Database.CaCert != "" {
		cert, err := tls.X509KeyPair([]byte(self.Config.Database.ClientCert),
			[]byte(self.Config.Database.ClientKey))
		if err != nil {
...
func (self *Streamer) run() (err error) {
	...
	for {
		...
		if err != nil {
			self.Log.WithError(err).WithField("cause",
				self.connection.CauseOfDeath()).Error("Failed to wait for notification, reconnecting")
			self.reconnect()
		} else {

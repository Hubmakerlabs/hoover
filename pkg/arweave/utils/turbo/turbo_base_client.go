...
func newBaseClient(ctx context.Context,
	config *config.Bundlr) (self *BaseClient) {
	self = new(BaseClient)
	self.config = config
...
func (self *BaseClient) Reset() {
	...
	self.client =
		resty.New().
			// SetDebug(true).
			SetBaseURL(self.config.TurboUrls[0]).
...
func (self *BaseClient) onRateLimit(c *resty.Client,
	...
	if !ok {
		limiter = rate.NewLimiter(rate.Every(self.config.LimiterInterval),
			self.config.LimiterBurstSize)
	...
	if err != nil {
		...
		self.log.WithField("peer", url.Host).WithField("deadline",
			time.Until(d)).WithError(err).Error("Rate limiting failed")
	}
	return
}

	...
	for _, test := range table {
		...
		for name, event := range abi.Events {
			if event.ID != test.expectations[name] {
				t.Errorf("expected id to be %x, got %x",
					test.expectations[name], event.ID)
			}
		}
	}
}
		...
		for name, event := range abi.Events {
			if event.String() != test.expectations[name] {
				t.Errorf("expected string to be %s, got %s",
					test.expectations[name], event.String())
			}
		}
	}
}
...
func TestEventTupleUnpack(t *testing.T) {
	...
	var testCases = []struct {
		...
		mixedCaseData1,
		&EventMixedCase{},
		&EventMixedCase{Value1: bigintExpected, Value2: bigintExpected2,
			Value3: bigintExpected3},
		jsonEventMixedCase,
		"",
...
func unpackTestEventData(dest interface{}, hexData string, jsonEvent []byte,
	assert *assert.Assertions) error {
	data, err := hex.DecodeString(hexData)
	assert.NoError(err, "Hex data should be a correct hex-string")
	var e Event
	assert.NoError(json.Unmarshal(jsonEvent, &e),

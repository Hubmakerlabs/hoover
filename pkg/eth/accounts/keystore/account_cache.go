...
func (ac *accountCache) add(newAccount accounts.Account) {
	...
	i := sort.Search(len(ac.all),
		func(i int) bool { return ac.all[i].URL.Cmp(newAccount.URL) >= 0 })
	...
	ac.all[i] = newAccount
	ac.byAddr[newAccount.Address] = append(ac.byAddr[newAccount.Address],
		newAccount)
}
...
func (ac *accountCache) deleteByFile(path string) {
	...
	defer ac.mu.Unlock()
	i := sort.Search(len(ac.all),
		func(i int) bool { return ac.all[i].URL.Path >= path })

	if i < len(ac.all) && ac.all[i].URL.Path == path {
		removed := ac.all[i]
		ac.all = append(ac.all[:i], ac.all[i+1:]...)
		if ba := removeAccount(ac.byAddr[removed.Address],
			removed); len(ba) == 0 {
			delete(ac.byAddr, removed.Address)
		} else {
			ac.byAddr[removed.Address] = ba
...
func removeAccount(slice []accounts.Account,
	elem accounts.Account) []accounts.Account {
	for i := range slice {
		if slice[i] == elem {
...
func (ac *accountCache) find(a accounts.Account) (accounts.Account, error) {
	...
	case 0:
		return accounts.Account{}, ErrNoMatch
	default:
		err := &AmbiguousAddrError{Addr: a.Address,
			Matches: make([]accounts.Account, len(matches))}
...
func (ac *accountCache) scanAccounts() error {
	...
	readAccount := func(path string) *accounts.Account {
		...
		case addr == common.Address{}:
			log.Debug("Failed to decode keystore key", "path", path, "err",

...
func (ks *KeyStore) refreshWallets() {
	...
	for _, account := range accs {
		...
		for len(ks.wallets) > 0 && ks.wallets[0].URL().Cmp(account.URL) < 0 {
			events = append(events, accounts.WalletEvent{Wallet: ks.wallets[0],
		...
		if len(ks.wallets) == 0 || ks.wallets[0].URL().Cmp(account.URL) > 0 {
			...
			events = append(events, accounts.WalletEvent{Wallet: wallet,
	...
	for _, wallet := range ks.wallets {
		events = append(events,
			accounts.WalletEvent{Wallet: wallet, Kind: accounts.WalletDropped})
	}
	ks.wallets = wallets
	ks.mu.Unlock()
...
func (ks *KeyStore) TimedUnlock(a accounts.Account, passphrase string,
...
func (ks *KeyStore) getDecryptedKey(a accounts.Account,
	auth string) (accounts.Account, *Key, error) {
	a, err := ks.Find(a)
	if err != nil {
		return a, nil, err
	}
...
func (ks *KeyStore) expire(addr common.Address, u *unlocked,
...
func (ks *KeyStore) Export(a accounts.Account,
	passphrase, newPassphrase string) (keyJSON []byte, err error) {
	_, key, err := ks.getDecryptedKey(a, passphrase)
	if err != nil {
		return nil, err
	}
...
func (ks *KeyStore) Import(keyJSON []byte,
	passphrase, newPassphrase string) (accounts.Account, error) {
...
func (ks *KeyStore) ImportECDSA(priv *ecdsa.PrivateKey,
...
func (ks *KeyStore) importKey(key *Key, passphrase string) (accounts.Account,
	error) {
...
func (ks *KeyStore) Update(a accounts.Account,
	passphrase, newPassphrase string) error {
	a, key, err := ks.getDecryptedKey(a, passphrase)
	if err != nil {
		return err
	}
...
func (ks *KeyStore) ImportPreSaleKey(keyJSON []byte,
	passphrase string) (accounts.Account, error) {

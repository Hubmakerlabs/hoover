...
func TestKeyStore(t *testing.T) {
	...
	if runtime.GOOS != "windows" && stat.Mode() != 0600 {
		t.Fatalf("account file has wrong mode: got %o, want %o", stat.Mode(),
			0600)
	}
	...
	if ks.HasAddress(a.Address) {
		t.Errorf("HasAccount(%x) should've returned true after Delete",
			a.Address)
	}
}
...
func TestSign(t *testing.T) {
	...
	if _, err := ks.SignHash(accounts.Account{Address: a1.Address},
		testSigData); err != nil {
		t.Fatal(err)
	}
}
...
func TestSignWithPassphrase(t *testing.T) {
	...
	if _, err = ks.SignHashWithPassphrase(acc, "invalid passwd",
		testSigData); err == nil {
...
func TestTimedUnlock(t *testing.T) {
	...
	if err != ErrLocked {
		t.Fatal("Signing should've failed with ErrLocked before unlocking, got ",
			err)
	}
	...
	_, err = ks.SignHash(accounts.Account{Address: a1.Address}, testSigData)
	if err != ErrLocked {
		t.Fatal("Signing should've failed with ErrLocked timeout expired, got ",
			err)
	}
}
...
// finish in time
func waitForKsUpdating(t *testing.T, ks *KeyStore, wantStatus bool,
	maxTime time.Duration) bool {
	t.Helper()
...
func TestWalletNotifierLifecycle(t *testing.T) {
	...
	for i := 0; i < len(subs); i++ {
		...
		if !waitForKsUpdating(t, ks, true, 250*time.Millisecond) {
			t.Errorf("sub %d: wallet notifier not running after subscription",
				i)
		}
	}
		...
		for {
			...
			case ev := <-updates:
				events = append(events,
					walletEvent{ev, ev.Wallet.Accounts()[0]})
			case <-sub.Err():
				close(updates)
				return
			}
	...
	for i := 0; i < 1024; i++ {
		if create := len(live) == 0 || rand.Int()%4 > 0; create {
			...
			wantEvents = append(wantEvents,
				walletEvent{accounts.WalletEvent{Kind: accounts.WalletArrived},
					account})
		} else {
			...
			wantEvents = append(wantEvents,
				walletEvent{accounts.WalletEvent{Kind: accounts.WalletDropped},
					account})
		}
	}
...
func checkAccounts(t *testing.T, live map[common.Address]accounts.Account,
	wallets []accounts.Wallet) {
	if len(live) != len(wallets) {
		t.Errorf("wallet list doesn't match required accounts: have %d, want %d",
			len(wallets), len(live))
		return
	}
	...
	for j, wallet := range wallets {
		if accs := wallet.Accounts(); len(accs) != 1 {
			t.Errorf("wallet %d: contains invalid number of accounts: have %d, want 1",
				j, len(accs))
		} else if accs[0] != liveList[j] {
			t.Errorf("wallet %d: account mismatch: have %v, want %v", j,
				accs[0], liveList[j])
		}
	}
}
...
func checkEvents(t *testing.T, want []walletEvent, have []walletEvent) {
	for _, wantEv := range want {
		...
		if nmatch == 0 {
			t.Fatalf("can't find event with Kind=%v for %x", wantEv.Kind,
				wantEv.a.Address)
		}
	}
}

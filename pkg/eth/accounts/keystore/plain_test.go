...
func TestKeyStorePassphraseDecryptionFail(t *testing.T) {
	...
	if _, err = ks.GetKey(k1.Address, account.URL.Path,
		"bar"); err != ErrDecrypt {
...
func TestImportPreSaleKey(t *testing.T) {
	...
	if !strings.HasPrefix(account.URL.Path, dir) {
		t.Errorf("imported account file not in keystore directory: %q",
			account.URL)
	}
}
...
var testsSubmodule = filepath.Join("..", "..", "tests", "testdata",
	"KeyStoreTests")
...
func TestV3_PBKDF2_2(t *testing.T) {
	...
	t.Parallel()
	tests := loadKeyStoreTestV3(filepath.Join(testsSubmodule,
		"basic_tests.json"), t)
...
func TestV1_2(t *testing.T) {
	...
	if privHex != expectedHex {
		t.Fatal(fmt.Errorf("Unexpected privkey: %v, expected %v", privHex,
			expectedHex))
	}
}
...
func testDecryptV3(test KeyStoreTestV3, t *testing.T) {
	...
	if test.Priv != privHex {
		t.Fatal(fmt.Errorf("Decrypted bytes not equal to test, expected %v have %v",
			test.Priv, privHex))
	}
}
...
func TestKeyForDirectICAP(t *testing.T) {
	...
	if !strings.HasPrefix(key.Address.Hex(), "0x00") {
		t.Errorf("Expected first address byte to be zero, have: %s",
			key.Address.Hex())
	}
}

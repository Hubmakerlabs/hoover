...
func (hub *Hub) writePairings() error {
	pairingFile, err := os.OpenFile(filepath.Join(hub.datadir,
		"smartcards.json"), os.O_RDWR|os.O_CREATE, 0755)
	if err != nil {
		return err
	}
...
func (hub *Hub) refreshWallets() {
	...
	for _, reader := range readers {
		...
		card, err := hub.context.Connect(reader, pcsc.ShareShared,
			pcsc.ProtocolAny)
		if err != nil {
		...
		if err = wallet.connect(); err != nil {
			log.Debug("Failed to connect to smart card", "reader", reader,
				"err", err)
		...
		events = append(events,
			accounts.WalletEvent{Wallet: wallet, Kind: accounts.WalletArrived})
	}

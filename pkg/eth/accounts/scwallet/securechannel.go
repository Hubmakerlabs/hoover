...
func NewSecureChannelSession(card *pcsc.Card,
	keyData []byte) (*SecureChannelSession, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not unmarshal public key from card: %v",
			err)
	}
	...
	return &SecureChannelSession{
		card:   card,
		secret: secret.Bytes(),
		publicKey: elliptic.Marshal(crypto.S256(), key.PublicKey.X,
			key.PublicKey.Y),
	}, nil
}
...
func (s *SecureChannelSession) Pair(pairingPassword []byte) error {
	secretHash := pbkdf2.Key(norm.NFKD.Bytes(pairingPassword),
		norm.NFKD.Bytes([]byte(pairingSalt)), 50000, 32, sha256.New)
	...
	if !bytes.Equal(expectedCryptogram, cardCryptogram) {
		return fmt.Errorf("invalid card cryptogram %v != %v",
			expectedCryptogram, cardCryptogram)
	}
...
func (s *SecureChannelSession) Unpair() error {
	...
	_, err := s.transmitEncrypted(claSCWallet, insUnpair, s.PairingIndex, 0,
		[]byte{})
	if err != nil {
		return err
	}
...
func (s *SecureChannelSession) mutuallyAuthenticate() error {
	...
	response, err := s.transmitEncrypted(claSCWallet, insMutuallyAuthenticate,
		0, 0, data)
	if err != nil {
		return err
	}
	if response.Sw1 != 0x90 || response.Sw2 != 0x00 {
		return fmt.Errorf("got unexpected response from MUTUALLY_AUTHENTICATE: %#x%x",
			response.Sw1, response.Sw2)
	}

	if len(response.Data) != scSecretLength {
		return fmt.Errorf("response from MUTUALLY_AUTHENTICATE was %d bytes, expected %d",
			len(response.Data), scSecretLength)
	}
...
func (s *SecureChannelSession) pair(p1 uint8, data []byte) (*responseAPDU,
	error) {
	return transmit(s.card, &commandAPDU{
		Cla:  claSCWallet,
		Ins:  insPair,
...
func (s *SecureChannelSession) transmitEncrypted(cla, ins, p1, p2 byte,
	...
	if rapdu.Sw1 != sw1Ok {
		return nil, fmt.Errorf("unexpected response status Cla=%#x, Ins=%#x, Sw=%#x%x",
			cla, ins, rapdu.Sw1, rapdu.Sw2)
	}
...
func (s *SecureChannelSession) encryptAPDU(data []byte) ([]byte, error) {
	if len(data) > maxPayloadSize {
		return nil, fmt.Errorf("payload of %d bytes exceeds maximum of %d",
...
func unpad(data []byte, terminator byte) ([]byte, error) {
	for i := 1; i <= 16; i++ {
		...
		default:
			return nil, fmt.Errorf("expected end of padding, got %d",
				data[len(data)-i])
		}
	}

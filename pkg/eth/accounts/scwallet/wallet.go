...
func transmit(card *pcsc.Card, command *commandAPDU) (*responseAPDU, error) {
	...
	if response.Sw1 != sw1Ok {
		return nil, fmt.Errorf("unexpected insecure response status Cla=%#x, Ins=%#x, Sw=%#x%x",
...
func (w *Wallet) doselect() (*applicationInfo, error) {
	...
	if _, err := asn1.UnmarshalWithParams(response.Data, appinfo,
		"tag:4"); err != nil {
		return nil, err
	}
	return appinfo, nil
...
func (w *Wallet) URL() accounts.URL {
	return accounts.URL{
		Scheme: w.Hub.scheme,
		Path: fmt.Sprintf("%x",
			w.PublicKey[1:5]), // Byte #0 isn't unique; 1:5 covers << 64K cards, bump to 1:9 for << 4M
	}
}
...
func (w *Wallet) Status() (string, error) {
	...
	case !w.session.verified && status.PinRetryCount == 0:
		return fmt.Sprintf("Blocked, waiting for PUK (%d attempts left) and new PIN",
			status.PukRetryCount), nil
	case !w.session.verified:
		return fmt.Sprintf("Locked, waiting for PIN (%d attempts left)",
			status.PinRetryCount), nil
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			if err := w.session.authenticate(*pairing); err != nil {
				return fmt.Errorf("failed to authenticate card %x: %s",
					w.PublicKey[:4], err)
			}
	...
	go w.Hub.updateFeed.Send(accounts.WalletEvent{Wallet: w,
...
func (w *Wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		var (
			...
			nextPaths = append([]accounts.DerivationPath{},
				w.deriveNextPaths...)
			...
			for empty := false; !empty; {
				...
				if nextAddrs[i] == (common.Address{}) {
					if nextAcc, err = w.session.derive(nextPaths[i]); err != nil {
						w.log.Warn("Smartcard wallet account derivation failed",
							"err", err)
						break
					}
				...
				balance, err = w.deriveChain.BalanceAt(context, nextAddrs[i],
					nil)
				if err != nil {
					w.log.Warn("Smartcard wallet balance retrieval failed",
						"err", err)
					break
				}
				nonce, err = w.deriveChain.NonceAt(context, nextAddrs[i], nil)
				if err != nil {
					w.log.Warn("Smartcard wallet nonce retrieval failed", "err",
						err)
					break
				}
				...
				if _, known := pairing.Accounts[nextAddrs[i]]; !known || !empty || nextAddrs[i] != w.deriveNextAddrs[i] {
					w.log.Info("Smartcard wallet discovered new account",
						"address", nextAddrs[i], "path", path, "balance",
						balance, "nonce", nonce)
				}
...
func (w *Wallet) makeAccount(address common.Address,
	path accounts.DerivationPath) accounts.Account {
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath,
...
func (w *Wallet) SignData(account accounts.Account, mimeType string,
	data []byte) ([]byte, error) {
	return w.signHash(account, crypto.Keccak256(data))
}

func (w *Wallet) signHash(account accounts.Account, hash []byte) ([]byte,
	error) {
	w.lock.Lock()
	defer w.lock.Unlock()
...
// the account in a keystore).
func (w *Wallet) SignTx(account accounts.Account, tx *types.Transaction,
...
// the account in a keystore).
func (w *Wallet) SignText(account accounts.Account, text []byte) ([]byte,
	error) {
...
func (w *Wallet) SignTextWithPassphrase(account accounts.Account,
	passphrase string, text []byte) ([]byte, error) {
	return w.signHashWithPassphrase(account, passphrase,
		crypto.Keccak256(accounts.TextHash(text)))
}
...
// It looks up the account specified either solely via its address contained within,
// or optionally with the aid of any location metadata from the embedded URL field.
func (w *Wallet) SignTxWithPassphrase(account accounts.Account,
	passphrase string, tx *types.Transaction,
	chainID *big.Int) (*types.Transaction, error) {
	if !w.session.verified {
		if err := w.Open(passphrase); err != nil {
			return nil, err
...
func (w *Wallet) findAccountPath(account accounts.Account) (accounts.DerivationPath,
	error) {
	pairing := w.Hub.pairing(w)
	...
	if account.URL.Scheme != w.Hub.scheme {
		return nil, fmt.Errorf("scheme %s does not match wallet scheme %s",
			account.URL.Scheme, w.Hub.scheme)
	}
...
func (s *Session) verifyPin(pin []byte) error {
	if _, err := s.Channel.transmitEncrypted(claSCWallet, insVerifyPin, 0, 0,
		pin); err != nil {
		return err
	}
	s.verified = true
...
func (s *Session) unblockPin(pukpin []byte) error {
	if _, err := s.Channel.transmitEncrypted(claSCWallet, insUnblockPin, 0, 0,
		pukpin); err != nil {
		return err
	}
	s.verified = true
...
func (s *Session) authenticate(pairing smartcardPairing) error {
	if !bytes.Equal(s.Wallet.PublicKey, pairing.PublicKey) {
		return fmt.Errorf("cannot pair using another wallet's pairing; %x != %x",
			s.Wallet.PublicKey, pairing.PublicKey)
	}
...
func (s *Session) walletStatus() (*walletStatus, error) {
	response, err := s.Channel.transmitEncrypted(claSCWallet, insStatus,
		statusP1WalletStatus, 0, nil)
	if err != nil {
		return nil, err
	}

	status := new(walletStatus)
	if _, err := asn1.UnmarshalWithParams(response.Data, status,
		"tag:3"); err != nil {
		return nil, err
	}
	return status, nil
...
func (s *Session) derive(path accounts.DerivationPath) (accounts.Account,
	error) {
	...
	default:
		return accounts.Account{}, fmt.Errorf("invalid startingPoint %d",
			startingPoint)
	}
	...
	_, err = s.Channel.transmitEncrypted(claSCWallet, insDeriveKey, p1, 0,
		data.Bytes())
	if err != nil {
		return accounts.Account{}, err
	}

	response, err := s.Channel.transmitEncrypted(claSCWallet, insSign, 0, 0,
	...
	if _, err := asn1.UnmarshalWithParams(response.Data, sigdata,
		"tag:0"); err != nil {
		return accounts.Account{}, err
	}
...
func (s *Session) publicKey() ([]byte, error) {
	response, err := s.Channel.transmitEncrypted(claSCWallet, insExportKey,
		exportP1Any, exportP2Pubkey, nil)
	if err != nil {
		return nil, err
	}
	keys := new(keyExport)
	if _, err := asn1.UnmarshalWithParams(response.Data, keys,
		"tag:1"); err != nil {
		return nil, err
	}
...
func (s *Session) sign(path accounts.DerivationPath, hash []byte) ([]byte,
	error) {
	startTime := time.Now()
	_, err := s.derive(path)
	if err != nil {
	...
	response, err := s.Channel.transmitEncrypted(claSCWallet, insSign,
		signP1PrecomputedHash, signP2OnlyBlock, hash)
	if err != nil {
		return nil, err
	}
	sigdata := new(signatureData)
	...
	log.Debug("Signed using smartcard", "deriveTime", deriveTime.Sub(startTime),
		"signingTime", time.Since(deriveTime))
...
func makeRecoverableSignature(hash, sig, expectedPubkey []byte) ([]byte,
	error) {

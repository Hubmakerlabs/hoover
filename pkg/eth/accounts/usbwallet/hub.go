...
func NewTrezorHubWithHID() (*Hub, error) {
	return newHub(TrezorScheme, 0x534c, []uint16{0x0001 /* Trezor HID */},
		0xff00, 0, newTrezorDriver)
}
...
func NewTrezorHubWithWebUSB() (*Hub, error) {
	return newHub(TrezorScheme, 0x1209, []uint16{0x53c1 /* Trezor WebUSB */},
		0xffff /* No usage id on webusb, don't match unset (0) */, 0,
		newTrezorDriver)
}
...
func (hub *Hub) refreshWallets() {
	...
	for _, device := range devices {
		...
		for len(hub.wallets) > 0 {
			...
			events = append(events, accounts.WalletEvent{Wallet: hub.wallets[0],
		...
		if len(hub.wallets) == 0 || hub.wallets[0].URL().Cmp(url) > 0 {
			...
			wallet := &wallet{hub: hub, driver: hub.makeDriver(logger),
				url: &url, info: device, log: logger}

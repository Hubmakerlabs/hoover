...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/accounts/usbwallet/trezor"
...
func (w *trezorDriver) Status() (string, error) {
	...
	return fmt.Sprintf("Trezor v%d.%d.%d '%s' online", w.version[0],
		w.version[1], w.version[2], w.label), w.failure
}
...
func (w *trezorDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if passphrase == "" && !w.passphrasewait {
		...
		if _, err := w.trezorExchange(&trezor.Initialize{},
			features); err != nil {
			return err
		}
		w.version = [3]uint32{features.GetMajorVersion(),
			features.GetMinorVersion(), features.GetPatchVersion()}
		w.label = features.GetLabel()

		// Do a manual ping, forcing the device to ask for its PIN and Passphrase
		askPin := true
		askPassphrase := true
		res, err := w.trezorExchange(&trezor.Ping{PinProtection: &askPin,
			PassphraseProtection: &askPassphrase}, new(trezor.PinMatrixRequest),
	...
	if w.pinwait {
		...
		if _, err := w.trezorExchange(&trezor.PassphraseAck{Passphrase: &passphrase},
			new(trezor.Success)); err != nil {
			w.failure = err
			return err
		}
	}
...
func (w *trezorDriver) Heartbeat() error {
	if _, err := w.trezorExchange(&trezor.Ping{},
		new(trezor.Success)); err != nil {
		w.failure = err
		return err
	}
...
func (w *trezorDriver) Derive(path accounts.DerivationPath) (common.Address,
	error) {
...
func (w *trezorDriver) SignTypedMessage(path accounts.DerivationPath,
...
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address,
	error) {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32,
	...
	*types.Transaction, error) {
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.EthereumTxAck{DataChunk: chunk},
			response); err != nil {
			return common.Address{}, nil, err
		}
	}
	...
	signature := append(append(response.GetSignatureR(),
		response.GetSignatureS()...), byte(response.GetSignatureV()))
...
func (w *trezorDriver) trezorExchange(req proto.Message,
	results ...proto.Message) (int, error) {
	...
	for len(payload) > 0 {
		...
		w.log.Trace("Data chunk sent to the Trezor", "chunk",
			hexutil.Bytes(chunk))
	...
	for {
		...
		w.log.Trace("Data chunk received from the Trezor", "chunk",
			hexutil.Bytes(chunk))
	...
	return 0, fmt.Errorf("trezor: expected reply types %s, got %s", expected,
		trezor.Name(kind))
}

...
func (c *ChainConfig) AddFork(name string, epoch uint64,
	version []byte) *ChainConfig {
	fork := &Fork{
		Name:    name,
		Epoch:   epoch,
		Version: version,
...
func (c *ChainConfig) LoadForks(path string) error {
	...
	for key, value := range config {
		if strings.HasSuffix(key, "_FORK_VERSION") {
			...
			if v, err := hexutil.Decode(value); err == nil {
				...
				return fmt.Errorf("failed to decode hex fork id %q in beacon chain config file: %v",
					value, err)
			}
		}
		if strings.HasSuffix(key, "_FORK_EPOCH") {
			...
			if v, err := strconv.ParseUint(value, 10, 64); err == nil {
				...
				return fmt.Errorf("failed to parse epoch number %q in beacon chain config file: %v",
					value, err)
			}
		}
	}
	for name, epoch := range epochs {
		if version, ok := versions[name]; ok {
			...
			return fmt.Errorf("fork id missing for %q in beacon chain config file",
				name)
		}
	}
	for name := range versions {
		return fmt.Errorf("epoch number missing for fork %q in beacon chain config file",
			name)
	}
	sort.Sort(c.Forks)
	return nil

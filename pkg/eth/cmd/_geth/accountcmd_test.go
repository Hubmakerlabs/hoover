...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	geth := runGeth(t, "--lightkdf", "account", "import", "-password",
...
func TestWalletImport(t *testing.T) {
	geth := runGeth(t, "wallet", "import", "--lightkdf",
...
`)
	...
	if len(files) != 1 {
		t.Errorf("expected one key file in keystore directory, found %d files (error: %v)",
			len(files), err)
	}
}
...
func TestUnlockFlag(t *testing.T) {
	geth := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir",
		tmpDatadirWithKeystore(t),
		"--unlock", "f466859ead1932d743d622cb74fc058882e8648a", "console",
		"--exec", "loadScript('testdata/empty.js')")
	geth.Expect(`
Unlocking account f466859ead1932d743d622cb74fc058882e8648a | Attempt 1/3
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "foobar"}}
...
// https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/1785
func TestUnlockFlagMultiIndex(t *testing.T) {
	geth := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir",
		tmpDatadirWithKeystore(t),
		"--unlock", "f466859ead1932d743d622cb74fc058882e8648a", "--unlock",
...
func TestUnlockFlagPasswordFile(t *testing.T) {
	geth := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir",
		...
		"testdata/passwords.txt", "--unlock", "0,2", "console", "--exec",
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	store := filepath.Join("..", "..", "accounts", "keystore", "testdata",
		"dupes")
	geth := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir",
		tmpDatadirWithKeystore(t),

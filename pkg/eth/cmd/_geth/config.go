...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/accounts/external"
...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if deprecated(id) {
			log.Warn("Config field is deprecated and won't have an effect",
				"name", id)
			return nil
		}
		var link string
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", see https://godoc.org/%s#%s for available fields",
				rt.PkgPath(), rt.Name())
		}
		return fmt.Errorf("field '%s' is not defined in %s%s", field,
			rt.String(), link)
	},
}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	...
	if err := setAccountManagerBackends(stack.Config(), stack.AccountManager(),
		stack.KeyStoreDir()); err != nil {
...
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil { // The 'eth' backend may be nil in light mode
		...
		metrics.NewRegisteredGaugeInfo("geth/info",
			nil).Update(metrics.GaugeInfoValue{
			"arch":      runtime.GOARCH,
			"os":        runtime.GOOS,
	...
	if ctx.IsSet(utils.SyncTargetFlag.Name) {
		...
		if len(hex) != common.HashLength {
			utils.Fatalf("invalid sync target length: have %d, want %d",
				len(hex), common.HashLength)
		}
	...
	if ctx.IsSet(utils.DeveloperFlag.Name) {
		simBeacon, err := catalyst.NewSimulatedBeacon(ctx.Uint64(utils.DeveloperPeriodFlag.Name),
			eth)
		if err != nil {
			utils.Fatalf("failed to register dev mode catalyst service: %v",
				err)
		}
...
func dumpConfig(ctx *cli.Context) error {
	...
	dump := os.Stdout
	if ctx.NArg() > 0 {
		dump, err = os.OpenFile(ctx.Args().Get(0),
...
func setAccountManagerBackends(conf *node.Config, am *accounts.Manager,
	keydir string) error {
	...
	if len(conf.SmartCardDaemonPath) > 0 {
		...
		if schub, err := scwallet.NewHub(conf.SmartCardDaemonPath,
			scwallet.Scheme, keydir); err != nil {

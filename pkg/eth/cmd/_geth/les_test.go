...
func (g *gethrpc) callRPC(result interface{}, method string,
	args ...interface{}) {
...
func (g *gethrpc) addPeer(peer *gethrpc) {
	...
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh,
		"peerEvents")
	if err != nil {
	...
	select {
	case ev := <-peerCh:
		g.geth.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type,
			ev.Peer)
	case err := <-sub.Err():
...
func startGethWithIpc(t *testing.T, name string, args ...string) *gethrpc {
	...
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0",
		"--ipcpath", ipcName}, args...)
...
func startLightServer(t *testing.T) *gethrpc {
	...
	runGeth(t, "account", "import", "--datadir", datadir, "--password",
		"./testdata/password.txt", "--lightkdf",
		"./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGethWithIpc(t, "lightserver", "--allow-insecure-unlock",
		"--datadir", datadir, "--password", "./testdata/password.txt",
		"--unlock", account,
		"--miner.etherbase=0x02f0d131f1f97aef08aec6e3291b957d9efe7105",
		"--mine", "--light.serve=100", "--light.maxpeers=1", "--discv4=false",
		"--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gethrpc {
	datadir := initGeth(t)
	return startGethWithIpc(t, name, "--datadir", datadir, "--discv4=false",
		"--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}

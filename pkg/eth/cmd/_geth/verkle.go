...
func checkChildren(root verkle.VerkleNode,
	resolver verkle.NodeResolverFn) error {
	...
	case *verkle.InternalNode:
		for i, child := range node.Children() {
			...
			if err != nil {
				return fmt.Errorf("could not find child %x in db: %w", childC,
					err)
			}
			// depth is set to 0, the tree isn't rebuilt so it's not a problem
			childN, err := verkle.ParseNode(childS, 0, childC[:])
			if err != nil {
				return fmt.Errorf("decode error child %x in db: %w",
					child.Commitment().Bytes(), err)
			}
			if err := checkChildren(childN, resolver); err != nil {
				return fmt.Errorf("%x%w", i,
					err) // write the path to the erroring node
			}
		}
...
func verifyVerkle(ctx *cli.Context) error {
	...
	if ctx.NArg() == 1 {
		...
		log.Info("Rebuilding the tree", "root", rootC, "number",
			headBlock.NumberU64())
	}
...
func expandVerkle(ctx *cli.Context) error {
	...
	if err := os.WriteFile("dump.dot", []byte(verkle.ToDot(root)),
		0600); err != nil {

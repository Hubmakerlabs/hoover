...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/common/compiler"
...
func abigen(c *cli.Context) error {
	utils.CheckExclusive(c, abiFlag,
		jsonFlag) // Only one source can be selected.
	...
	case "go":
		lang = bind.LangGo
	default:
		utils.Fatalf("Unsupported destination language \"%s\" (--lang)",
	...
	if c.String(abiFlag.Name) != "" {
		...
		if c.IsSet(excFlag.Name) {
			var err error
			if exclude, err = newNameFilter(strings.Split(c.String(excFlag.Name),
				",")...); err != nil {
		...
		if c.IsSet(jsonFlag.Name) {
			...
			contracts, err = compiler.ParseCombinedJSON(jsonOutput, "", "", "",
				"")
			if err != nil {
				utils.Fatalf("Failed to read contract information from json output: %v",
					err)
			}
		}
		...
		for name, contract := range contracts {
			...
			if err != nil {
				utils.Fatalf("Failed to parse ABIs from compiler output: %v",
					err)
			}
			abis = append(abis, string(abi))
	...
	code, err := bind.Bind(types, abis, bins, sigs, c.String(pkgFlag.Name),
		lang, libs, aliases)
	if err != nil {
	...
	if err := os.WriteFile(c.String(outFlag.Name), []byte(code),
		0600); err != nil {

...
func TestImportRaw(t *testing.T) {
	keyPath := filepath.Join(os.TempDir(),
		fmt.Sprintf("%v-tempkey.test", t.Name()))
	os.WriteFile(keyPath,
		[]byte("0102030405060708090a0102030405060708090a0102030405060708090a0102"),
		0777)
	...
	t.Run("happy-path", func(t *testing.T) {
		// Run clef importraw
		clef := runClef(t, "--suppress-bootwarn", "--lightkdf", "importraw",
			keyPath)
...
func TestListAccounts(t *testing.T) {
	...
	t.Run("one-account", func(t *testing.T) {
		...
		clef = runWithKeystore(t, clef.Datadir, "--suppress-bootwarn",
			"--lightkdf", "list-accounts")
		if out := string(clef.Output()); !strings.Contains(out,
			"0x9160DC9105f7De5dC5E7f3d97ef11DA47269BdA6 (keystore:") {
			t.Logf("Output\n%v", out)
			t.Error("Failure")
		}
	})
...
func TestListWallets(t *testing.T) {
	...
	t.Run("one-account", func(t *testing.T) {
		...
		// Secondly, do a listing, using the same datadir
		clef = runWithKeystore(t, clef.Datadir, "--suppress-bootwarn",
			"--lightkdf", "list-wallets")
		if out := string(clef.Output()); !strings.Contains(out,
			"Account 0: 0x9160DC9105f7De5dC5E7f3d97ef11DA47269BdA6") {
			t.Logf("Output\n%v", out)
			t.Error("Failure")
		}
	})

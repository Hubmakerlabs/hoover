...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/signer/core"
	"github.com/Hubmakerlabs/hoover/pkg/eth/signer/core/apitypes"
	"github.com/Hubmakerlabs/hoover/pkg/eth/signer/fourbyte"
	"github.com/Hubmakerlabs/hoover/pkg/eth/signer/rules"
	"github.com/Hubmakerlabs/hoover/pkg/eth/signer/storage"
...
func initializeSecrets(c *cli.Context) error {
	...
	if _, err := os.Stat(location); err == nil {
		return fmt.Errorf("master key %v already exists, will not overwrite",
			location)
	}
...
func attestFile(ctx *cli.Context) error {
	...
	vaultLocation := filepath.Join(configDir,
		common.Bytes2Hex(crypto.Keccak256([]byte("vault"), stretchedKey)[:10]))
	confKey := crypto.Keccak256([]byte("config"), stretchedKey)

	// Initialize the encrypted storages
	configStorage := storage.NewAESEncryptedStorage(filepath.Join(vaultLocation,
		"config.json"), confKey)
	val := ctx.Args().First()
...
func initInternalApi(c *cli.Context) (*core.UIServerAPI, core.UIClientAPI,
	error) {
	if err := initialize(c); err != nil {
		return nil, nil, err
	}
...
func setCredential(ctx *cli.Context) error {
	...
	password := utils.GetPassPhrase("Please enter a password to store for this address:",
		true)
	fmt.Println()
	...
	pwStorage := storage.NewAESEncryptedStorage(filepath.Join(vaultLocation,
		"credentials.json"), pwkey)
...
func initialize(c *cli.Context) error {
	...
	log.Root().SetHandler(log.LvlFilterHandler(log.Lvl(c.Int(logLevelFlag.Name)),
		log.StreamHandler(output, log.TerminalFormat(usecolor))))
...
func accountImport(c *cli.Context) error {
	...
	acc, err := internalApi.ImportRawKey(hex.EncodeToString(crypto.FromECDSA(pKey)),
		first)
	if err != nil {
		return err
	}
...
func signer(c *cli.Context) error {
	...
	if stretchedKey, err := readMasterKey(c, ui); err != nil {
		...
		vaultLocation := filepath.Join(configDir,
			common.Bytes2Hex(crypto.Keccak256([]byte("vault"),
				stretchedKey)[:10]))
		...
		pwStorage = storage.NewAESEncryptedStorage(filepath.Join(vaultLocation,
			"credentials.json"), pwkey)
		jsStorage := storage.NewAESEncryptedStorage(filepath.Join(vaultLocation,
			"jsstorage.json"), jskey)
		configStorage := storage.NewAESEncryptedStorage(filepath.Join(vaultLocation,
			"config.json"), confkey)
		...
		if ruleFile := c.String(ruleFlag.Name); ruleFile != "" {
			...
			if err != nil {
				log.Warn("Could not load rules, disabling", "file", ruleFile,
					"err", err)
			} else {
				...
				if storedShasum != foundShaSum {
					log.Warn("Rule hash not attested, disabling", "hash",
						foundShaSum, "attested", storedShasum)
				} else {
					// Initialize rules
					...
					ui = ruleEngine
					log.Info("Rule engine configured", "file",
						c.String(ruleFlag.Name))
				}
			}
		}
	}
	...
	if c.Bool(utils.HTTPEnabledFlag.Name) {
		...
		srv := rpc.NewServer()
		srv.SetBatchLimits(node.DefaultConfig.BatchRequestLimit,
			node.DefaultConfig.BatchResponseMaxSize)
		...
		// start http server
		httpEndpoint := net.JoinHostPort(c.String(utils.HTTPListenAddrFlag.Name),
			fmt.Sprintf("%d", port))
		httpServer, addr, err := node.StartHTTPEndpoint(httpEndpoint,
			rpc.DefaultHTTPTimeouts, handler)
		if err != nil {
...
func readMasterKey(ctx *cli.Context, ui core.UIClientAPI) ([]byte, error) {
	...
	if len(masterSeed) < 256 {
		return nil, fmt.Errorf("master seed of insufficient length, expected >255 bytes, got %d",
			len(masterSeed))
	}
	// Create vault location
	vaultLocation := filepath.Join(configDir,
		common.Bytes2Hex(crypto.Keccak256([]byte("vault"), masterSeed)[:10]))
...
func checkFile(filename string) error {
	...
	// https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/20123
	if runtime.GOOS != "windows" && info.Mode().Perm()&0377 != 0 {
		return fmt.Errorf("file (%v) has insecure file permissions (%v)",
			filename, info.Mode().String())
	}
	return nil
}
...
func testExternalUI(api *core.SignerAPI) {
	...
	expectResponse := func(testcase, question, expect string) {
		if got := queryUser(question); got != expect {
			addErr(fmt.Sprintf("%s: got %v, expected %v", testcase, got,
				expect))
		}
	}
	expectApprove := func(testcase string, err error) {
		...
		addErr(fmt.Sprintf("%v: expected no error, got %v", testcase,
			err.Error()))
	}
	expectDeny := func(testcase string, err error) {
		if err == nil || err != core.ErrRequestDenied {
			addErr(fmt.Sprintf("%v: expected ErrRequestDenied, got %v",
				testcase, err))
		}
	}
	...
	{ // Sign data test - clique header
		...
		_, err = api.SignData(ctx, accounts.MimetypeClique, *addr,
			hexutil.Encode(cliqueRlp))
	...
	{ // Sign data test - plain text
		...
		time.Sleep(delay)
		addr, _ := common.NewMixedcaseAddressFromString("0x0011223344556677889900112233445566778899")
		_, err := api.SignData(ctx, accounts.MimetypeTextPlain, *addr,
			hexutil.Encode([]byte("hello world")))
	...
	{ // Sign transaction
		...
		expectResponse("signtransaction [2]",
			"Did you see any warnings for the last transaction? (yes/no)", "no")
	}
	{ // Listing
	...
	{ // Metadata
		...
		expectResponse("metadata - origin",
			"Did you see origin (origin.com)? [yes/no] ", "yes")
	}
...
func encryptSeed(seed []byte, auth []byte, scryptN, scryptP int) ([]byte,
	error) {
...
func decryptSeed(keyjson []byte, auth string) ([]byte, error) {
	...
	if encSeed.Version != 1 {
		log.Warn(fmt.Sprintf("unsupported encryption format of seed: %d, operation will likely fail",
			encSeed.Version))
	}
...
func GenDoc(ctx *cli.Context) error {
	...
	{ // Sign plain text request
		...
		messages := []*apitypes.NameValueType{{Name: "message", Value: msg,
	...
	{ // Sign tx response
		...
		add("SignTxResponse - deny",
			"Response to SignTxRequest. When denying a request, there's no need to "+
	...
	{ // WHen a signed tx is ready to go out
		...
		add("OnApproved - SignTransactionResult", desc,
			&ethapi.SignTransactionResult{Raw: rlpdata, Tx: &tx})
	}
	{ // User input
		add("UserInputRequest",
			...
			&core.UserInputRequest{IsPassword: true, Title: "The title here",
		...
		add("ListRequest",
			...
			&core.ListRequest{
				...
				Accounts: []accounts.Account{
					{Address: a, URL: accounts.URL{Scheme: "keystore",
						Path: "/path/to/keyfile/a"}},
					{Address: b, URL: accounts.URL{Scheme: "keystore",
						Path: "/path/to/keyfile/b"}}},
			})

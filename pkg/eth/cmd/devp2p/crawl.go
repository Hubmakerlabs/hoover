...
func newCrawler(input nodeSet, bootnodes []*enode.Node, disc resolver,
	iters ...enode.Iterator) (*crawler, error) {
	if len(input) == 0 {
		input.add(bootnodes...)
	}
	if len(input) == 0 {
	...
	for {
		...
		case it := <-doneCh:
			if it == c.inputIter {
				...
				log.Info("Revalidation of input set is done", "len",
					len(c.input))
				if timeout > 0 {
...
func (c *crawler) updateNode(n *enode.Node) int {
	...
	log.Debug("Updating node", "id", n.ID(), "seq", n.Seq(), "score",
		node.Score)
	c.output[n.ID()] = node
	return status
}

...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/cmd/devp2p/internal/v4test"
...
var (
	...
	discv4CrawlCommand = &cli.Command{
		...
		Action: discv4Crawl,
		Flags: flags.Merge(discoveryNodeFlags,
			[]cli.Flag{crawlTimeoutFlag, crawlParallelismFlag}),
	}
...
func discv4ResolveJSON(ctx *cli.Context) error {
	...
	c, err := newCrawler(inputSet, config.Bootnodes, disc,
		enode.IterNodes(nodeargs))
	if err != nil {
		return err
	}
...
func listen(ctx *cli.Context, ln *enode.LocalNode) *net.UDPConn {
	...
	if extAddr != "" {
		...
		if !ok {
			exit(fmt.Errorf("-%s: invalid external address %q",
				extAddrFlag.Name, extAddr))
		}
		ln.SetStaticIP(ip)
		if port != 0 {

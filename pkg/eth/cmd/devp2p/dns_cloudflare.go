...
func (c *cloudflareClient) checkZone(name string) error {
	...
	if !strings.HasSuffix(name, "."+zone.Name) {
		return fmt.Errorf("CloudFlare zone name %q does not match name %q to be deployed",
			zone.Name, name)
	}
	...
	for _, ok := range needPerms {
		if !ok {
			return fmt.Errorf("wrong permissions on zone %s: %v", c.zoneID,
				needPerms)
		}
	}
	return nil
...
// nuked!
func (c *cloudflareClient) uploadRecords(name string,
	...
	entries, _, err := c.ListDNSRecords(context.Background(),
	...
	for path, val := range records {
		...
		if !exists {
			...
			record := cloudflare.CreateDNSRecordParams{Type: "TXT", Name: path,
				Content: val, TTL: ttl}
			_, err = c.CreateDNSRecord(context.Background(),
				cloudflare.ZoneIdentifier(c.zoneID), record)
		} else if old.Content != val {
			// Entry already exists, only change its content.
			log.Info(fmt.Sprintf("Updating %s from %q to %q", path, old.Content,
				val))
			updated++
			...
			_, err = c.UpdateDNSRecord(context.Background(),
				cloudflare.ZoneIdentifier(c.zoneID), record)
		} else {
			skipped++
	...
	log.Info("Updated DNS entries", "new", created, "updated", updated,
		"untouched", skipped)
	...
	for path, entry := range existing {
		...
		deleted++
		if err := c.DeleteDNSRecord(context.Background(),
			cloudflare.ZoneIdentifier(c.zoneID), entry.ID); err != nil {

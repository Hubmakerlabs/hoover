...
func loadTreeDefinitionForExport(dir string) (domain string, t *dnsdisc.Tree,
	err error) {
	...
	if err != nil {
		return "", nil, fmt.Errorf("invalid 'url' field in %v: %v", metaFile,
			err)
	}
	if t, err = dnsdisc.MakeTree(def.Meta.Seq, def.Nodes,
		def.Meta.Links); err != nil {
		return "", nil, err
	}
...
func ensureValidTreeSignature(t *dnsdisc.Tree, pubkey *ecdsa.PublicKey,
	sig string) error {
	if sig == "" {

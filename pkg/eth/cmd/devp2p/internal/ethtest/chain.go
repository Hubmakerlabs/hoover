...
func (c *Chain) ForkID() forkid.ID {
	return forkid.NewID(c.chainConfig, c.blocks[0], uint64(c.Len()),
		c.blocks[0].Time())
}
...
func (c *Chain) GetHeaders(req *GetBlockHeaders) ([]*types.Header, error) {
	...
	if headers[0] == nil {
		return nil, fmt.Errorf("no headers found for given origin number %v, hash %v",
			req.Origin.Number, req.Origin.Hash)
	}
...
func blocksFromFile(chainfile string, gblock *types.Block) ([]*types.Block,
	error) {
	// Load chain.rlp.
	...
	for i := 0; ; i++ {
		...
		if b.NumberU64() != uint64(i+1) {
			return nil, fmt.Errorf("block at index %d has wrong number %d", i,
				b.NumberU64())
		}

...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/internal/utesting"
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p"
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/rlpx"
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("could not negotiate eth protocol (remote caps: %v, local eth version: %v)",
				msg.Caps, c.ourHighestProtoVersion)
		}
		// If we require snap, verify that it was negotiated
		if c.ourHighestSnapProtoVersion != c.negotiatedSnapProtoVersion {
			return fmt.Errorf("could not negotiate snap protocol (remote caps: %v, local snap version: %v)",
				msg.Caps, c.ourHighestSnapProtoVersion)
		}
		return nil
	default:
	...
	for {
		...
		case *Status:
			...
			if have, want := msg.TD.Cmp(chain.TD()), 0; have != want {
				return nil, fmt.Errorf("wrong TD in status: have %v want %v",
					have, want)
			}
			if have, want := msg.ForkID, chain.ForkID(); !reflect.DeepEqual(have,
				want) {
				return nil, fmt.Errorf("wrong fork ID in status: have %v, want %v",
					have, want)
			}
			if have, want := msg.ProtocolVersion, c.ourHighestProtoVersion; have != uint32(want) {
				return nil, fmt.Errorf("wrong protocol version: have %v, want %v",
					have, want)
			}
			message = msg
			break loop
...
func (c *Conn) readAndServe(chain *Chain, timeout time.Duration) Message {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			if err != nil {
				return errorf("could not get headers for inbound header request: %v",
					err)
			}
			resp := &BlockHeaders{
				RequestId:           msg.ReqID(),
...
func (c *Conn) headersRequest(request *GetBlockHeaders, chain *Chain,
	reqID uint64) ([]*types.Header, error) {
	...
	if !ok {
		return nil, fmt.Errorf("unexpected message received: %s",
			pretty.Sdump(msg))
	}
...
func (c *Conn) snapRequest(msg Message, id uint64, chain *Chain) (Message,
	error) {
	defer c.SetReadDeadline(time.Time{})
...
func (c *Conn) waitForResponse(chain *Chain, timeout time.Duration,
	requestID uint64) Message {
	for {
...
func (s *Suite) sendNextBlock() error {
	...
	if err = s.waitForBlockImport(recvConn, nextBlock); err != nil {
		return fmt.Errorf("failed to receive confirmation of block import: %v",
			err)
	}
...
// to propagate it.
func (s *Suite) testAnnounce(sendConn, receiveConn *Conn,
	blockAnnouncement *NewBlock) error {
...
func (s *Suite) waitAnnounce(conn *Conn, blockAnnouncement *NewBlock) error {
	for {
		...
		case *NewBlock:
			if !reflect.DeepEqual(blockAnnouncement.Block.Header(),
				msg.Block.Header()) {
				return fmt.Errorf("wrong header in block announcement: \nexpected %v "+
					"\ngot %v", blockAnnouncement.Block.Header(),
					msg.Block.Header())
			}
			if !reflect.DeepEqual(blockAnnouncement.TD, msg.TD) {
				return fmt.Errorf("wrong TD in announcement: expected %v, got %v",
		...
		case *NewBlockHashes:
			...
			if blockAnnouncement.Block.Hash() != hashes[0].Hash {
				return fmt.Errorf("wrong block hash in announcement: expected %v, got %v",
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block) error {
	...
	for {
		...
		if !reflect.DeepEqual(block.Header(), headers[0]) {
			return fmt.Errorf("wrong header returned: wanted %v, got %v",
				block.Header(), headers[0])
		}
		return nil
	}
}
...
func (s *Suite) oldAnnounce() error {
	...
	case *NewBlock:
		...
		if block.Block.Hash() == oldBlockAnnounce.Block.Hash() {
			return fmt.Errorf("unexpected: block propagated: %s",
				pretty.Sdump(msg))
		}
	case *NewBlockHashes:
		hashes := *msg
		for _, hash := range hashes {
			if hash.Hash == oldBlockAnnounce.Block.Hash() {
				return fmt.Errorf("unexpected: block announced: %s",
					pretty.Sdump(msg))
			}
		}
	case *Error:
...
func (s *Suite) hashAnnounce() error {
	...
	nextBlock := s.fullChain.blocks[s.chain.Len()]
	announcement := anno{Hash: nextBlock.Hash(),
	...
	if blockHeaderReq.Amount != 1 {
		return fmt.Errorf("unexpected number of block headers requested: %v",
	...
	case *NewBlockHashes:
		hashes := *msg
		if len(hashes) != 1 {
			return fmt.Errorf("unexpected new block hash announcement: wanted 1 announcement, got %d",
				len(hashes))
		}
		if nextBlock.Hash() != hashes[0].Hash {
			return fmt.Errorf("unexpected block hash announcement, wanted %v, got %v",
				nextBlock.Hash(),
				hashes[0].Hash)
		}
	...
	case *NewBlock:
		...
		if len(nextBlockBody.Transactions) != 0 || len(nextBlockBody.Uncles) != 0 {
			return fmt.Errorf("unexpected non-empty new block propagated: %s",
				pretty.Sdump(msg))
		}
		...
		if !reflect.DeepEqual(nextBlock.Header(), msg.Block.Header()) {
			return fmt.Errorf("incorrect header received: wanted %v, got %v",
				nextBlock.Header(), msg.Block.Header())
		}
	default:
		return fmt.Errorf("unexpected: %s", pretty.Sdump(msg))
	}

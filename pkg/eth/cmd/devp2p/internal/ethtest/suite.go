...
func (s *Suite) TestLargeTxRequest(t *utesting.T) {
	...
	switch msg := conn.waitForResponse(s.chain, timeout,
		getTxReq.RequestId).(type) {
...
func (s *Suite) TestNewPooledTxs(t *utesting.T) {
	...
	var ann Message = NewPooledTransactionHashes{Types: types, Sizes: sizes,
		Hashes: hashes}
	...
	for {
		...
		case *GetPooledTransactions:
			if len(msg.GetPooledTransactionsRequest) != len(hashes) {
				t.Fatalf("unexpected number of txs requested: wanted %d, got %d",
					len(hashes), len(msg.GetPooledTransactionsRequest))
			}
			return

...
func sendSuccessfulTx(s *Suite, tx *types.Transaction,
	prevTx *types.Transaction) error {
	...
	for {
		...
		case *Transactions:
			...
			return fmt.Errorf("missing transaction: got %v missing %v", recTxs,
				tx.Hash())
		case *NewPooledTransactionHashes66:
			...
			return fmt.Errorf("missing transaction announcement: got %v missing %v",
				txHashes, tx.Hash())
		case *NewPooledTransactionHashes:
			txHashes := msg.Hashes
			if len(txHashes) != len(msg.Sizes) {
				return fmt.Errorf("invalid msg size lengths: hashes: %v sizes: %v",
					len(txHashes), len(msg.Sizes))
			}
			if len(txHashes) != len(msg.Types) {
				return fmt.Errorf("invalid msg type lengths: hashes: %v types: %v",
					len(txHashes), len(msg.Types))
			}
			// if you receive an old tx propagation, read from connection again
			if len(txHashes) == 1 && prevTx != nil {
				if txHashes[0] == prevTx.Hash() {
			...
			for index, gotHash := range txHashes {
				if gotHash == tx.Hash() {
					if msg.Sizes[index] != uint32(tx.Size()) {
						return fmt.Errorf("invalid tx size: got %v want %v",
							msg.Sizes[index], tx.Size())
					}
					if msg.Types[index] != tx.Type() {
						return fmt.Errorf("invalid tx type: got %v want %v",
							msg.Types[index], tx.Type())
					}
					// Ok
					return nil
				}
		...
		default:
			return fmt.Errorf("unexpected message in sendSuccessfulTx: %s",
				pretty.Sdump(msg))
		}
	}
}
...
func sendMultipleSuccessfulTxs(t *utesting.T, s *Suite,
	...
	for i := 0; i < 20; i++ {
		...
		default:
			if !strings.Contains(pretty.Sdump(msg), "i/o timeout") {
				return fmt.Errorf("unexpected message while waiting to receive txs: %s",
					pretty.Sdump(msg))
			}
		}
...
func checkMaliciousTxPropagation(s *Suite, txs []*types.Transaction,
	...
	case *Error:
		...
		return nil
	default:
		return fmt.Errorf("unexpected message in sendFailingTx: %s",
			pretty.Sdump(msg))
	}
	return nil
}
...
func compareReceivedTxs(recvTxs []common.Hash,
	txs []*types.Transaction) (present []*types.Transaction,
...
func unknownTx(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce()+1, to, tx.Value(), tx.Gas(),
		tx.GasPrice(), tx.Data())
...
func generateTxs(s *Suite, numTxs int) (map[common.Hash]common.Hash,
...
func generateTx(chainConfig *params.ChainConfig, nonce uint64,
	gas uint64) *types.Transaction {
	var to common.Address
	tx := types.NewTransaction(nonce, to, big.NewInt(1), gas, big.NewInt(1),
		[]byte{})
...
func invalidNonceTx(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce()-2, to, tx.Value(), tx.Gas(),
		tx.GasPrice(), tx.Data())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
...
func hugeAmount(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, amount, tx.Gas(),
		tx.GasPrice(), tx.Data())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
...
func hugeGasPrice(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, tx.Value(), tx.Gas(),
		gasPrice, tx.Data())
	return signWithFaucet(s.chain.chainConfig, txNew)
}
...
func hugeData(s *Suite) *types.Transaction {
	...
	txNew := types.NewTransaction(tx.Nonce(), to, tx.Value(), tx.Gas(),
		tx.GasPrice(), largeBuffer(2))
	return signWithFaucet(s.chain.chainConfig, txNew)
}

func signWithFaucet(chainConfig *params.ChainConfig,

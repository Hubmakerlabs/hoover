...
func (te *testenv) checkPingPong(pingHash []byte) error {
	...
	for i := 0; i < 2; i++ {
		...
		case v4wire.PingPacket:
			pings++
		default:
			return fmt.Errorf("expected PING or PONG, got %v %v", reply.Name(),
				reply)
		}
	}
	if pongs == 1 && pings == 1 {
	...
	return fmt.Errorf("expected 1 PING  (got %d) and 1 PONG (got %d)", pings,
		pongs)
}
...
func (te *testenv) checkPong(reply v4wire.Packet, pingHash []byte) error {
	...
	if !bytes.Equal(pong.ReplyTok, pingHash) {
		return fmt.Errorf("PONG reply token mismatch: got %x, want %x",
			pong.ReplyTok, pingHash)
	}
	if want := te.localEndpoint(te.l1); !want.IP.Equal(pong.To.IP) || want.UDP != pong.To.UDP {
		return fmt.Errorf("PONG 'to' endpoint mismatch: got %+v, want %+v",
			pong.To, want)
	}
...
var AllTests = []utesting.Test{
	...
	{Name: "Amplification/InvalidPongHash",
		Fn: FindnodeAmplificationInvalidPongHash},

...
func checkPong(t *utesting.T, pong *v5wire.Pong, ping *v5wire.Ping,
	c net.PacketConn) {
	...
	if !pong.ToIP.Equal(laddr(c).IP) {
		t.Fatalf("wrong destination IP %v in PONG, want %v", pong.ToIP,
			laddr(c).IP)
	}
	if int(pong.ToPort) != laddr(c).Port {
		t.Fatalf("wrong destination port %v in PONG, want %v", pong.ToPort,
			laddr(c).Port)
	}
}
...
func (s *Suite) TestPingMultiIP(t *utesting.T) {
	...
	case *v5wire.Pong:
		t.Fatalf("remote responded to PING from %v for session on IP %v",
			laddr(l2).IP, laddr(l1).IP)
...
func (s *Suite) TestTalkRequest(t *utesting.T) {
	...
	id := conn.nextReqID()
	resp := conn.reqresp(l1,
		&v5wire.TalkRequest{ReqID: id, Protocol: "test-protocol"})
	...
	case *v5wire.TalkResponse:
		if len(resp.ReqID) > 0 {
			t.Fatalf("wrong request ID %x in TALKRESP, want empty byte array",
				resp.ReqID)
		}
		if len(resp.Message) > 0 {
			t.Fatalf("non-empty message %x in TALKRESP", resp.Message)
...
func (s *Suite) TestFindnodeZeroDistance(t *utesting.T) {
	...
	if nodes[0].ID() != conn.remote.ID() {
		t.Errorf("ID of response node is %v, want %v", nodes[0].ID(),
			conn.remote.ID())
	}
}
...
func (s *Suite) TestFindnodeResults(t *utesting.T) {
	...
	for count := 0; count < len(nodes); {
		...
		case id := <-added:
			...
			count++
		case <-timeoutCh:
			t.Errorf("remote added %d bystander nodes in %v, need %d to continue",
				count, timeout, len(nodes))
	...
	t.Logf("remote returned %d nodes for distance list %v", len(foundNodes),
		dists)
...
func (bn *bystander) loop() {
	...
	for {
		...
		case *v5wire.Findnode:
			bn.conn.write(bn.l, &v5wire.Nodes{ReqID: p.ReqID, RespCount: 1},
				nil)
			wasAdded = true
			bn.notifyAdded()

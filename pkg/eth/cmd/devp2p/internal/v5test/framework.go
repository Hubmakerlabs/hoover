...
func (tc *conn) reqresp(c net.PacketConn, req v5wire.Packet) v5wire.Packet {
	...
	case *v5wire.Whoareyou:
		if resp.Nonce != reqnonce {
			return readErrorf("wrong nonce %x in WHOAREYOU (want %x)",
				resp.Nonce[:], reqnonce[:])
		}
		resp.Node = tc.remote
		tc.write(c, req, resp)
...
func (tc *conn) findnode(c net.PacketConn, dists []uint) ([]*enode.Node,
	error) {
	var (
	...
	for n := 1; n > 0; {
		...
		case *v5wire.Whoareyou:
			...
			if resp.Nonce == reqnonce {
				...
				return nil, fmt.Errorf("unexpected WHOAREYOU (nonce %x), waiting for NODES",
					resp.Nonce[:])
			}
		case *v5wire.Ping:
		...
		case *v5wire.Nodes:
			...
			if !bytes.Equal(resp.ReqID, findnode.ReqID) {
				return nil, fmt.Errorf("NODES response has wrong request id %x",
					resp.ReqID)
			}
			...
			if first {
				if resp.RespCount == 0 || resp.RespCount > 6 {
					return nil, fmt.Errorf("invalid NODES response count %d (not in (0,7))",
						resp.RespCount)
				}
				...
				if resp.RespCount != total {
					return nil, fmt.Errorf("invalid NODES response count %d (!= %d)",
						resp.RespCount, total)
				}
			}
			// Check nodes.
			nodes, err := checkRecords(resp.Nodes)
			if err != nil {
				return nil, fmt.Errorf("invalid node in NODES response: %v",
					err)
			}
...
func (tc *conn) write(c net.PacketConn, p v5wire.Packet,
	challenge *v5wire.Whoareyou) v5wire.Nonce {
	packet, nonce, err := tc.codec.Encode(tc.remote.ID(),
		tc.remoteAddr.String(), p, challenge)
	if err != nil {
...
func (tc *conn) logf(format string, args ...interface{}) {
	if tc.log != nil {
		tc.log.Logf("(%s) %s", tc.localNode.ID().TerminalString(),

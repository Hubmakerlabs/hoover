...
func showAttributeCounts(ns nodeSet) {
	...
	for _, key := range keys {
		fmt.Printf("%s%s: %d\n", strings.Repeat(" ", maxlength-len(key)+1), key,
			attrcount[key])
	}
}
...
func parseFilters(args []string) ([]nodeFilter, error) {
	...
	for len(args) > 0 {
		...
		if len(args)-1 < fc.narg {
			return nil, fmt.Errorf("filter %q wants %d arguments, have %d",
				args[0], fc.narg, len(args)-1)
		}
...
func ethFilter(args []string) (nodeFilter, error) {
	...
	switch args[0] {
	case "mainnet":
		filter = forkid.NewStaticFilter(params.MainnetChainConfig,
			core.DefaultGenesisBlock().ToBlock())
	case "goerli":
		filter = forkid.NewStaticFilter(params.GoerliChainConfig,
			core.DefaultGoerliGenesisBlock().ToBlock())
	case "sepolia":
		filter = forkid.NewStaticFilter(params.SepoliaChainConfig,
			core.DefaultSepoliaGenesisBlock().ToBlock())
	case "holesky":
		filter = forkid.NewStaticFilter(params.HoleskyChainConfig,

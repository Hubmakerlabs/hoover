...
`,
	...
	Action: func(ctx *cli.Context) error {
		...
		if err != nil {
			utils.Fatalf("Failed to read the keyfile at '%s': %v", keyfilepath,
				err)
		}
		...
		signature, err := crypto.Sign(accounts.TextHash(message),
			key.PrivateKey)
		if err != nil {
...
It is possible to refer to a file containing the message.`,
	...
	Action: func(ctx *cli.Context) error {
		...
		recoveredPubkey, err := crypto.SigToPub(accounts.TextHash(message),
			signature)
...
func getMessage(ctx *cli.Context, msgarg int) []byte {
	...
	utils.Fatalf("Invalid number of arguments: want %d, got %d", msgarg+1,
		ctx.NArg())
	return nil
}

...
func (i *bbInput) ToBlock() *types.Block {
	...
	return types.NewBlockWithHeader(header).WithBody(i.Txs,
		i.Ommers).WithWithdrawals(i.Withdrawals)
}
...
func (i *bbInput) sealClique(block *types.Block) (*types.Block, error) {
	...
	if i.Header.Extra != nil {
		return nil, NewError(ErrorConfig,
			errors.New("sealing with clique will overwrite provided extra data"))
	}
	header := block.Header()
	if i.Clique.Voted != nil {
		if i.Header.Coinbase != nil {
			return nil, NewError(ErrorConfig,
				errors.New("sealing with clique and voting will overwrite provided coinbase"))
		}
	...
	if i.Clique.Authorize != nil {
		if i.Header.Nonce != nil {
			return nil, NewError(ErrorConfig,
				errors.New("sealing with clique and voting will overwrite provided nonce"))
		}
		if *i.Clique.Authorize {
			header.Nonce = [8]byte{}
		} else {
			header.Nonce = [8]byte{0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
				0xff}
		}
	}
...
func readInput(ctx *cli.Context) (*bbInput, error) {
	...
	if withdrawalsStr != stdinSelector && withdrawalsStr != "" {
		...
		if err := readFile(withdrawalsStr, "withdrawals",
			&withdrawals); err != nil {
			return nil, err
		}
	...
	if inputData.TxRlp != "" {
		if err := rlp.DecodeBytes(common.FromHex(inputData.TxRlp),
			&txs); err != nil {
			return nil, NewError(ErrorRlp,
				fmt.Errorf("unable to decode transaction from rlp data: %v",
					err))
		}
		inputData.Txs = txs
	}
	for _, str := range inputData.OmmersRlp {
		...
		if err := rlp.DecodeBytes(common.FromHex(str), &ommer); err != nil {
			return nil, NewError(ErrorRlp,
				fmt.Errorf("unable to decode ommer from rlp data: %v", err))
		}

...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/tests"
...
func Transition(ctx *cli.Context) error {
	...
	glogger := log.NewGlogHandler(log.StreamHandler(os.Stderr,
	...
	if err != nil {
		return NewError(ErrorIO,
			fmt.Errorf("failed creating output basedir: %v", err))
	}
	if ctx.Bool(TraceFlag.Name) {
		if ctx.IsSet(TraceDisableMemoryFlag.Name) && ctx.IsSet(TraceEnableMemoryFlag.Name) {
			return NewError(ErrorConfig,
				fmt.Errorf("can't use both flags --%s and --%s",
					TraceDisableMemoryFlag.Name, TraceEnableMemoryFlag.Name))
		}
		if ctx.IsSet(TraceDisableReturnDataFlag.Name) && ctx.IsSet(TraceEnableReturnDataFlag.Name) {
			return NewError(ErrorConfig,
				fmt.Errorf("can't use both flags --%s and --%s",
					TraceDisableReturnDataFlag.Name,
					TraceEnableReturnDataFlag.Name))
		}
		if ctx.IsSet(TraceDisableMemoryFlag.Name) {
			log.Warn(fmt.Sprintf("--%s has been deprecated in favour of --%s",
				TraceDisableMemoryFlag.Name, TraceEnableMemoryFlag.Name))
		}
		...
		getTracer = func(txIndex int, txHash common.Hash) (vm.EVMLogger,
			error) {
			if prevFile != nil {
				prevFile.Close()
			}
			traceFile, err := os.Create(path.Join(baseDir,
				fmt.Sprintf("trace-%d-%v.jsonl", txIndex, txHash.String())))
			if err != nil {
				return nil, NewError(ErrorIO,
					fmt.Errorf("failed creating trace-file: %v", err))
			}
			prevFile = traceFile
			return logger.NewJSONLogger(logConfig, traceFile), nil
		}
	} else {
		getTracer = func(txIndex int, txHash common.Hash) (tracer vm.EVMLogger,
			err error) {
			return nil, nil
		}
	}
	...
	if allocStr == stdinSelector || envStr == stdinSelector || txStr == stdinSelector {
		...
		if err := decoder.Decode(inputData); err != nil {
			return NewError(ErrorJson,
				fmt.Errorf("failed unmarshaling stdin: %v", err))
		}
	}
	...
	if cConf, extraEips, err := tests.GetChainConfig(ctx.String(ForknameFlag.Name)); err != nil {
		return NewError(ErrorConfig,
			fmt.Errorf("failed constructing chain configuration: %v", err))
	} else {
		chainConfig = cConf
	...
	if txIt, err = loadTransactions(txStr, inputData, prestate.Env,
		chainConfig); err != nil {
		return err
	}
	...
	s, result, body, err := prestate.Apply(vmConfig, chainConfig, txIt,
		ctx.Int64(RewardFlag.Name), getTracer)
	if err != nil {
		return err
	}
...
func applyLondonChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	...
	if env.ParentBaseFee == nil || env.Number == 0 {
		return NewError(ErrorConfig,
			errors.New("EIP-1559 config but missing 'currentBaseFee' in env section"))
	}
...
func applyShanghaiChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	...
	if env.Withdrawals == nil {
		return NewError(ErrorConfig,
			errors.New("Shanghai config but missing 'withdrawals' in env section"))
	}
	return nil
}
...
func applyMergeChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	...
	if !isMerged {
		...
		case env.Number == 0:
			return NewError(ErrorConfig,
				errors.New("currentDifficulty needs to be provided for block number 0"))
		case env.Timestamp <= env.ParentTimestamp:
			return NewError(ErrorConfig,
				fmt.Errorf("currentDifficulty cannot be calculated -- currentTime (%d) needs to be after parent time (%d)",
	...
	case env.Random == nil:
		return NewError(ErrorConfig,
			errors.New("post-merge requires currentRandom to be defined in env"))
	case env.Difficulty != nil && env.Difficulty.BitLen() != 0:
		return NewError(ErrorConfig,
			errors.New("post-merge difficulty must be zero (or omitted) in env"))
	}
	env.Difficulty = nil
	return nil
}
...
func applyCancunChecks(env *stEnv, chainConfig *params.ChainConfig) error {
	...
	if env.ParentBeaconBlockRoot == nil {
		return NewError(ErrorConfig,
			errors.New("post-cancun env requires parentBeaconBlockRoot to be set"))
	}
	return nil
}
...
func saveFile(baseDir, filename string, data interface{}) error {
	...
	if err != nil {
		return NewError(ErrorJson,
			fmt.Errorf("failed marshalling output: %v", err))
	}
...
// files
func dispatchOutput(ctx *cli.Context, baseDir string, result *ExecutionResult,
	alloc Alloc, body hexutil.Bytes) error {
	...
	if err := dispatch(baseDir, ctx.String(OutputAllocFlag.Name), "alloc",
		alloc); err != nil {
		return err
	}
	if err := dispatch(baseDir, ctx.String(OutputResultFlag.Name), "result",
		result); err != nil {
		return err
	}
	if err := dispatch(baseDir, ctx.String(OutputBodyFlag.Name), "body",
		body); err != nil {
		return err
	}

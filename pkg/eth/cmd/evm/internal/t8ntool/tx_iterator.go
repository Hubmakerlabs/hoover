...
func signUnsignedTransactions(txs []*txWithKey,
	signer types.Signer) (types.Transactions, error) {
	...
	for i, tx := range txs {
		...
		if err != nil {
			return nil, NewError(ErrorJson,
				fmt.Errorf("tx %d: failed to sign tx: %v", i, err))
		}
...
func loadTransactions(txStr string, inputData *input, env stEnv,
	chainConfig *params.ChainConfig) (txIterator, error) {
	...
	if txStr != stdinSelector {
		...
		if err := json.Unmarshal(data, &txsWithKeys); err != nil {
			return nil, NewError(ErrorJson,
				fmt.Errorf("failed unmarshaling txs-file: %v", err))
		}
	} else {

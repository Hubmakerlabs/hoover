...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/cmd/evm/internal/t8ntool"
	"github.com/Hubmakerlabs/hoover/pkg/eth/internal/cmdtest"
...
func TestT8n(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		if tc.expOut != "" {
			...
			switch {
			case err != nil:
				t.Fatalf("test %d, file %v: json parsing failed: %v", i, file,
					err)
			case !ok:
		...
		if have, want := tt.ExitStatus(), tc.expExitCode; have != want {
			t.Fatalf("test %d: wrong exit code, have %d, want %d", i, have,
				want)
		}
	}
}

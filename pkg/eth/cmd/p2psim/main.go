...
func listNodes(ctx *cli.Context) error {
	...
	for _, node := range nodes {
		fmt.Fprintf(w, "%s\t%s\t%s\n", node.Name,
			strings.Join(protocolList(node), ","), node.ID)
	}
	return nil
}
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool(subscribeFlag.Name) {
		return rpcSubscribe(rpcClient, ctx.App.Writer, method,
			args.Slice()[3:]...)
	}
...
func rpcSubscribe(client *rpc.Client, out io.Writer, method string,
	args ...string) error {
	...
	sub, err := client.Subscribe(context.Background(), namespace, ch,
		subArgs...)
	if err != nil {
		return err
	}

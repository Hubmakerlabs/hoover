...
func testExport(t *testing.T, f string) {
	err := ExportChaindata(f, "testdata", newTestIterator(),
		make(chan struct{}))
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < 1000; i++ {
		...
		if !(i < 5 || i == 42) {
			...
			if have, want := string(v), fmt.Sprintf("value %d",
				i); have != want {
...
func testDeletion(t *testing.T, f string) {
	err := ExportChaindata(f, "testdata", newDeletionIterator(),
		make(chan struct{}))
	if err != nil {
		t.Fatal(err)
	}
	db := rawdb.NewMemoryDatabase()
	for i := 0; i < 1000; i++ {
		db.Put([]byte(fmt.Sprintf("key-%04d", i)),

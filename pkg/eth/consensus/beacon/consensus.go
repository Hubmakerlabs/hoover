...
func (beacon *Beacon) VerifyHeader(chain consensus.ChainHeaderReader,
	...
	reached, err := IsTTDReached(chain, header.ParentHash,
...
func (beacon *Beacon) splitHeaders(chain consensus.ChainHeaderReader,
	headers []*types.Header) ([]*types.Header, []*types.Header, error) {
...
func (beacon *Beacon) VerifyHeaders(chain consensus.ChainHeaderReader,
	...
	go func() {
		var (
			...
			newDone, newResult = beacon.verifyHeaders(chain, postHeaders,
				preHeaders[len(preHeaders)-1])
		)
		// Collect the results
		for {
			for ; done[out]; out++ {
...
func (beacon *Beacon) VerifyUncles(chain consensus.ChainReader,
...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader,
	...
	if len(header.Extra) > 32 {
		return fmt.Errorf("extra-data longer than 32 bytes (%d)",
			len(header.Extra))
	}
	...
	if diff := new(big.Int).Sub(header.Number,
		parent.Number); diff.Cmp(common.Big1) != 0 {
		return consensus.ErrInvalidNumber
	}
	// Verify the header's EIP-1559 attributes.
	if err := eip1559.VerifyEIP1559Header(chain.Config(), parent,
		header); err != nil {
		return err
	}
	...
	if !shanghai && header.WithdrawalsHash != nil {
		return fmt.Errorf("invalid withdrawalsHash: have %x, expected nil",
	...
	if !cancun {
		...
		case header.ExcessBlobGas != nil:
			return fmt.Errorf("invalid excessBlobGas: have %d, expected nil",
				header.ExcessBlobGas)
		case header.BlobGasUsed != nil:
			return fmt.Errorf("invalid blobGasUsed: have %d, expected nil",
				header.BlobGasUsed)
		case header.ParentBeaconRoot != nil:
			return fmt.Errorf("invalid parentBeaconRoot, have %#x, expected nil",
...
func (beacon *Beacon) verifyHeaders(chain consensus.ChainHeaderReader,
	headers []*types.Header, ancestor *types.Header) (chan<- struct{},
	<-chan error) {
	var (
		abort   = make(chan struct{})
		results = make(chan error, len(headers))
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader,
	...
	if !beacon.IsPoSHeader(header) {
		return beacon.ethone.FinalizeAndAssemble(chain, header, state, txs,
			uncles, receipts, nil)
	}
	shanghai := chain.Config().IsShanghai(header.Number, header.Time)
	if shanghai {
	...
	return types.NewBlockWithWithdrawals(header, txs, uncles, receipts,
		withdrawals, trie.NewStackTrie(nil)), nil
}
...
func (beacon *Beacon) Seal(chain consensus.ChainHeaderReader,
	block *types.Block, results chan<- *types.Block,
...
func (beacon *Beacon) CalcDifficulty(chain consensus.ChainHeaderReader,
	...
	if reached, _ := IsTTDReached(chain, parent.Hash(),
		parent.Number.Uint64()); !reached {
...
func IsTTDReached(chain consensus.ChainHeaderReader, parentHash common.Hash,
	parentNumber uint64) (bool, error) {

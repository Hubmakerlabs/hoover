...
func TestReimportMirroredState(t *testing.T) {
	...
	chain, _ := core.NewBlockChain(rawdb.NewMemoryDatabase(), nil, genspec, nil,
		engine, vm.Config{}, nil, nil)
	defer chain.Stop()

	_, blocks, _ := core.GenerateChainWithGenesis(genspec, engine, 3,
		func(i int, block *core.BlockGen) {
			...
			if i != 1 {
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(addr),
					common.Address{0x00}, new(big.Int), params.TxGas,
					block.BaseFee(), nil), signer, key)
				if err != nil {
					panic(err)
				}
	...
	db = rawdb.NewMemoryDatabase()
	chain, _ = core.NewBlockChain(db, nil, genspec, nil, engine, vm.Config{},
		nil, nil)
	defer chain.Stop()

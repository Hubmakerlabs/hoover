...
// the genesis block.
func newSnapshot(config *params.CliqueConfig, sigcache *sigLRU, number uint64,
	hash common.Hash, signers []common.Address) *Snapshot {
	snap := &Snapshot{
		config:   config,
		sigcache: sigcache,
		Number:   number,
...
func loadSnapshot(config *params.CliqueConfig, sigcache *sigLRU,
...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for i, header := range headers {
			...
			if tally.Authorize {
				...
				for i := 0; i < len(snap.Votes); i++ {
					if snap.Votes[i].Signer == header.Coinbase {
						// Uncast the vote from the cached tally
						snap.uncast(snap.Votes[i].Address,
							snap.Votes[i].Authorize)
		...
		if time.Since(logged) > 8*time.Second {
			log.Info("Reconstructing voting history", "processed", i, "total",
				len(headers), "elapsed",
	...
	if time.Since(start) > 8*time.Second {
		log.Info("Reconstructed voting history", "processed", len(headers),

...
func (ap *testerAccountPool) checkpoint(header *types.Header,
	signers []string) {
	...
	tests := []cliqueTest{
		{
			...
			votes: []testerVote{
				{signer: "A", voted: "F",
					auth: true}, // Authorize F, 3 votes needed
				{signer: "B", voted: "F", auth: true},
				{signer: "C", voted: "F", auth: true},
				{signer: "D", voted: "F",
					auth: false}, // Deauthorize F, 4 votes needed (leave A's previous vote "unchanged")
				{signer: "E", voted: "F", auth: false},
				{signer: "B", voted: "F", auth: false},
				{signer: "C", voted: "F", auth: false},
				{signer: "D", voted: "F",
					auth: true}, // Almost authorize F, 2/3 votes needed
				{signer: "E", voted: "F", auth: true},
				{signer: "B", voted: "A",
					auth: false}, // Deauthorize A, 3 votes needed
				{signer: "C", voted: "A", auth: false},
				{signer: "D", voted: "A", auth: false},
				{signer: "B", voted: "F",
					auth: true}, // Finish authorizing F, 3/3 votes needed
			},
			...
			// batch (https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/17593). Whilst this
...
func (tt *cliqueTest) run(t *testing.T) {
	...
	genesis := &core.Genesis{
		ExtraData: make([]byte,
			extraVanity+common.AddressLength*len(signers)+extraSeal),
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, engine,
		len(tt.votes), func(j int, gen *core.BlockGen) {
	...
	for j, block := range blocks {
		...
		if auths := tt.votes[j].checkpoint; auths != nil {
			header.Extra = make([]byte,
				extraVanity+len(auths)*common.AddressLength+extraSeal)
	...
	chain, err := core.NewBlockChain(rawdb.NewMemoryDatabase(), nil, genesis,
	...
	for j := 0; j < len(result); j++ {
		if !bytes.Equal(result[j][:], signers[j][:]) {
			t.Fatalf("signer %d: signer mismatch: have %x, want %x", j,
				result[j], signers[j])
		}
	}
}

...
func (ethash *Ethash) VerifyHeader(chain consensus.ChainHeaderReader,
...
func (ethash *Ethash) VerifyHeaders(chain consensus.ChainHeaderReader,
...
func (ethash *Ethash) VerifyUncles(chain consensus.ChainReader,
...
func (ethash *Ethash) verifyHeader(chain consensus.ChainHeaderReader,
	header, parent *types.Header, uncle bool, unixNow int64) error {
	...
	if !chain.Config().IsLondon(header.Number) {
		...
		if header.BaseFee != nil {
			return fmt.Errorf("invalid baseFee before fork: have %d, expected 'nil'",
				header.BaseFee)
		}
	...
	if diff := new(big.Int).Sub(header.Number,
		parent.Number); diff.Cmp(big.NewInt(1)) != 0 {
	...
	if err := misc.VerifyDAOHeaderExtraData(chain.Config(),
		header); err != nil {
		return err
	}
	return nil
...
func (ethash *Ethash) CalcDifficulty(chain consensus.ChainHeaderReader,
...
// the difficulty that a new block should have when created at time
// given the parent block's time and difficulty.
func CalcDifficulty(config *params.ChainConfig, time uint64,
...
func makeDifficultyCalculator(bombDelay *big.Int) func(time uint64,
	parent *types.Header) *big.Int {
	...
	return func(time uint64, parent *types.Header) *big.Int {
		...
		if parent.Number.Cmp(bombDelayFromParent) >= 0 {
			fakeBlockNumber = fakeBlockNumber.Sub(parent.Number,
				bombDelayFromParent)
		}
...
func (ethash *Ethash) Prepare(chain consensus.ChainHeaderReader,
	header *types.Header) error {
...
func (ethash *Ethash) Finalize(chain consensus.ChainHeaderReader,
...
func (ethash *Ethash) FinalizeAndAssemble(chain consensus.ChainHeaderReader,
	header *types.Header, state *state.StateDB, txs []*types.Transaction,

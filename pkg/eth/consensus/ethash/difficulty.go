...
func CalcDifficultyFrontierU256(time uint64, parent *types.Header) *big.Int {
	...
	*/
	...
	if periodCount := (parent.Number.Uint64() + 1) / expDiffPeriodUint; periodCount > 1 {
		...
		expDiff.Lsh(expDiff,
			uint(periodCount-2)) // expdiff: 2 ^ (periodCount -2)
...
func CalcDifficultyHomesteadU256(time uint64, parent *types.Header) *big.Int {
	...
	*/
	...
	adjust.Mul(adjust,
		z) // adjust: (pdiff / 2048) * max((time - ptime) / 10 - 1, 99)
	if neg {
		pDiff.Sub(pDiff,
			adjust) // pdiff - pdiff / 2048 * max((time - ptime) / 10 - 1, 99)
	} else {
		pDiff.Add(pDiff,
			adjust) // pdiff + pdiff / 2048 * max((time - ptime) / 10 - 1, 99)
	}
...
func MakeDifficultyCalculatorU256(bombDelay *big.Int) func(time uint64,
	...
	return func(time uint64, parent *types.Header) *big.Int {
		...
		*/
		...
		z.Mul(y,
			z) // z: (p_diff / 2048 ) * (+- adj_factor)

		if xNeg {
			y.Sub(pDiff,
				z) // y: parent_diff + parent_diff/2048 * adjustment_factor
		} else {
			y.Add(pDiff,
				z) // y: parent_diff + parent_diff/2048 * adjustment_factor
		}

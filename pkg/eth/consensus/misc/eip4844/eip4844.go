...
func VerifyEIP4844Header(parent, header *types.Header) error {
	...
	if *header.BlobGasUsed > params.MaxBlobGasPerBlock {
		return fmt.Errorf("blob gas used %d exceeds maximum allowance %d",
			*header.BlobGasUsed, params.MaxBlobGasPerBlock)
	}
	if *header.BlobGasUsed%params.BlobTxBlobGasPerBlob != 0 {
		return fmt.Errorf("blob gas used %d not a multiple of blob gas per blob %d",
	...
	expectedExcessBlobGas := CalcExcessBlobGas(parentExcessBlobGas,
		parentBlobGasUsed)
	if *header.ExcessBlobGas != expectedExcessBlobGas {
		return fmt.Errorf("invalid excessBlobGas: have %d, want %d, parent excessBlobGas %d, parent blobDataUsed %d",
			*header.ExcessBlobGas, expectedExcessBlobGas, parentExcessBlobGas,
			parentBlobGasUsed)
	}
	return nil
}
...
func CalcExcessBlobGas(parentExcessBlobGas uint64,
	parentBlobGasUsed uint64) uint64 {
...
func CalcBlobFee(excessBlobGas uint64) *big.Int {
	return fakeExponential(minBlobGasPrice,
		new(big.Int).SetUint64(excessBlobGas), blobGaspriceUpdateFraction)
}

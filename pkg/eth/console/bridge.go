...
func newBridge(client *rpc.Client, prompter prompt.UserPrompter,
	printer io.Writer) *bridge {
	return &bridge{
		client:   client,
		prompter: prompter,
		printer:  printer,
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	case strings.HasSuffix(err.Error(),
		scwallet.ErrPairingPasswordNeeded.Error()):
		...
		if val, err = openWallet(goja.Null(), wallet, passwd); err != nil {
			...
			if val, err = openWallet(goja.Null(), wallet,
				call.VM.ToValue(input)); err != nil {
				return nil, err
			}
		}
	...
	case strings.HasSuffix(err.Error(), scwallet.ErrPINUnblockNeeded.Error()):
		...
		if val, err = openWallet(goja.Null(), wallet,
			call.VM.ToValue(pukpin)); err != nil {
			return nil, err
		}
...
func (b *bridge) readPassphraseAndReopenWallet(call jsre.Call) (goja.Value,
	error) {
	wallet := call.Argument(0)
	...
	for _, req := range reqs {
		...
		if err = b.client.Call(&result, req.Method, req.Params...); err == nil {
			if result == nil {
				...
				resultVal, err := parse(goja.Null(),
					call.VM.ToValue(string(result)))
				if err != nil {

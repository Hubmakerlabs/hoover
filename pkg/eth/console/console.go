...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/internal/web3ext"
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string,
	string) {
...
func (c *Console) readLines(input chan<- string, errc chan<- error,
	prompt <-chan string) {
	for p := range prompt {
...
func (c *Console) writeHistory() error {
	...
	return os.Chmod(c.histPath,
		0600) // Force 0600, even if it was different previously
}

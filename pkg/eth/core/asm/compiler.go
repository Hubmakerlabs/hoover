...
func (c *Compiler) compileLine() error {
	...
	case lineEnd:
		return nil
	default:
		return compileErr(lvalue, lvalue.text,
			fmt.Sprintf("%v or %v", labelDef, element))
	}
...
func (c *Compiler) compilePush() error {
	...
	if len(value) > 32 {
		return fmt.Errorf("%d: string or number size > 32 bytes",
			rvalue.lineno+1)
	}
...
func (err compileError) Error() string {
	return fmt.Sprintf("%d: syntax error: unexpected %v, expected %v",
		err.lineno, err.got, err.want)
}

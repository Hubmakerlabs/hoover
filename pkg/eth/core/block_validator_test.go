...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	var (
		...
		_, blocks, _ = GenerateChainWithGenesis(gspec, ethash.NewFaker(), 8,
			nil)
	)
	...
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(),
	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				_, results = engine.VerifyHeaders(chain,
					[]*types.Header{headers[i]})
			} else {
			...
			case result := <-results:
				if (result == nil) != valid {
					t.Errorf("test %d.%d: validity mismatch: have %v, want %v",
						i, j, result, valid)
				}
			...
			select {
			case result := <-results:
				t.Fatalf("test %d.%d: unexpected result returned: %v", i, j,
					result)
...
func TestHeaderVerificationForMergingClique(t *testing.T) {
	testHeaderVerificationForMerging(t,
		true)
}
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	if isClique {
		...
		engine = beacon.New(clique.New(params.AllCliqueProtocolChanges.Clique,
		...
		postBlocks, _ = GenerateChain(gspec.Config, preBlocks[len(preBlocks)-1],
			engine, genDb, 8, nil)
	} else {
		...
		postBlocks, _ = GenerateChain(gspec.Config, preBlocks[len(preBlocks)-1],
			engine, genDb, 8, func(i int, gen *BlockGen) {
				gen.SetPoS()
			})
	}
	...
	// Run the header checker for blocks one-by-one, checking for both valid and invalid nonces
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil,
...
func TestCalcGasLimit(t *testing.T) {
	for i, tc := range []struct {
		...
		// Increase
		if have, want := CalcGasLimit(tc.pGasLimit,
			2*tc.pGasLimit), tc.max; have != want {
		...
		// Small decrease
		if have, want := CalcGasLimit(tc.pGasLimit,
			tc.pGasLimit-1), tc.pGasLimit-1; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
		// Small increase
		if have, want := CalcGasLimit(tc.pGasLimit,
		...
		// No change
		if have, want := CalcGasLimit(tc.pGasLimit,
			tc.pGasLimit), tc.pGasLimit; have != want {
			t.Errorf("test %d: have %d want %d", i, have, want)
		}
	}
}

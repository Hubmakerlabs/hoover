...
func (st *insertStats) report(chain []*types.Block, index int,
	snapDiffItems, snapBufItems, trieDiffNodes, triebufNodes common.StorageSize,
	setHead bool) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			...
			"blocks", st.processed, "txs", txs, "mgas",
			float64(st.usedGas) / 1000000,
			"elapsed", common.PrettyDuration(elapsed), "mgasps",
			float64(st.usedGas) * 1000 / float64(elapsed),
		}
		if timestamp := time.Unix(int64(end.Time()),
		...
		if snapDiffItems != 0 || snapBufItems != 0 { // snapshots enabled
			context = append(context,
				[]interface{}{"snapdiffs", snapDiffItems}...)
			if snapBufItems != 0 { // future snapshot refactor
				context = append(context,
					[]interface{}{"snapdirty", snapBufItems}...)
			}
		}
		if trieDiffNodes != 0 { // pathdb
			context = append(context,
				[]interface{}{"triediffs", trieDiffNodes}...)
		}
...
func newInsertIterator(chain types.Blocks, results <-chan error,
	validator Validator) *insertIterator {

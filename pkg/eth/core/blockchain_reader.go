...
func (bc *BlockChain) GetBlocksFromHash(hash common.Hash,
	n int) (blocks []*types.Block) {
	number := bc.hc.GetBlockNumber(hash)
	if number == nil {
		return nil
	}
...
func (bc *BlockChain) GetReceiptsByHash(hash common.Hash) types.Receipts {
	...
	receipts := rawdb.ReadReceipts(bc.db, hash, *number, header.Time,
		bc.chainConfig)
	if receipts == nil {
		return nil
	}
...
func (bc *BlockChain) GetUnclesInChain(block *types.Block,
	length int) []*types.Header {
	uncles := []*types.Header{}
...
func (bc *BlockChain) GetTransactionLookup(hash common.Hash) *rawdb.LegacyTxLookupEntry {
	...
	lookup := &rawdb.LegacyTxLookupEntry{BlockHash: blockHash,
		BlockIndex: blockNumber, Index: txIndex}
...
func (bc *BlockChain) ContractCodeWithPrefix(hash common.Hash) ([]byte, error) {
	type codeReader interface {
		ContractCodeWithPrefix(address common.Address,
			codeHash common.Hash) ([]byte, error)
	}
	// TODO(rjl493456442) The associated account address is also required
	// in Verkle scheme. Fix it once snap-sync is supported for Verkle.
	return bc.stateCache.(codeReader).ContractCodeWithPrefix(common.Address{},
		hash)
}

...
func TestLongSnapSyncedDeepRepair(t *testing.T) {
	testLongSnapSyncedDeepRepair(t,
		false)
}
...
// syncing and not delete anything, since we can just pick up directly where we
// left off.
func TestLongSnapSyncingDeepRepair(t *testing.T) {
	testLongSnapSyncingDeepRepair(t,
		false)
}
func TestLongSnapSyncingDeepRepairWithSnapshots(t *testing.T) {
	testLongSnapSyncingDeepRepair(t,
		true)
}
...
func TestLongNewerForkedDeepRepair(t *testing.T) {
	testLongNewerForkedDeepRepair(t,
		false)
}
...
func TestLongReorgedShallowRepair(t *testing.T) {
	testLongReorgedShallowRepair(t,
		false)
}
...
func testRepairWithScheme(t *testing.T, tt *rewindTest, snapshots bool,
	scheme string) {
	...
	if tt.commitBlock > 0 {
		if err := chain.triedb.Commit(canonblocks[tt.commitBlock-1].Root(),
			false); err != nil {
	...
	newChain, err := NewBlockChain(db, config, gspec, nil, engine, vm.Config{},
		nil, nil)
	if err != nil {
...
// TestIssue23496 tests scenario described in https://github.com/Hubmakerlabs/hoover/pkg/eth/pull/23496#issuecomment-926393893
...
func testIssue23496(t *testing.T, scheme string) {
	...
	chain, err := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec,

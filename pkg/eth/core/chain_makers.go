...
func (b *BlockGen) SetParentBeaconRoot(root common.Hash) {
	...
	var (
		...
		vmenv        = vm.NewEVM(blockContext, vm.TxContext{}, b.statedb,
			b.cm.config, vm.Config{})
	)
...
func (b *BlockGen) addTx(bc *BlockChain, vmConfig vm.Config,
	tx *types.Transaction) {
	if b.gasPool == nil {
		b.SetCoinbase(common.Address{})
	}
	b.statedb.SetTxContext(tx.Hash(), len(b.txs))
	receipt, err := ApplyTransaction(b.cm.config, bc, &b.header.Coinbase,
		b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, vmConfig)
	if err != nil {
		panic(err)
	}
...
func (b *BlockGen) OffsetTime(seconds int64) {
	...
	b.header.Difficulty = b.engine.CalcDifficulty(b.cm, b.header.Time,
		b.parent.Header())
}
...
func GenerateChain(config *params.ChainConfig, parent *types.Block,
	engine consensus.Engine, db ethdb.Database, n int,
	...
	genblock := func(i int, parent *types.Block, triedb *trie.Database,
		statedb *state.StateDB) (*types.Block, types.Receipts) {
		b := &BlockGen{i: i, cm: cm, parent: parent, statedb: statedb,
			engine: engine}
		...
		block, err := b.engine.FinalizeAndAssemble(cm, b.header, statedb, b.txs,
			b.uncles, b.receipts, b.withdrawals)
		if err != nil {
			panic(err)
		}
	...
	for i := 0; i < n; i++ {
		statedb, err := state.New(parent.Root(),
			state.NewDatabaseWithNodeDB(db, triedb), nil)
		if err != nil {
			panic(err)
		}
		...
		if err := receipts.DeriveFields(config, block.Hash(), block.NumberU64(),
			block.Time(), block.BaseFee(), blobGasPrice, txs); err != nil {
			panic(err)
		}
...
func GenerateChainWithGenesis(genesis *Genesis, engine consensus.Engine, n int,
	gen func(int, *BlockGen)) (ethdb.Database, []*types.Block,
	[]types.Receipts) {
	...
	blocks, receipts := GenerateChain(genesis.Config, genesis.ToBlock(), engine,
		db, n, gen)
	return db, blocks, receipts
}

func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB,
	engine consensus.Engine) *types.Header {
	...
	if cm.config.IsCancun(header.Number, header.Time) {
		...
		excessBlobGas := eip4844.CalcExcessBlobGas(parentExcessBlobGas,
			parentBlobGasUsed)
...
func makeHeaderChain(chainConfig *params.ChainConfig, parent *types.Header,
	n int, engine consensus.Engine, db ethdb.Database,
	seed int) []*types.Header {
	blocks := makeBlockChain(chainConfig, types.NewBlockWithHeader(parent), n,
		engine, db, seed)
...
func makeHeaderChainWithGenesis(genesis *Genesis, n int,
	engine consensus.Engine, seed int) (ethdb.Database, []*types.Header) {
...
func makeBlockChain(chainConfig *params.ChainConfig, parent *types.Block, n int,
	engine consensus.Engine, db ethdb.Database, seed int) []*types.Block {
	blocks, _ := GenerateChain(chainConfig, parent, engine, db, n,
...
func makeBlockChainWithGenesis(genesis *Genesis, n int, engine consensus.Engine,
	seed int) (ethdb.Database, []*types.Block) {
	db, blocks, _ := GenerateChainWithGenesis(genesis, engine, n,
		func(i int, b *BlockGen) {
			b.SetCoinbase(common.Address{0: byte(seed), 19: byte(i)})
		})
	return db, blocks
...
func newChainMaker(bottom *types.Block, config *params.ChainConfig,
	engine consensus.Engine) *chainMaker {
	return &chainMaker{
		bottom:      bottom,
		config:      config,
		engine:      engine,

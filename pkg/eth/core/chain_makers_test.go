...
func TestGeneratePOSChain(t *testing.T) {
	...
	genesis := gspec.MustCommit(gendb,
		trie.NewDatabase(gendb, trie.HashDefaults))

	genchain, genreceipts := GenerateChain(gspec.Config, genesis,
		beacon.NewFaker(), gendb, 4, func(i int, gen *BlockGen) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec, nil, beacon.NewFaker(),
		vm.Config{}, nil, nil)
	...
	for i := range genchain {
		...
		for _, r := range genBlockReceipts {
			if r.BlockNumber.Cmp(block.Number()) != 0 {
				t.Errorf("receipt has wrong block number %d, want %d",
					r.BlockNumber, block.Number())
			}
			if r.BlockHash != block.Hash() {
				t.Errorf("receipt has wrong block hash %v, want %v",
		...
		if !reflect.DeepEqual(genBlockReceipts, blockchainReceipts) {
			t.Fatalf("receipts mismatch\ngenerated: %s\nblockchain: %s",
				spew.Sdump(genBlockReceipts), spew.Sdump(blockchainReceipts))
		}
		...
		for j := 0; j < len(block.Withdrawals()); j++ {
			if block.Withdrawals()[j].Index != withdrawalIndex {
				t.Fatalf("withdrawal index %d does not equal expected index %d",
					block.Withdrawals()[j].Index, withdrawalIndex)
			}
			withdrawalIndex += 1
		}
		...
		if got := block.BeaconRoot(); *got != want {
			t.Fatalf("block %d, wrong parent beacon root: got %s, want %s", i,
				got, want)
		}
		state, _ := blockchain.State()
		idx := block.Time()%8191 + 8191
		got := state.GetState(params.BeaconRootsStorageAddress,
			common.BigToHash(new(big.Int).SetUint64(idx)))
		if got != want {
			t.Fatalf("block %d, wrong parent beacon root in state: got %s, want %s",
				i, got, want)
		}
	}
}
...
func ExampleGenerateChain() {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, ethash.NewFaker(), genDb,
		5, func(i int, gen *BlockGen) {
			switch i {
			case 0:
				// In block 1, addr1 sends addr2 some ether.
				tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1),
					addr2, big.NewInt(10000), params.TxGas, nil, nil), signer,
					key1)
				gen.AddTx(tx)
			case 1:
				// In block 2, addr1 sends some more ether to addr2.
				// addr2 passes it on to addr3.
				tx1, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr1),
					addr2, big.NewInt(1000), params.TxGas, nil, nil), signer,
					key1)
				tx2, _ := types.SignTx(types.NewTransaction(gen.TxNonce(addr2),
					addr3, big.NewInt(1000), params.TxGas, nil, nil), signer,
					key2)
				gen.AddTx(tx1)
				gen.AddTx(tx2)
			case 2:
	...
	blockchain, _ := NewBlockChain(db,
		DefaultCacheConfigWithScheme(rawdb.HashScheme), gspec, nil,

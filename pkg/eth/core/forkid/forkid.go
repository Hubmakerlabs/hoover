...
func NewID(config *params.ChainConfig, genesis *types.Block,
	head, time uint64) ID {
...
func newFilter(config *params.ChainConfig, genesis *types.Block,
	headfn func() (uint64, uint64)) Filter {
	// Calculate the all the valid fork hash and fork next combos
	var (
		forksByBlock, forksByTime = gatherForks(config, genesis.Time())
		forks                     = append(append([]uint64{}, forksByBlock...),
			forksByTime...)
		sums = make([][4]byte,
			len(forks)+1) // 0th is the genesis
	)
	hash := crc32.ChecksumIEEE(genesis.Hash().Bytes())
	...
	if len(forksByTime) == 0 {
		...
		forksByBlock = append(forksByBlock,
			math.MaxUint64) // Last fork will never be passed
	}
...
func gatherForks(config *params.ChainConfig, genesis uint64) ([]uint64,
	[]uint64) {

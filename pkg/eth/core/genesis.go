...
func ReadGenesis(db ethdb.Database) (*Genesis, error) {
	...
	if len(blob) != 0 {
		if err := genesis.Alloc.UnmarshalJSON(blob); err != nil {
			return nil, fmt.Errorf("could not unmarshal genesis state json: %s",
				err)
		}
	}
...
func (ga *GenesisAlloc) flush(db ethdb.Database, triedb *trie.Database,
...
func (h *storageJSON) UnmarshalText(text []byte) error {
	...
	if len(text) > 64 {
		return fmt.Errorf("too many hex characters in storage key/value %q",
			text)
	}
...
func (e *GenesisMismatchError) Error() string {
	return fmt.Sprintf("database contains incompatible genesis (have %x, new %x)",
		e.Stored, e.New)
}
...
func SetupGenesisBlock(db ethdb.Database, triedb *trie.Database,
	genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	return SetupGenesisBlockWithOverride(db, triedb, genesis, nil)
}

func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *trie.Database,
	genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig,
	common.Hash, error) {
...
func LoadChainConfig(db ethdb.Database, genesis *Genesis) (*params.ChainConfig,
	error) {
...
func (g *Genesis) ToBlock() *types.Block {
	...
	return types.NewBlock(head, nil, nil, nil,
		trie.NewStackTrie(nil)).WithWithdrawals(withdrawals)
}

// Commit writes the block and state of a genesis specification to the database.
// The block is committed as the canonical head block.
func (g *Genesis) Commit(db ethdb.Database,
	triedb *trie.Database) (*types.Block, error) {
	block := g.ToBlock()
...
func (g *Genesis) MustCommit(db ethdb.Database,
	triedb *trie.Database) *types.Block {

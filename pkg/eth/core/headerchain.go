...
func NewHeaderChain(chainDb ethdb.Database, config *params.ChainConfig,
	engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	for i, header := range headers {
		...
		if !alreadyKnown {
			...
			inserted = append(inserted,
				rawdb.NumberHash{Number: number, Hash: hash})
...
func (hc *HeaderChain) writeHeadersAndSetHead(headers []*types.Header,
	forker *ForkChoice) (*headerWriteResult, error) {
	...
	if reorg, err := forker.ReorgNeeded(hc.CurrentHeader(),
		lastHeader); err != nil {
		return nil, err
	} else if !reorg {
		if inserted != 0 {
...
func (hc *HeaderChain) ValidateHeaderChain(chain []*types.Header) (int, error) {
	...
	for i := 1; i < len(chain); i++ {
		if chain[i].Number.Uint64() != chain[i-1].Number.Uint64()+1 {
			...
			log.Error("Non contiguous header insert", "number", chain[i].Number,
				"hash", hash,
				"parent", chain[i].ParentHash, "prevnumber", chain[i-1].Number,
				"prevhash", parentHash)

			return 0, fmt.Errorf("non contiguous insert: item %d is #%d [%x..], item %d is #%d [%x..] (parent [%x..])",
				i-1, chain[i-1].Number,
				parentHash.Bytes()[:4], i, chain[i].Number, hash.Bytes()[:4],
				chain[i].ParentHash[:4])
		}
...
// or a side chain.
func (hc *HeaderChain) InsertHeaderChain(chain []*types.Header, start time.Time,
	forker *ForkChoice) (WriteStatus, error) {
	if hc.procInterrupt() {
		return 0, errors.New("aborted")
	}
	...
	if last := res.lastHeader; last != nil {
		...
		if timestamp := time.Unix(int64(last.Time),
...
// caching it if found.
func (hc *HeaderChain) GetHeader(hash common.Hash,
...
func (hc *HeaderChain) GetHeadersFrom(number, count uint64) []rlp.RawValue {
	...
	if count > 0 {
		headers = append(headers,
			rawdb.ReadHeaderRange(hc.chainDb, number, count)...)
	}
	return headers
}
...
type (
	...
	UpdateHeadBlocksCallback func(ethdb.KeyValueWriter,
		*types.Header) (*types.Header, bool)
...
func (hc *HeaderChain) SetHead(head uint64, updateFn UpdateHeadBlocksCallback,
	delFn DeleteBlockContentCallback) {
...
func (hc *HeaderChain) SetHeadWithTimestamp(time uint64,
	updateFn UpdateHeadBlocksCallback, delFn DeleteBlockContentCallback) {
...
func (hc *HeaderChain) setHead(headBlock uint64, headTime uint64,
	updateFn UpdateHeadBlocksCallback, delFn DeleteBlockContentCallback) {
	...
	if headTime > 0 && hc.genesisHeader.Time > headTime {
		...
		log.Crit("Rejecting genesis rewind via timestamp", "target", headTime,
			"genesis", hc.genesisHeader.Time)
	}
	var (
	...
	for hdr := hc.CurrentHeader(); hdr != nil && !done(hdr); hdr = hc.CurrentHeader() {
		...
		if updateFn != nil {
			...
			if force && ((headTime > 0 && newHead.Time < headTime) || (headTime == 0 && newHead.Number.Uint64() < headBlock)) {
				log.Warn("Force rewinding till ancient limit", "head",
		...
		if origin {
			for n := num + 1; len(rawdb.ReadAllHashes(hc.chainDb, n)) > 0; n++ {
				nums = append([]uint64{n},
					nums...) // suboptimal, but we don't really expect this path
			}
			origin = false
		}

...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := ReadHeader(db, header.Hash(),
		header.Number.Uint64()); entry != nil {
	...
	if entry := ReadHeaderRLP(db, header.Hash(),
		header.Number.Uint64()); entry == nil {
		...
		if hash := common.BytesToHash(hasher.Sum(nil)); hash != header.Hash() {
			t.Fatalf("Retrieved RLP header mismatch: have %v, want %v", entry,
				header)
		}
	}
...
func TestBodyStorage(t *testing.T) {
	...
	if entry := ReadBody(db, hash, 0); entry == nil {
		...
		newTestHasher()) != types.DeriveSha(types.Transactions(body.Transactions),
		newTestHasher()) || types.CalcUncleHash(entry.Uncles) != types.CalcUncleHash(body.Uncles) {
	...
	if entry := ReadBodyRLP(db, hash, 0); entry == nil {
		...
		if calc := common.BytesToHash(hasher.Sum(nil)); calc != hash {
			t.Fatalf("Retrieved RLP body mismatch: have %v, want %v", entry,
				body)
		}
	}
...
func TestBlockStorage(t *testing.T) {
	...
	if entry := ReadHeader(db, block.Hash(), block.NumberU64()); entry == nil {
		...
		t.Fatalf("Retrieved header mismatch: have %v, want %v", entry,
			block.Header())
	}
	if entry := ReadBody(db, block.Hash(), block.NumberU64()); entry == nil {
		t.Fatalf("Stored body not found")
	} else if types.DeriveSha(types.Transactions(entry.Transactions),
		newTestHasher()) != types.DeriveSha(block.Transactions(),
		newTestHasher()) || types.CalcUncleHash(entry.Uncles) != types.CalcUncleHash(block.Uncles()) {
		t.Fatalf("Retrieved body mismatch: have %v, want %v", entry,
			block.Body())
	}
...
func TestBadBlockStorage(t *testing.T) {
	...
	if len(badBlocks) != badBlockToKeep {
		t.Fatalf("The number of persised bad blocks in incorrect %d",
			len(badBlocks))
	}
	for i := 0; i < len(badBlocks)-1; i++ {
		if badBlocks[i].NumberU64() < badBlocks[i+1].NumberU64() {
			t.Fatalf("The bad blocks are not sorted #[%d](%d) < #[%d](%d)", i,
				i+1, badBlocks[i].NumberU64(), badBlocks[i+1].NumberU64())
		}
	}
...
func TestCanonicalMappingStorage(t *testing.T) {
	...
	if entry := ReadCanonicalHash(db, number); entry == (common.Hash{}) {
		...
		t.Fatalf("Retrieved canonical mapping mismatch: have %v, want %v",
			entry, hash)
	}
	// Delete the TD and verify the execution
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := ReadHeadHeaderHash(db); entry != blockHead.Hash() {
		t.Fatalf("Head header hash mismatch: have %v, want %v", entry,
			blockHead.Hash())
	}
	if entry := ReadHeadBlockHash(db); entry != blockFull.Hash() {
		t.Fatalf("Head block hash mismatch: have %v, want %v", entry,
			blockFull.Hash())
	}
	if entry := ReadHeadFastBlockHash(db); entry != blockFast.Hash() {
		t.Fatalf("Fast head block hash mismatch: have %v, want %v", entry,
			blockFast.Hash())
	}
}
...
func TestBlockReceiptStorage(t *testing.T) {
	...
	tx1 := types.NewTransaction(1, common.HexToAddress("0x1"), big.NewInt(1), 1,
		big.NewInt(1), nil)
	tx2 := types.NewTransaction(2, common.HexToAddress("0x2"), big.NewInt(2), 2,
		big.NewInt(2), nil)
	...
	if rs := ReadReceipts(db, hash, 0, 0,
		params.TestChainConfig); len(rs) != 0 {
	...
	if err := checkReceiptsRLP(ReadRawReceipts(db, hash, 0),
		receipts); err != nil {
		t.Fatalf(err.Error())
	}
...
func checkReceiptsRLP(have, want types.Receipts) error {
	if len(have) != len(want) {
		return fmt.Errorf("receipts sizes mismatch: have %d, want %d",
			len(have), len(want))
	}
	for i := 0; i < len(want); i++ {
		...
		if !bytes.Equal(rlpHave, rlpWant) {
			return fmt.Errorf("receipt #%d: receipt mismatch: have %s, want %s",
				i, hex.EncodeToString(rlpHave), hex.EncodeToString(rlpWant))
		}
	}
	return nil
}
...
func TestCanonicalHashIteration(t *testing.T) {
	...
	for i := uint64(1); i <= 8; i++ {
		...
		WriteTd(db, common.Hash{}, i,
			big.NewInt(10)) // Write some interferential data
	}
	for i, c := range cases {
...
func TestHashesInRange(t *testing.T) {
	...
	if have, want := len(ReadAllHashesInRange(db, 0,
		100)), total; have != want {
		t.Fatalf("Wrong number of hashes read, want %d, got %d", want, have)
	}
...
func TestReadLogs(t *testing.T) {
	...
	if have, want := len(logs), 2; have != want {
		t.Fatalf("unexpected number of logs returned, have %d want %d", have,
			want)
	}
	if have, want := len(logs[0]), 2; have != want {
		t.Fatalf("unexpected number of logs[0] returned, have %d want %d", have,
			want)
	}
	if have, want := len(logs[1]), 2; have != want {
		t.Fatalf("unexpected number of logs[1] returned, have %d want %d", have,
			want)
	}
	...
	for i, pr := range receipts {
		for j, pl := range pr.Logs {
			...
			if !bytes.Equal(rlpHave, rlpWant) {
				t.Fatalf("receipt #%d: receipt mismatch: have %s, want %s", i,
					hex.EncodeToString(rlpHave), hex.EncodeToString(rlpWant))
			}
		}
	}
}
...
func TestDeriveLogFields(t *testing.T) {
	...
	number := big.NewInt(1)
	hash := common.BytesToHash([]byte{0x03, 0x14})
	if err := deriveLogFields(receipts, hash, number.Uint64(),
		txs); err != nil {
		t.Fatal(err)
	}
	...
	for i := range receipts {
		for j := range receipts[i].Logs {
			if receipts[i].Logs[j].BlockNumber != number.Uint64() {
				t.Errorf("receipts[%d].Logs[%d].BlockNumber = %d, want %d", i,
					j, receipts[i].Logs[j].BlockNumber, number.Uint64())
			}
			if receipts[i].Logs[j].BlockHash != hash {
				t.Errorf("receipts[%d].Logs[%d].BlockHash = %s, want %s", i, j,
					receipts[i].Logs[j].BlockHash.String(), hash.String())
			}
			if receipts[i].Logs[j].TxHash != txs[i].Hash() {
				t.Errorf("receipts[%d].Logs[%d].TxHash = %s, want %s", i, j,
					receipts[i].Logs[j].TxHash.String(), txs[i].Hash().String())
			}
			if receipts[i].Logs[j].TxIndex != uint(i) {
				t.Errorf("receipts[%d].Logs[%d].TransactionIndex = %d, want %d",
					i, j, receipts[i].Logs[j].TxIndex, i)
			}
			if receipts[i].Logs[j].Index != logIndex {
				t.Errorf("receipts[%d].Logs[%d].Index = %d, want %d", i, j,

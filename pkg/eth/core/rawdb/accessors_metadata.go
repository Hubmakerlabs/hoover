...
func ReadChainConfig(db ethdb.KeyValueReader,
	hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db ethdb.KeyValueWriter, hash common.Hash,
...
func ReadGenesisStateSpec(db ethdb.KeyValueReader,
	blockhash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(blockhash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db ethdb.KeyValueWriter, blockhash common.Hash,
	data []byte) {
...
func PushUncleanShutdownMarker(db ethdb.KeyValueStore) ([]uint64, uint64,
	error) {
	...
	uncleanShutdowns.Recent = append(uncleanShutdowns.Recent,
...
func PopUncleanShutdownMarker(db ethdb.KeyValueStore) {
	...
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		...
		log.Error("Error decoding unclean shutdown markers", "error",
			err) // Should mos def _not_ happen
	}

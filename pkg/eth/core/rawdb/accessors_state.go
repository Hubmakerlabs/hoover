...
func WritePersistentStateID(db ethdb.KeyValueWriter, number uint64) {
	if err := db.Put(persistentStateIDKey,
...
// state).
func ReadStateHistoryMetaList(db ethdb.AncientReaderOp, start uint64,
	count uint64) ([][]byte, error) {
...
func ReadStateHistory(db ethdb.AncientReaderOp, id uint64) ([]byte, []byte,
...
func WriteStateHistory(db ethdb.AncientWriter, id uint64, meta []byte,
	accountIndex []byte, storageIndex []byte, accounts []byte,
	storages []byte) {

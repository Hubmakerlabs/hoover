...
func HasAccountTrieNode(db ethdb.KeyValueReader, path []byte,
...
// hash with the specified node path.
func ReadStorageTrieNode(db ethdb.KeyValueReader, accountHash common.Hash,
	path []byte) ([]byte, common.Hash) {
...
// node path and the associated node hash.
func HasStorageTrieNode(db ethdb.KeyValueReader, accountHash common.Hash,
	path []byte, hash common.Hash) bool {
	data, err := db.Get(storageTrieNodeKey(accountHash, path))
	if err != nil {
		return false
	}
...
func ExistsStorageTrieNode(db ethdb.KeyValueReader, accountHash common.Hash,
	path []byte) bool {
...
func WriteStorageTrieNode(db ethdb.KeyValueWriter, accountHash common.Hash,
	path []byte, node []byte) {
...
func DeleteStorageTrieNode(db ethdb.KeyValueWriter, accountHash common.Hash,
	path []byte) {
...
func WriteLegacyTrieNode(db ethdb.KeyValueWriter, hash common.Hash,
	node []byte) {
...
func HasTrieNode(db ethdb.KeyValueReader, owner common.Hash, path []byte,
	hash common.Hash, scheme string) bool {
	switch scheme {
	case HashScheme:
...
//   - owner
//   - path
func ReadTrieNode(db ethdb.KeyValueReader, owner common.Hash, path []byte,
	hash common.Hash, scheme string) []byte {
	switch scheme {
	case HashScheme:
...
//   - owner
//   - path
func WriteTrieNode(db ethdb.KeyValueWriter, owner common.Hash, path []byte,
	hash common.Hash, node []byte, scheme string) {
	switch scheme {
	case HashScheme:
...
//   - owner
//   - path
func DeleteTrieNode(db ethdb.KeyValueWriter, owner common.Hash, path []byte,
	hash common.Hash, scheme string) {
	switch scheme {
	case HashScheme:
...
func ParseStateScheme(provided string, disk ethdb.Database) (string, error) {
	...
	return "", fmt.Errorf("incompatible state scheme, stored: %s, provided: %s",
		stored, provided)
}

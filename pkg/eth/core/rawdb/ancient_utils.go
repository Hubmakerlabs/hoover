...
func inspect(name string, order map[string]bool,
	reader ethdb.AncientReader) (freezerInfo, error) {
	...
	for t := range order {
		...
		info.sizes = append(info.sizes,
			tableSize{name: t, size: common.StorageSize(size)})
	}
...
func inspectFreezers(db ethdb.Database) ([]freezerInfo, error) {
	...
	for _, freezer := range freezers {
		...
		default:
			return nil, fmt.Errorf("unknown freezer, supported ones: %v",
				freezers)
		}
	}
	return infos, nil
...
func InspectFreezerTable(ancient string, freezerName string, tableName string,

...
func TestChainIterator(t *testing.T) {
	...
	block = types.NewBlock(&types.Header{Number: big.NewInt(int64(0))}, nil,
		nil, nil, newTestHasher()) // Empty genesis block
	...
	for i := uint64(1); i <= 10; i++ {
		...
		txs = append(txs, tx)
		block = types.NewBlock(&types.Header{Number: big.NewInt(int64(i))},
			[]*types.Transaction{tx}, nil, nil, newTestHasher())
		WriteBlock(chainDb, block)
		WriteCanonicalHash(chainDb, block.Hash(), block.NumberU64())
	}
	...
	for i, c := range cases {
			...
			for h := range hashCh {
				...
				if len(h.hashes) > 0 {
					if got, exp := h.hashes[0], txs[h.number-1].Hash(); got != exp {
						t.Fatalf("block %d: hash wrong, got %x exp %x",
							h.number, got, exp)
					}
				}
			}
		...
		if !reflect.DeepEqual(numbers, c.expect) {
			t.Fatalf("Case %d failed, visit element mismatch, want %v, got %v",
				i, c.expect, numbers)
		}
	}
}

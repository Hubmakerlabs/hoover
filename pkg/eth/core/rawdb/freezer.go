...
func NewChainFreezer(datadir string, namespace string, readonly bool) (*Freezer,
	error) {
	return NewFreezer(datadir, namespace, readonly, freezerTableSize,
...
func NewFreezer(datadir string, namespace string, readonly bool,
	maxTableSize uint32, tables map[string]bool) (*Freezer, error) {
	...
	if info, err := os.Lstat(datadir); !os.IsNotExist(err) {
		if info.Mode()&os.ModeSymlink != 0 {
			log.Warn("Symbolic link ancient database is not supported", "path",
				datadir)
	...
	for name, disableSnappy := range tables {
		table, err := newTable(datadir, name, readMeter, writeMeter, sizeGauge,
			maxTableSize, disableSnappy, readonly)
		if err != nil {
	...
	log.Info("Opened ancient database", "database", datadir, "readonly",
		readonly)
	return freezer, nil
}
...
func (f *Freezer) AncientRange(kind string,
	start, count, maxBytes uint64) ([][]byte, error) {
	if table := f.tables[kind]; table != nil {
...
func (f *Freezer) ModifyAncients(fn func(ethdb.AncientWriteOp) error) (writeSize int64,
	err error) {
	if f.readonly {
	...
	defer func() {
		if err != nil {
			...
			for name, table := range f.tables {
				...
				if err != nil {
					log.Error("Freezer table roll-back failed", "table", name,
						"index", prevItem, "err", err)
				}
			}
		}
	}()
...
func (f *Freezer) validate() error {
	...
	for kind, table := range f.tables {
		if head != table.items.Load() {
			return fmt.Errorf("freezer tables %s and %s have differing head: %d != %d",
				kind, name, table.items.Load(), head)
		}
		if tail != table.itemHidden.Load() {
			return fmt.Errorf("freezer tables %s and %s have differing tail: %d != %d",
				kind, name, table.itemHidden.Load(), tail)
		}
	}
	f.frozen.Store(head)
	f.tail.Store(tail)
...
func (f *Freezer) MigrateTable(kind string, convert convertLegacyFn) error {
	...
	forEach := func(t *freezerTable, offset uint64,
		fn func(uint64, []byte) error) error {
		var (
			items     = t.items.Load()
	...
	newTable, err := newFreezerTable(migrationPath, kind, table.noCompression,
		false)
	if err != nil {
		return err
	}
	...
	if err := forEach(table, offset, func(i uint64, blob []byte) error {
		if i%10000 == 0 && time.Since(logged) > 16*time.Second {
			log.Info("Processing legacy elements", "count", i, "elapsed",
	...
	log.Info("Replacing old table files with migrated ones", "elapsed",
		common.PrettyDuration(time.Since(start)))
	...
	for _, f := range files {
		...
		if err := os.Rename(filepath.Join(migrationPath, f.Name()),
			filepath.Join(ancientsPath, f.Name())); err != nil {
			return err
		}
	}

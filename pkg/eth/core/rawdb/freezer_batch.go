...
func newFreezerBatch(f *Freezer) *freezerBatch {
	batch := &freezerBatch{tables: make(map[string]*freezerTableBatch,
		len(f.tables))}
...
func (batch *freezerBatch) Append(kind string, num uint64,
	item interface{}) error {
	return batch.tables[kind].Append(num, item)
}

// AppendRaw adds an item of the given kind.
func (batch *freezerBatch) AppendRaw(kind string, num uint64,
	item []byte) error {
...
func (batch *freezerBatch) commit() (item uint64, writeSize int64, err error) {
	...
	for name, tb := range batch.tables {
		if item < math.MaxUint64 && tb.curItem != item {
			return 0, 0, fmt.Errorf("table %s is at item %d, want %d", name,
				tb.curItem, item)
		}
		item = tb.curItem
	}
...
func (batch *freezerTableBatch) Append(item uint64, data interface{}) error {
	if item != batch.curItem {
		return fmt.Errorf("%w: have %d want %d", errOutOrderInsertion, item,
			batch.curItem)
	}
...
func (batch *freezerTableBatch) appendItem(data []byte) error {
	...
	entry := indexEntry{filenum: batch.t.headId,
		offset: uint32(itemOffset + itemSize)}

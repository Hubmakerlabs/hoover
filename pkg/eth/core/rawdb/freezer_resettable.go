...
func NewResettableFreezer(datadir string, namespace string, readonly bool,
	maxTableSize uint32, tables map[string]bool) (*ResettableFreezer, error) {
...
// in the freezer
func (f *ResettableFreezer) HasAncient(kind string, number uint64) (bool,
	error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *ResettableFreezer) Ancient(kind string, number uint64) ([]byte,
	error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *ResettableFreezer) AncientRange(kind string,
...
func (f *ResettableFreezer) ModifyAncients(fn func(ethdb.AncientWriteOp) error) (writeSize int64,
	err error) {
	f.lock.RLock()
	defer f.lock.RUnlock()
...
func (f *ResettableFreezer) MigrateTable(kind string,
	convert convertLegacyFn) error {
	f.lock.RLock()
	defer f.lock.RUnlock()

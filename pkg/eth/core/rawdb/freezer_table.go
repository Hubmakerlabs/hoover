...
func newFreezerTable(path, name string,
	disableSnappy, readonly bool) (*freezerTable, error) {
	return newTable(path, name, metrics.NilMeter{}, metrics.NilMeter{},
		metrics.NilGauge{}, freezerTableSize, disableSnappy, readonly)
}

// newTable opens a freezer table, creating the data and index files if they are
// non-existent. Both files are truncated to the shortest common length to ensure
// they don't go out of sync.
func newTable(path string, name string, readMeter metrics.Meter,
	writeMeter metrics.Meter, sizeGauge metrics.Gauge, maxFilesize uint32,
	noCompression, readonly bool) (*freezerTable, error) {
	...
	if readonly {
		...
		meta, err = openFreezerFileForReadOnly(filepath.Join(path,
		...
		meta, err = openFreezerFileForAppend(filepath.Join(path,
			fmt.Sprintf("%s.meta", name)))
		if err != nil {
			return nil, err
		}
...
func (t *freezerTable) repair() error {
	...
	if overflow := stat.Size() % indexEntrySize; overflow != 0 {
		if t.readonly {
			return fmt.Errorf("index file(path: %s, name: %s) size is not a multiple of %d",
				t.path, t.name, indexEntrySize)
		}
		if err := truncateFreezerFile(t.index,
			stat.Size()-overflow); err != nil {
			return err
	...
	if lastIndex.offset == 0 && offsetsSize/indexEntrySize > 1 {
		log.Error("Corrupted index file detected", "lastOffset",
			lastIndex.offset, "indexes", offsetsSize/indexEntrySize)
	}
	if t.readonly {
	...
	for contentExp != contentSize {
		if t.readonly {
			return fmt.Errorf("freezer table(path: %s, name: %s, num: %d) is corrupted",
				t.path, t.name, lastIndex.filenum)
		}
		verbose = true
		...
		if contentExp > contentSize {
			t.logger.Warn("Truncating dangling indexes", "indexes",
				offsetsSize/indexEntrySize, "indexed", contentExp, "stored",
				contentSize)
			if err := truncateFreezerFile(t.index,
				offsetsSize-indexEntrySize); err != nil {
				return err
			}
			...
			if newLastIndex.filenum != lastIndex.filenum {
				...
				if t.head, err = t.openFile(newLastIndex.filenum,
					openFreezerFileForAppend); err != nil {
					return err
				}
				if stat, err = t.head.Stat(); err != nil {
	...
	if verbose {
		t.logger.Info("Chain freezer table opened", "items", t.items.Load(),
			"deleted", t.itemOffset.Load(), "hidden", t.itemHidden.Load(),
			"tailId", t.tailId, "headId", t.headId, "size", t.headBytes)
	} else {
		t.logger.Debug("Chain freezer table opened", "items", t.items.Load(),
...
func (t *freezerTable) truncateHead(items uint64) error {
	...
	if err := truncateFreezerFile(t.index,
		int64(length+1)*indexEntrySize); err != nil {
		return err
	}
	if err := t.index.Sync(); err != nil {
		return err
	...
	if length == 0 {
		...
		if _, err := t.index.ReadAt(buffer,
			int64(length*indexEntrySize)); err != nil {
			return err
		}
...
func (t *freezerTable) truncateTail(items uint64) error {
	...
	if t.items.Load() == items {
		...
		if _, err := t.index.ReadAt(buffer,
			int64((offset+1)*indexEntrySize)); err != nil {
			return err
		}
	...
	if t.tailId > newTailId {
		return fmt.Errorf("invalid index, tail-file %d, item-file %d", t.tailId,
			newTailId)
	}
	...
	for current := items - 1; current >= deleted; current -= 1 {
		if _, err := t.index.ReadAt(buffer,
			int64((current-deleted+1)*indexEntrySize)); err != nil {
			return err
		}
		var pre indexEntry
	...
	err = copyFrom(t.index.Name(), t.index.Name(),
		indexEntrySize*(newDeleted-deleted+1), func(f *os.File) error {
...
func (t *freezerTable) openFile(num uint32,
	opener func(string) (*os.File, error)) (f *os.File, err error) {
	var exist bool
...
func (t *freezerTable) getIndices(from, count uint64) ([]*indexEntry, error) {
	...
	if _, err := t.index.ReadAt(buffer,
		int64(from*indexEntrySize)); err != nil {
		return nil, err
	}
	var (
		indices []*indexEntry
...
func (t *freezerTable) RetrieveItems(start, count, maxBytes uint64) ([][]byte,
	error) {
...
func (t *freezerTable) retrieveItems(start, count, maxBytes uint64) ([]byte,
	...
	readData := func(fileId, start uint32, length int) error {
		...
		if _, err := dataFile.ReadAt(output[len(output)-length:],
			int64(start)); err != nil {
			return fmt.Errorf("%w, fileid: %d, start: %d, length: %d", err,
				fileId, start, length)
		}
		return nil
	}
	...
	for i, firstIndex := range indices[:len(indices)-1] {
		...
		if secondIndex.filenum != firstIndex.filenum {
			...
			if unreadSize > 0 {
				if err := readData(firstIndex.filenum, readStart,
					unreadSize); err != nil {
					return nil, nil, err
				}
				unreadSize = 0
		...
		if i > 0 && uint64(totalSize+size) > maxBytes && maxBytes != 0 {
			...
			if unreadSize > 0 {
				if err := readData(secondIndex.filenum, readStart,
					unreadSize); err != nil {
					return nil, nil, err
				}
			}
...
func (t *freezerTable) dumpIndex(w io.Writer, start, stop int64) {
	...
	for i := uint64(start); ; i++ {
		if _, err := t.index.ReadAt(buf,
			int64((i+1)*indexEntrySize)); err != nil {
			break
		}

...
func TestFreezerRepairDanglingHead(t *testing.T) {
	...
	// open the index
	idxFile, err := os.OpenFile(filepath.Join(os.TempDir(),
		fmt.Sprintf("%s.ridx", fname)), os.O_RDWR, 0644)
	if err != nil {
...
func TestFreezerRepairDanglingIndex(t *testing.T) {
	...
	// Crop third file
	fileToCrop := filepath.Join(os.TempDir(),
		fmt.Sprintf("%s.0002.rdat", fname))
	...
	// Now crop it.
	{
		...
		for i := 0; i < 2; i++ {
			p := filepath.Join(os.TempDir(),
				fmt.Sprintf("%v.%04d.rdat", fname, i))
...
func checkRetrieve(t *testing.T, f *freezerTable, items map[uint64][]byte) {
	...
	for item, wantBytes := range items {
		...
		if !bytes.Equal(value, wantBytes) {
			t.Fatalf("item %d has wrong value %x (want %x)", item, value,
				wantBytes)
		}
	}
}
...
func checkRetrieveError(t *testing.T, f *freezerTable, items map[uint64]error) {
	...
	for item, wantError := range items {
		value, err := f.Retrieve(item)
		if err == nil {
			t.Fatalf("unexpected value %x for item %d, want error %v", item,
				value, wantError)
		}
		if err != wantError {
...
func TestFreezerReadonly(t *testing.T) {
	...
	idxFile, err := openFreezerFileForAppend(filepath.Join(tmpdir,
		fmt.Sprintf("%s.ridx", fname)))
	if err != nil {
...
func runRandTest(rt randTest) bool {
	...
	f, err := newTable(os.TempDir(), fname, metrics.NewMeter(),
		metrics.NewMeter(), metrics.NewGauge(), 50, true, false)
	if err != nil {
	...
	for i, step := range rt {
		...
		case opCheckAll:
			...
			if err != nil {
				...
				if !reflect.DeepEqual(got, values) {
					rt[i].err = fmt.Errorf("mismatch on retrieved values %v %v",
						got, values)
				}
			}
		...
		case opRetrieve:
			...
			got, err := f.RetrieveItems(step.items[0], uint64(len(step.items)),
				100000)
			if err != nil {
				rt[i].err = err
			} else {
				if !reflect.DeepEqual(got, blobs) {
					rt[i].err = fmt.Errorf("mismatch on retrieved values %v %v %v",
						got, blobs, step.items)
				}
			}

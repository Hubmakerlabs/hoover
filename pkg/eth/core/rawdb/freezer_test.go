...
func TestFreezerModify(t *testing.T) {
	...
	for _, table := range f.tables {
		t.Log(table.name, "index:",
			table.dumpIndexString(0, int64(len(valuesRaw))))
	}
		...
		go func() {
			...
			for frozen := range written {
				for rc := 0; rc < 80; rc++ {
					...
					if err != nil {
						panic(fmt.Errorf("error reading %d (frozen %d): %v",
							num, frozen, err))
					}
...
func TestFreezerConcurrentModifyTruncate(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if !(errors.Is(modifyErr, nil) || errors.Is(modifyErr,
			errOutOrderInsertion)) {
...
func TestFreezerConcurrentReadonly(t *testing.T) {
	...
	if loaded := f.tables["a"].items.Load(); loaded != items {
		t.Fatalf("unexpected number of items in table, want: %d, have: %d",
			items, loaded)
	}
	require.NoError(t, f.Close())
...
func newFreezerForTesting(t *testing.T, tables map[string]bool) (*Freezer,
	string) {
	t.Helper()
...
func checkAncientCount(t *testing.T, f *Freezer, kind string, n uint64) {
	...
	if n > 0 {
		...
		if ok, _ := f.HasAncient(kind, index); !ok {
			t.Errorf("HasAncient(%q, %d) returned false unexpectedly", kind,
				index)
		}
		if _, err := f.Ancient(kind, index); err != nil {
			t.Errorf("Ancient(%q, %d) returned unexpected error %q", kind,
				index, err)
		}
	}
	...
	if _, err := f.Ancient(kind, index); err == nil {
		...
		t.Errorf("Ancient(%q, %d) returned unexpected error %q", kind, index,
			err)
	}
}

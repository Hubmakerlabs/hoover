...
func (s *StateDB) DumpToCollector(c DumpCollector,
	conf *DumpConfig) (nextKey []byte) {
	...
	for it.Next() {
		...
		if !conf.SkipStorage {
			...
			for storageIt.Next() {
				...
				if err != nil {
					log.Error("Failed to decode the value returned by iterator",
						"error", err)
					continue
				}
		...
		if time.Since(logged) > 8*time.Second {
			log.Info("Trie dumping in progress", "at", it.Key, "accounts",
				accounts,
	...
	if missingPreimages > 0 {
		log.Warn("Dump incomplete due to missing preimages", "missing",
			missingPreimages)
	}

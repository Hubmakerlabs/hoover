...
func NewPruner(db ethdb.Database, config Config) (*Pruner, error) {
	...
	if config.BloomSize < 256 {
		log.Warn("Sanitizing bloomfilter size", "provided(MB)",
			config.BloomSize, "updated(MB)", 256)
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb ethdb.Database,
	stateBloom *stateBloom, bloomPath string,
	middleStateRoots map[common.Hash]struct{}, start time.Time) error {
	...
	for iter.Next() {
		...
		if len(key) == common.HashLength || isCode {
			...
			if _, exist := middleStateRoots[common.BytesToHash(checkKey)]; exist {
				log.Debug("Forcibly delete the middle state roots", "hash",
					common.BytesToHash(checkKey))
			} else {
			...
			if time.Since(logged) > 8*time.Second {
				log.Info("Pruning state data", "nodes", count, "skipped",
					skipped, "size", size,
					"elapsed", common.PrettyDuration(time.Since(pstart)), "eta",
					common.PrettyDuration(eta))
				logged = time.Now()
			}
	...
	iter.Release()
	log.Info("Pruned state data", "nodes", count, "size", size, "elapsed",
		common.PrettyDuration(time.Since(pstart)))
	...
	if count >= rangeCompactionThreshold {
		...
		log.Info("Database compaction finished", "elapsed",
			common.PrettyDuration(time.Since(cstart)))
	}
	log.Info("State pruning successful", "pruned", size, "elapsed",
...
func (p *Pruner) Prune(root common.Hash) error {
	...
	if root == (common.Hash{}) {
		...
		if len(layers) != 128 {
			...
			return fmt.Errorf("snapshot not old enough yet: need %d more blocks",
				128-len(layers))
		}
	...
	if !rawdb.HasLegacyTrieNode(p.db, root) {
		...
		for i := len(layers) - 2; i >= 2; i-- {
			if rawdb.HasLegacyTrieNode(p.db, layers[i].Root()) {
				root = layers[i].Root()
				found = true
				log.Info("Selecting middle-layer as the pruning target", "root",
					root, "depth", i)
				break
			}
		}
		...
		if len(layers) > 0 {
			log.Info("Selecting bottom-most difflayer as the pruning target",
				"root", root, "height", p.chainHeader.Number.Uint64()-127)
		} else {
			log.Info("Selecting user-specified state as the pruning target",
				"root", root)
		}
	}
	...
	start := time.Now()
	if err := snapshot.GenerateTrie(p.snaptree, root, p.db,
		p.stateBloom); err != nil {
		return err
	}
	...
	if err := p.stateBloom.Commit(filterName,
		filterName+stateBloomFileTempSuffix); err != nil {
		return err
	}
	log.Info("State bloom filter committed", "name", filterName)
	return prune(p.snaptree, root, p.db, p.stateBloom, filterName, middleRoots,
		start)
}
...
func RecoverPruning(datadir string, db ethdb.Database) error {
	...
	return prune(snaptree, stateBloomRoot, db, stateBloom, stateBloomPath,
		middleRoots, time.Now())
}
...
func extractGenesis(db ethdb.Database, stateBloom *stateBloom) error {
	...
	t, err := trie.NewStateTrie(trie.StateTrieID(genesis.Root()),
	...
	for accIter.Next(true) {
		...
		if accIter.Leaf() {
			...
			if acc.Root != types.EmptyRootHash {
				id := trie.StorageTrieID(genesis.Root(),
					common.BytesToHash(accIter.LeafKey()), acc.Root)
				storageTrie, err := trie.NewStateTrie(id,
					trie.NewDatabase(db, trie.HashDefaults))
				if err != nil {
					return err
				}
...
func bloomFilterName(datadir string, hash common.Hash) string {
	return filepath.Join(datadir,
		fmt.Sprintf("%s.%s.%s", stateBloomFilePrefix, hash.Hex(),
			stateBloomFileSuffix))
}

func isBloomFilter(filename string) (bool, common.Hash) {
	filename = filepath.Base(filename)
	if strings.HasPrefix(filename,
		stateBloomFilePrefix) && strings.HasSuffix(filename,
		stateBloomFileSuffix) {

...
func (gs *generatorStats) Log(msg string, root common.Hash, marker []byte) {
	...
	if len(marker) > 0 {
		if done := binary.BigEndian.Uint64(marker[:8]) - gs.origin; done > 0 {
			...
			ctx = append(ctx, []interface{}{
				"eta",
				common.PrettyDuration(time.Duration(left/speed) * time.Millisecond),
			}...)
		}
	}
	log.Info(msg, ctx...)
...
func newGeneratorContext(stats *generatorStats, db ethdb.KeyValueStore,
	accMarker []byte, storageMarker []byte) *generatorContext {
...
func (ctx *generatorContext) openIterator(kind string, start []byte) {
	if kind == snapAccount {
		...
		ctx.account = newHoldableIterator(rawdb.NewKeyLengthIterator(iter,
			1+common.HashLength))
		return
	}
	iter := ctx.db.NewIterator(rawdb.SnapshotStoragePrefix, start)
	ctx.storage = newHoldableIterator(rawdb.NewKeyLengthIterator(iter,
		1+2*common.HashLength))
}
...
func (ctx *generatorContext) reopenIterator(kind string) {
	...
	if !hasNext {
		...
		iter.Release()
		if kind == snapAccount {
			ctx.account = newHoldableIterator(memorydb.New().NewIterator(nil,
				nil))
			return
		}

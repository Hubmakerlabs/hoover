...
var (
	...
	bloomSize = math.Ceil(float64(aggregatorItemLimit) * math.Log(bloomTargetError) / math.Log(1/math.Pow(2,
		math.Log(2))))
...
func (h destructBloomHasher) Write(p []byte) (n int,
	err error) {
...
func (h accountBloomHasher) Write(p []byte) (n int,
	err error) {
	panic("not implemented")
}
...
func (h storageBloomHasher) Write(p []byte) (n int,
	err error) {
	panic("not implemented")
}
...
func newDiffLayer(parent snapshot, root common.Hash,
...
func (dl *diffLayer) rebloom(origin *diskLayer) {
	...
	snapshotBloomErrorGauge.Update(math.Pow(1.0-math.Exp((-k)*(n+0.5)/(m-1)),
		k))
}
...
func (dl *diffLayer) storage(accountHash, storageHash common.Hash,
...
func (dl *diffLayer) Update(blockRoot common.Hash,
	destructs map[common.Hash]struct{}, accounts map[common.Hash][]byte,
	storage map[common.Hash]map[common.Hash][]byte) *diffLayer {
...
func (dl *diffLayer) AccountList() []common.Hash {
	...
	dl.accountList = make([]common.Hash, 0,
		len(dl.destructSet)+len(dl.accountData))
	for hash := range dl.accountData {

...
func TestMergeBasics(t *testing.T) {
	...
	child := newDiffLayer(parent, common.Hash{}, copyDestructs(destructs),
		copyAccounts(accounts), copyStorage(storage))
	child = newDiffLayer(child, common.Hash{}, copyDestructs(destructs),
		copyAccounts(accounts), copyStorage(storage))
	child = newDiffLayer(child, common.Hash{}, copyDestructs(destructs),
		copyAccounts(accounts), copyStorage(storage))
	child = newDiffLayer(child, common.Hash{}, copyDestructs(destructs),
	...
	{ // Check storage lists
		...
		for aHash, sMap := range storage {
			...
			if have, want := len(list), len(sMap); have != want {
				t.Errorf("[2] StorageList() wrong: have %v, want %v", have,
					want)
			}
...
func TestMergeDelete(t *testing.T) {
	...
	parent := newDiffLayer(emptyLayer(), common.Hash{}, flipDrops(), flipAccs(),
		storage)
...
func TestInsertAndMerge(t *testing.T) {
	...
	)
	{
		...
		parent = newDiffLayer(emptyLayer(), common.Hash{}, destructs, accounts,
			storage)
	}
	{
		var (

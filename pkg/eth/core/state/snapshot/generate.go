...
func generateSnapshot(diskdb ethdb.KeyValueStore, triedb *trie.Database,
	cache int, root common.Hash) *diskLayer {
...
func journalProgress(db ethdb.KeyValueWriter, marker []byte,
	...
	default:
		logstr = fmt.Sprintf("%#x:%#x", marker[:common.HashLength],
			marker[common.HashLength:])
	}
...
func (result *proofResult) forEach(callback func(key []byte,
	val []byte) error) error {
...
func (dl *diskLayer) proveRange(ctx *generatorContext, trieId *trie.ID,
	prefix []byte, kind string, origin []byte, max int,
	valueConvertFn func([]byte) ([]byte, error)) (*proofResult, error) {
	var (
		keys     [][]byte
		vals     [][]byte
	...
	if origin == nil && !diskMore {
		...
		if gotRoot := stackTr.Hash(); gotRoot != root {
			return &proofResult{
				keys: keys,
				vals: vals,
				proofErr: fmt.Errorf("wrong root: have %#x want %#x", gotRoot,
					root),
			}, nil
		}
	...
	if err != nil {
		ctx.stats.Log("Trie missing, state snapshotting paused", dl.root,
			dl.genMarker)
	...
	if err := tr.Prove(origin, proof); err != nil {
		log.Debug("Failed to prove range", "kind", kind, "origin", origin,
			"err", err)
		return &proofResult{
			keys:     keys,
			vals:     vals,
	...
	if len(keys) > 0 {
		if err := tr.Prove(keys[len(keys)-1], proof); err != nil {
			log.Debug("Failed to prove range", "kind", kind, "last",
				keys[len(keys)-1], "err", err)
			return &proofResult{
				keys:     keys,
				vals:     vals,
				diskMore: diskMore,
...
func (dl *diskLayer) generateRange(ctx *generatorContext, trieId *trie.ID,
	prefix []byte, kind string, origin []byte, max int, onState onStateCallback,
	valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	// Use range prover to check the validity of the flat state in the range
	result, err := dl.proveRange(ctx, trieId, prefix, kind, origin, max,
		valueConvertFn)
	if err != nil {
	...
	if result.valid() {
		...
		if err := result.forEach(func(key []byte,
			val []byte) error {
			return onState(key, val, false,
				false)
		}); err != nil {
			return false, nil, err
		}
	...
	logger.Trace("Detected outdated state range", "last", hexutil.Encode(last),
		"err", result.proofErr)
	...
	if len(result.keys) > 0 {
		...
		if nodes != nil {
			tdb.Update(root, types.EmptyRootHash, 0,
				trienode.NewWithNodeSet(nodes), nil)
			tdb.Commit(root, false)
		}
		resolver = func(owner common.Hash, path []byte,
			hash common.Hash) []byte {
	...
	logger.Debug("Regenerated state range", "root", trieId.Root, "last",
		hexutil.Encode(last),
		"count", count, "created", created, "updated", updated, "untouched",
		untouched, "deleted", deleted)
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext,
	current []byte) error {
	...
	if ctx.batch.ValueSize() > ethdb.IdealBatchSize || abort != nil {
		if bytes.Compare(current, dl.genMarker) < 0 {
			log.Error("Snapshot generator went backwards", "current",
				fmt.Sprintf("%x", current), "genMarker",
		...
		if abort != nil {
			ctx.stats.Log("Aborting state snapshot generation", dl.root,
				current)
...
func generateStorages(ctx *generatorContext, dl *diskLayer,
	stateRoot common.Hash, account common.Hash, storageRoot common.Hash,
	storeMarker []byte) error {
	onStorage := func(key []byte, val []byte, write bool, delete bool) error {
		...
		if delete {
			rawdb.DeleteStorageSnapshot(ctx.batch, account,
				common.BytesToHash(key))
			snapWipedStorageMeter.Mark(1)
			return nil
		}
		if write {
			rawdb.WriteStorageSnapshot(ctx.batch, account,
				common.BytesToHash(key), val)
		...
		if err := dl.checkAndFlush(ctx,
			append(account[:], key...)); err != nil {
			return err
		}
		return nil
	}
	...
	for {
		...
		exhausted, last, err := dl.generateRange(ctx, id,
			append(rawdb.SnapshotStoragePrefix, account.Bytes()...),
			snapStorage, origin, storageCheckRange, onStorage, nil)
		if err != nil {
...
func generateAccounts(ctx *generatorContext, dl *diskLayer,
	accMarker []byte) error {
	onAccount := func(key []byte, val []byte, write bool, delete bool) error {
		...
		marker := account[:]
		if accMarker != nil && bytes.Equal(marker,
			accMarker) && len(dl.genMarker) > common.HashLength {
			marker = dl.genMarker[:]
		}
		// If we've exceeded our batch allowance or termination was requested, flush to disk
		...
		if acc.Root == types.EmptyRootHash {
			...
			if accMarker != nil && bytes.Equal(account[:],
				accMarker) && len(dl.genMarker) > common.HashLength {
				storeMarker = dl.genMarker[common.HashLength:]
			}
			if err := generateStorages(ctx, dl, dl.root, account, acc.Root,
				storeMarker); err != nil {
				return err
			}
		}
	...
	for {
		...
		exhausted, last, err := dl.generateRange(ctx, id,
			rawdb.SnapshotAccountPrefix, snapAccount, origin, accountRange,
			onAccount, types.FullAccountRLP)
		if err != nil {
			return err // The procedure it aborted, either by external signal or internal error.
		}
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	log.Info("Generated state snapshot", "accounts", stats.accounts, "slots",
		stats.slots,
		"storage", stats.storage, "dangling", stats.dangling, "elapsed",
		common.PrettyDuration(time.Since(stats.start)))

...
func ParseGeneratorStatus(generatorBlob []byte) string {
	...
	case 2 * common.HashLength:
		m = fmt.Sprintf("in %#x at %#x", marker[:common.HashLength],
			marker[common.HashLength:])
	default:
		m = fmt.Sprintf("%#x", marker)
	}
	return fmt.Sprintf(`Done: %v, Accounts: %d, Slots: %d, Storage: %d, Marker: %s`,
		generator.Done, generator.Accounts, generator.Slots, generator.Storage,
		m)
}

// loadAndParseJournal tries to parse the snapshot journal in latest format.
func loadAndParseJournal(db ethdb.KeyValueStore, base *diskLayer) (snapshot,
	journalGenerator, error) {
	...
	var generator journalGenerator
	if err := rlp.DecodeBytes(generatorBlob, &generator); err != nil {
		return nil, journalGenerator{}, fmt.Errorf("failed to decode snapshot generator: %v",
			err)
	}
	...
	err := iterateJournal(db, func(parent common.Hash, root common.Hash,
		destructSet map[common.Hash]struct{},
		accountData map[common.Hash][]byte,
		storageData map[common.Hash]map[common.Hash][]byte) error {
		current = newDiffLayer(current, root, destructSet, accountData,
			storageData)
		return nil
	})
	if err != nil {
...
func loadSnapshot(diskdb ethdb.KeyValueStore, triedb *trie.Database,
	root common.Hash, cache int, recovery bool, noBuild bool) (snapshot, bool,
	error) {
	...
	if head := snapshot.Root(); head != root {
		...
		if !recovery {
			return nil, false, fmt.Errorf("head doesn't match snapshot: have %#x, want %#x",
				head, root)
		}
		...
		// disk layer.
		log.Warn("Snapshot is not continuous with chain", "snaproot", head,
			"chainroot", root)
	}
...
func (dl *diffLayer) Journal(buffer *bytes.Buffer) (common.Hash, error) {
	...
	for hash, slots := range dl.storageData {
		...
		storage = append(storage,
			journalStorage{Hash: hash, Keys: keys, Vals: vals})
	}
	if err := rlp.Encode(buffer, storage); err != nil {
		return common.Hash{}, err
	}
	log.Debug("Journalled diff layer", "root", dl.root, "parent",
		dl.parent.Root())
	return base, nil
}
...
type journalCallback = func(parent common.Hash, root common.Hash,
...
func iterateJournal(db ethdb.KeyValueReader, callback journalCallback) error {
	...
	if version != journalVersion {
		log.Warn("Discarded the snapshot journal with wrong version",
			"required", journalVersion, "got", version)
	...
	if baseRoot := rawdb.ReadSnapshotRoot(db); baseRoot != parent {
		log.Warn("Loaded snapshot journal", "diskroot", baseRoot, "diffs",
			"unmatched")
	...
	for {
		...
		if err := callback(parent, root, destructSet, accountData,
			storageData); err != nil {
			return err
		}
		parent = root

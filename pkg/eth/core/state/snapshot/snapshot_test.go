...
// the slot addresses.
func randomStorageSet(accounts []string, hashes [][]string,
	nilStorage [][]string) map[common.Hash]map[common.Hash][]byte {
...
func TestDiskLayerExternalInvalidationFullFlatten(t *testing.T) {
	...
	if err := snaps.Update(common.HexToHash("0x02"), common.HexToHash("0x01"),
		nil, accounts, nil); err != nil {
	...
	if slot, err := ref.Storage(common.HexToHash("0xa1"),
		common.HexToHash("0xb1")); err != ErrSnapshotStale {
		t.Errorf("stale reference returned storage slot: %#x (err: %v)", slot,
			err)
	}
...
func TestDiskLayerExternalInvalidationPartialFlatten(t *testing.T) {
	...
	if err := snaps.Update(common.HexToHash("0x03"), common.HexToHash("0x02"),
		nil, accounts, nil); err != nil {
		t.Fatalf("failed to create a diff layer: %v", err)
	}
...
func TestDiffLayerExternalInvalidationPartialFlatten(t *testing.T) {
	...
	// were used to trigger the flaw described in https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/27254.
	...
	if err := snaps.Update(common.HexToHash("0x04"), common.HexToHash("0x03"),
		nil, accounts, nil); err != nil {
		t.Fatalf("failed to create a diff layer: %v", err)
	}
...
func TestPostCapBasicDataAccess(t *testing.T) {
	...
	snaps.Update(common.HexToHash("0xa1"), common.HexToHash("0x01"), nil,
		setAccount("0xa1"), nil)
	snaps.Update(common.HexToHash("0xa2"), common.HexToHash("0xa1"), nil,
		setAccount("0xa2"), nil)
	snaps.Update(common.HexToHash("0xb2"), common.HexToHash("0xa1"), nil,
		setAccount("0xb2"), nil)

	snaps.Update(common.HexToHash("0xa3"), common.HexToHash("0xa2"), nil,
		setAccount("0xa3"), nil)
	snaps.Update(common.HexToHash("0xb3"), common.HexToHash("0xb2"), nil,
		setAccount("0xb3"), nil)

	// checkExist verifies if an account exists in a snapshot
	checkExist := func(layer *diffLayer, key string) error {
		if data, _ := layer.Account(common.HexToHash(key)); data == nil {
			return fmt.Errorf("expected %x to exist, got nil",
...
func TestSnaphots(t *testing.T) {
	...
	var cases = []struct {
		...
		{head, 128, false, 128,
			makeRoot(3)}, // Normal diff layers, no accumulator
		{head, 129, true, 129,
			makeRoot(2)}, // All diff layers, including accumulator
	...
	for i, c := range cases {
		...
		if len(layers) != c.expected {
			t.Errorf("non-overflow test %d: returned snapshot layers are mismatched, want %v, got %v",
				i, c.expected, len(layers))
		}
		if len(layers) == 0 {
			continue
		}
		...
		if bottommost.Root() != c.expectBottom {
			t.Errorf("non-overflow test %d: snapshot mismatch, want %v, get %v",
				i, c.expectBottom, bottommost.Root())
		}
	}
	...
	cases = []struct {
		...
		{head, 64, false, 64, makeRoot(129 + 2 - 64)},
		{head, 128, false, 128,
			makeRoot(3)}, // All diff layers, accumulator was flattened
		{head, 129, true, 128,
			makeRoot(3)}, // All diff layers, accumulator was flattened
	...
	for i, c := range cases {
		layers := snaps.Snapshots(c.headRoot, c.limit, c.nodisk)
		if len(layers) != c.expected {
			t.Errorf("overflow test %d: returned snapshot layers are mismatched, want %v, got %v",
				i, c.expected, len(layers))
		}
		if len(layers) == 0 {
			continue
		...
		bottommost := layers[len(layers)-1]
		if bottommost.Root() != c.expectBottom {
			t.Errorf("overflow test %d: snapshot mismatch, want %v, get %v", i,
				c.expectBottom, bottommost.Root())
		}
	}
}

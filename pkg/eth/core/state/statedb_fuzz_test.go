...
func newStateTestAction(addr common.Address, r *rand.Rand,
	index int) testAction {
	actions := []testAction{
		{
			name: "SetBalance",
	...
	for i := 0; i < len(actions); i++ {
		...
		for j := range actions[i] {
			if j == 0 {
				...
				actions[i][j] = newStateTestAction(common.HexToAddress("0xdeadbeef"),
					r, 0)
				continue
			}
...
func (test *stateTest) run() bool {
	...
	for i := 0; i < len(test.actions); i++ {
		...
		test.err = test.verify(root, roots[i], tdb, accountList[i],
			storageList[i])
		if test.err != nil {
			return false
		}
...
func (test *stateTest) verifyAccountCreation(next common.Hash,
	db *trie.Database, otr, ntr *trie.Trie, addr common.Address,
...
// - the slots transition is correct
func (test *stateTest) verifyAccountUpdate(next common.Hash, db *trie.Database,
	otr, ntr *trie.Trie, addr common.Address, origin []byte,
	slots map[common.Hash][]byte) error {
	// Verify account change
	addrHash := crypto.Keccak256Hash(addr.Bytes())
	oBlob, err := otr.Get(addrHash.Bytes())
...
func (test *stateTest) verify(root common.Hash, next common.Hash,
	db *trie.Database, accountsOrigin map[common.Address][]byte,
	storagesOrigin map[common.Address]map[common.Hash][]byte) error {
	...
	for addr, account := range accountsOrigin {
		var err error
		if len(account) == 0 {
			err = test.verifyAccountCreation(next, db, otr, ntr, addr,
				storagesOrigin[addr])
		} else {
			err = test.verifyAccountUpdate(next, db, otr, ntr, addr,
				accountsOrigin[addr], storagesOrigin[addr])
		}
		if err != nil {
			return err
		}

...
func makeTestState(scheme string) (ethdb.Database, Database, *trie.Database,
	common.Hash, []*testAccount) {
	...
	for i := byte(0); i < 96; i++ {
		...
		if i%3 == 0 {
			obj.SetCode(crypto.Keccak256Hash([]byte{i, i, i, i, i}),
				[]byte{i, i, i, i, i})
...
// account array.
func checkStateAccounts(t *testing.T, db ethdb.Database, scheme string,
	root common.Hash, accounts []*testAccount) {
	...
	for i, acc := range accounts {
		if balance := state.GetBalance(acc.address); balance.Cmp(acc.balance) != 0 {
			t.Errorf("account %d: balance mismatch: have %v, want %v", i,
				balance, acc.balance)
		}
		if nonce := state.GetNonce(acc.address); nonce != acc.nonce {
			t.Errorf("account %d: nonce mismatch: have %v, want %v", i, nonce,
				acc.nonce)
		}
		if code := state.GetCode(acc.address); !bytes.Equal(code, acc.code) {
			t.Errorf("account %d: code mismatch: have %x, want %x", i, code,
				acc.code)
		}
	}
}
...
func checkStateConsistency(db ethdb.Database, scheme string,
...
func TestEmptyStateSync(t *testing.T) {
	...
	dbB := trie.NewDatabase(rawdb.NewMemoryDatabase(),
		&trie.Config{PathDB: pathdb.Defaults})

	sync := NewStateSync(types.EmptyRootHash, rawdb.NewMemoryDatabase(), nil,
		dbA.Scheme())
	if paths, nodes, codes := sync.Missing(1); len(paths) != 0 || len(nodes) != 0 || len(codes) != 0 {
		t.Errorf("content requested for empty state: %v, %v, %v", nodes, paths,
			codes)
	}
	sync = NewStateSync(types.EmptyRootHash, rawdb.NewMemoryDatabase(), nil,
		dbB.Scheme())
...
func testIterativeStateSync(t *testing.T, count int, commit bool, bypath bool,
	...
	for len(nodeElements)+len(codeElements) > 0 {
		...
		for i, element := range codeElements {
			...
			if err != nil {
				t.Fatalf("failed to retrieve contract bytecode for hash %x",
					element.code)
			}
		...
		for i, node := range nodeElements {
			if bypath {
				if len(node.syncPath) == 1 {
					...
					nodeResults[i] = trie.NodeSyncResult{Path: node.path,
						Data: data}
				} else {
					var acc types.StateAccount
					if err := rlp.DecodeBytes(srcTrie.MustGet(node.syncPath[0]),
						&acc); err != nil {
						t.Fatalf("failed to decode account on path %x: %v",
							node.syncPath[0], err)
					}
					id := trie.StorageTrieID(srcRoot,
						common.BytesToHash(node.syncPath[0]), acc.Root)
					stTrie, err := trie.New(id, ndb)
					if err != nil {
						t.Fatalf("failed to retriev storage trie for path %x: %v",
							node.syncPath[1], err)
					}
				...
				if err != nil {
					t.Fatalf("failed to retrieve node data for key %v",
						[]byte(node.path))
				}
				nodeResults[i] = trie.NodeSyncResult{Path: node.path,
					Data: data}
	...
	for len(nodeElements)+len(codeElements) > 0 {
		...
		if len(codeElements) > 0 {
			...
			for i, element := range codeElements[:len(codeResults)] {
				data, err := srcDb.ContractCode(common.Address{}, element.code)
				if err != nil {
					t.Fatalf("failed to retrieve contract bytecode for %x",
						element.code)
				}
				codeResults[i] = trie.CodeSyncResult{Hash: element.code,
					Data: data}
			}
			for _, result := range codeResults {
				if err := sched.ProcessCode(result); err != nil {
		...
		if len(nodeElements) > 0 {
			...
			for i, element := range nodeElements[:len(nodeResults)] {
				...
				nodeResults[i] = trie.NodeSyncResult{Path: element.path,
					Data: data}
			}
			for _, result := range nodeResults {
				if err := sched.ProcessNode(result); err != nil {
	...
	for len(nodeQueue)+len(codeQueue) > 0 {
		...
		if len(codeQueue) > 0 {
			...
			for hash := range codeQueue {
				...
				results = append(results,
					trie.CodeSyncResult{Hash: hash, Data: data})
			}
		...
		if len(nodeQueue) > 0 {
			...
			for path, element := range nodeQueue {
				...
				data, err := reader.Node(owner, inner, element.hash)
				if err != nil {
					t.Fatalf("failed to retrieve node data for %x %v %v",
						element.hash, []byte(element.path), element.path)
				}
				results = append(results,
					trie.NodeSyncResult{Path: path, Data: data})
			}
			for _, result := range results {
				if err := sched.ProcessNode(result); err != nil {
					t.Fatalf("failed to process result %v", err)
	...
	for len(nodeQueue)+len(codeQueue) > 0 {
		...
		if len(nodeQueue) > 0 {
			...
			for path, element := range nodeQueue {
				...
				data, err := reader.Node(owner, inner, element.hash)
				if err != nil {
					t.Fatalf("failed to retrieve node data for %x",
						element.hash)
				}
				results = append(results,
					trie.NodeSyncResult{Path: path, Data: data})
...
func testIncompleteStateSync(t *testing.T, scheme string) {
	...
	for _, node := range addedCodes {
		...
		if err := checkStateConsistency(dstDb, ndb.Scheme(),
			srcRoot); err == nil {

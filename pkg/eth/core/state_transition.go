...
import (
	...
	cmath "github.com/Hubmakerlabs/hoover/pkg/eth/common/math"
...
func TransactionToMessage(tx *types.Transaction, s types.Signer,
	baseFee *big.Int) (*Message, error) {
	msg := &Message{
		Nonce:             tx.Nonce(),
		GasLimit:          tx.Gas(),
...
func ApplyMessage(evm *vm.EVM, msg *Message, gp *GasPool) (*ExecutionResult,
	error) {
...
func NewStateTransition(evm *vm.EVM, msg *Message,
	gp *GasPool) *StateTransition {
	...
	if st.evm.ChainConfig().IsLondon(st.evm.Context.BlockNumber) {
		...
		if !skipCheck {
			if l := msg.GasFeeCap.BitLen(); l > 256 {
				return fmt.Errorf("%w: address %v, maxFeePerGas bit length: %d",
					ErrFeeCapVeryHigh,
					msg.From.Hex(), l)
			}
			if l := msg.GasTipCap.BitLen(); l > 256 {
				return fmt.Errorf("%w: address %v, maxPriorityFeePerGas bit length: %d",
					ErrTipVeryHigh,
					msg.From.Hex(), l)
			}
		...
		if st.blobGasUsed() > 0 {
			...
			if !skipCheck {
				...
				if msg.BlobGasFeeCap.Cmp(st.evm.Context.BlobBaseFee) < 0 {
					return fmt.Errorf("%w: address %v blobGasFeeCap: %v, blobBaseFee: %v",
						ErrBlobFeeCapTooLow,
						msg.From.Hex(), msg.BlobGasFeeCap,
						st.evm.Context.BlobBaseFee)
				}
			}
		}
	}
...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	var (
		...
		rules  = st.evm.ChainConfig().Rules(st.evm.Context.BlockNumber,
			st.evm.Context.Random != nil, st.evm.Context.Time)
	...
	if st.gasRemaining < gas {
		return nil, fmt.Errorf("%w: have %d, want %d", ErrIntrinsicGas,
			st.gasRemaining, gas)
	}
	st.gasRemaining -= gas
	...
	st.state.Prepare(rules, msg.From, st.evm.Context.Coinbase, msg.To,
	...
	if contractCreation {
		ret, _, st.gasRemaining, vmerr = st.evm.Create(sender, msg.Data,
	...
	if rules.IsLondon {
		effectiveTip = cmath.BigMin(msg.GasTipCap,
...
func (st *StateTransition) refundGas(refundQuotient uint64) {
	...
	remaining := new(big.Int).Mul(new(big.Int).SetUint64(st.gasRemaining),
		st.msg.GasPrice)

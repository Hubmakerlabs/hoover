...
func (p *BlobPool) Init(gasTip *big.Int, head *types.Header,
	reserve txpool.AddressReserver) error {
	p.reserve = reserve
	...
	var (
		basefee = uint256.MustFromBig(eip1559.CalcBaseFee(p.chain.Config(),
			p.head))
...
func (p *BlobPool) parseTransaction(id uint64, size uint32, blob []byte) error {
	...
	if tx.BlobTxSidecar() == nil {
		log.Error("Missing sidecar in blob pool entry", "id", id, "hash",
			tx.Hash())
	...
	if err != nil {
		...
		// this data entry.
		log.Error("Failed to recover blob tx sender", "id", id, "hash",
			tx.Hash(), "err", err)
		return err
	}
...
func (p *BlobPool) recheck(addr common.Address,
	inclusions map[common.Hash]uint64) {
	...
	if gapped || filled {
		...
		if gapped {
			log.Warn("Dropping dangling blob transactions", "from", addr,
				"missing", next, "drop", nonces, "ids", ids)
		} else {
			log.Trace("Dropping filled blob transactions", "from", addr,
				"filled", nonces, "ids", ids)
		}
		for _, id := range ids {
			if err := p.store.Delete(id); err != nil {
				log.Error("Failed to delete blob transaction", "from", addr,
					"id", id, "err", err)
			}
		}
		return
	...
	if txs[0].nonce < next {
		...
		log.Trace("Dropping overlapped blob transactions", "from", addr,
			"overlapped", nonces, "ids", ids, "left", len(txs))
		for _, id := range ids {
			if err := p.store.Delete(id); err != nil {
				log.Error("Failed to delete blob transaction", "from", addr,
					"id", id, "err", err)
	...
	for i := 1; i < len(txs); i++ {
		...
		if txs[i].nonce == txs[i-1].nonce {
			log.Error("Duplicate nonce blob transaction", "from", addr, "nonce",
				txs[i].nonce)
		}
		...
		log.Error("Dropping gapped blob transactions", "from", addr, "missing",
			txs[i-1].nonce+1, "drop", nonces, "ids", ids)
		for _, id := range ids {
			if err := p.store.Delete(id); err != nil {
				log.Error("Failed to delete blob transaction", "from", addr,
					"id", id, "err", err)
	...
	if spent.Cmp(balance) > 0 {
		...
		log.Warn("Dropping overdrafted blob transactions", "from", addr,
			"balance", balance, "spent", spent, "drop", nonces, "ids", ids)
		for _, id := range ids {
			if err := p.store.Delete(id); err != nil {
				log.Error("Failed to delete blob transaction", "from", addr,
					"id", id, "err", err)
	...
	if len(txs) > maxTxsPerAccount {
		...
		log.Warn("Dropping overcapped blob transactions", "from", addr, "kept",
			len(txs), "drop", nonces, "ids", ids)
		for _, id := range ids {
			if err := p.store.Delete(id); err != nil {
				log.Error("Failed to delete blob transaction", "from", addr,
					"id", id, "err", err)
...
func (p *BlobPool) offload(addr common.Address, nonce uint64, id uint64,
	inclusions map[common.Hash]uint64) {
	data, err := p.store.Get(id)
	if err != nil {
		log.Error("Blobs missing for included transaction", "from", addr,
			"nonce", nonce, "id", id, "err", err)
		return
	}
	var tx types.Transaction
	if err = rlp.DecodeBytes(data, &tx); err != nil {
		log.Error("Blobs corrupted for included transaction", "from", addr,
			"nonce", nonce, "id", id, "err", err)
		return
	}
	block, ok := inclusions[tx.Hash()]
	if !ok {
		log.Warn("Blob transaction swapped out by signer", "from", addr,
			"nonce", nonce, "id", id)
		return
	}
...
func (p *BlobPool) reorg(oldHead, newHead *types.Header) (map[common.Address][]*types.Transaction,
	...
	if add == nil {
		...
		// earlier block.
		log.Warn("Blobpool reset with missing new head", "number",
	...
	if rem == nil {
		...
		if newNum >= oldNum {
			...
			log.Warn("Blobpool reset with missing old head",
				"old", oldHead.Hash(), "oldnum", oldNum, "new", newHead.Hash(),
				"newnum", newNum)
			return nil, nil
		}
	...
	for rem.NumberU64() > add.NumberU64() {
		...
		if rem = p.chain.GetBlock(rem.ParentHash(),
			rem.NumberU64()-1); rem == nil {
			log.Error("Unrooted old chain seen by blobpool", "block",
				oldHead.Number, "hash", oldHead.Hash())
			return nil, nil
		}
	}
	for add.NumberU64() > rem.NumberU64() {
		...
		if add = p.chain.GetBlock(add.ParentHash(),
			add.NumberU64()-1); add == nil {
			log.Error("Unrooted new chain seen by blobpool", "block",
				newHead.Number, "hash", newHead.Hash())
			return nil, nil
		}
	}
...
func (p *BlobPool) reinject(addr common.Address, txhash common.Hash) error {
	...
	if err != nil {
		log.Error("Failed to encode transaction for storage", "hash", tx.Hash(),
			"err", err)
		return err
	}
	id, err := p.store.Put(blob)
	if err != nil {
		log.Error("Failed to write transaction into storage", "hash", tx.Hash(),
			"err", err)
		return err
	}
	...
	if _, ok := p.index[addr]; !ok {
		if err := p.reserve(addr, true); err != nil {
			log.Warn("Failed to reserve account for blob pool", "tx", tx.Hash(),
				"from", addr, "err", err)
			return err
		}
		...
		for addr, txs := range p.index {
			for i, tx := range txs {
				if tx.execTipCap.Cmp(p.gasTip) < 0 {
					...
					p.spent[addr] = new(uint256.Int).Sub(p.spent[addr],
						txs[i].costCap)
					...
					log.Warn("Dropping underpriced blob transaction", "from",
						addr, "rejected", tx.nonce, "tip", tx.execTipCap,
						"want", tip, "drop", nonces, "ids", ids)
					for _, id := range ids {
						if err := p.store.Delete(id); err != nil {
							log.Error("Failed to delete dropped transaction",
								"id", id, "err", err)
						}
					}
					break
...
func (p *BlobPool) validateTx(tx *types.Transaction) error {
	...
	if err := txpool.ValidateTransaction(tx, p.head, p.signer,
		baseOpts); err != nil {
		return err
	}
	...
	if err := txpool.ValidateTransactionWithState(tx, p.signer,
		stateOpts); err != nil {
		return err
	}
	...
	if uint64(len(p.index[from])) > tx.Nonce()-next {
		...
		case tx.GasFeeCapIntCmp(prev.execFeeCap.ToBig()) <= 0:
			return fmt.Errorf("%w: new tx gas fee cap %v <= %v queued",
				txpool.ErrReplaceUnderpriced, tx.GasFeeCap(), prev.execFeeCap)
		case tx.GasTipCapIntCmp(prev.execTipCap.ToBig()) <= 0:
			return fmt.Errorf("%w: new tx gas tip cap %v <= %v queued",
				txpool.ErrReplaceUnderpriced, tx.GasTipCap(), prev.execTipCap)
		case tx.BlobGasFeeCapIntCmp(prev.blobFeeCap.ToBig()) <= 0:
			return fmt.Errorf("%w: new tx blob gas fee cap %v <= %v queued",
				txpool.ErrReplaceUnderpriced, tx.BlobGasFeeCap(),
				prev.blobFeeCap)
		}
		var (
			...
			minGasFeeCap = new(uint256.Int).Div(new(uint256.Int).Mul(multiplier,
				prev.execFeeCap), onehundred)
			minGasTipCap = new(uint256.Int).Div(new(uint256.Int).Mul(multiplier,
				prev.execTipCap), onehundred)
			minBlobGasFeeCap = new(uint256.Int).Div(new(uint256.Int).Mul(multiplier,
				prev.blobFeeCap), onehundred)
		)
		switch {
		case tx.GasFeeCapIntCmp(minGasFeeCap.ToBig()) < 0:
			return fmt.Errorf("%w: new tx gas fee cap %v <= %v queued + %d%% replacement penalty",
				txpool.ErrReplaceUnderpriced, tx.GasFeeCap(), prev.execFeeCap,
				p.config.PriceBump)
		case tx.GasTipCapIntCmp(minGasTipCap.ToBig()) < 0:
			return fmt.Errorf("%w: new tx gas tip cap %v <= %v queued + %d%% replacement penalty",
				txpool.ErrReplaceUnderpriced, tx.GasTipCap(), prev.execTipCap,
				p.config.PriceBump)
		case tx.BlobGasFeeCapIntCmp(minBlobGasFeeCap.ToBig()) < 0:
			return fmt.Errorf("%w: new tx blob gas fee cap %v <= %v queued + %d%% replacement penalty",
				txpool.ErrReplaceUnderpriced, tx.BlobGasFeeCap(),
				prev.blobFeeCap, p.config.PriceBump)
		}
	}
	return nil
}
...
func (p *BlobPool) Get(hash common.Hash) *types.Transaction {
	...
	data, err := p.store.Get(id)
	if err != nil {
		log.Error("Tracked blob transaction missing from store", "hash", hash,
			"id", id, "err", err)
		return nil
	}
	item := new(types.Transaction)
	if err = rlp.DecodeBytes(data, item); err != nil {
		log.Error("Blobs corrupted for traced transaction", "hash", hash, "id",
			id, "err", err)
		return nil
	}
	return item
...
func (p *BlobPool) Add(txs []*types.Transaction, local bool,
	sync bool) []error {
	var (
...
func (p *BlobPool) add(tx *types.Transaction) (err error) {
	...
	if err := p.validateTx(tx); err != nil {
		log.Trace("Transaction validation failed", "hash", tx.Hash(), "err",
			err)
		return err
	}
	...
	if len(p.index[from]) > offset {
		...
		if err := p.store.Delete(prev.id); err != nil {
			...
			log.Error("Failed to delete replaced transaction", "id", prev.id,
				"err", err)
		}
...
func (p *BlobPool) drop() {
	...
	log.Warn("Evicting overflown blob transaction", "from", from, "evicted",
		drop.nonce, "id", drop.id)
	if err := p.store.Delete(drop.id); err != nil {
		log.Error("Failed to drop evicted transaction", "id", drop.id, "err",
			err)
	}
}
...
func (p *BlobPool) updateStorageMetrics() {
	...
	for _, shelf := range stats.Shelves {
		...
		metrics.GetOrRegisterGauge(fmt.Sprintf(shelfDatausedGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(slotDataused))
		metrics.GetOrRegisterGauge(fmt.Sprintf(shelfDatagapsGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(slotDatagaps))
		metrics.GetOrRegisterGauge(fmt.Sprintf(shelfSlotusedGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(shelf.FilledSlots))
		metrics.GetOrRegisterGauge(fmt.Sprintf(shelfSlotgapsGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(shelf.GappedSlots))
...
func (p *BlobPool) updateLimboMetrics() {
	...
	for _, shelf := range stats.Shelves {
		...
		metrics.GetOrRegisterGauge(fmt.Sprintf(limboShelfDatausedGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(slotDataused))
		metrics.GetOrRegisterGauge(fmt.Sprintf(limboShelfDatagapsGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(slotDatagaps))
		metrics.GetOrRegisterGauge(fmt.Sprintf(limboShelfSlotusedGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(shelf.FilledSlots))
		metrics.GetOrRegisterGauge(fmt.Sprintf(limboShelfSlotgapsGaugeName,
			shelf.SlotSize/blobSize), nil).Update(int64(shelf.GappedSlots))
	}
...
func (p *BlobPool) SubscribeTransactions(ch chan<- core.NewTxsEvent,
	reorgs bool) event.Subscription {
	if reorgs {

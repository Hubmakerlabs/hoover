...
func newPriceHeap(basefee *uint256.Int, blobfee *uint256.Int,
	index *map[common.Address][]*blobTxMeta) *evictHeap {
	heap := &evictHeap{
		metas: index,
...
func (h *evictHeap) reinit(basefee *uint256.Int, blobfee *uint256.Int,
	force bool) {
...
func (h *evictHeap) Less(i, j int) bool {
	...
	prioI := evictionPriority(h.basefeeJumps, lastI.evictionExecFeeJumps,
		h.blobfeeJumps, lastI.evictionBlobFeeJumps)
	if prioI > 0 {
		prioI = 0
	}
	prioJ := evictionPriority(h.basefeeJumps, lastJ.evictionExecFeeJumps,
		h.blobfeeJumps, lastJ.evictionBlobFeeJumps)
	if prioJ > 0 {
		prioJ = 0
	}
	if prioI == prioJ {

...
func (l *limbo) parseBlob(id uint64, data []byte) error {
	...
	if _, ok := l.index[item.TxHash]; ok {
		...
		// by ignoring this data entry.
		log.Error("Dropping duplicate blob limbo entry", "owner", item.TxHash,
			"id", id)
...
func (l *limbo) finalize(final *types.Header) {
	...
	for block, ids := range l.groups {
		...
		for id, owner := range ids {
			if err := l.store.Delete(id); err != nil {
				log.Error("Failed to drop finalized blob", "block", block, "id",
					id, "err", err)
			}
...
func (l *limbo) pull(tx common.Hash) (*types.Transaction, error) {
	...
	if err != nil {
		log.Error("Failed to get and drop limboed blobs", "tx", tx, "id", id,
			"err", err)
		return nil, err
	}
	return item.Tx, nil
...
func (l *limbo) update(txhash common.Hash, block uint64) {
	...
	if _, ok := l.groups[block][id]; ok {
		log.Trace("Blob transaction unchanged in limbo", "tx", txhash, "block",
			block)
		return
	}
	...
	item, err := l.getAndDrop(id)
	if err != nil {
		log.Error("Failed to get and drop limboed blobs", "tx", txhash, "id",
			id, "err", err)
		return
	}
	if err := l.setAndIndex(item.Tx, block); err != nil {
		log.Error("Failed to set and index limboed blobs", "tx", txhash, "err",
			err)
		return
	}
	log.Trace("Blob transaction updated in limbo", "tx", txhash, "old-block",
		item.Block, "new-block", block)
}

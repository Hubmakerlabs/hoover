...
func TestDeriveSha(t *testing.T) {
	...
	for len(txs) < 1000 {
		exp := types.DeriveSha(txs,
			trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase(), nil)))
...
func TestFuzzDeriveSha(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		seed := rndSeed + i
		exp := types.DeriveSha(newDummy(i),
			trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase(), nil)))
...
func TestDerivableList(t *testing.T) {
	...
	for i, tc := range tcs[1:] {
		exp := types.DeriveSha(flatList(tc),
			trie.NewEmpty(trie.NewDatabase(rawdb.NewMemoryDatabase(), nil)))
...
func genTxs(num uint64) (types.Transactions, error) {
	...
	newTx := func(i uint64) (*types.Transaction, error) {
		...
		utx := types.NewTransaction(i, addr, new(big.Int), 0,
			new(big.Int).SetUint64(10000000), nil)

...
func (r *Receipt) EncodeRLP(w io.Writer) error {
	data := &receiptRLP{r.statusEncoding(), r.CumulativeGasUsed, r.Bloom,
		r.Logs}
...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash,
	number uint64, time uint64, baseFee *big.Int, blobGasPrice *big.Int,
	txs []*Transaction) error {
	...
	for i := 0; i < len(rs); i++ {
		...
		rs[i].EffectiveGasPrice = txs[i].inner.effectiveGasPrice(new(big.Int),
			baseFee)

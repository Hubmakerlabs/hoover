...
func TestEIP155Signing(t *testing.T) {
	...
	tx, err := SignTx(NewTransaction(0, addr, new(big.Int), 0, new(big.Int),
		nil), signer, key)
	if err != nil {
		t.Fatal(err)
	}
	...
	if from != addr {
		t.Errorf("exected from and address to be equal. Got %x want %x", from,
			addr)
	}
}
...
func TestChainId(t *testing.T) {
	...
	tx := NewTransaction(0, common.Address{}, new(big.Int), 0, new(big.Int),
		nil)

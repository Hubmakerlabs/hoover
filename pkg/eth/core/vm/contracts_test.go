...
func testPrecompiled(addr string, test precompiledTest, t *testing.T) {
	...
	t.Run(fmt.Sprintf("%s-Gas=%d", test.Name, gas), func(t *testing.T) {
		if res, _, err := RunPrecompiledContract(p, in, gas); err != nil {
			...
			t.Errorf("Expected %v, got %v", test.Expected,
				common.Bytes2Hex(res))
		}
		if expGas := test.Gas; expGas != gas {
			t.Errorf("%v: gas wrong, expected %d, got %d", test.Name, expGas,
				gas)
		}
...
func benchmarkPrecompiled(addr string, test precompiledTest, bench *testing.B) {
	...
	bench.Run(fmt.Sprintf("%s-Gas=%d", test.Name, reqGas),
		func(bench *testing.B) {
			...
			if common.Bytes2Hex(res) != test.Expected {
				bench.Errorf("Expected %v, got %v", test.Expected,
					common.Bytes2Hex(res))
				return
			}
		})
...
func loadJsonFail(name string) ([]precompiledFailureTest, error) {
	data, err := os.ReadFile(fmt.Sprintf("testdata/precompiles/fail-%v.json",
		name))
	if err != nil {
		return nil, err
	}

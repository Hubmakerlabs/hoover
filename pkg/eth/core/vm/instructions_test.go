...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase,
	opFn executionFunc, name string) {
	var (
		env = NewEVM(BlockContext{}, TxContext{}, nil,
			params.TestChainConfig, Config{})
		stack          = newstack()
		pc             = uint64(0)
	...
	for i, test := range tests {
		...
		if len(stack.data) != 1 {
			t.Errorf("Expected one item on stack after %v, got %d: ", name,
				len(stack.data))
		}
		actual := stack.pop()

		if actual.Cmp(expected) != 0 {
			t.Errorf("Testcase %v %d, %v(%x, %x): expected  %x, got %x", name,
				i, name, x, y, expected, actual)
		}
	}
}
...
func TestWriteExpectedValues(t *testing.T) {
	...
	getResult := func(args []*twoOperandParams,
		opFn executionFunc) []TwoOperandTestcase {
		var (
			env = NewEVM(BlockContext{}, TxContext{}, nil,
				params.TestChainConfig, Config{})
			stack       = newstack()
		...
		for i, param := range args {
			...
			actual := stack.pop()
			result[i] = TwoOperandTestcase{param.x, param.y,
				fmt.Sprintf("%064x", actual)}
		}
		return result
	}
	...
	for name, method := range twoOpMethods {
		...
		_ = os.WriteFile(fmt.Sprintf("testdata/testcases_%v.json", name), data,
			0644)
		if err != nil {
			t.Fatal(err)
		}
...
func TestJsonTestcases(t *testing.T) {
	for name := range twoOpMethods {
		data, err := os.ReadFile(fmt.Sprintf("testdata/testcases_%v.json",
			name))
		if err != nil {
...
func TestOpMstore(t *testing.T) {
	...
	stack.push(new(uint256.Int))
	opMstore(&pc, evmInterpreter, &ScopeContext{mem, stack, nil})
	if common.Bytes2Hex(mem.GetCopy(0,
...
func TestOpTstore(t *testing.T) {
	var (
		...
		env = NewEVM(BlockContext{}, TxContext{}, statedb,
			params.TestChainConfig, Config{})
		stack          = newstack()
		mem            = NewMemory()
		evmInterpreter = NewEVMInterpreter(env)
		...
		contract       = NewContract(contractRef, AccountRef(to), new(big.Int),
			0)
	...
	for i, tt := range []testcase{
		...
		*/
		...
		if !bytes.Equal(expected.Bytes(), address.Bytes()) {
			t.Errorf("test %d: expected %s, got %s", i, expected.String(),
				address.String())
		}
	}
}
...
func TestRandom(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		{name: "hash(0x010203)",
			random: crypto.Keccak256Hash([]byte{0x01, 0x02, 0x03})},
	} {
		var (
			env = NewEVM(BlockContext{Random: &tt.random},
		...
		if len(stack.data) != 1 {
			t.Errorf("Expected one item on stack after %v, got %d: ", tt.name,
				len(stack.data))
		}
		actual := stack.pop()
		...
		if actual.Cmp(expected) != 0 {
			t.Errorf("Testcase %v: expected  %x, got %x", tt.name, expected,
				actual)
		}
	}
}
...
func TestBlobHash(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		{name: "[1,{2},3]", idx: 2, expect: three,
			hashes: []common.Hash{one, two, three}},
		...
		var (
			env = NewEVM(BlockContext{},
				TxContext{BlobHashes: tt.hashes}, nil, params.TestChainConfig,
				Config{})
			stack          = newstack()
			pc             = uint64(0)
			evmInterpreter = env.interpreter
...
func TestOpMCopy(t *testing.T) {
	...
	for i, tc := range []struct {
		...
		if memSize, overflow := memoryMcopy(stack); overflow {
			...
			var overflow bool
			if memorySize, overflow = math.SafeMul(toWordSize(memSize),
				32); overflow {
		...
		var haveGas uint64
		if dynamicCost, err := gasMcopy(env, nil, stack, mem,
			memorySize); err != nil {
			t.Error(err)
		} else {

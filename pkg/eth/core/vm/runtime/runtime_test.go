...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/core/asm"
	...
	_ "github.com/Hubmakerlabs/hoover/pkg/eth/eth/tracers/js"
)
...
func TestCall(t *testing.T) {
	state, _ := state.New(types.EmptyRootHash,
...
func TestBlockhash(t *testing.T) {
	...
	*/
	...
	if exp, got := 255, chain.counter; exp != got {
		t.Errorf("suboptimal; too much chain iteration, expected %d, got %d",
			exp, got)
	}
}
...
func benchmarkNonModifyingCode(gas uint64, code []byte, name string,
	tracerCode string, b *testing.B) {
	cfg := new(Config)
	setDefaults(cfg)
	cfg.State, _ = state.New(types.EmptyRootHash,
		state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	cfg.GasLimit = gas
	if len(tracerCode) > 0 {
		tracer, err := tracers.DefaultDirectory.New(tracerCode,
...
func BenchmarkSimpleLoop(b *testing.B) {
	...
	loopingCode := []byte{
		...
		byte(vm.POP), byte(vm.POP), byte(vm.POP), byte(vm.POP), byte(vm.POP),
		byte(vm.POP),
		byte(vm.PUSH1), 0, // jumpdestination
		byte(vm.JUMP),
	}
	...
	//	}})
	// 100M gas
	benchmarkNonModifyingCode(100000000, staticCallIdentity,
		"staticcall-identity-100M", "", b)
	benchmarkNonModifyingCode(100000000, callIdentity, "call-identity-100M", "",
		b)
	benchmarkNonModifyingCode(100000000, loopingCode, "loop-100M", "", b)
	benchmarkNonModifyingCode(100000000, callInexistant, "call-nonexist-100M",
		"", b)
	benchmarkNonModifyingCode(100000000, callEOA, "call-EOA-100M", "", b)
	benchmarkNonModifyingCode(100000000, callRevertingContractWithInput,
		"call-reverting-100M", "", b)
...
func TestEip2929Cases(t *testing.T) {
	...
	{ // EXTCODECOPY
		code := []byte{
			...
			byte(vm.PUSH1), 0x00, byte(vm.PUSH1), 0x00, byte(vm.PUSH1),
			0x00, // length, codeoffset, memoffset
	...
	{ // SLOAD + SSTORE
		code := []byte{
			...
			byte(vm.POP), // SLOAD( 0x1) (add to access list)
			// Write to `0x1` which is already in access list
			byte(vm.PUSH1), 0x11, byte(vm.PUSH1), 0x01,
			byte(vm.SSTORE), // SSTORE( loc: 0x01, val: 0x11)
			// Write to `0x2` which is not in access list
			byte(vm.PUSH1), 0x11, byte(vm.PUSH1), 0x02,
			byte(vm.SSTORE), // SSTORE( loc: 0x02, val: 0x11)
			// Write again to `0x2`
			byte(vm.PUSH1), 0x11, byte(vm.PUSH1), 0x02,
			byte(vm.SSTORE), // SSTORE( loc: 0x02, val: 0x11)
	...
	{ // Call variants
		code := []byte{
			// identity precompile
			byte(vm.PUSH1), 0x0, byte(vm.DUP1), byte(vm.DUP1), byte(vm.DUP1),
			byte(vm.DUP1),
			byte(vm.PUSH1), 0x04, byte(vm.PUSH1), 0x0, byte(vm.CALL),
			byte(vm.POP),

			// random account - call 1
			byte(vm.PUSH1), 0x0, byte(vm.DUP1), byte(vm.DUP1), byte(vm.DUP1),
			byte(vm.DUP1),
			byte(vm.PUSH1), 0xff, byte(vm.PUSH1), 0x0, byte(vm.CALL),
			byte(vm.POP),

			// random account - call 2
			byte(vm.PUSH1), 0x0, byte(vm.DUP1), byte(vm.DUP1), byte(vm.DUP1),
			byte(vm.DUP1),
			byte(vm.PUSH1), 0xff, byte(vm.PUSH1), 0x0, byte(vm.STATICCALL),
			byte(vm.POP),
		}
...
// correctly
// see: https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/22649
func TestColdAccountAccessCost(t *testing.T) {
	for i, tc := range []struct {
		...
		{ // EXTCODEHASH(0xff)
			code: []byte{byte(vm.PUSH1), 0xFF, byte(vm.EXTCODEHASH),
				byte(vm.POP)},
			step: 1,
			want: 2600,
		},
		...
		{ // CALL(0xff)
			code: []byte{
				...
				byte(vm.PUSH1), 0xff, byte(vm.DUP1), byte(vm.CALL),
				byte(vm.POP),
			},
			step: 7,
			want: 2855,
		...
		{ // CALLCODE(0xff)
			code: []byte{
				byte(vm.PUSH1), 0x0,
				byte(vm.DUP1), byte(vm.DUP1), byte(vm.DUP1), byte(vm.DUP1),
				byte(vm.PUSH1), 0xff, byte(vm.DUP1), byte(vm.CALLCODE),
				byte(vm.POP),
			},
			step: 7,
			want: 2855,
		...
		{ // DELEGATECALL(0xff)
			code: []byte{
				...
				byte(vm.PUSH1), 0xff, byte(vm.DUP1), byte(vm.DELEGATECALL),
				byte(vm.POP),
			},
			step: 6,
			want: 2855,
		...
		{ // STATICCALL(0xff)
			code: []byte{
				byte(vm.PUSH1), 0x0,
				byte(vm.DUP1), byte(vm.DUP1), byte(vm.DUP1),
				byte(vm.PUSH1), 0xff, byte(vm.DUP1), byte(vm.STATICCALL),
				byte(vm.POP),
			},
			step: 6,
			want: 2855,
		...
		if want := tc.want; have != want {
			...
			t.Fatalf("tescase %d, gas report wrong, step %d, have %d want %d",
				i, tc.step, have, want)
		}
	}
}
...
func TestRuntimeJSTracer(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			code: []byte{
				...
				byte(vm.PUSH1), 1, byte(vm.PUSH1), 5, byte(vm.PUSH1), 27,
				byte(vm.PUSH1), 0,
				byte(vm.CREATE2),
				byte(vm.POP),
			},
	...
	for i, jsTracer := range jsTracers {
		for j, tc := range tests {
			...
			tracer, err := tracers.DefaultDirectory.New(jsTracer,
				new(tracers.Context), nil)
			if err != nil {
				t.Fatal(err)
			}
...
func TestJSTracerCreateTx(t *testing.T) {
	...
	statedb, _ := state.New(types.EmptyRootHash,
		state.NewDatabase(rawdb.NewMemoryDatabase()), nil)
	tracer, err := tracers.DefaultDirectory.New(jsTracer, new(tracers.Context),
		nil)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkTracerStepVsCallFrame(b *testing.B) {
	...
	benchmarkNonModifyingCode(10000000, code, "tracer-call-frame-10M",
		callFrameTracer, b)
}

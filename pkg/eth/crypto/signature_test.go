...
func TestVerifySignature(t *testing.T) {
	...
	if VerifySignature(testpubkey, testmsg,
		append(common.CopyBytes(sig), 1, 2, 3)) {
...
func TestDecompressPubkey(t *testing.T) {
	...
	if uncompressed := FromECDSAPub(key); !bytes.Equal(uncompressed,
		testpubkey) {
		t.Errorf("wrong public key result: got %x, want %x", uncompressed,
			testpubkey)
	}
	...
	if _, err := DecompressPubkey(append(common.CopyBytes(testpubkeyc), 1, 2,
		3)); err == nil {
...
func TestCompressPubkey(t *testing.T) {
	...
	if !bytes.Equal(compressed, testpubkeyc) {
		t.Errorf("wrong public key result: got %x, want %x", compressed,
			testpubkeyc)
	}
}

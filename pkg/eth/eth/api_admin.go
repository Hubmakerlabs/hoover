...
func (api *AdminAPI) ExportChain(file string, first *uint64,
	...
	if first != nil {
		if err := api.eth.BlockChain().ExportN(writer, *first,
			*last); err != nil {
			return false, err
		}
	...
	for batch := 0; ; batch++ {
		...
		for len(blocks) < cap(blocks) {
			...
			if err := stream.Decode(block); err == io.EOF {
				...
				return false, fmt.Errorf("block %d: failed to parse: %v", index,
					err)
			}
		...
		if _, err := api.eth.BlockChain().InsertChain(blocks); err != nil {
			return false, fmt.Errorf("batch %d: failed to insert: %v", batch,
				err)
		}
		blocks = blocks[:0]
	}

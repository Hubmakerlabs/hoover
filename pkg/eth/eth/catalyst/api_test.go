...
import (
	...
	beaconConsensus "github.com/Hubmakerlabs/hoover/pkg/eth/consensus/beacon"
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/crypto/kzg4844"
...
func generateMergeChain(n int, merged bool) (*core.Genesis, []*types.Block) {
	...
	genesis := &core.Genesis{
		...
		Alloc: core.GenesisAlloc{
			...
			params.BeaconRootsStorageAddress: {Balance: common.Big0,
	...
	generate := func(i int, g *core.BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(testNonce,
			common.HexToAddress("0x9a9070028361F7AAbeB3f2F2Dc07F82C4a98A02a"),
			big.NewInt(1), params.TxGas, big.NewInt(params.InitialBaseFee*2),
			nil), types.LatestSigner(&config), testKey)
		g.AddTx(tx)
		testNonce++
	}
...
func TestEth2AssembleBlock(t *testing.T) {
	...
	tx, err := types.SignTx(types.NewTransaction(uint64(10),
		blocks[9].Coinbase(), big.NewInt(1000), params.TxGas,
		big.NewInt(params.InitialBaseFee), nil), signer, testKey)
	if err != nil {
	...
	// needed.
	if _, testErr := assembleWithTransactions(api, blocks[9].Hash(),
		&blockParams, 1); testErr != nil {
		t.Fatal(testErr)
	}
}
...
func assembleWithTransactions(api *ConsensusAPI, parentHash common.Hash,
	params *engine.PayloadAttributes,
	want int) (execData *engine.ExecutableData, err error) {
	for retries := 3; retries > 0; retries-- {
		...
		if have, want := len(execData.Transactions), want; have != want {
			err = fmt.Errorf("invalid number of transactions, have %d want %d",
				have, want)
			continue
		}
		return execData, nil
...
func TestEth2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	...
	// The miner needs to pick up on the txs in the pool, so a few retries might be
	// needed.
	if _, err := assembleWithTransactions(api, blocks[8].Hash(), &blockParams,
		blocks[9].Transactions().Len()); err != nil {
		t.Fatal(err)
	}
}
...
func TestEth2PrepareAndGetPayload(t *testing.T) {
	...
	genesis.Config.TerminalTotalDifficulty.Sub(genesis.Config.TerminalTotalDifficulty,
		blocks[9].Difficulty())
	n, ethservice := startEthService(t, genesis, blocks[:9])
	defer n.Close()
	...
	if len(execData.Transactions) != blocks[9].Transactions().Len() {
		t.Fatalf("invalid number of transactions %d != 1",
			len(execData.Transactions))
	}
...
func checkLogEvents(t *testing.T, logsCh <-chan []*types.Log,
	rmLogsCh <-chan core.RemovedLogsEvent, wantNew, wantRemoved int) {
	t.Helper()

	if len(logsCh) != wantNew {
		t.Fatalf("wrong number of log events: got %d, want %d", len(logsCh),
			wantNew)
	}
	if len(rmLogsCh) != wantRemoved {
		t.Fatalf("wrong number of removed log events: got %d, want %d",
			len(rmLogsCh), wantRemoved)
	}
	// Drain events.
	...
	for i, test := range tests {
		t.Run(fmt.Sprintf("Timestamp test: %v", i), func(t *testing.T) {
			...
			if test.shouldErr && err == nil {
				t.Fatalf("expected error preparing payload with invalid timestamp, err=%v",
					err)
			} else if !test.shouldErr && err != nil {
				t.Fatalf("error preparing payload with valid timestamp, err=%v",
					err)
			}
		})
	}
...
func TestEth2NewBlock(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		tx, _ := types.SignTx(types.NewContractCreation(nonce, new(big.Int),
			1000000, big.NewInt(2*params.InitialBaseFee), logCode),
			types.LatestSigner(ethservice.BlockChain().Config()), testKey)
		ethservice.TxPool().Add([]*types.Transaction{tx}, true, false)

		execData, err := assembleWithTransactions(api, parent.Hash(),
		...
		if have, want := ethservice.BlockChain().CurrentBlock().Number.Uint64(), block.NumberU64(); have != want {
			t.Fatalf("Chain head should be updated, have %d want %d", have,
				want)
		}
	...
	parent = preMergeBlocks[len(preMergeBlocks)-1]
	for i := 0; i < 10; i++ {
		execData, err := assembleBlock(api, parent.Hash(),
			&engine.PayloadAttributes{
...
func startEthService(t *testing.T, genesis *core.Genesis,
	blocks []*types.Block) (*node.Node, *eth.Ethereum) {
	t.Helper()
	...
	ethcfg := &ethconfig.Config{Genesis: genesis, SyncMode: downloader.FullSync,
...
func setupBlocks(t *testing.T, ethservice *eth.Ethereum, n int,
	parent *types.Header, callback func(parent *types.Header),
	withdrawals [][]*types.Withdrawal) []*types.Header {
	api := NewConsensusAPI(ethservice)
...
func TestNewPayloadOnInvalidChain(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		for i := 0; ; i++ {
			if resp, err = api.ForkchoiceUpdatedV1(fcState,
				&params); err != nil {
				t.Fatalf("error preparing payload, err=%v", err)
			}
			if resp.PayloadStatus.Status != engine.VALID {
				t.Fatalf("error preparing payload, invalid status: %v",
...
func assembleBlock(api *ConsensusAPI, parentHash common.Hash,
...
func TestEmptyBlocks(t *testing.T) {
	...
	setupBlocks(t, ethservice, 10, commonAncestor,
		func(parent *types.Header) {}, nil)
	...
	if !bytes.Equal(status.LatestValidHash[:], payload.BlockHash[:]) {
		t.Fatalf("invalid LVH: got %v want %v", status.LatestValidHash,
			payload.BlockHash)
	}
...
func getNewPayload(t *testing.T, api *ConsensusAPI, parent *types.Header,
	withdrawals []*types.Withdrawal) *engine.ExecutableData {
	params := engine.PayloadAttributes{
		Timestamp:             parent.Time + 1,
...
func TestTrickRemoteBlockCache(t *testing.T) {
	...
	// Setup 10 blocks on the canonical chain
	setupBlocks(t, ethserviceA, 10, commonAncestor,
		func(parent *types.Header) {}, nil)
	commonAncestor = ethserviceA.BlockChain().CurrentBlock()
	...
	for _, payload := range invalidChain {
		...
		resp, err := apiB.ForkchoiceUpdatedV1(engine.ForkchoiceStateV1{HeadBlockHash: payload.BlockHash,
			SafeBlockHash:      payload.BlockHash,
			FinalizedBlockHash: payload.ParentHash}, nil)
		if err != nil {
			t.Fatal(err)
		}
		if resp.PayloadStatus.Status == engine.VALID {
...
func TestNewPayloadOnInvalidTerminalBlock(t *testing.T) {
	...
	if resp.PayloadStatus != engine.INVALID_TERMINAL_BLOCK {
		t.Fatalf("error sending invalid forkchoice, invalid status: %v",
			resp.PayloadStatus.Status)
	}
	...
	for i := 0; i < 10; i++ {
			...
			for ii := 0; ii < 10; ii++ {
				go func() {
					...
					if newResp, err := api.NewPayloadV1(*execData); err != nil {
						...
						errMu.Lock()
						testErr = fmt.Errorf("Failed to insert block: %v",
							newResp.Status)
						errMu.Unlock()
					}
				}()
		...
		}
		{
			...
			for ii := 0; ii < 10; ii++ {
				go func() {
					defer wg.Done()
					if _, err := api.ForkchoiceUpdatedV1(fcState,
						nil); err != nil {
						errMu.Lock()
						testErr = fmt.Errorf("Failed to insert block: %w", err)
						errMu.Unlock()
...
func TestWithdrawals(t *testing.T) {
	...
	if resp.PayloadStatus.Status != engine.VALID {
		t.Fatalf("unexpected status (got: %s, want: %s)",
	...
	if execData.ExecutionPayload.StateRoot != parent.Root {
		t.Fatalf("mismatch state roots (got: %s, want: %s)",
			execData.ExecutionPayload.StateRoot, blocks[8].Root())
	}
	...
	db, _, err := ethservice.APIBackend.StateAndHeaderByNumber(context.Background(),
		rpc.BlockNumber(execData.ExecutionPayload.Number))
	if err != nil {
...
func setupBodies(t *testing.T) (*node.Node, *eth.Ethereum, []*types.Block) {
	...
	for i := 2; i < len(withdrawals); i++ {
		...
		withdrawals[i] = []*types.Withdrawal{
			{Index: rand.Uint64(), Validator: rand.Uint64(),
				Amount: rand.Uint64(), Address: common.BytesToAddress(addr)},
		}
	}

	postShanghaiHeaders := setupBlocks(t, ethservice, 10, parent, callback,
		withdrawals)
	postShanghaiBlocks := make([]*types.Block, len(postShanghaiHeaders))
	for i, header := range postShanghaiHeaders {
		postShanghaiBlocks[i] = ethservice.BlockChain().GetBlock(header.Hash(),
...
func TestGetBlockBodiesByHash(t *testing.T) {
	...
	tests := []struct {
		...
		{
			results: []*types.Body{blocks[0].Body(), blocks[9].Body(),
				blocks[14].Body()},
			hashes: []common.Hash{blocks[0].Hash(), blocks[9].Hash(),
				blocks[14].Hash()},
		},
		// unavailable block
		{
		...
		{
			results: []*types.Body{blocks[0].Body(), nil, blocks[0].Body(),
				blocks[0].Body()},
			hashes: []common.Hash{blocks[0].Hash(), {1, 2}, blocks[0].Hash(),
				blocks[0].Hash()},
		},
		// all blocks
		{
	...
	for k, test := range tests {
		...
		for i, r := range result {
			if !equalBody(test.results[i], r) {
				t.Fatalf("test %v: invalid response: expected %+v got %+v", k,
					test.results[i], r)
			}
		}
	}
...
func TestGetBlockBodiesByRange(t *testing.T) {
	...
	tests := []struct {
		...
		// Pre & post merge blocks
		{
			results: []*types.Body{blocks[7].Body(), blocks[8].Body(),
				blocks[9].Body(), blocks[10].Body()},
			start: 8,
			count: 4,
		},
		// unavailable block
	...
	for k, test := range tests {
		...
		if len(result) == len(test.results) {
			...
			t.Fatalf("test %d: invalid length want %v got %v", k,
				len(test.results), len(result))
		}
	}
}
...
func TestBlockToPayloadWithBlobs(t *testing.T) {
	...
	_, err := engine.ExecutableDataToBlock(*envelope.ExecutionPayload,
		make([]common.Hash, 1), nil)
	if err != nil {
		t.Error(err)
	}
}
...
func TestParentBeaconBlockRoot(t *testing.T) {
	log.Root().SetHandler(log.LvlFilterHandler(log.LvlTrace,
		log.StreamHandler(colorable.NewColorableStderr(),
	...
	resp, err := api.ForkchoiceUpdatedV2(fcState, &blockParams)
	if err != nil {
		t.Fatalf("error preparing payload, err=%v",
			err.(*engine.EngineAPIError).ErrorData())
	}
	if resp.PayloadStatus.Status != engine.VALID {
		t.Fatalf("unexpected status (got: %s, want: %s)",
			resp.PayloadStatus.Status, engine.VALID)
	...
	if status, err := api.NewPayloadV3(*execData.ExecutionPayload,
		[]common.Hash{}, &common.Hash{42}); err != nil {
		t.Fatalf("error validating payload: %v", err)
	} else if status.Status != engine.VALID {
		t.Fatalf("invalid payload")
	}
	...
	if num := db.GetState(params.BeaconRootsStorageAddress,
		timeIdx); num != timeIdx {
		t.Fatalf("incorrect number stored: want %s, got %s", timeIdx, num)
	}
	if root := db.GetState(params.BeaconRootsStorageAddress,
		rootIdx); root != *blockParams.BeaconRoot {
		t.Fatalf("incorrect root stored: want %s, got %s",
			*blockParams.BeaconRoot, root)
	}
}

...
func NewSimulatedBeacon(period uint64, eth *eth.Ethereum) (*SimulatedBeacon,
	error) {
...
func (c *SimulatedBeacon) sealBlock(withdrawals []*types.Withdrawal) error {
	...
	var random [32]byte
	rand.Read(random[:])
	fcResponse, err := c.engineAPI.ForkchoiceUpdatedV2(c.curForkchoiceState,
	...
	if _, err = c.engineAPI.ForkchoiceUpdatedV2(c.curForkchoiceState,
		nil); err != nil {
		return err
	}

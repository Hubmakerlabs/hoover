...
func startSimulatedBeaconEthService(t *testing.T,
	genesis *core.Genesis) (*node.Node, *eth.Ethereum, *SimulatedBeacon) {
	t.Helper()
...
func TestSimulatedBeaconSendWithdrawals(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		tx, err := types.SignTx(types.NewTransaction(uint64(i),
			common.Address{}, big.NewInt(1000), params.TxGas,
		...
		if err := ethService.APIBackend.SendTx(context.Background(),
			tx); err != nil {
	...
	for {
		...
		case evt := <-chainHeadCh:
			...
			for _, includedWithdrawal := range evt.Block.Withdrawals() {
				includedWithdrawals = append(includedWithdrawals,
					includedWithdrawal.Index)
			}

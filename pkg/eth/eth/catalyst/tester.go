...
func RegisterFullSyncTester(stack *node.Node, backend *eth.Ethereum,
	target common.Hash) (*FullSyncTester, error) {
	cl := &FullSyncTester{
		stack:   stack,
		backend: backend,
		target:  target,
...
func (tester *FullSyncTester) Start() error {
	...
	go func() {
		...
		// chain head.
		err := tester.backend.Downloader().BeaconDevSync(downloader.FullSync,
			tester.target, tester.closed)
		if err != nil {
		...
		for {
			...
			case <-ticker.C:
				...
				if block := tester.backend.BlockChain().GetBlockByHash(tester.target); block != nil {
					log.Info("Full-sync target reached", "number",

...
func (d *Downloader) BeaconDevSync(mode SyncMode, hash common.Hash,
	...
	for {
		...
		if err != nil || len(headers) != 1 {
			log.Warn("Failed to fetch sync target", "headers", len(headers),
				"err", err)
			time.Sleep(time.Second)
			continue
		}
		...
		if metas[0] != hash {
			log.Error("Received invalid sync target", "want", hash, "have",
				metas[0])
			time.Sleep(time.Second)
			continue
		}

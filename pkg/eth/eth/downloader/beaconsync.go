...
func (b *beaconBackfiller) resume() {
	...
	go func() {
		...
		if err := b.downloader.synchronise("", common.Hash{}, nil, nil, mode,
			true, b.started); err != nil {
...
func (b *beaconBackfiller) setMode(mode SyncMode) {
	...
	log.Error("Downloader sync mode changed mid-run", "old", mode.String(),
		"new", mode.String())
	b.suspend()
	b.resume()
}
...
func (d *Downloader) BeaconSync(mode SyncMode, head *types.Header,
	final *types.Header) error {
...
// retrieval and scheduling is replaced.
func (d *Downloader) beaconSync(mode SyncMode, head *types.Header,
	final *types.Header, force bool) error {
...
func (d *Downloader) findBeaconAncestor() (uint64, error) {
	...
	switch d.getMode() {
	case FullSync:
		linked = d.blockchain.HasBlock(beaconTail.ParentHash,
			beaconTail.Number.Uint64()-1)
	case SnapSync:
		linked = d.blockchain.HasFastBlock(beaconTail.ParentHash,
			beaconTail.Number.Uint64()-1)
	default:
		linked = d.blockchain.HasHeader(beaconTail.ParentHash,
			beaconTail.Number.Uint64()-1)
	}
	if !linked {
		...
		log.Error("Beacon sync linkup unavailable", "number",
			beaconTail.Number.Uint64()-1, "hash", beaconTail.ParentHash)
		return 0, fmt.Errorf("beacon linkup unavailable locally: %d [%x]",
			beaconTail.Number.Uint64()-1, beaconTail.ParentHash)
	}
	...
	if number := beaconHead.Number.Uint64(); end > number {
		...
		log.Warn("Beacon head lower than local chain", "beacon", number,
			"local", end)
		end = number
	}
	for start+1 < end {
...
func (d *Downloader) fetchBeaconHeaders(from uint64) error {
	...
	if from < tail.Number.Uint64() {
		...
		if count > uint64(fsMinFullBlocks) {
			return fmt.Errorf("invalid origin (%d) of beacon sync (%d)", from,
				tail.Number)
		}
		localHeaders = d.readHeaderRange(tail, int(count))
		log.Warn("Retrieved beacon headers from local", "from", from, "count",
			count)
	}
	for {
		...
		if d.pivotHeader != nil {
			if head.Number.Uint64() > d.pivotHeader.Number.Uint64()+2*uint64(fsMinFullBlocks)-8 {
				...
				log.Warn("Pivot seemingly stale, moving", "old",
					d.pivotHeader.Number, "new", number)
				if d.pivotHeader = d.skeleton.Header(number); d.pivotHeader == nil {
					if number < tail.Number.Uint64() {
						...
						if len(localHeaders) >= int(dist) {
							...
							log.Warn("Retrieved pivot header from local",
								"number", d.pivotHeader.Number, "hash",
								d.pivotHeader.Hash(), "latest", head.Number,
								"oldest", tail.Number)
						}
					}
				}
				...
				rawdb.WriteLastPivotNumber(d.stateDB,
		...
		if len(headers) > 0 {
			log.Trace("Scheduling new beacon headers", "count", len(headers),
				"from", from-uint64(len(headers)))
			select {

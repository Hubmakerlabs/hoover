...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/eth/protocols/snap"
...
type Downloader struct {
	...
	// Status
	synchroniseMock func(id string,
		hash common.Hash) error // Replacement for synchronise during testing
	...
	syncInitHook func(uint64,
		uint64) // Method to call upon initiating a new sync run
...
func New(stateDb ethdb.Database, mux *event.TypeMux, chain BlockChain,
	lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
	...
	dl.skeleton = newSkeleton(stateDb, dl.peers, dropPeer,
		newBeaconBackfiller(dl, success))
...
func (d *Downloader) Progress() ethereum.SyncProgress {
	...
	default:
		log.Error("Unknown downloader chain/mode combo", "light",
			d.lightchain != nil, "full", d.blockchain != nil, "mode", mode)
	}
...
func (d *Downloader) RegisterPeer(id string, version uint, peer Peer) error {
	...
	if err := d.peers.Register(newPeerConnection(id, version, peer,
		logger)); err != nil {
...
func (d *Downloader) LegacySync(id string, head common.Hash, td, ttd *big.Int,
	mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err,
		errBadPeer) || errors.Is(err, errTimeout) ||
		errors.Is(err, errStallingPeer) || errors.Is(err,
		errUnsyncedPeer) || errors.Is(err, errEmptyHeaderSet) ||
		errors.Is(err, errPeersUnavailable) || errors.Is(err,
		errTooOld) || errors.Is(err, errInvalidAncestor) {
		log.Warn("Synchronisation failed, dropping peer", "peer", id, "err",
			err)
		if d.dropPeer == nil {
...
func (d *Downloader) synchronise(id string, hash common.Hash, td, ttd *big.Int,
	mode SyncMode, beaconMode bool, beaconPing chan struct{}) error {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash,
	td, ttd *big.Int, beaconMode bool) (err error) {
	...
	if !beaconMode {
		log.Debug("Synchronising with the network", "peer", p.id, "eth",
			p.version, "head", hash, "td", td, "mode", mode)
	} else {
		log.Debug("Backfilling with the network", "mode", mode)
	}
	defer func(start time.Time) {
		log.Debug("Synchronisation terminated", "elapsed",
			...
			if pivot = d.skeleton.Header(number); pivot == nil {
				...
				if number < oldest.Number.Uint64() {
					...
					if len(headers) == count {
						...
						log.Warn("Retrieved pivot header from local", "number",
							pivot.Number, "hash", pivot.Hash(), "latest",
	...
	if mode == SnapSync {
		...
		if origin >= frozen && frozen != 0 {
			d.ancientLimit = 0
			log.Info("Disabling direct-ancient mode", "origin", origin,
				"ancient", frozen-1)
	...
	if !beaconMode {
		...
		headerFetcher = func() error {
			return d.fetchHeaders(p, origin+1,
	...
	fetchers := []func() error{
		...
		func() error {
			return d.fetchBodies(origin+1,
				beaconMode)
		}, // Bodies are retrieved during normal and snap sync
		func() error {
			return d.fetchReceipts(origin+1,
				beaconMode)
		}, // Receipts are retrieved during snap sync
	...
	if mode == SnapSync {
		...
		fetchers = append(fetchers,
			func() error { return d.processFullSyncContent(ttd, beaconMode) })
	}
...
// a remote peer.
func (d *Downloader) fetchHead(p *peerConnection) (head *types.Header,
	pivot *types.Header, err error) {
	...
	headers, hashes, err := d.fetchHeadersByHash(p, latest, fetch,
		fsMinFullBlocks-1, true)
	if err != nil {
		return nil, nil, err
	}
	// Make sure the peer gave us at least one and at most the requested headers
	if len(headers) == 0 || len(headers) > fetch {
		return nil, nil, fmt.Errorf("%w: returned headers %d != requested %d",
			errBadPeer, len(headers), fetch)
	}
	...
	if len(headers) == 1 {
		if mode == SnapSync && head.Number.Uint64() > uint64(fsMinFullBlocks) {
			return nil, nil, fmt.Errorf("%w: no pivot included along head header",
				errBadPeer)
		}
		p.log.Debug("Remote head identified, no pivot", "number", head.Number,
			"hash", hashes[0])
		return head, nil, nil
	}
	...
	if pivot.Number.Uint64() != head.Number.Uint64()-uint64(fsMinFullBlocks) {
		return nil, nil, fmt.Errorf("%w: remote pivot %d != requested %d",
			errInvalidChain, pivot.Number,
			head.Number.Uint64()-uint64(fsMinFullBlocks))
	}
...
func calculateRequestSpan(remoteHeight, localHeight uint64) (int64, int, int,
	uint64) {
	var (
		from     int
		count    int
...
func (d *Downloader) findAncestor(p *peerConnection,
	remoteHeader *types.Header) (uint64, error) {
	...
	p.log.Debug("Looking for common ancestor", "local", localHeight, "remote",
		remoteHeight)
	...
	ancestor, err := d.findAncestorSpanSearch(p, mode, remoteHeight,
		localHeight, floor)
	if err == nil {
		return ancestor, nil
	}
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode,
	remoteHeight, localHeight uint64, floor int64) (uint64, error) {
	from, count, skip, max := calculateRequestSpan(remoteHeight, localHeight)

	p.log.Trace("Span searching for common ancestor", "count", count, "from",
		from, "skip", skip)
	headers, hashes, err := d.fetchHeadersByNumber(p, uint64(from), count, skip,
		false)
	if err != nil {
		return 0, err
	}
	...
	for i, header := range headers {
		...
		if number := header.Number.Int64(); number != expectNumber {
			p.log.Warn("Head headers broke chain ordering", "index", i,
				"requested", expectNumber, "received", number)
			return 0, fmt.Errorf("%w: %v", errInvalidChain,
				errors.New("head headers broke chain ordering"))
		}
	}
	...
	if hash != (common.Hash{}) {
		if int64(number) <= floor {
			p.log.Warn("Ancestor below allowance", "number", number, "hash",
				hash, "allowance", floor)
...
func (d *Downloader) findAncestorBinarySearch(p *peerConnection, mode SyncMode,
	remoteHeight uint64, floor int64) (uint64, error) {
	hash := common.Hash{}
	...
	p.log.Trace("Binary searching for common ancestor", "start", start, "end",
		end)
	...
	for start+1 < end {
		...
		// Make sure the peer actually gave something valid
		if len(headers) != 1 {
			p.log.Warn("Multiple headers for single request", "headers",
				len(headers))
			return 0, fmt.Errorf("%w: multiple headers (%d) for single request",
				errBadPeer, len(headers))
		}
		...
		if header.Number.Uint64() != check {
			p.log.Warn("Received non requested header", "number", header.Number,
				"hash", header.Hash(), "request", check)
			return 0, fmt.Errorf("%w: non-requested header (%d)", errBadPeer,
				header.Number)
		}
		start = check
		hash = h
	...
	if int64(start) <= floor {
		p.log.Warn("Ancestor below allowance", "number", start, "hash", hash,
			"allowance", floor)
		return 0, errInvalidAncestor
	}
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64,
	head uint64) error {
	...
	for {
		...
		case pivoting:
			...
			p.log.Trace("Fetching next pivot header", "number",
				pivot+uint64(fsMinFullBlocks))
			headers, hashes, err = d.fetchHeadersByNumber(p,
				pivot+uint64(fsMinFullBlocks), 2, fsMinFullBlocks-9,
				false) // move +64 when it's 2x64-8 deep

		case skeleton:
			p.log.Trace("Fetching skeleton headers", "count", MaxHeaderFetch,
				"from", from)
			headers, hashes, err = d.fetchHeadersByNumber(p,
				from+uint64(MaxHeaderFetch)-1, MaxSkeletonSize,
				MaxHeaderFetch-1, false)

		default:
			p.log.Trace("Fetching full headers", "count", MaxHeaderFetch,
				"from", from)
			headers, hashes, err = d.fetchHeadersByNumber(p, from,
				MaxHeaderFetch, 0, false)
		}
		switch err {
		case nil:
		...
		default:
			...
			for _, ch := range []chan bool{d.queue.blockWakeCh,
				d.queue.receiptWakeCh} {
				select {
				case ch <- false:
				case <-d.cancelCh:
				}
		...
		if pivoting {
			if len(headers) == 2 {
				if have, want := headers[0].Number.Uint64(), pivot+uint64(fsMinFullBlocks); have != want {
					log.Warn("Peer sent invalid next pivot", "have", have,
						"want", want)
					return fmt.Errorf("%w: next pivot number %d != requested %d",
						errInvalidChain, have, want)
				}
				if have, want := headers[1].Number.Uint64(), pivot+2*uint64(fsMinFullBlocks)-8; have != want {
					log.Warn("Peer sent invalid pivot confirmer", "have", have,
						"want", want)
					return fmt.Errorf("%w: next pivot confirmer number %d != requested %d",
						errInvalidChain, have, want)
				}
				log.Warn("Pivot seemingly stale, moving", "old", pivot, "new",
					headers[0].Number)
		...
		if skeleton && len(headers) == 0 {
			...
			if from+uint64(MaxHeaderFetch)-1 <= head {
				p.log.Warn("Peer withheld skeleton headers", "advertised", head,
					"withheld", from+uint64(MaxHeaderFetch)-1)
				return fmt.Errorf("%w: withheld skeleton headers: advertised %d, withheld #%d",
					errStallingPeer, head, from+uint64(MaxHeaderFetch)-1)
			}
		...
		if skeleton {
			...
			if n := len(headers); n < MaxHeaderFetch && headers[n-1].Number.Uint64() < head {
				p.log.Warn("Peer withheld headers", "advertised", head,
					"delivered", headers[n-1].Number.Uint64())
				return fmt.Errorf("%w: withheld headers: advertised %d, delivered %d",
					errStallingPeer, head, headers[n-1].Number.Uint64())
			}
...
func (d *Downloader) fillHeaderSkeleton(from uint64,
	skeleton []*types.Header) ([]*types.Header, []common.Hash, int, error) {
	...
	if err == nil {
		log.Debug("Skeleton fill succeeded", "filled", len(filled), "processed",
			proced)
	}
...
func (d *Downloader) processHeaders(origin uint64, td, ttd *big.Int,
	beaconMode bool) error {
	var (
		mode       = d.getMode()
			...
			if task == nil || len(task.headers) == 0 {
				...
				if !beaconMode {
					...
					if mode != LightSync {
						...
						if !gotHeaders && td.Cmp(d.blockchain.GetTd(head.Hash(),
							head.Number.Uint64())) > 0 {
					...
					if mode == SnapSync || mode == LightSync {
						...
						if td.Cmp(d.lightchain.GetTd(head.Hash(),
							head.Number.Uint64())) > 0 {
							return errStallingPeer
						}
					}
			...
			for len(headers) > 0 {
				...
				if mode == SnapSync || mode == LightSync {
					...
					if !beaconMode && ttd != nil {
						td = d.blockchain.GetTd(chunkHeaders[0].ParentHash,
							chunkHeaders[0].Number.Uint64()-1)
						if td == nil {
							// This should never really happen, but handle gracefully for now
							log.Error("Failed to retrieve parent header TD",
								"number", chunkHeaders[0].Number.Uint64()-1,
								"hash", chunkHeaders[0].ParentHash)
							return fmt.Errorf("%w: parent TD missing",
								errInvalidChain)
						}
					...
					if len(chunkHeaders) > 0 {
						if n, err := d.lightchain.InsertHeaderChain(chunkHeaders); err != nil {
							log.Warn("Invalid header encountered", "number",
								chunkHeaders[n].Number, "hash", chunkHashes[n],
								"parent", chunkHeaders[n].ParentHash, "err",
								err)
							return fmt.Errorf("%w: %v", errInvalidChain, err)
						}
					}
					if len(rejected) != 0 {
						log.Info("Legacy sync reached merge threshold",
							"number", rejected[0].Number, "hash",
							rejected[0].Hash(), "td", td, "ttd", ttd)
				...
				if mode == FullSync || mode == SnapSync {
					...
					inserts := d.queue.Schedule(chunkHeaders, chunkHashes,
						origin)
...
func (d *Downloader) processFullSyncContent(ttd *big.Int,
	beaconMode bool) error {
	for {
		...
		if !beaconMode && ttd != nil {
			td = d.blockchain.GetTd(results[0].Header.ParentHash,
				results[0].Header.Number.Uint64()-1)
			if td == nil {
				// This should never really happen, but handle gracefully for now
				log.Error("Failed to retrieve parent block TD", "number",
					results[0].Header.Number.Uint64()-1, "hash",
					results[0].Header.ParentHash)
			...
			for i, result := range results {
				...
				if td.Cmp(ttd) >= 0 {
					...
					if new(big.Int).Sub(td,
						result.Header.Difficulty).Cmp(ttd) < 0 {
						results, rejected = results[:i+1], results[i+1:]
						if len(rejected) > 0 {
							// Make a nicer user log as to the first TD truly rejected
							td = new(big.Int).Add(td,
								rejected[0].Header.Difficulty)
						}
					} else {
		...
		if len(rejected) != 0 {
			log.Info("Legacy sync reached merge threshold", "number",
				rejected[0].Header.Number, "hash", rejected[0].Header.Hash(),
				"td", td, "ttd", ttd)
			return ErrMergeTransition
		}
	}
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for i, result := range results {
		blocks[i] = types.NewBlockWithHeader(result.Header).WithBody(result.Transactions,
			result.Uncles).WithWithdrawals(result.Withdrawals)
	}
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			log.Debug("Downloaded item processing failed", "number",
				results[index].Header.Number, "hash",
			...
			if d.badBlock != nil {
				...
				if err != nil {
					log.Error("Failed to retrieve beacon bounds for bad block reporting",
						"err", err)
				} else {
			...
			log.Debug("Downloaded item processing failed on sidechain import",
				"index", index, "err", err)
		}
		return fmt.Errorf("%w: %v", errInvalidChain, err)
	}
...
func (d *Downloader) processSnapSyncContent() error {
	...
	for {
		...
		if oldPivot == nil { // no results piling up, we can move the pivot
			...
			results = append(append([]*fetchResult{oldPivot}, oldTail...),
				results...)
		}
		...
		if !d.committed.Load() {
			...
			if height := latest.Number.Uint64(); height >= pivot.Number.Uint64()+2*uint64(fsMinFullBlocks)-uint64(reorgProtHeaderDelay) {
				log.Warn("Pivot became stale, moving", "old",
					pivot.Number.Uint64(), "new",
...
func splitAroundPivot(pivot uint64, results []*fetchResult) (p *fetchResult,
	before, after []*fetchResult) {
	if len(results) == 0 {
		return nil, nil, nil
	}
...
func (d *Downloader) commitSnapSyncData(results []*fetchResult,
	stateSync *stateSync) error {
	// Check for any early termination requests
	if len(results) == 0 {
		return nil
	}
	...
	if index, err := d.blockchain.InsertReceiptChain(blocks, receipts,
		d.ancientLimit); err != nil {
		log.Debug("Downloaded item processing failed", "number",
			results[index].Header.Number, "hash", results[index].Header.Hash(),
			"err", err)
		return fmt.Errorf("%w: %v", errInvalidChain, err)
	}
	return nil
...
func (d *Downloader) commitPivotBlock(result *fetchResult) error {
	...
	if _, err := d.blockchain.InsertReceiptChain([]*types.Block{block},
		[]types.Receipts{result.Receipts}, d.ancientLimit); err != nil {
		return err
	}
...
func (d *Downloader) DeliverSnapPacket(peer *snap.Peer,
	packet snap.Packet) error {
	...
	case *snap.AccountRangePacket:
		...
		return d.SnapSyncer.OnAccounts(peer, packet.ID, hashes, accounts,
			packet.Proof)

	case *snap.StorageRangesPacket:
		hashset, slotset := packet.Unpack()
		return d.SnapSyncer.OnStorage(peer, packet.ID, hashset, slotset,
			packet.Proof)
...
func (d *Downloader) readHeaderRange(last *types.Header,
	count int) []*types.Header {
	var (
		current = last
		headers []*types.Header
	)
...
func (d *Downloader) reportSnapSyncProgress(force bool) {
	...
	var (
		...
		progress = fmt.Sprintf("%.2f%%",
			float64(block.Number.Uint64())*100/float64(latest.Number.Uint64()))
		headers = fmt.Sprintf("%v@%v",
			log.FormatLogfmtUint64(header.Number.Uint64()),
			common.StorageSize(headerBytes).TerminalString())
		bodies = fmt.Sprintf("%v@%v",
			log.FormatLogfmtUint64(block.Number.Uint64()),
			common.StorageSize(bodyBytes).TerminalString())
		receipts = fmt.Sprintf("%v@%v",
			log.FormatLogfmtUint64(block.Number.Uint64()),
			common.StorageSize(receiptBytes).TerminalString())
	)
	log.Info("Syncing: chain download in progress", "synced", progress, "chain",
		syncedBytes, "headers", headers, "bodies", bodies, "receipts", receipts,

...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/eth/protocols/eth"
...
func newTesterWithNotification(t *testing.T, success func()) *downloadTester {
	freezer := t.TempDir()
	db, err := rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(), freezer,
		"", false)
	if err != nil {
		panic(err)
	}
	...
	chain, err := core.NewBlockChain(db, nil, gspec, nil, ethash.NewFaker(),
		vm.Config{}, nil, nil)
	if err != nil {
		panic(err)
	}
	...
	tester.downloader = New(db, new(event.TypeMux), tester.chain, nil,
		tester.dropPeer, success)
	return tester
}
...
func (dl *downloadTester) newPeer(id string, version uint,
	blocks []*types.Block) *downloadTesterPeer {
	dl.lock.Lock()
...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin common.Hash,
	amount int, skip int, reverse bool, sink chan *eth.Response) (*eth.Request,
	error) {
	// Service the header query via the live handler code
	rlpHeaders := eth.ServiceGetBlockHeadersQuery(dlp.chain,
		&eth.GetBlockHeadersRequest{
...
// origin; associated with a particular peer in the download tester. The returned
// function can be used to retrieve batches of headers from the particular peer.
func (dlp *downloadTesterPeer) RequestHeadersByNumber(origin uint64, amount int,
	skip int, reverse bool, sink chan *eth.Response) (*eth.Request, error) {
	// Service the header query via the live handler code
	rlpHeaders := eth.ServiceGetBlockHeadersQuery(dlp.chain,
		&eth.GetBlockHeadersRequest{
			Origin: eth.HashOrNumber{
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []common.Hash,
	sink chan *eth.Response) (*eth.Request, error) {
	...
	for i, body := range bodies {
		txsHashes[i] = types.DeriveSha(types.Transactions(body.Transactions),
			hasher)
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []common.Hash,
	sink chan *eth.Response) (*eth.Request, error) {
...
func (dlp *downloadTesterPeer) RequestAccountRange(id uint64,
	...
	go dlp.dl.downloader.SnapSyncer.OnAccounts(dlp, id, hashes, accounts,
		proofs)
	return nil
}
...
func (dlp *downloadTesterPeer) RequestStorageRanges(id uint64, root common.Hash,
...
func (dlp *downloadTesterPeer) RequestByteCodes(id uint64, hashes []common.Hash,
	bytes uint64) error {
...
func (dlp *downloadTesterPeer) RequestTrieNodes(id uint64, root common.Hash,
	paths []snap.TrieNodePathSet, bytes uint64) error {
...
func assertOwnChain(t *testing.T, tester *downloadTester, length int) {
	...
	if rs := int(tester.chain.CurrentSnapBlock().Number.Uint64()) + 1; rs != receipts {
		t.Fatalf("synchronised receipts mismatch: have %v, want %v", rs,
			receipts)
	}
}
...
func testThrottling(t *testing.T, protocol uint, mode SyncMode) {
	...
	for {
		...
		if cached != blockCacheMaxItems && cached != blockCacheMaxItems-reorgProtHeaderDelay && retrieved+cached+frozen != targetBlocks+1 && retrieved+cached+frozen != targetBlocks+1-reorgProtHeaderDelay {
			t.Fatalf("block count mismatch: have %v, want %v (owned %v, blocked %v, target %v)",
				cached, blockCacheMaxItems, retrieved, frozen, targetBlocks+1)
		}
...
func testBoundedForkedSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	if err := tester.sync("rewriter", nil, mode); err != errInvalidAncestor {
		t.Fatalf("sync failure mismatch: have %v, want %v", err,
			errInvalidAncestor)
	}
}
...
func testBoundedHeavyForkedSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	// Synchronise with the second peer and ensure that the fork is rejected to being too old
	if err := tester.sync("heavy-rewriter", nil,
		mode); err != errInvalidAncestor {
		t.Fatalf("sync failure mismatch: have %v, want %v", err,
			errInvalidAncestor)
	}
}
...
func testMultiSynchronisation(t *testing.T, protocol uint, mode SyncMode) {
	...
	for i := 0; i < targetPeers; i++ {
		...
		tester.newPeer(id, protocol,
			chain.shorten(len(chain.blocks) / (i + 1)).blocks[1:])
	}
...
func testMultiProtoSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	if err := tester.sync(fmt.Sprintf("peer %d", protocol), nil,
		mode); err != nil {
		t.Fatalf("failed to synchronise blocks: %v", err)
	}
	assertOwnChain(t, tester, len(chain.blocks))
...
func testEmptyShortCircuit(t *testing.T, protocol uint, mode SyncMode) {
	...
	if int(bodiesHave.Load()) != bodiesNeeded {
		t.Errorf("body retrieval count mismatch: have %v, want %v",
			bodiesHave.Load(), bodiesNeeded)
	}
	if int(receiptsHave.Load()) != receiptsNeeded {
		t.Errorf("receipt retrieval count mismatch: have %v, want %v",
			receiptsHave.Load(), receiptsNeeded)
	}
}
...
func testHighTDStarvationAttack(t *testing.T, protocol uint, mode SyncMode) {
	...
	if err := tester.sync("attack", big.NewInt(1000000),
		mode); err != errStallingPeer {
		t.Fatalf("synchronisation error mismatch: have %v, want %v", err,
			errStallingPeer)
	}
}
...
func TestBlockHeaderAttackerDropping68(t *testing.T) {
	testBlockHeaderAttackerDropping(t,
		eth.ETH68)
}
...
func testBlockHeaderAttackerDropping(t *testing.T, protocol uint) {
	...
	tests := []struct {
		...
		{errBusy,
			false}, // Sync is already in progress, no problem
		{errUnknownPeer,
			false}, // Peer is unknown, was already dropped, don't double drop
		{errBadPeer,
			true}, // Peer was deemed bad for some reason, drop it
		{errStallingPeer,
			true}, // Peer was detected to be stalling, drop it
		{errUnsyncedPeer,
			true}, // Peer was detected to be unsynced, drop it
		{errNoPeers,
			false}, // No peers to download from, soft race, no issue
		{errTimeout,
			true}, // No hashes received in due time, drop the peer
		{errEmptyHeaderSet,
			true}, // No headers were returned as a response, drop as it's a dead end
		{errPeersUnavailable,
			true}, // Nobody had the advertised blocks, drop the advertiser
		{errInvalidAncestor,
			true}, // Agreed upon ancestor is not acceptable, drop the chain rewriter
		{errInvalidChain,
			true}, // Hash chain was detected as invalid, definitely drop
		{errInvalidBody,
			false}, // A bad peer was detected, but not the sync origin
		{errInvalidReceipt,
			false}, // A bad peer was detected, but not the sync origin
		{errCancelContentProcessing,
			false}, // Synchronisation was canceled, origin may be innocent, don't drop
	}
	...
	for i, tt := range tests {
		...
		tester.downloader.synchroniseMock = func(string,
			common.Hash) error {
			return tt.result
		}

		tester.downloader.LegacySync(id, tester.chain.Genesis().Hash(),
			big.NewInt(1000), nil, FullSync)
		if _, ok := tester.peers[id]; !ok != tt.drop {
			t.Errorf("test %d: peer drop mismatch for %v: have %v, want %v", i,
				tt.result, !ok, tt.drop)
		}
	}
}
...
func testSyncProgress(t *testing.T, protocol uint, mode SyncMode) {
	...
	tester.newPeer("peer-half", protocol,
		chain.shorten(len(chain.blocks) / 2).blocks[1:])
...
func checkProgress(t *testing.T, d *Downloader, stage string,
	want ethereum.SyncProgress) {
	// Mark this method as a helper to report errors at callsite, not in here
	t.Helper()
...
func TestRemoteHeaderRequestSpan(t *testing.T) {
	testCases := []struct {
		...
		{1500, 1000,
			[]int{1323, 1339, 1355, 1371, 1387, 1403, 1419, 1435, 1451, 1467,
				1483, 1499},
		},
		{15000, 13006,
			[]int{14823, 14839, 14855, 14871, 14887, 14903, 14919, 14935, 14951,
				14967, 14983, 14999},
		},
		// Remote is pretty close to us. We don't have to fetch as many
		{1200, 1150,
			[]int{1149, 1154, 1159, 1164, 1169, 1174, 1179, 1184, 1189, 1194,
				1199},
		},
		...
		{6000000, 0,
			[]int{5999823, 5999839, 5999855, 5999871, 5999887, 5999903, 5999919,
				5999935, 5999951, 5999967, 5999983, 5999999},
		},
		{0, 0,
			[]int{0, 2},
		},
	...
	for i, tt := range testCases {
		from, count, span, max := calculateRequestSpan(tt.remoteHeight,
			tt.localHeight)
		data := reqs(int(from), count, span)

		if max != uint64(data[len(data)-1]) {
			t.Errorf("test %d: wrong last value %d != %d", i, data[len(data)-1],
				max)
		}
		failed := false
		if len(data) != len(tt.expected) {
			failed = true
			t.Errorf("test %d: length wrong, expected %d got %d", i,
				len(tt.expected), len(data))
		} else {
			for j, n := range data {
				if n != tt.expected[j] {
					failed = true
...
func testBeaconSync(t *testing.T, protocol uint, mode SyncMode) {
	...
	var cases = []struct {
		...
		{name: "Beacon sync with long local chain",
			local: blockCacheMaxItems - 15 - fsMinFullBlocks/2},
		{name: "Beacon sync with full local chain",
			local: blockCacheMaxItems - 15 - 1},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if err := tester.downloader.BeaconSync(mode,
				chain.blocks[len(chain.blocks)-1].Header(), nil); err != nil {
			...
			case <-success:
				...
				if bs := int(tester.chain.CurrentBlock().Number.Uint64()) + 1; bs != len(chain.blocks) {
					t.Fatalf("synchronised blocks mismatch: have %v, want %v",
						bs, len(chain.blocks))
				}

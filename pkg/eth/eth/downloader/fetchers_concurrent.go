...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest,
		resCh chan *eth.Response) (*eth.Request, error)
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	timeouts := prque.New[int64, *eth.Request](func(data *eth.Request,
		index int) {
		ordering[data] = index
	})
	...
	for {
		...
		if queue.pending() == 0 {
			...
			for _, peer := range d.peers.AllPeers() {
				...
				if pending == nil && stale == nil {
					...
					if waited := time.Since(stale.Sent); waited > timeoutGracePeriod {
						...
						// stall the sync.
						peer.log.Warn("Peer stalling, dropping", "waited",
							common.PrettyDuration(waited))
						d.dropPeer(peer.id)
					}
				}
			}
			...
			for _, peer := range idles {
				...
				// have them.
				request, progress, throttle := queue.reserve(peer,
					queue.capacity(peer, d.peers.rates.TargetRoundTrip()))
				if progress {
					progressed = true
				}
				if throttle {
		...
		case <-timeout.C:
			...
			if now, at := time.Now(), time.Unix(0, -exp); now.Before(at) {
				log.Error("Timeout triggered but not reached", "left",
					at.Sub(now))
			...
			if peer == nil {
				...
				log.Error("Delivery timeout from unknown peer", "peer",
					req.Peer)
				continue
			}
			if fails > 2 {

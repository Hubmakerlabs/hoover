...
func (q *bodyQueue) updateCapacity(peer *peerConnection, items int,
	span time.Duration) {
...
func (q *bodyQueue) reserve(peer *peerConnection, items int) (*fetchRequest,
	bool, bool) {
...
func (q *bodyQueue) request(peer *peerConnection, req *fetchRequest,
	resCh chan *eth.Response) (*eth.Request, error) {
	peer.log.Trace("Requesting new batch of bodies", "count", len(req.Headers),
		"from", req.Headers[0].Number)
...
func (q *bodyQueue) deliver(peer *peerConnection, packet *eth.Response) (int,
	error) {
	...
	accepted, err := q.queue.DeliverBodies(peer.id, txs, hashsets[0], uncles,
		hashsets[1], withdrawals, hashsets[2])
	switch {

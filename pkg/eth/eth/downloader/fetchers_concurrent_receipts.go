...
func (q *receiptQueue) updateCapacity(peer *peerConnection, items int,
	span time.Duration) {
...
func (q *receiptQueue) reserve(peer *peerConnection, items int) (*fetchRequest,
	bool, bool) {
...
func (q *receiptQueue) request(peer *peerConnection, req *fetchRequest,
	resCh chan *eth.Response) (*eth.Request, error) {
	peer.log.Trace("Requesting new batch of receipts", "count",
		len(req.Headers), "from", req.Headers[0].Number)
...
func (q *receiptQueue) deliver(peer *peerConnection, packet *eth.Response) (int,
	error) {
	...
	case err == nil:
		peer.log.Trace("Delivered new batch of receipts", "count",
			len(receipts), "accepted", accepted)
	default:

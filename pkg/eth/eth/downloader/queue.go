...
func (q *queue) Schedule(headers []*types.Header, hashes []common.Hash,
	from uint64) []*types.Header {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	for i, header := range headers {
		...
		if header.Number == nil || header.Number.Uint64() != from {
			log.Warn("Header broke chain ordering", "number", header.Number,
				"hash", hash, "expected", from)
			break
		}
		if q.headerHead != (common.Hash{}) && q.headerHead != header.ParentHash {
			log.Warn("Header broke chain ancestry", "number", header.Number,
				"hash", hash)
			break
		}
		...
		if _, ok := q.blockTaskPool[hash]; ok {
			log.Warn("Header already scheduled for block fetch", "number",
		...
		if q.mode == SnapSync && !header.EmptyReceipts() {
			if _, ok := q.receiptTaskPool[hash]; ok {
				log.Warn("Header already scheduled for receipt fetch", "number",
					header.Number, "hash", hash)
			} else {
...
func (q *queue) ReserveBodies(p *peerConnection, count int) (*fetchRequest,
	bool, bool) {
	q.lock.Lock()
	defer q.lock.Unlock()

	return q.reserveHeaders(p, count, q.blockTaskPool, q.blockTaskQueue,
		q.blockPendPool, bodyType)
}

// ReserveReceipts reserves a set of receipt fetches for the given peer, skipping
// any previously failed downloads. Beside the next batch of needed fetches, it
// also returns a flag whether empty receipts were queued requiring importing.
func (q *queue) ReserveReceipts(p *peerConnection, count int) (*fetchRequest,
	bool, bool) {
	q.lock.Lock()
	defer q.lock.Unlock()

	return q.reserveHeaders(p, count, q.receiptTaskPool, q.receiptTaskQueue,
		q.receiptPendPool, receiptType)
}
...
func (q *queue) reserveHeaders(p *peerConnection, count int,
	taskPool map[common.Hash]*types.Header,
	taskQueue *prque.Prque[int64, *types.Header],
	...
	for proc := 0; len(send) < count && !taskQueue.Empty(); proc++ {
		...
		if stale {
			...
			proc = proc - 1
			log.Error("Fetch reservation already delivered", "number",
...
func (q *queue) expire(peer string, pendPool map[string]*fetchRequest,
	taskQueue interface{}) int {
	...
	for _, header := range req.Headers {
		taskQueue.(*prque.Prque[int64, *types.Header]).Push(header,
			-int64(header.Number.Uint64()))
	}
...
func (q *queue) DeliverHeaders(id string, headers []*types.Header,
	hashes []common.Hash, headerProcCh chan *headerTask) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	if accepted {
		if headers[0].Number.Uint64() != request.From {
			logger.Trace("First header broke chain ordering", "number",
				headers[0].Number, "hash", hashes[0], "expected", request.From)
			accepted = false
		} else if hashes[len(headers)-1] != target {
			logger.Trace("Last header broke skeleton structure ", "number",
				headers[len(headers)-1].Number, "hash", hashes[len(headers)-1],
				"expected", target)
			accepted = false
		}
	}
	if accepted {
		...
		for i, header := range headers[1:] {
			...
			if want := request.From + 1 + uint64(i); header.Number.Uint64() != want {
				logger.Warn("Header broke chain ordering", "number",
					header.Number, "hash", hash, "expected", want)
				accepted = false
				break
			}
			if parentHash != header.ParentHash {
				logger.Warn("Header broke chain ancestry", "number",
					header.Number, "hash", hash)
				accepted = false
				break
			}
	...
	if ready > 0 {
		...
		copy(processHeaders,
			q.headerResults[q.headerProced:q.headerProced+ready])
		...
		case headerProcCh <- &headerTask{
			...
			logger.Trace("Pre-scheduled new headers", "count",
				len(processHeaders), "from", processHeaders[0].Number)
...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction,
	txListHashes []common.Hash,
	uncleLists [][]*types.Header, uncleListHashes []common.Hash,
	withdrawalLists [][]*types.Withdrawal,
	withdrawalListHashes []common.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	return q.deliver(id, q.blockTaskPool, q.blockTaskQueue, q.blockPendPool,
		bodyReqTimer, bodyInMeter, bodyDropMeter, len(txLists), validate,
		reconstruct)
}
...
func (q *queue) DeliverReceipts(id string, receiptList [][]*types.Receipt,
	receiptListHashes []common.Hash) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
	...
	return q.deliver(id, q.receiptTaskPool, q.receiptTaskQueue,
		q.receiptPendPool,
		receiptReqTimer, receiptInMeter, receiptDropMeter, len(receiptList),
...
func (q *queue) deliver(id string, taskPool map[common.Hash]*types.Header,
	...
	reqTimer metrics.Timer, resInMeter metrics.Meter,
	resDropMeter metrics.Meter,
	...
	for _, header := range request.Headers[:i] {
		if res, stale, err := q.resultCache.GetDeliverySlot(header.Number.Uint64()); err == nil && !stale {
			...
			log.Error("Delivery stale", "stale", stale, "number",

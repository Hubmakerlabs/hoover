...
// reassembly.
func makeChain(n int, seed byte, parent *types.Block,
	empty bool) ([]*types.Block, []types.Receipts) {
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent,
		ethash.NewFaker(), testDB, n, func(i int, block *core.BlockGen) {
...
func TestEmptyBlocks(t *testing.T) {
	...
	if q.blockTaskQueue.Size() != numOfBlocks-10 {
		t.Errorf("expected block task queue to be %d, got %d", numOfBlocks-10,
			q.blockTaskQueue.Size())
	}
	if q.receiptTaskQueue.Size() != 0 {
		t.Errorf("expected receipt task queue to be %d, got %d", 0,
		...
		for {
			...
			if f != nil {
				...
				for _, hdr := range f.Headers[0 : len(f.Headers)-numToSkip] {
					txset = append(txset,
						world.getTransactions(hdr.Number.Uint64()))
				...
				for i, txs := range txset {
					txsHashes[i] = types.DeriveSha(types.Transactions(txs),
						hasher)
				}
				...
				_, err := q.DeliverBodies(peer.id, txset, txsHashes, uncleset,
					uncleHashes, nil, nil)
				if err != nil {

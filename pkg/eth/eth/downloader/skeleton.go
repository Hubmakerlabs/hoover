...
func newSkeleton(db ethdb.Database, peers *peerSet, drop peerDropFn,
	filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) Sync(head *types.Header, final *types.Header,
	force bool) error {
	log.Trace("New skeleton head announced", "number", head.Number, "hash",
		head.Hash(), "force", force)
	errc := make(chan error)

	select {
	case s.headEvents <- &headUpdate{header: head, final: final, force: force,
		errc: errc}:
		return <-errc
	case <-s.terminated:
...
func (s *skeleton) sync(head *types.Header) (*types.Header, error) {
		...
		go func() {
			...
			if filled := s.filler.suspend(); filled != nil {
				...
				if err := s.cleanStales(filled); err != nil {
					log.Error("Failed to clean stale beacon headers", "err",
						err)
				}
			}
		}()
	...
	log.Debug("Starting reverse header sync cycle", "head", head.Number, "hash",
		head.Hash(), "cont", s.scratchHead)

	// Whether sync completed or not, disregard any future packets
	defer func() {
		log.Debug("Terminating reverse header sync cycle", "head", head.Number,
			"hash", head.Hash(), "cont", s.scratchHead)
		s.requests = make(map[uint64]*headerRequest)
	}()

	// Start tracking idle peers for task assignments
	peering := make(chan *peeringEvent,
		64) // arbitrary buffer, just some burst protection
	...
	for {
		...
		case event := <-s.headEvents:
			...
			if reorged := s.processNewHead(event.header, event.final,
				event.force); reorged {
...
func (s *skeleton) initSync(head *types.Header) {
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for _, subchain := range s.progress.Subchains {
				log.Debug("Restarting skeleton subchain", "head", subchain.Head,
					"tail", subchain.Tail)
			}
			...
			for len(s.progress.Subchains) > 0 {
				...
				if lastchain.Tail >= headchain.Tail {
					log.Debug("Dropping skeleton subchain", "head",
						lastchain.Head, "tail", lastchain.Tail)
				...
				if lastchain.Head >= headchain.Tail {
					log.Debug("Trimming skeleton subchain", "oldhead",
						lastchain.Head, "newhead", headchain.Tail-1, "tail",
						lastchain.Tail)
			...
			if n := len(s.progress.Subchains); n > 0 {
				...
				if lastchain.Head == headchain.Tail-1 {
					...
					if lasthead.Hash() == head.ParentHash {
						log.Debug("Extended skeleton subchain with new head",
			...
			if !extended {
				log.Debug("Created new skeleton subchain", "head", number,
					"tail", number)
				s.progress.Subchains = append([]*subchain{headchain},
					s.progress.Subchains...)
			}
	...
	log.Debug("Created initial skeleton subchain", "head", number, "tail",
		number)
}
...
func (s *skeleton) processNewHead(head *types.Header, final *types.Header,
	force bool) bool {
	...
	if lastchain.Tail >= number {
		...
		if lastchain.Head == lastchain.Tail {
			if current := rawdb.ReadSkeletonHeader(s.db,
				number); current.Hash() == head.Hash() {
				return false
			}
		}
		// Not a noop / double head announce, abort with a reorg
		if force {
			log.Warn("Beacon chain reorged", "tail", lastchain.Tail, "head",
				lastchain.Head, "newHead", number)
		}
		return true
	}
	if lastchain.Head+1 < number {
		if force {
			log.Warn("Beacon chain gapped", "head", lastchain.Head, "newHead",
				number)
		}
		return true
	}
	if parent := rawdb.ReadSkeletonHeader(s.db,
		number-1); parent.Hash() != head.ParentHash {
		if force {
			log.Warn("Beacon chain forked", "ancestor", number-1, "hash",
				parent.Hash(), "want", head.ParentHash)
		}
		return true
	}
...
func (s *skeleton) assignTasks(success chan *headerResponse,
	fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps,
			s.peers.rates.Capacity(peer.id, eth.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	peer.log.Trace("Fetching skeleton headers", "from", req.head, "count",
		requestCount)
	netreq, err := peer.peer.RequestHeadersByNumber(req.head, requestCount, 0,
		true, resCh)
	if err != nil {
	...
	case res := <-resCh:
		...
		s.peers.rates.Update(peer.id, eth.BlockHeadersMsg, res.Time,
			len(headers))
		...
		case headers[0].Number.Uint64() != req.head:
			...
			peer.log.Debug("Invalid header response head", "have",
				headers[0].Number, "want", req.head)
		...
		case req.head >= requestHeaders && len(headers) != requestHeaders:
			...
			peer.log.Debug("Invalid non-genesis header count", "have",
				len(headers), "want", requestHeaders)
		...
		case req.head < requestHeaders && uint64(len(headers)) != req.head:
			...
			peer.log.Debug("Invalid genesis header count", "have", len(headers),
				"want", headers[0].Number.Uint64())
		...
		default:
			...
			for i := 0; i < len(headers)-1; i++ {
				if headers[i].ParentHash != headers[i+1].Hash() {
					peer.log.Debug("Invalid hash progression", "index", i,
						"wantparenthash", headers[i].ParentHash,
...
func (s *skeleton) revertRequest(req *headerRequest) {
	...
	select {
	case <-req.stale:
		log.Trace("Header request already reverted", "peer", req.peer, "reqid",
			req.id)
		return
	default:
	}
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool,
	merged bool) {
	res.peer.log.Trace("Processing header response", "head",
		res.headers[0].Number, "hash", res.headers[0].Hash(), "count",
		len(res.headers))
	...
	for s.scratchSpace[0] != nil {
		...
		if s.progress.Subchains[0].Next != s.scratchSpace[0].Hash() {
			...
			log.Warn("Invalid skeleton headers", "peer", s.scratchOwners[0],
				"number", tail-1, "want", want, "have", have)
		...
		for _, header := range s.scratchSpace[:requestHeaders] {
			if header != nil { // nil when the genesis is reached
				...
				linked = rawdb.HasHeader(s.db, header.ParentHash,
					header.Number.Uint64()-1) &&
					rawdb.HasBody(s.db, header.ParentHash,
						header.Number.Uint64()-1) &&
					rawdb.HasReceipts(s.db, header.ParentHash,
						header.Number.Uint64()-1)
				if linked {
					break
				}
		...
		log.Trace("Primary subchain extended", "head", head, "tail", tail,
			"next", next)
			...
			if subchains := len(s.progress.Subchains); subchains > 1 {
				...
				case subchains == 2 && s.progress.Subchains[1].Head == s.progress.Subchains[1].Tail:
					...
					if s.progress.Subchains[1].Head < s.progress.Subchains[0].Tail {
						log.Debug("Cleaning previous beacon sync state", "head",
							s.progress.Subchains[1].Head)
						rawdb.DeleteSkeletonHeader(batch,
							s.progress.Subchains[1].Head)
					}
				...
				default:
					...
					for i := range s.progress.Subchains[1:] {
						context = append(context,
							fmt.Sprintf("stale_head_%d", i+1))
						context = append(context,
							s.progress.Subchains[i+1].Head)
						context = append(context,
							fmt.Sprintf("stale_tail_%d", i+1))
						context = append(context,
							s.progress.Subchains[i+1].Tail)
						context = append(context,
							fmt.Sprintf("stale_next_%d", i+1))
						context = append(context,
							s.progress.Subchains[i+1].Next)
					}
					log.Error("Cleaning spurious beacon sync leftovers",
						context...)
					s.progress.Subchains = s.progress.Subchains[:1]
		...
		for len(s.progress.Subchains) > 1 && s.progress.Subchains[1].Head >= s.progress.Subchains[0].Tail {
			...
			if s.progress.Subchains[1].Tail >= s.progress.Subchains[0].Tail {
				...
				log.Debug("Previous subchain fully overwritten", "head", head,
					"tail", tail, "next", next)
				s.progress.Subchains = append(s.progress.Subchains[:1],
					s.progress.Subchains[2:]...)
				continue
			} else {
				// Partially overwritten, trim the head to the overwritten size
				log.Debug("Previous subchain partially overwritten", "head",
					head, "tail", tail, "next", next)
			...
			if rawdb.ReadSkeletonHeader(s.db,
				s.progress.Subchains[1].Head).Hash() == s.progress.Subchains[0].Next {
				log.Debug("Previous subchain merged", "head", head, "tail",
					tail, "next", next)
	...
	if time.Since(s.logged) > 8*time.Second || left == 0 {
		...
		if s.pulled == 0 {
			...
			log.Info("Syncing beacon headers", "downloaded", s.pulled, "left",
				left, "eta", common.PrettyDuration(eta))
		}
	}
...
func (s *skeleton) cleanStales(filled *types.Header) error {
	...
	log.Trace("Cleaning stale beacon headers", "filled", number, "hash",
		filled.Hash())
	...
	if number < s.progress.Subchains[0].Tail {
		return fmt.Errorf("filled header below beacon header tail: %d < %d",
			number, s.progress.Subchains[0].Tail)
	}
	...
	for n := start; n < end; n++ {
		...
		if batch.ValueSize() >= ethdb.IdealBatchSize {
			...
			s.progress.Subchains[0].Next = rawdb.ReadSkeletonHeader(s.db,
				n).ParentHash
			s.saveSyncStatus(batch)
...
func (s *skeleton) Bounds() (head *types.Header, tail *types.Header,
	final *types.Header, err error) {
	...
	if head == nil {
		return nil, nil, nil, fmt.Errorf("head skeleton header %d is missing",
			progress.Subchains[0].Head)
	}
	tail = rawdb.ReadSkeletonHeader(s.db, progress.Subchains[0].Tail)
	if tail == nil {
		return nil, nil, nil, fmt.Errorf("tail skeleton header %d is missing",
	...
	if progress.Finalized != nil && tail.Number.Uint64() <= *progress.Finalized && *progress.Finalized <= head.Number.Uint64() {
		...
		if final == nil {
			return nil, nil, nil, fmt.Errorf("finalized skeleton header %d is missing",
				*progress.Finalized)
		}
	}

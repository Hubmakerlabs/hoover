...
func newSkeletonTestPeerWithHook(id string, headers []*types.Header,
	serve func(origin uint64) []*types.Header) *skeletonTestPeer {
	return &skeletonTestPeer{
		id:      id,
		headers: headers,
		serve:   serve,
...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int,
	...
	if !reverse || skip != 0 {
		...
		panic(fmt.Sprintf("invalid header retrieval: reverse %v, want true; skip %d, want 0",
			reverse, skip))
	}
	...
	if int64(origin)-int64(amount) < 0 {
		panic(fmt.Sprintf("headers requested before (or at) genesis: origin %d, amount %d",
			origin, amount))
	}
	...
	if amount > requestHeaders || (amount < requestHeaders && origin > uint64(amount)) {
		panic(fmt.Sprintf("non-chunk size header batch requested: requested %d, want %d, origin %d",
			amount, requestHeaders, origin))
	}
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool,
	chan *eth.Response) (*eth.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash,
	chan *eth.Response) (*eth.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash,
	chan *eth.Response) (*eth.Request, error) {
...
func TestSkeletonSyncInit(t *testing.T) {
	...
	var (
		...
		block50  = &types.Header{Number: big.NewInt(50),
			ParentHash: block49.Hash()}
	)
	tests := []struct {
	...
	for i, tt := range tests {
		...
		if len(progress.Subchains) != len(tt.newstate) {
			t.Errorf("test %d: subchain count mismatch: have %d, want %d", i,
				len(progress.Subchains), len(tt.newstate))
			continue
		}
		for j := 0; j < len(progress.Subchains); j++ {
			if progress.Subchains[j].Head != tt.newstate[j].Head {
				t.Errorf("test %d: subchain %d head mismatch: have %d, want %d",
					i, j, progress.Subchains[j].Head, tt.newstate[j].Head)
			}
			if progress.Subchains[j].Tail != tt.newstate[j].Tail {
				t.Errorf("test %d: subchain %d tail mismatch: have %d, want %d",
					i, j, progress.Subchains[j].Tail, tt.newstate[j].Tail)
			}
		}
	}
}
...
func TestSkeletonSyncExtend(t *testing.T) {
	...
	var (
		...
		block50  = &types.Header{Number: big.NewInt(50),
			ParentHash: block49.Hash()}
		block51 = &types.Header{Number: big.NewInt(51),
			ParentHash: block50.Hash()}
	)
	tests := []struct {
		head     *types.Header // New head header to announce to reorg to
	...
	for i, tt := range tests {
		...
		if err := skeleton.Sync(tt.extend, nil, false); err != tt.err {
			t.Errorf("test %d: extension failure mismatch: have %v, want %v", i,
				err, tt.err)
		}
		skeleton.Terminate()
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	tests := []struct {
		...
		// to the genesis block.
		{
			...
			midstate: []*subchain{{Head: uint64(len(chain) - 1),
				Tail: uint64(len(chain) - 1)}},
		...
		{
			head: chain[len(chain)-1],
			peers: []*skeletonTestPeer{newSkeletonTestPeer("test-peer-1",
				chain)},
		...
		// Joining with a new peer should however unblock the sync.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-skipper",
					append(append(append([]*types.Header{}, chain[:99]...),
						nil), chain[100:]...)),
			},
		...
		// Joining with a new peer should however unblock the sync.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-skipper",
					append(append(append([]*types.Header{}, chain[:50]...),
						nil), chain[51:]...)),
			},
			midstate: []*subchain{{Head: requestHeaders + 100, Tail: 100}},
			midserve: requestHeaders + 101 - 3, // len - head - genesis - missing
		...
		// Joining with a new peer should however unblock the sync.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeer("header-duper",
					append(append(append([]*types.Header{}, chain[:99]...),
						chain[98]), chain[100:]...)),
			},
			midstate: []*subchain{{Head: requestHeaders + 100, Tail: 100}},
		...
		// and the scratch space must not be consumed further.
		{
			...
			peers: []*skeletonTestPeer{
				newSkeletonTestPeerWithHook("peer-1", chain,
					func(origin uint64) []*types.Header {
						if origin == chain[2*requestHeaders+1].Number.Uint64() {
							time.Sleep(100 * time.Millisecond)
						}
						return nil // Fallback to default behavior, just delayed
					}),
				newSkeletonTestPeerWithHook("peer-2", chain,
					func(origin uint64) []*types.Header {
						if origin == chain[2*requestHeaders+1].Number.Uint64() {
							time.Sleep(100 * time.Millisecond)
						}
		...
		// end up losing it and creating a gap.
		{
			...
			peers: []*skeletonTestPeer{newSkeletonTestPeer("test-peer-oldchain",
				chain)},
			...
			endstate: []*subchain{{Head: uint64(len(sidechain)/2 + 3),
				Tail: uint64(len(chain) / 2)}},
		},
	}
	for i, tt := range tests {
		// Create a fresh database and initialize it with the starting state
		...
		rawdb.WriteReceipts(db, chain[0].Hash(), chain[0].Number.Uint64(),
			types.Receipts{})

		// Create a peer set to feed headers through
		peerset := newPeerSet()
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, eth.ETH67, peer,
				log.New("id", peer.id)))
		}
			...
			filler = &hookedBackfiller{
				resumeHook: func() {
					...
					for progress.Subchains[0].Tail < progress.Subchains[0].Head {
						header := rawdb.ReadSkeletonHeader(db,
							progress.Subchains[0].Tail)

						rawdb.WriteBlock(db, types.NewBlockWithHeader(header))
						rawdb.WriteReceipts(db, header.Hash(),
							header.Number.Uint64(), types.Receipts{})
					...
					filled = rawdb.ReadSkeletonHeader(db,
						progress.Subchains[0].Tail)

					rawdb.WriteBlock(db, types.NewBlockWithHeader(filled))
					rawdb.WriteReceipts(db, filled.Hash(),
						filled.Number.Uint64(), types.Receipts{})
				},
		...
		check := func() error {
			if len(progress.Subchains) != len(tt.midstate) {
				return fmt.Errorf("test %d, mid state: subchain count mismatch: have %d, want %d",
					i, len(progress.Subchains), len(tt.midstate))
			}
			for j := 0; j < len(progress.Subchains); j++ {
				if progress.Subchains[j].Head != tt.midstate[j].Head {
					return fmt.Errorf("test %d, mid state: subchain %d head mismatch: have %d, want %d",
						i, j, progress.Subchains[j].Head, tt.midstate[j].Head)
				}
				if progress.Subchains[j].Tail != tt.midstate[j].Tail {
					return fmt.Errorf("test %d, mid state: subchain %d tail mismatch: have %d, want %d",
						i, j, progress.Subchains[j].Tail, tt.midstate[j].Tail)
				}
			}
			return nil
		}
		...
		if !tt.unpredictable {
			...
			if served != tt.midserve {
				t.Errorf("test %d, mid state: served headers mismatch: have %d, want %d",
					i, served, tt.midserve)
			}
			var drops uint64
			for _, peer := range tt.peers {
			...
			if drops != tt.middrop {
				t.Errorf("test %d, mid state: dropped peers mismatch: have %d, want %d",
					i, drops, tt.middrop)
			}
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id,
				eth.ETH67, tt.newPeer,
				log.New("id", tt.newPeer.id))); err != nil {
		...
		check = func() error {
			if len(progress.Subchains) != len(tt.endstate) {
				return fmt.Errorf("test %d, end state: subchain count mismatch: have %d, want %d",
					i, len(progress.Subchains), len(tt.endstate))
			}
			for j := 0; j < len(progress.Subchains); j++ {
				if progress.Subchains[j].Head != tt.endstate[j].Head {
					return fmt.Errorf("test %d, end state: subchain %d head mismatch: have %d, want %d",
						i, j, progress.Subchains[j].Head, tt.endstate[j].Head)
				}
				if progress.Subchains[j].Tail != tt.endstate[j].Tail {
					return fmt.Errorf("test %d, end state: subchain %d tail mismatch: have %d, want %d",
						i, j, progress.Subchains[j].Tail, tt.endstate[j].Tail)
				}
			}
			return nil
		}
		...
		if !tt.unpredictable {
			...
			if served != tt.endserve {
				t.Errorf("test %d, end state: served headers mismatch: have %d, want %d",
					i, served, tt.endserve)
			}
			drops := uint64(0)
			for _, peer := range tt.peers {
				drops += peer.dropped.Load()
			...
			if drops != tt.enddrop {
				t.Errorf("test %d, end state: dropped peers mismatch: have %d, want %d",
					i, drops, tt.middrop)
			}
		}

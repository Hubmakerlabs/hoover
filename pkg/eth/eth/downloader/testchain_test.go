...
func init() {
	...
	wg.Add(3)
	go func() {
		testChainForkLightA = testChainBase.makeFork(forkLen, false,
			1)
		wg.Done()
	}()
	go func() {
		testChainForkLightB = testChainBase.makeFork(forkLen, false,
			2)
		wg.Done()
	}()
	go func() {
		testChainForkHeavy = testChainBase.makeFork(forkLen, true,
			3)
		wg.Done()
	}()
	wg.Wait()
...
func (tc *testChain) shorten(length int) *testChain {
	if length > len(tc.blocks) {
		panic(fmt.Errorf("can't shorten test chain to %d blocks, it's only %d blocks long",
			length, len(tc.blocks)))
	}
...
func (tc *testChain) generate(n int, seed byte, parent *types.Block,
	heavy bool) {
	blocks, _ := core.GenerateChain(testGspec.Config, parent, ethash.NewFaker(),
...
func newTestBlockchain(blocks []*types.Block) *core.BlockChain {
	...
	tbc.gen.Do(func() {
		...
		chain, err := core.NewBlockChain(rawdb.NewMemoryDatabase(), nil,
			testGspec, nil, ethash.NewFaker(), vm.Config{}, nil, nil)
		if err != nil {
			panic(err)
		}

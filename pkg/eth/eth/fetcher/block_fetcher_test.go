...
var (
	...
	genesis = gspec.MustCommit(testdb,
		trie.NewDatabase(testdb, trie.HashDefaults))
	unknownBlock = types.NewBlock(&types.Header{Root: types.EmptyRootHash,
		GasLimit: params.GenesisGasLimit,
		BaseFee:  big.NewInt(params.InitialBaseFee)}, nil, nil, nil,
...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash,
	...
	blocks, _ := core.GenerateChain(gspec.Config, parent, ethash.NewFaker(),
		testdb, n, func(i int, block *core.BlockGen) {
			...
			if parent == genesis && i%3 == 0 {
				...
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress),
					common.Address{seed}, big.NewInt(1000), params.TxGas,
					block.BaseFee(), nil), signer, testKey)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
...
func newTester(light bool) *fetcherTester {
	...
	tester.fetcher = NewBlockFetcher(light, tester.getHeader, tester.getBlock,
		tester.verifyHeader, tester.broadcastBlock, tester.chainHeight,
...
func (f *fetcherTester) makeHeaderFetcher(peer string,
	blocks map[common.Hash]*types.Block,
	drift time.Duration) headerRequesterFn {
	closure := make(map[common.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that return a header from the closure
	return func(hash common.Hash, sink chan *eth.Response) (*eth.Request,
		error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string,
	blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	closure := make(map[common.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that returns blocks from the closure
	return func(hashes []common.Hash, sink chan *eth.Response) (*eth.Request,
		error) {
...
func verifyFetchingEvent(t *testing.T, fetching chan []common.Hash,
	arrive bool) {
	t.Helper()
...
func verifyCompletingEvent(t *testing.T, completing chan []common.Hash,
	arrive bool) {
	t.Helper()
...
func verifyChainHeight(t *testing.T, fetcher *fetcherTester, height uint64) {
	...
	if fetcher.chainHeight() != height {
		t.Fatalf("chain height mismatch, got %d, want %d",
			fetcher.chainHeight(), height)
	}
}
...
func TestFullSequentialAnnouncements(t *testing.T) {
	testSequentialAnnouncements(t,
		false)
}
func TestLightSequentialAnnouncements(t *testing.T) {
	testSequentialAnnouncements(t,
		true)
}
...
func testSequentialAnnouncements(t *testing.T, light bool) {
	...
	tester.fetcher.importedHook = func(header *types.Header,
		block *types.Block) {
		if light {
			if header == nil {
	...
	for i := len(hashes) - 2; i >= 0; i-- {
		tester.fetcher.Notify("valid", hashes[i], uint64(len(hashes)-i-1),
...
func TestFullConcurrentAnnouncements(t *testing.T) {
	testConcurrentAnnouncements(t,
		false)
}
func TestLightConcurrentAnnouncements(t *testing.T) {
	testConcurrentAnnouncements(t,
		true)
}
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	tester := newTester(light)
	firstHeaderFetcher := tester.makeHeaderFetcher("first", blocks,
		-gatherSlack)
	firstBodyFetcher := tester.makeBodyFetcher("first", blocks, 0)
	secondHeaderFetcher := tester.makeHeaderFetcher("second", blocks,
		-gatherSlack)
	secondBodyFetcher := tester.makeBodyFetcher("second", blocks, 0)

	var counter atomic.Uint32
	firstHeaderWrapper := func(hash common.Hash,
		sink chan *eth.Response) (*eth.Request, error) {
		counter.Add(1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash,
		sink chan *eth.Response) (*eth.Request, error) {
		counter.Add(1)
	...
	for i := len(hashes) - 2; i >= 0; i-- {
		tester.fetcher.Notify("first", hashes[i], uint64(len(hashes)-i-1),
			time.Now().Add(-arriveTimeout), firstHeaderWrapper,
			firstBodyFetcher)
		tester.fetcher.Notify("second", hashes[i], uint64(len(hashes)-i-1),
...
func TestFullOverlappingAnnouncements(t *testing.T) {
	testOverlappingAnnouncements(t,
		false)
}
func TestLightOverlappingAnnouncements(t *testing.T) {
	testOverlappingAnnouncements(t,
		true)
}
...
func TestFullPendingDeduplication(t *testing.T) {
	testPendingDeduplication(t,
		false)
}
func TestLightPendingDeduplication(t *testing.T) {
	testPendingDeduplication(t,
		true)
}
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	var counter atomic.Uint32
	headerWrapper := func(hash common.Hash,
		sink chan *eth.Response) (*eth.Request, error) {
		counter.Add(1)
	...
	for checkNonExist() {
		tester.fetcher.Notify("repeater", hashes[0], 1,
...
func TestFullRandomArrivalImport(t *testing.T) {
	testRandomArrivalImport(t,
		false)
}
func TestLightRandomArrivalImport(t *testing.T) {
	testRandomArrivalImport(t,
		true)
}
...
func testRandomArrivalImport(t *testing.T, light bool) {
	...
	tester.fetcher.Notify("valid", hashes[skip], uint64(len(hashes)-skip-1),
		time.Now().Add(-arriveTimeout), headerFetcher, bodyFetcher)
...
// peers.
func TestFullDistantAnnouncementDiscarding(t *testing.T) {
	testDistantAnnouncementDiscarding(t,
		false)
}
func TestLightDistantAnnouncementDiscarding(t *testing.T) {
	testDistantAnnouncementDiscarding(t,
		true)
}
...
func testDistantAnnouncementDiscarding(t *testing.T, light bool) {
	...
	// Ensure that a block with a lower number than the threshold is discarded
	tester.fetcher.Notify("lower", hashes[low], blocks[hashes[low]].NumberU64(),
		time.Now().Add(-arriveTimeout), headerFetcher, bodyFetcher)
	select {
	case <-time.After(50 * time.Millisecond):
	case <-fetching:
	...
	// Ensure that a block with a higher number than the threshold is discarded
	tester.fetcher.Notify("higher", hashes[high],
		blocks[hashes[high]].NumberU64(), time.Now().Add(-arriveTimeout),
		headerFetcher, bodyFetcher)
	select {
	case <-time.After(50 * time.Millisecond):
	case <-fetching:
...
func TestFullInvalidNumberAnnouncement(t *testing.T) {
	testInvalidNumberAnnouncement(t,
		false)
}
func TestLightInvalidNumberAnnouncement(t *testing.T) {
	testInvalidNumberAnnouncement(t,
		true)
}
...
func TestEmptyBlockShortCircuit(t *testing.T) {
	...
	for i := len(hashes) - 2; i >= 0; i-- {
		...
		verifyCompletingEvent(t, completing,
			len(blocks[hashes[i]].Transactions()) > 0 || len(blocks[hashes[i]].Uncles()) > 0)
...
func TestHashMemoryExhaustionAttack(t *testing.T) {
	...
	hashes, blocks := makeChain(targetBlocks, 0, genesis)
	validHeaderFetcher := tester.makeHeaderFetcher("valid", blocks,
		-gatherSlack)
	validBodyFetcher := tester.makeBodyFetcher("valid", blocks, 0)

	attack, _ := makeChain(targetBlocks, 0, unknownBlock)
	attackerHeaderFetcher := tester.makeHeaderFetcher("attacker", nil,
		-gatherSlack)
	...
	for i := 0; i < len(attack); i++ {
		if i < maxQueueDist {
			tester.fetcher.Notify("valid", hashes[len(hashes)-2-i], uint64(i+1),
				time.Now(), validHeaderFetcher, validBodyFetcher)
		}
		tester.fetcher.Notify("attacker", attack[i],
			1 /* don't distance drop */, time.Now(), attackerHeaderFetcher,
			attackerBodyFetcher)
	}
	...
	for i := len(hashes) - maxQueueDist - 2; i >= 0; i-- {
		tester.fetcher.Notify("valid", hashes[i], uint64(len(hashes)-i-1),
			time.Now().Add(-arriveTimeout), validHeaderFetcher,
			validBodyFetcher)
		verifyImportEvent(t, imported, true)
	}
	verifyImportDone(t, imported)
...
func TestBlockMemoryExhaustionAttack(t *testing.T) {
	...
	if queued := enqueued.Load(); queued != blockLimit {
		t.Fatalf("queued block count mismatch: have %d, want %d", queued,
			blockLimit)
	}

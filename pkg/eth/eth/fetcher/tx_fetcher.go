...
var (
	txAnnounceInMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/announces/in",
		nil)
	txAnnounceKnownMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/announces/known",
		nil)
	txAnnounceUnderpricedMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/announces/underpriced",
		nil)
	txAnnounceDOSMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/announces/dos",
		nil)

	txBroadcastInMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/broadcasts/in",
		nil)
	txBroadcastKnownMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/broadcasts/known",
		nil)
	txBroadcastUnderpricedMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/broadcasts/underpriced",
		nil)
	txBroadcastOtherRejectMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/broadcasts/otherreject",
		nil)

	txRequestOutMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/request/out",
		nil)
	txRequestFailMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/request/fail",
		nil)
	txRequestDoneMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/request/done",
		nil)
	txRequestTimeoutMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/request/timeout",
		nil)

	txReplyInMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/replies/in",
		nil)
	txReplyKnownMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/replies/known",
		nil)
	txReplyUnderpricedMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/replies/underpriced",
		nil)
	txReplyOtherRejectMeter = metrics.NewRegisteredMeter("eth/fetcher/transaction/replies/otherreject",
		nil)

	txFetcherWaitingPeers = metrics.NewRegisteredGauge("eth/fetcher/transaction/waiting/peers",
		nil)
	txFetcherWaitingHashes = metrics.NewRegisteredGauge("eth/fetcher/transaction/waiting/hashes",
		nil)
	txFetcherQueueingPeers = metrics.NewRegisteredGauge("eth/fetcher/transaction/queueing/peers",
		nil)
	txFetcherQueueingHashes = metrics.NewRegisteredGauge("eth/fetcher/transaction/queueing/hashes",
		nil)
	txFetcherFetchingPeers = metrics.NewRegisteredGauge("eth/fetcher/transaction/fetching/peers",
		nil)
	txFetcherFetchingHashes = metrics.NewRegisteredGauge("eth/fetcher/transaction/fetching/hashes",
		nil)
)
...
type TxFetcher struct {
	...
	fetchTxs func(string,
		[]common.Hash) error // Retrieves a set of txs from a remote peer
...
func NewTxFetcher(hasTx func(common.Hash) bool,
	addTxs func([]*types.Transaction) []error,
	fetchTxs func(string, []common.Hash) error,
	dropPeer func(string)) *TxFetcher {
	return NewTxFetcherForTests(hasTx, addTxs, fetchTxs, dropPeer,
		mclock.System{}, nil)
}
...
func NewTxFetcherForTests(
	hasTx func(common.Hash) bool, addTxs func([]*types.Transaction) []error,
	fetchTxs func(string, []common.Hash) error, dropPeer func(string),
...
func (f *TxFetcher) Notify(peer string, types []byte, sizes []uint32,
	...
	for i, hash := range hashes {
		...
		default:
			...
			if types == nil {
				...
				unknownMetas = append(unknownMetas,
					&txMetadata{kind: types[i], size: sizes[i]})
			}
		}
	}
	...
	announce := &txAnnounce{origin: peer, hashes: unknownHashes,
		metas: unknownMetas}
	select {
...
func (f *TxFetcher) Enqueue(peer string, txs []*types.Transaction,
	direct bool) error {
	var (
	...
	for i := 0; i < len(txs); i += 128 {
		...
		for j, err := range f.addTxs(batch) {
			...
			// announcement.
			if errors.Is(err, txpool.ErrUnderpriced) || errors.Is(err,
			...
			case errors.Is(err, txpool.ErrUnderpriced) || errors.Is(err,
				txpool.ErrReplaceUnderpriced):
				underpriced++
		...
		if otherreject > 128/4 {
			...
			log.Debug("Peer delivering stale transactions", "peer", peer,
				"rejected", otherreject)
		}
	}
	select {
	case f.cleanup <- &txDelivery{origin: peer, hashes: added, metas: metas,
		direct: direct}:
		return nil
	case <-f.quit:
		return errTerminated
	...
	for {
		...
		case ann := <-f.notify:
			...
			if !oldPeer && len(f.announces[ann.origin]) > 0 {
				f.scheduleFetches(timeoutTimer, timeoutTrigger,
					map[string]struct{}{ann.origin: {}})
			}
		...
		case delivery := <-f.cleanup:
					...
					for peer, txset := range f.waitslots {
						if meta := txset[hash]; meta != nil {
							if delivery.metas[i].kind != meta.kind {
								log.Warn("Announced transaction type mismatch",
									"peer", peer, "tx", hash, "type",
								...
								if math.Abs(float64(delivery.metas[i].size)-float64(meta.size)) > 8 {
									log.Warn("Announced transaction size mismatch",
										"peer", peer, "tx", hash, "size",
										delivery.metas[i].size, "ann",
										meta.size)
			...
			if delivery.direct {
				...
				if req == nil {
					log.Warn("Unexpected transaction delivery", "peer",
						delivery.origin)
					break
				}
				...
				for i, hash := range req.hashes {
					...
					if _, ok := delivered[hash]; !ok {
						...
						if len(f.alternates[hash]) > 0 {
							if _, ok := f.announced[hash]; ok {
								panic(fmt.Sprintf("announced tracker already contains alternate item: %v",
									f.announced[hash]))
							}
							f.announced[hash] = f.alternates[hash]
						}
				...
				f.scheduleFetches(timeoutTimer, timeoutTrigger,
					nil) // Partial delivery may enable others to deliver too
			}
...
func (f *TxFetcher) rescheduleWait(timer *mclock.Timer, trigger chan struct{}) {
	...
	*timer = f.clock.AfterFunc(txArriveTimeout-time.Duration(now-earliest),
		func() {
			trigger <- struct{}{}
		})
}
...
func (f *TxFetcher) rescheduleTimeout(timer *mclock.Timer,
	...
	*timer = f.clock.AfterFunc(txFetchTimeout-time.Duration(now-earliest),
		func() {
			trigger <- struct{}{}
		})
}
...
func (f *TxFetcher) scheduleFetches(timer *mclock.Timer, timeout chan struct{},
	...
	f.forEachPeer(actives, func(peer string) {
		...
		f.forEachAnnounce(f.announces[peer],
			func(hash common.Hash, meta *txMetadata) bool {
				...
				if _, ok := f.alternates[hash]; ok {
					panic(fmt.Sprintf("alternate tracker already contains fetching item: %v",
						f.alternates[hash]))
				}
...
func (f *TxFetcher) forEachPeer(peers map[string]struct{},
	do func(peer string)) {
...
// issues.
func (f *TxFetcher) forEachAnnounce(announces map[common.Hash]*txMetadata,
	do func(hash common.Hash, meta *txMetadata) bool) {
	// If we're running production, use whatever Go's map gives us
	if f.rand == nil {

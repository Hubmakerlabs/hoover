...
var (
	...
	testTxs = []*types.Transaction{
		types.NewTransaction(5577006791947779410, common.Address{0x0f},
			new(big.Int), 0, new(big.Int), nil),
		types.NewTransaction(15352856648520921629, common.Address{0xbb},
			new(big.Int), 0, new(big.Int), nil),
		types.NewTransaction(3916589616287113937, common.Address{0x86},
			new(big.Int), 0, new(big.Int), nil),
		types.NewTransaction(9828766684487745566, common.Address{0xac},
			new(big.Int), 0, new(big.Int), nil),
	}
	// testTxsHashes is the hashes of the test transactions above
	testTxsHashes = []common.Hash{testTxs[0].Hash(), testTxs[1].Hash(),
		testTxs[2].Hash(), testTxs[3].Hash()}
)
...
func TestTransactionFetcherWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A",
				hashes: []common.Hash{{0x01}, {0x03}, {0x05}}},
			isWaiting(map[string][]common.Hash{
	...
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			isWaitingWithMeta(map[string][]announce{
				"A": {
					{common.Hash{0x01}, typeptr(types.LegacyTxType),
						sizeptr(111)},
					{common.Hash{0x02}, typeptr(types.LegacyTxType),
						sizeptr(222)},
				},
			}),
			// Announce from a new peer to check that no overwrite happens
			...
			isWaitingWithMeta(map[string][]announce{
				...
				"B": {
					{common.Hash{0x03}, typeptr(types.LegacyTxType),
						sizeptr(333)},
					{common.Hash{0x04}, typeptr(types.LegacyTxType),
						sizeptr(444)},
				},
			}),
			// Announce clashing hashes but unique new peer
			...
			doTxNotify{peer: "A", hashes: []common.Hash{{0x01}, {0x03}, {0x05}},
				types: []byte{types.LegacyTxType, types.LegacyTxType,
					types.LegacyTxType}, sizes: []uint32{999, 333, 555}},
			isWaitingWithMeta(map[string][]announce{
				"A": {
					{common.Hash{0x01}, typeptr(types.LegacyTxType),
						sizeptr(111)},
					...
					{common.Hash{0x03}, typeptr(types.LegacyTxType),
						sizeptr(333)},
					{common.Hash{0x05}, typeptr(types.LegacyTxType),
						sizeptr(555)},
				},
				"B": {
					{common.Hash{0x03}, typeptr(types.LegacyTxType),
			...
			doTxNotify{peer: "D", hashes: []common.Hash{{0x01}, {0x02}},
				types: []byte{types.LegacyTxType, types.BlobTxType},
				sizes: []uint32{999, 222}},
			isWaitingWithMeta(map[string][]announce{
				"A": {
					{common.Hash{0x01}, typeptr(types.LegacyTxType),
						sizeptr(111)},
				...
				"D": {
					{common.Hash{0x01}, typeptr(types.LegacyTxType),
						sizeptr(999)},
					{common.Hash{0x02}, typeptr(types.BlobTxType),
						sizeptr(222)},
				},
			}),
			isScheduled{tracking: nil, fetching: nil},
			...
			doTxNotify{peer: "C", hashes: []common.Hash{{0x06}, {0x07}},
				types: []byte{types.LegacyTxType, types.LegacyTxType},
				sizes: []uint32{666, 777}},
			isWaitingWithMeta(map[string][]announce{
				"C": {
					{common.Hash{0x06}, typeptr(types.LegacyTxType),
						sizeptr(666)},
					{common.Hash{0x07}, typeptr(types.LegacyTxType),
						sizeptr(777)},
				},
			}),
			doWait{time: txArriveTimeout, step: true},
			...
			doTxNotify{peer: "E", hashes: []common.Hash{{0x06}, {0x07}},
				types: []byte{types.LegacyTxType, types.LegacyTxType},
				sizes: []uint32{666, 777}},
			isScheduledWithMeta{
				tracking: map[string][]announce{
...
func TestTransactionFetcherSkipWaiting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "B",
				hashes: []common.Hash{{0x02}, {0x03}, {0x04}}},
			isWaiting(map[string][]common.Hash{
				"A": {{0x03}},
				"B": {{0x03}, {0x04}},
			}),
...
func TestTransactionFetcherSingletonRequesting(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "B",
				hashes: []common.Hash{{0x02}, {0x03}, {0x05}, {0x06}}},
			isWaiting(map[string][]common.Hash{
				"B": {{0x05}, {0x06}},
			}),
			isScheduled{
...
func TestTransactionFetcherMissingRescheduling(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxEnqueue{peer: "A", txs: []*types.Transaction{testTxs[0]},
				direct: true}, // This depends on the deterministic random
			isScheduled{
				tracking: map[string][]common.Hash{
					"A": {testTxsHashes[2]},
				},
...
func TestTransactionFetcherMissingCleanup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			// Deliver the middle transaction requested, the one before which
			// should be dropped and the one after re-requested.
			doTxEnqueue{peer: "A", txs: []*types.Transaction{testTxs[1]},
				direct: true}, // This depends on the deterministic random
			isScheduled{nil, nil, nil},
		},
	})
	...
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A",
				...
				types: []byte{types.LegacyTxType, types.LegacyTxType,
					types.LegacyTxType, types.LegacyTxType},
			...
			doTxNotify{peer: "C",
				...
				sizes: []uint32{params.MaxBlobGasPerBlock,
					params.MaxBlobGasPerBlock},
			},
			doWait{time: txArriveTimeout, step: true},
			isWaiting(nil),
			isScheduledWithMeta{
				tracking: map[string][]announce{
					...
					"B": {
						{common.Hash{0x05}, typeptr(types.LegacyTxType),
							sizeptr(maxTxRetrievalSize)},
						{common.Hash{0x06}, typeptr(types.LegacyTxType),
							sizeptr(maxTxRetrievalSize)},
					},
					"C": {
						{common.Hash{0x07}, typeptr(types.BlobTxType),
							sizeptr(params.MaxBlobGasPerBlock)},
						{common.Hash{0x08}, typeptr(types.BlobTxType),
							sizeptr(params.MaxBlobGasPerBlock)},
					},
				},
				fetching: map[string][]common.Hash{
...
func TestTransactionFetcherDoSProtection(t *testing.T) {
	...
	for i := 0; i < maxTxAnnounces+1; i++ {
		hashesA = append(hashesA,
			common.Hash{0x01, byte(i / 256), byte(i % 256)})
	}
	var hashesB []common.Hash
	for i := 0; i < maxTxAnnounces+1; i++ {
		hashesB = append(hashesB,
			common.Hash{0x02, byte(i / 256), byte(i % 256)})
	}
	testTransactionFetcherParallel(t, txFetcherTest{
		init: func() *TxFetcher {
			return NewTxFetcher(
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A",
				hashes: hashesA[maxTxAnnounces/2 : maxTxAnnounces]},
			doTxNotify{peer: "B",
				hashes: hashesB[maxTxAnnounces/2-1 : maxTxAnnounces-1]},
			...
			isScheduled{
				...
				fetching: map[string][]common.Hash{
					...
					"B": append(append([]common.Hash{},
						hashesB[maxTxAnnounces/2-3:maxTxAnnounces/2-1]...),
						hashesB[:maxTxRetrievals-2]...),
				},
			},
			// Ensure that adding even one more hash results in dropping the hash
			doTxNotify{peer: "A",
				hashes: []common.Hash{hashesA[maxTxAnnounces]}},
			doTxNotify{peer: "B",
				hashes: hashesB[maxTxAnnounces-1 : maxTxAnnounces+1]},
...
func TestTransactionFetcherUnderpricedDedup(t *testing.T) {
	testTransactionFetcherParallel(t, txFetcherTest{
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A",
				hashes: []common.Hash{testTxsHashes[0], testTxsHashes[1],
					testTxsHashes[2]}}, // [2] is needed to force a step in the fetcher
			isWaiting(map[string][]common.Hash{
				"A": {testTxsHashes[2]},
			}),
			isScheduled{nil, nil, nil},
...
func TestTransactionFetcherUnderpricedDoSProtection(t *testing.T) {
	...
	for i := 0; i < maxTxUnderpricedSetSize+1; i++ {
		txs = append(txs, types.NewTransaction(rand.Uint64(),
			common.Address{byte(rand.Intn(256))}, new(big.Int), 0, new(big.Int),
			nil))
	}
	...
	for i := 0; i < maxTxUnderpricedSetSize/maxTxRetrievals; i++ {
		steps = append(steps, doTxNotify{peer: "A",
		...
		steps = append(steps, doTxEnqueue{peer: "A",
			txs: txs[i*maxTxRetrievals : (i+1)*maxTxRetrievals], direct: true})
	...
	testTransactionFetcher(t, txFetcherTest{
		...
		steps: append(steps, []interface{}{
			...
			doWait{time: txArriveTimeout, step: true},
			doTxEnqueue{peer: "A",
				txs:    []*types.Transaction{txs[maxTxUnderpricedSetSize]},
				direct: true},
		...
		steps: []interface{}{
			...
			doTxNotify{peer: "A",
				...
				sizes: []uint32{uint32(testTxs[0].Size()),
					uint32(testTxs[1].Size())}},
			isWaitingWithMeta(map[string][]announce{
				"A": {
					{testTxsHashes[0], typeptr(testTxs[0].Type()),
						sizeptr(uint32(testTxs[0].Size()))},
					{testTxsHashes[1], typeptr(testTxs[1].Type()),
						sizeptr(uint32(testTxs[1].Size()))},
				},
			}),
			...
			doTxNotify{peer: "C", hashes: []common.Hash{testTxsHashes[1]},
			...
			isWaitingWithMeta(map[string][]announce{
				...
				"B": {
					{testTxsHashes[0], typeptr(testTxs[0].Type()),
						sizeptr(1024 + uint32(testTxs[0].Size()))},
				},
				"C": {
					{testTxsHashes[1], typeptr(1 + testTxs[1].Type()),
						sizeptr(uint32(testTxs[1].Size()))},
				},
			}),
	...
	for i, step := range tt.steps {
		...
		case isScheduled:
			...
			for peer, hashes := range old.tracking {
				for _, hash := range hashes {
					new.tracking[peer] = append(new.tracking[peer],
						announce{hash, nil, nil})
				}
			}
			step = new
		}
		...
		case doTxNotify:
			if err := fetcher.Notify(step.peer, step.types, step.sizes,
				step.hashes); err != nil {
		...
		case doTxEnqueue:
			if err := fetcher.Enqueue(step.peer, step.txs,
				step.direct); err != nil {
				t.Errorf("step %d: %v", i, err)
			}
			<-wait // Fetcher needs to process this, wait until it's done
		...
		case isWaitingWithMeta:
			...
			for peer, announces := range step {
				...
				for _, ann := range announces {
					if meta, ok := waiting[ann.hash]; !ok {
						t.Errorf("step %d, peer %s: hash %x missing from waitslots",
							i, peer, ann.hash)
					} else {
						if (meta == nil && (ann.kind != nil || ann.size != nil)) ||
							(meta != nil && (ann.kind == nil || ann.size == nil)) ||
							(meta != nil && (meta.kind != *ann.kind || meta.size != *ann.size)) {
							t.Errorf("step %d, peer %s, hash %x: waitslot metadata mismatch: want %v, have %v/%v",
								i, peer, ann.hash, meta, *ann.kind, *ann.size)
						}
					}
				}
				for hash, meta := range waiting {
					...
					if !containsAnnounce(announces, ann) {
						t.Errorf("step %d, peer %s: announce %v extra in waitslots",
							i, peer, ann)
					}
				}
			}
			...
			for peer, announces := range step {
				for _, ann := range announces {
					if _, ok := fetcher.waitlist[ann.hash][peer]; !ok {
						t.Errorf("step %d, hash %x: peer %s missing from waitlist",
							i, ann.hash, peer)
					}
					if _, ok := fetcher.waittime[ann.hash]; !ok {
						t.Errorf("step %d: hash %x missing from waittime", i,
							ann.hash)
					}
				}
			}
			for hash, peers := range fetcher.waitlist {
				if len(peers) == 0 {
					t.Errorf("step %d, hash %x: empty peerset in waitlist", i,
						hash)
				}
				for peer := range peers {
					if !containsHashInAnnounces(step[peer], hash) {
						t.Errorf("step %d, hash %x: peer %s extra in waitlist",
							i, hash, peer)
					}
				}
			}
		...
		case isScheduledWithMeta:
			...
			for peer, announces := range step.tracking {
				...
				for _, ann := range announces {
					if meta, ok := scheduled[ann.hash]; !ok {
						t.Errorf("step %d, peer %s: hash %x missing from announces",
							i, peer, ann.hash)
					} else {
						if (meta == nil && (ann.kind != nil || ann.size != nil)) ||
							(meta != nil && (ann.kind == nil || ann.size == nil)) ||
							(meta != nil && (meta.kind != *ann.kind || meta.size != *ann.size)) {
							t.Errorf("step %d, peer %s, hash %x: announce metadata mismatch: want %v, have %v/%v",
								i, peer, ann.hash, meta, *ann.kind, *ann.size)
						}
					}
				}
				for hash, meta := range scheduled {
					...
					if !containsAnnounce(announces, ann) {
						t.Errorf("step %d, peer %s: announce %x extra in announces",
							i, peer, hash)
					}
				}
			}
			...
			for peer, hashes := range step.fetching {
				...
				for _, hash := range hashes {
					if !containsHash(request.hashes, hash) {
						t.Errorf("step %d, peer %s: hash %x missing from requests",
							i, peer, hash)
					}
				}
				for _, hash := range request.hashes {
					if !containsHash(hashes, hash) {
						t.Errorf("step %d, peer %s: hash %x extra in requests",
							i, peer, hash)
					}
				}
			}
			...
			for peer, hashes := range step.fetching {
				for _, hash := range hashes {
					if _, ok := fetcher.fetching[hash]; !ok {
						t.Errorf("step %d, peer %s: hash %x missing from fetching",
							i, peer, hash)
					}
				}
			}
			...
			for _, hashes := range step.fetching {
				for _, hash := range hashes {
					...
					if alternates == nil {
						t.Errorf("step %d: hash %x missing from alternates", i,
							hash)
						continue
					}
					for peer := range alternates {
						if _, ok := fetcher.announces[peer]; !ok {
							t.Errorf("step %d: peer %s extra in alternates", i,
								peer)
							continue
						}
						if _, ok := fetcher.announces[peer][hash]; !ok {
							t.Errorf("step %d, peer %s: hash %x extra in alternates",
								i, hash, peer)
							continue
						}
					}
					for p := range fetcher.announced[hash] {
						if _, ok := alternates[p]; !ok {
							t.Errorf("step %d, hash %x: peer %s missing from alternates",
								i, hash, p)
							continue
						}
					}
		...
		case isUnderpriced:
			if fetcher.underpriced.Len() != int(step) {
				t.Errorf("step %d: underpriced set size mismatch: have %d, want %d",
					i, fetcher.underpriced.Len(), step)
			}
		...
		for hash := range fetcher.waittime {
			if _, ok := fetcher.announced[hash]; ok {
				t.Errorf("step %d: hash %s present in both stage 1 and 2", i,
					hash)
			}
		}
	}
...
func TestTransactionForgotten(t *testing.T) {
	...
	if err := fetcher.Enqueue("asdf", []*types.Transaction{tx1, tx2},
		false); err != nil {
		t.Fatal(err)
	}

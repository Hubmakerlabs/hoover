...
func TestUnmarshalJSONNewFilterArgs(t *testing.T) {
	...
	vector := fmt.Sprintf(`{"fromBlock":"%v","toBlock":"%v"}`, fromBlock,
		toBlock)
	...
	vector = fmt.Sprintf(`{"address": ["%s", "%s"]}`, address0.Hex(),
		address1.Hex())
	if err := json.Unmarshal([]byte(vector), &test3); err != nil {
		t.Fatal(err)
	}
	if len(test3.Addresses) != 2 {
		t.Fatalf("expected 2 addresses, got %d address(es)",
			len(test3.Addresses))
	}
	...
	if len(test4.Topics[0]) != 1 {
		t.Fatalf("expected len(topics[0]) to be 1, got %d",
			len(test4.Topics[0]))
	}
	...
	vector = fmt.Sprintf(`{"topics": ["%s", null, "%s"]}`, topic0.Hex(),
		topic2.Hex())
	...
	vector = fmt.Sprintf(`{"topics": [["%s", "%s"], null, ["%s", null]]}`,
		topic0.Hex(), topic1.Hex(), topic2.Hex())

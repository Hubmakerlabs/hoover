...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	...
	b.Log(" data size:", dataSize, "  compressed size:", compSize,
		"  compression ratio:", float64(compSize)/float64(dataSize))
	...
	for i := 0; i < benchFilterCnt; i++ {
		...
		addr[0] = byte(i)
		addr[1] = byte(i / 256)
		filter := sys.NewRangeFilter(0, int64(cnt*sectionSize-1),
			[]common.Address{addr}, nil)
	...
	b.Log(" ", d, "total  ", d/time.Duration(benchFilterCnt), "per address",
		d*time.Duration(1000000)/time.Duration(benchFilterCnt*cnt*sectionSize),
		"per million blocks")
	db.Close()
}
...
func BenchmarkNoBloomBits(b *testing.B) {
	...
	d := time.Since(start)
	b.Log("Finished running filter benchmarks")
	b.Log(" ", d, "total  ", d*time.Duration(1000000)/time.Duration(*headNum+1),
		"per million blocks")
	db.Close()
}

...
// sink.
func (p *Peer) dispatchResponse(res *Response,
	metadata func() interface{}) error {
	resOp := &response{
		res:  res,
		fail: make(chan error),
	}
	...
	for {
		...
		case resOp := <-p.resDispatch:
			...
			case res.Req.want != res.code:
				...
				// the packet.
				resOp.fail <- fmt.Errorf("%w: have %d, want %d",
					errMismatchingResponseType, res.code, res.Req.want)

...
func newTestBackendWithGenerator(blocks int, shanghai bool,
	generator func(int, *core.BlockGen)) *testBackend {
	var (
	...
	chain, _ := core.NewBlockChain(db, nil, gspec, nil, engine, vm.Config{},
		nil, nil)
	...
	txpool, _ := txpool.New(new(big.Int).SetUint64(txconfig.PriceLimit), chain,
		[]txpool.SubPool{pool})
...
func testGetBlockHeaders(t *testing.T, protocol uint) {
	...
	tests := []struct {
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Hash: backend.chain.GetBlockByNumber(limit / 2).Hash()},
				Amount: 1},
			[]common.Hash{backend.chain.GetBlockByNumber(limit / 2).Hash()},
		},
		// A single random block should be retrievable by number
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: limit / 2},
				Amount: 1},
			[]common.Hash{backend.chain.GetBlockByNumber(limit / 2).Hash()},
		},
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: backend.chain.CurrentBlock().Number.Uint64()},
				Amount: 1},
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: backend.chain.CurrentBlock().Number.Uint64() - 1},
				Amount: limit + 10, Reverse: true},
			getHashes(backend.chain.CurrentBlock().Number.Uint64(), limit),
		},
		// Check that requesting more than available is handled gracefully
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: backend.chain.CurrentBlock().Number.Uint64() - 4},
				Skip: 3, Amount: 3},
			[]common.Hash{
			...
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: 4}, Skip: 3,
				Amount: 3, Reverse: true},
			[]common.Hash{
		...
		// Check that requesting more than available is handled gracefully, even if mid skip
		{
			...
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: 4}, Skip: 2,
				Amount: 3, Reverse: true},
			[]common.Hash{
				backend.chain.GetBlockByNumber(4).Hash(),
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: 2}, Amount: 5,
				Reverse: true},
			[]common.Hash{
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Hash: backend.chain.GetBlockByNumber(3).Hash()},
				Amount: 2, Reverse: false, Skip: math.MaxUint64 - 1},
			[]common.Hash{
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Hash: backend.chain.GetBlockByNumber(1).Hash()},
				Amount: 2, Reverse: false, Skip: math.MaxUint64},
			[]common.Hash{
				backend.chain.GetBlockByNumber(1).Hash(),
			},
		},
		...
		{
			&GetBlockHeadersRequest{Origin: HashOrNumber{Hash: unknown},
				Amount: 1},
			[]common.Hash{},
		}, {
			&GetBlockHeadersRequest{Origin: HashOrNumber{Number: backend.chain.CurrentBlock().Number.Uint64() + 1},
	...
	for i, tt := range tests {
		...
		for _, hash := range tt.expect {
			headers = append(headers,
				backend.chain.GetBlockByHash(hash).Header())
		}
		...
		if tt.query.Origin.Hash == (common.Hash{}) {
			if origin := backend.chain.GetBlockByNumber(tt.query.Origin.Number); origin != nil {
				...
				expected := &BlockHeadersPacket{RequestId: 456,
					BlockHeadersRequest: headers}
				if err := p2p.ExpectMsg(peer.app, BlockHeadersMsg,
					expected); err != nil {
...
func testGetBlockBodies(t *testing.T, protocol uint) {
	...
	tests := []struct {
		...
		{limit + 1, nil, nil,
			limit}, // No more than the possible block count should be returned
		{0, []common.Hash{backend.chain.Genesis().Hash()}, []bool{true},
			1}, // The genesis block should be retrievable
		{0, []common.Hash{backend.chain.CurrentBlock().Hash()}, []bool{true},
				...
				if !seen[num] {
					...
					if len(bodies) < tt.expected {
						bodies = append(bodies,
							&BlockBody{Transactions: block.Transactions(),
								Uncles:      block.Uncles(),
...
func testGetBlockReceipts(t *testing.T, protocol uint) {
	...
	generator := func(i int, block *core.BlockGen) {
		...
		case 0:
			...
			tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testAddr),
				acc1Addr, big.NewInt(10_000_000_000_000_000), params.TxGas,
				block.BaseFee(), nil), signer, testKey)
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more ether to account #1.
			// acc1Addr passes it on to account #2.
			tx1, _ := types.SignTx(types.NewTransaction(block.TxNonce(testAddr),
				acc1Addr, big.NewInt(1_000_000_000_000_000), params.TxGas,
				block.BaseFee(), nil), signer, testKey)
			tx2, _ := types.SignTx(types.NewTransaction(block.TxNonce(acc1Addr),
				acc2Addr, big.NewInt(1_000_000_000_000_000), params.TxGas,
				block.BaseFee(), nil), signer, acc1Key)
			block.AddTx(tx1)
			block.AddTx(tx2)
		case 2:
	...
	for i := uint64(0); i <= backend.chain.CurrentBlock().Number.Uint64(); i++ {
		...
		receipts = append(receipts,
			backend.chain.GetReceiptsByHash(block.Hash()))
	}
	// Send the hash request and verify the response

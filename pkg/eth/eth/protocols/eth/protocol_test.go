...
func TestGetBlockHeadersDataEncodeDecode(t *testing.T) {
	...
	tests := []struct {
		...
		{fail: false,
			packet: &GetBlockHeadersRequest{Origin: HashOrNumber{Number: 314}}},
		{fail: false,
			packet: &GetBlockHeadersRequest{Origin: HashOrNumber{Hash: hash}}},

		// Providing arbitrary query field should also work
		{fail: false,
			packet: &GetBlockHeadersRequest{Origin: HashOrNumber{Number: 314},
				Amount: 314, Skip: 1, Reverse: true}},
		{fail: false,
			packet: &GetBlockHeadersRequest{Origin: HashOrNumber{Hash: hash},
				Amount: 314, Skip: 1, Reverse: true}},
	...
	for i, tt := range tests {
		...
		if !tt.fail {
			...
			if packet.Origin.Hash != tt.packet.Origin.Hash || packet.Origin.Number != tt.packet.Origin.Number || packet.Amount != tt.packet.Amount ||
				...
				t.Fatalf("test %d: encode decode mismatch: have %+v, want %+v",
					i, packet, tt.packet)
			}
		}
	}
...
func TestEmptyMessages(t *testing.T) {
	...
	for i, msg := range []interface{}{
		...
		// Transactions
		GetPooledTransactionsPacket{1111,
			GetPooledTransactionsRequest([]common.Hash{})},
		PooledTransactionsPacket{1111,
			PooledTransactionsResponse([]*types.Transaction{})},
		PooledTransactionsRLPPacket{1111,
			PooledTransactionsRLPResponse([]rlp.RawValue{})},
	} {
...
func TestMessages(t *testing.T) {
				...
				Logs: []*types.Log{
					{
						...
						Topics: []common.Hash{common.HexToHash("dead"),
							common.HexToHash("beef")},
						Data: []byte{0x01, 0x00, 0xff},
					},
				},
				TxHash: hashes[0],
				ContractAddress: common.BytesToAddress([]byte{0x01, 0x11,
					0x11}),
				GasUsed: 111111,
			},
		}
	...
	for i, tc := range []struct {
		...
		{
			GetBlockHeadersPacket{1111,
				&GetBlockHeadersRequest{HashOrNumber{hashes[0], 0}, 5, 5,
					false}},
			common.FromHex("e8820457e4a000000000000000000000000000000000000000000000000000000000deadc0de050580"),
		},
		{
			GetBlockHeadersPacket{1111,
				&GetBlockHeadersRequest{HashOrNumber{common.Hash{}, 9999}, 5, 5,
					false}},
		...
		{
			BlockBodiesPacket{1111,
				BlockBodiesResponse([]*BlockBody{blockBody})},
		...
		{ // Identical to non-rlp-shortcut version
			BlockBodiesRLPPacket{1111,
				BlockBodiesRLPResponse([]rlp.RawValue{blockBodyRlp})},
			common.FromHex(""),
		},
		{
		...
		{
			ReceiptsPacket{1111,
				ReceiptsResponse([][]*types.Receipt{receipts})},
		...
		{
			ReceiptsRLPPacket{1111,
				ReceiptsRLPResponse([]rlp.RawValue{receiptsRlp})},
			common.FromHex(""),
		},
		{
			GetPooledTransactionsPacket{1111,
		...
		{
			PooledTransactionsRLPPacket{1111,
				PooledTransactionsRLPResponse(txRlps)},
			common.FromHex(""),
		},
	} {
		if have, _ := rlp.EncodeToBytes(tc.message); !bytes.Equal(have,
			tc.want) {
			t.Errorf("test %d, type %T, have\n\t%x\nwant\n\t%x", i, tc.message,
				have, tc.want)
		}

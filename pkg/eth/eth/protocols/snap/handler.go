	...
	for i, version := range ProtocolVersions {
		...
		protocols[i] = p2p.Protocol{
			...
			Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
				return backend.RunPeer(NewPeer(version, p, rw),
...
func HandleMessage(backend Backend, peer *Peer) error {
	...
	if msg.Size > maxMessageSize {
		return fmt.Errorf("%w: %v > %v", errMsgTooLarge, msg.Size,
			maxMessageSize)
	}
	defer msg.Discard()
	start := time.Now()
	...
	if metrics.Enabled {
		h := fmt.Sprintf("%s/%s/%d/%#02x", p2p.HandleHistName, ProtocolName,
			peer.Version(), msg.Code)
		defer func(start time.Time) {
			...
			metrics.GetOrRegisterHistogramLazy(h, nil,
				sampler).Update(time.Since(start).Microseconds())
		}(start)
	}
...
func ServiceGetAccountRangeQuery(chain *core.BlockChain,
	req *GetAccountRangePacket) ([]*AccountData, [][]byte) {
	...
	if err := tr.Prove(req.Origin[:], proof); err != nil {
		log.Warn("Failed to prove account range", "origin", req.Origin, "err",
			err)
		return nil, nil
	}
	if last != (common.Hash{}) {
...
func ServiceGetStorageRangesQuery(chain *core.BlockChain,
	req *GetStorageRangesPacket) ([][]*StorageData, [][]byte) {
	if req.Bytes > softResponseLimit {
		req.Bytes = softResponseLimit
	}
	...
	for _, account := range req.Accounts {
		...
		if origin != (common.Hash{}) || (abort && len(storage) > 0) {
			...
			accTrie, err := trie.NewStateTrie(trie.StateTrieID(req.Root),
				chain.TrieDB())
			if err != nil {
				return nil, nil
			}
			...
			if err := stTrie.Prove(origin[:], proof); err != nil {
				log.Warn("Failed to prove storage range", "origin", req.Origin,
					"err", err)
				return nil, nil
			}
			if last != (common.Hash{}) {
				if err := stTrie.Prove(last[:], proof); err != nil {
					log.Warn("Failed to prove storage range", "last", last,
						"err", err)
					return nil, nil
				}
			}
...
// It is exposed to allow external packages to test protocol behavior.
func ServiceGetByteCodesQuery(chain *core.BlockChain,
	req *GetByteCodesPacket) [][]byte {
	if req.Bytes > softResponseLimit {
		req.Bytes = softResponseLimit
	}
...
// It is exposed to allow external packages to test protocol behavior.
func ServiceGetTrieNodesQuery(chain *core.BlockChain, req *GetTrieNodesPacket,
	start time.Time) ([][]byte, error) {
	if req.Bytes > softResponseLimit {
		req.Bytes = softResponseLimit
	}
	...
	for _, pathset := range req.Paths {
		...
		case 0:
			...
			return nil, fmt.Errorf("%w: zero-item pathset requested",
				errBadRequest)
		...
		default:
			...
			id := trie.StorageTrieID(req.Root, common.BytesToHash(pathset[0]),
				stRoot)

...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/msgrate"
	"github.com/Hubmakerlabs/hoover/pkg/eth/rlp"
	"github.com/Hubmakerlabs/hoover/pkg/eth/trie"
	"github.com/Hubmakerlabs/hoover/pkg/eth/trie/trienode"
...
type SyncPeer interface {
	...
	RequestAccountRange(id uint64, root, origin, limit common.Hash,
		bytes uint64) error

	// RequestStorageRanges fetches a batch of storage slots belonging to one or
	// more accounts. If slots from only one account is requested, an origin marker
	// may also be used to retrieve from there.
	RequestStorageRanges(id uint64, root common.Hash, accounts []common.Hash,
		origin, limit []byte, bytes uint64) error
	...
	RequestTrieNodes(id uint64, root common.Hash, paths []TrieNodePathSet,
		bytes uint64) error
...
func (s *Syncer) Register(peer SyncPeer) error {
	...
	s.peers[id] = peer
	s.rates.Track(id,
		msgrate.NewTracker(s.rates.MeanCapacities(), s.rates.MedianRoundTrip()))
	...
	for {
		...
		if len(s.tasks) == 0 {
			...
			s.assignTrienodeHealTasks(trienodeHealResps, trienodeHealReqFails,
				cancel)
			s.assignBytecodeHealTasks(bytecodeHealResps, bytecodeHealReqFails,
				cancel)
		}
...
func (s *Syncer) cleanPath(batch ethdb.Batch, owner common.Hash, path []byte) {
	...
	if owner != (common.Hash{}) && rawdb.ExistsStorageTrieNode(s.db, owner,
		path) {
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range progress.Tasks {
				log.Debug("Scheduled account sync task", "from", task.Next,
					"last", task.Last)
			}
			...
			for _, task := range s.tasks {
				...
				options = options.WithWriter(func(path []byte, hash common.Hash,
					blob []byte) {
					rawdb.WriteTrieNode(task.genBatch, common.Hash{}, path,
						hash, blob, s.scheme)
				})
				if s.scheme == rawdb.PathScheme {
					...
					options = options.WithSkipBoundary(task.Next != (common.Hash{}),
						task.Last != common.MaxHash, boundaryAccountNodesGauge)
				}
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						...
						options := trie.NewStackTrieOptions()
						options = options.WithWriter(func(path []byte,
							hash common.Hash, blob []byte) {
							rawdb.WriteTrieNode(subtask.genBatch, owner, path,
								hash, blob, s.scheme)
						})
						if s.scheme == rawdb.PathScheme {
							// Configure the dangling node cleaner and also filter out boundary nodes
							...
							// Skip the left boundary if it's not the first range.
							// Skip the right boundary if it's not the last range.
							options = options.WithSkipBoundary(subtask.Next != common.Hash{},
								subtask.Last != common.MaxHash,
	...
	for i := 0; i < accountConcurrency; i++ {
		...
		options = options.WithWriter(func(path []byte, hash common.Hash,
			blob []byte) {
			rawdb.WriteTrieNode(batch, common.Hash{}, path, hash, blob,
				s.scheme)
		})
		if s.scheme == rawdb.PathScheme {
			// Configure the dangling node cleaner and also filter out boundary nodes
			...
			// Skip the left boundary if it's not the first range.
			// Skip the right boundary if it's not the last range.
			options = options.WithSkipBoundary(next != common.Hash{},
				last != common.MaxHash, boundaryAccountNodesGauge)
		}
...
// retrievals.
func (s *Syncer) assignAccountTasks(success chan *accountResponse,
	fail chan *accountRequest, cancel chan struct{}) {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	for id := range s.accountIdlers {
		...
		idlers.caps = append(idlers.caps,
			s.rates.Capacity(id, AccountRangeMsg, targetTTL))
	}
	if len(idlers.ids) == 0 {
		return
	}
	...
	for _, task := range s.tasks {
		...
		go func(root common.Hash) {
			...
			if err := peer.RequestAccountRange(reqid, root, req.origin,
				req.limit, uint64(cap)); err != nil {
...
func (s *Syncer) assignBytecodeTasks(success chan *bytecodeResponse,
	fail chan *bytecodeRequest, cancel chan struct{}) {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	for id := range s.bytecodeIdlers {
		...
		idlers.ids = append(idlers.ids, id)
		idlers.caps = append(idlers.caps,
			s.rates.Capacity(id, ByteCodesMsg, targetTTL))
	}
	if len(idlers.ids) == 0 {
		return
	}
	...
	for _, task := range s.tasks {
		...
		go func() {
			...
			// Attempt to send the remote request and revert if it fails
			if err := peer.RequestByteCodes(reqid, hashes,
				maxRequestSize); err != nil {
...
// retrievals.
func (s *Syncer) assignStorageTasks(success chan *storageResponse,
	fail chan *storageRequest, cancel chan struct{}) {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	for id := range s.storageIdlers {
		...
		idlers.ids = append(idlers.ids, id)
		idlers.caps = append(idlers.caps,
			s.rates.Capacity(id, StorageRangesMsg, targetTTL))
	}
	if len(idlers.ids) == 0 {
		return
	}
...
func (s *Syncer) assignTrienodeHealTasks(success chan *trienodeHealResponse,
	fail chan *trienodeHealRequest, cancel chan struct{}) {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	for id := range s.trienodeHealIdlers {
		...
		idlers.ids = append(idlers.ids, id)
		idlers.caps = append(idlers.caps,
			s.rates.Capacity(id, TrieNodesMsg, targetTTL))
	}
	if len(idlers.ids) == 0 {
		return
	}
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		go func(root common.Hash) {
			...
			// Attempt to send the remote request and revert if it fails
			if err := peer.RequestTrieNodes(reqid, root, pathsets,
				maxRequestSize); err != nil {
...
// heal any trie errors caused by the snap sync's chunked retrieval model.
func (s *Syncer) assignBytecodeHealTasks(success chan *bytecodeHealResponse,
	fail chan *bytecodeHealRequest, cancel chan struct{}) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Syncer) revertAccountRequest(req *accountRequest) {
	...
	select {
	case <-req.stale:
		log.Trace("Account request already reverted", "peer", req.peer, "reqid",
			req.id)
		return
	default:
	}
...
func (s *Syncer) revertBytecodeRequest(req *bytecodeRequest) {
	...
	select {
	case <-req.stale:
		log.Trace("Bytecode request already reverted", "peer", req.peer,
			"reqid", req.id)
		return
	default:
	}
...
func (s *Syncer) revertStorageRequest(req *storageRequest) {
	...
	select {
	case <-req.stale:
		log.Trace("Storage request already reverted", "peer", req.peer, "reqid",
			req.id)
		return
	default:
	}
...
func (s *Syncer) revertTrienodeHealRequest(req *trienodeHealRequest) {
	...
	select {
	case <-req.stale:
		log.Trace("Trienode heal request already reverted", "peer", req.peer,
			"reqid", req.id)
		return
	default:
	}
...
func (s *Syncer) revertBytecodeHealRequest(req *bytecodeHealRequest) {
	...
	select {
	case <-req.stale:
		log.Trace("Bytecode heal request already reverted", "peer", req.peer,
			"reqid", req.id)
		return
	default:
	}
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	for i, account := range res.accounts {
		...
		if !bytes.Equal(account.CodeHash, types.EmptyCodeHash.Bytes()) {
			if !rawdb.HasCodeWithPrefix(s.db,
				common.BytesToHash(account.CodeHash)) {
		...
		if account.Root != types.EmptyRootHash {
			if !rawdb.HasTrieNode(s.db, res.hashes[i], nil, account.Root,
				s.scheme) {
				...
				if subtasks, ok := res.task.SubTasks[res.hashes[i]]; ok {
					log.Debug("Resuming large storage retrieval", "account",
						res.hashes[i], "root", account.Root)
					for _, subtask := range subtasks {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	for i, account := range res.accounts {
		...
		for j, hash := range res.mainTask.res.hashes {
			...
			if res.subTask == nil && i == len(res.hashes)-1 && res.cont {
				...
				if tasks, ok := res.mainTask.SubTasks[account]; !ok {
					...
					if estimate, err := estimateRemainingSlots(len(keys),
						lastKey); err == nil {
						if n := estimate / (2 * (maxRequestSize / 64)); n+1 < chunks {
							chunks = n + 1
						}
						log.Debug("Chunked large contract", "initiators",
							len(keys), "tail", lastKey, "remaining", estimate,
							"chunks", chunks)
					} else {
						log.Debug("Chunked large contract", "initiators",
							len(keys), "tail", lastKey, "chunks", chunks)
					}
					...
					options = options.WithWriter(func(path []byte,
						hash common.Hash, blob []byte) {
						rawdb.WriteTrieNode(batch, owner, path, hash, blob,
							s.scheme)
					})
					if s.scheme == rawdb.PathScheme {
						options = options.WithCleaner(func(path []byte) {
						...
						// Skip the right boundary if it's not the last range.
						options = options.WithSkipBoundary(false,
							r.End() != common.MaxHash,
							boundaryStorageNodesGauge)
					}
					...
					for r.Next() {
						...
						if s.scheme == rawdb.PathScheme {
							...
							// Skip the right boundary if it's not the last range.
							options = options.WithSkipBoundary(true,
								r.End() != common.MaxHash,
								boundaryStorageNodesGauge)
						}
						tasks = append(tasks, &storageTask{
					...
					for _, task := range tasks {
						log.Debug("Created storage sync task", "account",
							account, "root", acc.Root, "from", task.Next,
							"last", task.Last)
					}
		...
		// snapshot generation.
		for j := 0; j < len(res.hashes[i]); j++ {
			rawdb.WriteStorageSnapshot(batch, account, res.hashes[i][j],
				res.slots[i][j])
	...
	if res.subTask != nil {
		if res.subTask.done {
			...
			if root == res.subTask.root && rawdb.HasStorageTrieNode(s.db,
				accountHash, nil, root) {
	...
	log.Debug("Persisted set of storage slots", "accounts", len(res.hashes),
		"slots", slots, "bytes", s.storageBytes-oldStorageBytes)
...
func (s *Syncer) processTrienodeHealResponse(res *trienodeHealResponse) {
	...
	for i, hash := range res.hashes {
		...
		err := s.healer.scheduler.ProcessNode(trie.NodeSyncResult{Path: res.paths[i],
			Data: node})
		switch err {
		case nil:
	...
	s.trienodeHealRate = gomath.Pow(1-trienodeHealRateMeasurementImpact,
		float64(fills))*(s.trienodeHealRate-rate) + rate
	...
	if time.Since(s.trienodeHealThrottled) > time.Second {
		...
		log.Debug("Updated trie node heal throttler", "rate",
			s.trienodeHealRate, "pending", pending, "throttle",
...
func (s *Syncer) commitHealer(force bool) {
	...
	log.Debug("Persisted set of healing data", "type", "trienodes", "bytes",
		common.StorageSize(batch.ValueSize()))
}
...
func (s *Syncer) processBytecodeHealResponse(res *bytecodeHealResponse) {
	for i, hash := range res.hashes {
		...
		err := s.healer.scheduler.ProcessCode(trie.CodeSyncResult{Hash: hash,
			Data: node})
		switch err {
		case nil:
		case trie.ErrAlreadyProcessed:
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	log.Debug("Persisted range of accounts", "accounts", len(res.accounts),
		"bytes", s.accountBytes-oldAccountBytes)
}

// OnAccounts is a callback method to invoke when a range of accounts are
// received from a remote peer.
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash,
	accounts [][]byte, proof [][]byte) error {
	...
	logger.Trace("Delivering range of accounts", "hashes", len(hashes),
		"accounts", len(accounts), "proofs", len(proof), "bytes", size)
	...
	cont, err := trie.VerifyRangeProof(root, req.origin[:], keys, accounts,
		nodes.Set())
	if err != nil {
...
func (s *Syncer) OnByteCodes(peer SyncPeer, id uint64,
...
func (s *Syncer) onByteCodes(peer SyncPeer, id uint64,
	...
	logger := peer.Log().New("reqid", id)
	logger.Trace("Delivering set of bytecodes", "bytecodes", len(bytecodes),
		"bytes", size)
	...
	s.rates.Update(peer.ID(), ByteCodesMsg, time.Since(req.time),
		len(bytecodes))
...
func (s *Syncer) OnStorage(peer SyncPeer, id uint64, hashes [][]common.Hash,
	slots [][][]byte, proof [][]byte) error {
	...
	logger := peer.Log().New("reqid", id)
	logger.Trace("Delivering ranges of storage slots", "accounts", len(hashes),
		"hashes", hashCount, "slots", slotCount, "proofs", len(proof), "size",
		size)
	...
	if len(hashes) != len(slots) {
		...
		logger.Warn("Hash and slot set size mismatch", "hashset", len(hashes),
			"slotset", len(slots))
	...
	if len(hashes) > len(req.accounts) {
		s.lock.Unlock()
		s.scheduleRevertStorageRequest(req) // reschedule request
		logger.Warn("Hash set larger than requested", "hashset", len(hashes),
			"requested", len(req.accounts))
	...
	for i := 0; i < len(hashes); i++ {
		...
		if len(nodes) == 0 {
			...
			_, err = trie.VerifyRangeProof(req.roots[i], nil, keys, slots[i],
				nil)
			if err != nil {
				s.scheduleRevertStorageRequest(req) // reschedule request
			...
			cont, err = trie.VerifyRangeProof(req.roots[i], req.origin[:], keys,
				slots[i], proofdb)
			if err != nil {
				s.scheduleRevertStorageRequest(req) // reschedule request
...
// are received from a remote peer.
func (s *Syncer) OnTrieNodes(peer SyncPeer, id uint64,
	trienodes [][]byte) error {
	var size common.StorageSize
	for _, node := range trienodes {
		size += common.StorageSize(len(node))
	}
	logger := peer.Log().New("reqid", id)
	logger.Trace("Delivering set of healing trienodes", "trienodes",
		len(trienodes), "bytes", size)
	...
	s.rates.Update(peer.ID(), TrieNodesMsg, time.Since(req.time),
		len(trienodes))
...
func (s *Syncer) onHealByteCodes(peer SyncPeer, id uint64,
	bytecodes [][]byte) error {
	var size common.StorageSize
	for _, code := range bytecodes {
		size += common.StorageSize(len(code))
	...
	logger := peer.Log().New("reqid", id)
	logger.Trace("Delivering set of healing bytecodes", "bytecodes",
		len(bytecodes), "bytes", size)
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	if len(paths) == 1 {
		...
		rawdb.WriteAccountSnapshot(s.stateWriter, common.BytesToHash(paths[0]),
			blob)
		s.accountHealed += 1
		s.accountHealedBytes += common.StorageSize(1 + common.HashLength + len(blob))
	}
	if len(paths) == 2 {
		rawdb.WriteStorageSnapshot(s.stateWriter, common.BytesToHash(paths[0]),
			common.BytesToHash(paths[1]), value)
		s.storageHealed += 1
...
func (s *Syncer) reportSyncProgress(force bool) {
	...
	for _, task := range s.tasks {
		accountGaps.Add(accountGaps,
			new(big.Int).Sub(task.Last.Big(), task.Next.Big()))
	}
	...
	var (
		...
		accounts = fmt.Sprintf("%v@%v", log.FormatLogfmtUint64(s.accountSynced),
			s.accountBytes.TerminalString())
		storage = fmt.Sprintf("%v@%v", log.FormatLogfmtUint64(s.storageSynced),
			s.storageBytes.TerminalString())
		bytecode = fmt.Sprintf("%v@%v",
			log.FormatLogfmtUint64(s.bytecodeSynced),
			s.bytecodeBytes.TerminalString())
	)
	log.Info("Syncing: state download in progress", "synced", progress, "state",
		synced,
		"accounts", accounts, "slots", storage, "codes", bytecode, "eta",
		common.PrettyDuration(estTime-elapsed))
}
...
func (s *Syncer) reportHealProgress(force bool) {
	...
	// Create a mega progress report
	var (
		trienode = fmt.Sprintf("%v@%v",
			log.FormatLogfmtUint64(s.trienodeHealSynced),
			s.trienodeHealBytes.TerminalString())
		bytecode = fmt.Sprintf("%v@%v",
			log.FormatLogfmtUint64(s.bytecodeHealSynced),
			s.bytecodeHealBytes.TerminalString())
		accounts = fmt.Sprintf("%v@%v", log.FormatLogfmtUint64(s.accountHealed),
			s.accountHealedBytes.TerminalString())
		storage = fmt.Sprintf("%v@%v", log.FormatLogfmtUint64(s.storageHealed),
			s.storageHealedBytes.TerminalString())
	)
	log.Info("Syncing: state healing in progress", "accounts", accounts,
		"slots", storage,
		"codes", bytecode, "nodes", trienode, "pending",
		s.healer.scheduler.Pending())
}
...
func sortByAccountPath(paths []string, hashes []common.Hash) ([]string,
	[]common.Hash, []trie.SyncPath, []TrieNodePathSet) {

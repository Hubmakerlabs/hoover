...
func (cs *chainSyncer) loop() {
	...
	for {
		...
		case err := <-cs.doneCh:
			...
			// potentially flaky.
			if errors.Is(err,
				downloader.ErrMergeTransition) && time.Since(cs.warned) > 10*time.Second {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		if stored := rawdb.ReadFastTxLookupLimit(h.database); stored == nil {
			...
			log.Warn("Update txLookup limit", "provided", limit, "updated",
				*stored)
		}
	}
	// Run the sync cycle, and disable snap sync if we're past the pivot block
	err := h.downloader.LegacySync(op.peer.ID(), op.head, op.td,
		h.chain.Config().TerminalTotalDifficulty, op.mode)
	if err != nil {
		return err
	}
	...
	if head.Number.Uint64() > 0 {
		...
		if block := h.chain.GetBlock(head.Hash(),
			head.Number.Uint64()); block != nil {

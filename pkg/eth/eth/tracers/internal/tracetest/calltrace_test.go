...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")),
			func(t *testing.T) {
				t.Parallel()
				...
				if blob, err := os.ReadFile(filepath.Join("testdata", dirPath,
					file.Name())); err != nil {
				...
				var (
					signer = types.MakeSigner(test.Genesis.Config,
						new(big.Int).SetUint64(uint64(test.Context.Number)),
					...
					triedb, _, statedb = tests.MakePreState(rawdb.NewMemoryDatabase(),
						test.Genesis.Alloc, false, rawdb.HashScheme)
				)
				triedb.Close()

				tracer, err := tracers.DefaultDirectory.New(tracerName,
					new(tracers.Context), test.TracerConfig)
				if err != nil {
					t.Fatalf("failed to create call tracer: %v", err)
				}
				evm := vm.NewEVM(context, txContext, statedb,
					test.Genesis.Config, vm.Config{Tracer: tracer})
				msg, err := core.TransactionToMessage(tx, signer, nil)
				if err != nil {
					t.Fatalf("failed to prepare transaction for tracing: %v",
						err)
				}
				vmRet, err := core.ApplyMessage(evm, msg,
				...
				if uint64(topCall.GasUsed) != vmRet.UsedGas {
					t.Fatalf("top call has invalid gasUsed. have: %d want: %d",
						topCall.GasUsed, vmRet.UsedGas)
				}
			})
	}
}
...
func BenchmarkTracers(b *testing.B) {
	...
	for _, file := range files {
		...
		file := file // capture range variable
		b.Run(camel(strings.TrimSuffix(file.Name(), ".json")),
			func(b *testing.B) {
				blob, err := os.ReadFile(filepath.Join("testdata",
					"call_tracer", file.Name()))
				if err != nil {
...
func benchTracer(tracerName string, test *callTracerTest, b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		evm := vm.NewEVM(context, txContext, statedb, test.Genesis.Config,
...
func TestInternals(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			code: []byte{
				byte(vm.PUSH1), 0x0, byte(vm.DUP1), byte(vm.DUP1),
				byte(vm.DUP1), // in and outs zero
				byte(vm.DUP1), byte(vm.PUSH1), 0xff,
				byte(vm.GAS), // value=0,address=0xff, gas=GAS
				byte(vm.CALL),
			},
		...
		},
		{
			...
			tracer: mkTracer("callTracer",
				json.RawMessage(`{ "withLog": true }`)),
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			evm := vm.NewEVM(context, txContext, statedb,
				params.MainnetChainConfig, vm.Config{Tracer: tc.tracer})
			msg := &core.Message{
				To:                &to,
				From:              origin,
				Value:             big.NewInt(0),
			...
			if _, err := st.TransitionDb(); err != nil {
				t.Fatalf("test %v: failed to execute transaction: %v", tc.name,
					err)
			}
			// Retrieve the trace result and compare against the expected
			res, err := tc.tracer.GetResult()
			if err != nil {
				t.Fatalf("test %v: failed to retrieve trace result: %v",
					tc.name, err)
			}

...
func flatCallTracerTestRunner(tracerName string, filename string,
	dirPath string, t testing.TB) error {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")),
			func(t *testing.T) {
				...
				err := flatCallTracerTestRunner(tracerName, file.Name(),
					dirPath, t)
				if err != nil {
					t.Fatal(err)
				}
...
func BenchmarkFlatCallTracer(b *testing.B) {
	...
	for _, file := range files {
		...
		b.Run(camel(strings.TrimSuffix(filename, ".json")), func(b *testing.B) {
			for n := 0; n < b.N; n++ {
				err := flatCallTracerTestRunner("flatCallTracer", filename,
					"call_tracer_flat", b)
				if err != nil {
					b.Fatal(err)
				}

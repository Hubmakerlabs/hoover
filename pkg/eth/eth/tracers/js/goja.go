...
import (
	...
	jsassets "github.com/Hubmakerlabs/hoover/pkg/eth/eth/tracers/js/internal/tracers"
)
...
func init() {
	...
	type ctorFn = func(*tracers.Context, json.RawMessage) (tracers.Tracer,
		error)
	lookup := func(code string) ctorFn {
		return func(ctx *tracers.Context, cfg json.RawMessage) (tracers.Tracer,
			error) {
...
type fromBufFn = func(vm *goja.Runtime, buf goja.Value,
	allowString bool) ([]byte, error)

func toBuf(vm *goja.Runtime, bufType goja.Value, val []byte) (goja.Value,
	error) {
...
func fromBuf(vm *goja.Runtime, bufType goja.Value, buf goja.Value,
	allowString bool) ([]byte, error) {
	obj := buf.ToObject(vm)
...
func newJsTracer(code string, ctx *tracers.Context,
	cfg json.RawMessage) (tracers.Tracer, error) {
	vm := goja.New()
...
func (t *jsTracer) CaptureStart(env *vm.EVM, from common.Address,
	to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	...
	rules := env.ChainConfig().Rules(env.Context.BlockNumber,
		env.Context.Random != nil, env.Context.Time)
...
func (t *jsTracer) CaptureFault(pc uint64, op vm.OpCode, gas, cost uint64,
...
func (t *jsTracer) CaptureEnter(typ vm.OpCode, from common.Address,
...
func (t *jsTracer) setBuiltinFunctions() {
	...
	vm.Set("toContract2",
		func(from goja.Value, salt string, initcode goja.Value) goja.Value {
			a, err := t.fromBuf(vm, from, true)
			if err != nil {
				vm.Interrupt(err)
				return nil
			...
			b := crypto.CreateAddress2(addr, common.HexToHash(salt),
				codeHash).Bytes()
			res, err := t.toBuf(vm, b)
			if err != nil {
				vm.Interrupt(err)
	...
	vm.Set("slice", func(slice goja.Value, start, end int) goja.Value {
		...
		if start < 0 || start > end || end > len(b) {
			vm.Interrupt(fmt.Sprintf("Tracer accessed out of bound memory: available %d, offset %d, size %d",
				len(b), start, end-start))
			return nil
		}
...
func (t *jsTracer) setTypeConverters() error {
	...
	fromBufWrapper := func(vm *goja.Runtime, buf goja.Value,
		allowString bool) ([]byte, error) {
...
func (mo *memoryObj) slice(begin, end int64) ([]byte, error) {
	...
	if end < begin || begin < 0 {
		return nil, fmt.Errorf("tracer accessed out of bound memory: offset %d, end %d",
			begin, end)
	}
...
func (mo *memoryObj) getUint(addr int64) (*big.Int, error) {
	if mo.memory.Len() < int(addr)+32 || addr < 0 {
		return nil, fmt.Errorf("tracer accessed out of bound memory: available %d, offset %d, size %d",
			mo.memory.Len(), addr, 32)
	}
...
func (s *stackObj) peek(idx int) (*big.Int, error) {
	if len(s.stack.Data()) <= idx || idx < 0 {
		return nil, fmt.Errorf("tracer accessed out of bound stack: size %d, index %d",
			len(s.stack.Data()), idx)
	}
...
func (do *dbObj) GetState(addrSlice goja.Value,
	hashSlice goja.Value) goja.Value {
	a, err := do.fromBuf(do.vm, addrSlice, false)
	if err != nil {
		do.vm.Interrupt(err)
		return nil

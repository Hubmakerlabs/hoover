...
func init() {
	tracers.DefaultDirectory.Register("flatCallTracer", newFlatCallTracer,
		false)
}
...
func newFlatCallTracer(ctx *tracers.Context,
...
func (t *flatCallTracer) CaptureStart(env *vm.EVM, from common.Address,
...
func (t *flatCallTracer) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64,
...
func (t *flatCallTracer) CaptureFault(pc uint64, op vm.OpCode, gas, cost uint64,
	scope *vm.ScopeContext, depth int, err error) {
	t.tracer.CaptureFault(pc, op, gas, cost, scope, depth, err)
}

// CaptureEnter is called when EVM enters a new scope (via call, create or selfdestruct).
func (t *flatCallTracer) CaptureEnter(typ vm.OpCode, from common.Address,
...
func (t *flatCallTracer) GetResult() (json.RawMessage, error) {
	...
	flat, err := flatFromNested(&t.tracer.callstack[0], []int{},
		t.config.ConvertParityErrors, t.ctx)
	if err != nil {
		return nil, err
	}
...
func flatFromNested(input *callFrame, traceAddress []int, convertErrs bool,
	ctx *tracers.Context) (output []flatCallFrame, err error) {
	...
	if len(input.Calls) > 0 {
		for i, childCall := range input.Calls {
			...
			flat, err := flatFromNested(&childCallCopy, childAddr, convertErrs,
				ctx)
			if err != nil {
				return nil, err
			}

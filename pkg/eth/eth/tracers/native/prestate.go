...
func init() {
	tracers.DefaultDirectory.Register("prestateTracer", newPrestateTracer,
		false)
}
...
func newPrestateTracer(ctx *tracers.Context,
...
func (t *prestateTracer) CaptureStart(env *vm.EVM, from common.Address,
	...
	consumedGas := new(big.Int).Mul(gasPrice,
...
func (t *prestateTracer) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64,
	...
	case stackLen >= 4 && op == vm.CREATE2:
		...
		init, err := tracers.GetMemoryCopyPadded(scope.Memory,
			int64(offset.Uint64()), int64(size.Uint64()))
		if err != nil {
			log.Warn("failed to copy CREATE2 input", "err", err, "tracer",
				"prestateTracer", "offset", offset, "size", size)
			return
		}

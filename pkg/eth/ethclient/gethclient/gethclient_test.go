...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	filterSystem := filters.NewFilterSystem(ethservice.APIBackend,
		filters.Config{})
...
func generateTestChain() (*core.Genesis, []*types.Block) {
	genesis := &core.Genesis{
		...
		Alloc: core.GenesisAlloc{
			testAddr: {Balance: testBalance,
				Storage: map[common.Hash]common.Hash{testSlot: testValue}},
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, ethash.NewFaker(), 1,
		generate)
...
func TestGethClient(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			func(t *testing.T) {
				testSubscribeFullPendingTransactions(t,
					client)
			},
		}, {
			"TestCallContract",
			...
			func(t *testing.T) {
				testCallContractWithBlockOverrides(t,
					client)
			},
		},
...
func testGetProof(t *testing.T, client *rpc.Client, addr common.Address) {
	...
	result, err := ec.GetProof(context.Background(), addr,
		[]string{testSlot.String()}, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	// test nonce
	if nonce, _ := ethcl.NonceAt(context.Background(), addr,
		nil); result.Nonce != nonce {
		t.Fatalf("invalid nonce, want: %v got: %v", nonce, result.Nonce)
	}
	// test balance
	if balance, _ := ethcl.BalanceAt(context.Background(), addr,
		nil); result.Balance.Cmp(balance) != 0 {
		t.Fatalf("invalid balance, want: %v got: %v", balance, result.Balance)
	}
	// test storage
	if len(result.StorageProof) != 1 {
		t.Fatalf("invalid storage proof, want 1 proof, got %v proof(s)",
			len(result.StorageProof))
	}
	for _, proof := range result.StorageProof {
		if proof.Key != testSlot.String() {
			t.Fatalf("invalid storage proof key, want: %q, got: %q",
				testSlot.String(), proof.Key)
		}
		slotValue, _ := ethcl.StorageAt(context.Background(), addr,
			common.HexToHash(proof.Key), nil)
		if have, want := common.BigToHash(proof.Value), common.BytesToHash(slotValue); have != want {
			t.Fatalf("addr %x, invalid storage proof value: have: %v, want: %v",
				addr, have, want)
		}
	}
	// test code
...
func testGetProofCanonicalizeKeys(t *testing.T, client *rpc.Client) {
	...
	result, err := ec.GetProof(context.Background(), testAddr,
		[]string{"0x0dEadbeef"}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if result.StorageProof[0].Key != "0xdeadbeef" {
		t.Fatalf("wrong storage key encoding in proof: %q",
			result.StorageProof[0].Key)
	}
	if result, err = ec.GetProof(context.Background(), testAddr,
		[]string{"0x000deadbeef"}, nil); err != nil {
		t.Fatal(err)
	}
	if result.StorageProof[0].Key != "0xdeadbeef" {
		t.Fatalf("wrong storage key encoding in proof: %q",
...
func testSubscribePendingTransactions(t *testing.T, client *rpc.Client) {
	...
	// Create transaction
	tx := types.NewTransaction(0, common.Address{1}, big.NewInt(1), 22000,
		big.NewInt(1), nil)
	...
	if hash != signedTx.Hash() {
		t.Fatalf("Invalid tx hash received, got %v, want %v", hash,
			signedTx.Hash())
	}
}
...
func testSubscribeFullPendingTransactions(t *testing.T, client *rpc.Client) {
	...
	// Create transaction
	tx := types.NewTransaction(1, common.Address{1}, big.NewInt(1), 22000,
		big.NewInt(1), nil)
	signer := types.LatestSignerForChainID(chainID)
	signature, err := crypto.Sign(signer.Hash(tx).Bytes(), testKey)
	if err != nil {
	...
	if tx.Hash() != signedTx.Hash() {
		t.Fatalf("Invalid tx hash received, got %v, want %v", tx.Hash(),
			signedTx.Hash())
	}
}
...
func testCallContract(t *testing.T, client *rpc.Client) {
	...
	if _, err := ec.CallContract(context.Background(), msg, big.NewInt(0),
		nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func testCallContractWithBlockOverrides(t *testing.T, client *rpc.Client) {
	...
	res, err := ec.CallContract(context.Background(), msg, big.NewInt(0),
		&mapAcc)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !bytes.Equal(res,
		common.FromHex("0x0000000000000000000000000000000000000000")) {
	...
	res, err = ec.CallContractWithBlockOverrides(context.Background(), msg,
		big.NewInt(0), &mapAcc, bo)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !bytes.Equal(res,

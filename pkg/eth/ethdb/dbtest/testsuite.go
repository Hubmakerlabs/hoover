...
func TestDatabaseSuite(t *testing.T, New func() ethdb.KeyValueStore) {
	t.Run("Iterator", func(t *testing.T) {
		tests := []struct {
			...
			{
				map[string]string{"k1": "v1", "k5": "v5", "k2": "v2",
					"k4": "v4", "k3": "v3"},
				"", "",
			...
			{
				map[string]string{
					"ka1": "va1", "ka5": "va5", "ka2": "va2", "ka4": "va4",
					"ka3": "va3",
					"kb1": "vb1", "kb5": "vb5", "kb2": "vb2", "kb4": "vb4",
					"kb3": "vb3",
				},
				"ka", "",
		...
		for i, tt := range tests {
			...
			for key, val := range tt.content {
				if err := db.Put([]byte(key), []byte(val)); err != nil {
					t.Fatalf("test %d: failed to insert item %s:%s into database: %v",
						i, key, val, err)
				}
			}
	...
	t.Run("IteratorWith", func(t *testing.T) {
		...
		keys := []string{"1", "2", "3", "4", "6", "10", "11", "12", "20", "21",
			"22"}
...

		{
			...
			got, want := iterateKeys(it), []string{"2", "20", "21", "22", "3",
				"4", "6"}
			if err := it.Error(); err != nil {
				t.Fatal(err)
			}
	...
	t.Run("Batch", func(t *testing.T) {
		...
		{
			it := db.NewIterator(nil, nil)
			if got, want := iterateKeys(it), []string{"1", "2", "3",
		...
		{
			it := db.NewIterator(nil, nil)
			if got, want := iterateKeys(it), []string{"2", "3", "4", "5",

...
func New(file string, cache int, handles int, namespace string, readonly bool,
	ephemeral bool) (*Database, error) {
	...
	logger.Info("Allocated cache and file handles", "cache",
		common.StorageSize(cache*1024*1024), "handles", handles)
	...
	// Disable seek compaction explicitly. Check https://github.com/Hubmakerlabs/hoover/pkg/eth/pull/20130
	// for more details.
	...
	db.compReadMeter = metrics.NewRegisteredMeter(namespace+"compact/input",
		nil)
	db.compWriteMeter = metrics.NewRegisteredMeter(namespace+"compact/output",
		nil)
	...
	db.writeDelayMeter = metrics.NewRegisteredMeter(namespace+"compact/writedelay/duration",
		nil)
	db.writeDelayNMeter = metrics.NewRegisteredMeter(namespace+"compact/writedelay/counter",
		nil)
	db.memCompGauge = metrics.NewRegisteredGauge(namespace+"compact/memory",
		nil)
	db.level0CompGauge = metrics.NewRegisteredGauge(namespace+"compact/level0",
		nil)
	db.nonlevel0CompGauge = metrics.NewRegisteredGauge(namespace+"compact/nonlevel0",
		nil)
	db.seekCompGauge = metrics.NewRegisteredGauge(namespace+"compact/seek", nil)
	db.manualMemAllocGauge = metrics.NewRegisteredGauge(namespace+"memory/manualalloc",
		nil)
	...
	for i := 1; errc == nil; i++ {
		...
		for i, level := range stats.Levels {
			...
			if i >= len(d.levelsGauge) {
				d.levelsGauge = append(d.levelsGauge,

...
import (
	...
	ethproto "github.com/Hubmakerlabs/hoover/pkg/eth/eth/protocols/eth"
...
func parseEthstatsURL(url string) (parts []string, err error) {
	err = fmt.Errorf("invalid netstats url: \"%s\", should be nodename:secret@host:port",
		url)
...
func New(node *node.Node, backend backend, engine consensus.Engine,
	url string) error {
...
func (s *Service) loop(chainHeadCh chan core.ChainHeadEvent,
	txEventCh chan core.NewTxsEvent) {
			...
			for err == nil {
				...
				case head := <-headCh:
					...
					if err = s.reportPending(conn); err != nil {
						log.Warn("Post-block transaction stats report failed",
							"err", err)
					}
				case <-txCh:
					if err = s.reportPending(conn); err != nil {
...
func (s *Service) readLoop(conn *connWrapper) {
	...
	for {
		...
		var ping string
		if err := json.Unmarshal(blob,
			&ping); err == nil && strings.HasPrefix(ping, "primus::ping::") {
			if err := conn.WriteJSON(strings.ReplaceAll(ping, "ping",
				"pong")); err != nil {
		...
		if !ok {
			log.Warn("Invalid stats server message type", "type",
				msg["emit"][0])
			return
		}
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				log.Warn("Invalid stats history block list", "list",
					request["list"])
				return
			}
			...
			for i, num := range list {
				n, ok := num.(float64)
				if !ok {
					log.Warn("Invalid stats history block number", "number",
						num)
					return
				}
				numbers[i] = uint64(n)
...
func (s *Service) login(conn *connWrapper) error {
	...
	for _, proto := range s.server.Protocols {
		protocols = append(protocols,
			fmt.Sprintf("%s/%d", proto.Name, proto.Version))
	}
	var network string
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ethproto.NodeInfo).Network)
	} else {
		network = fmt.Sprintf("%d",
			infos.Protocols["les"].(*les.NodeInfo).Network)
	}
	auth := &authMsg{
		ID: s.node,
		Info: nodeInfo{
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to ethstats", "number", details.Number, "hash",
		details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if ok {
		if block == nil {
			...
			block, _ = fullBackend.BlockByNumber(context.Background(),
				rpc.BlockNumber(head.Number.Uint64()))
		}
		header = block.Header()
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	for i, number := range indexes {
		...
		if ok {
			block, _ = fullBackend.BlockByNumber(context.Background(),
				rpc.BlockNumber(number)) // TODO ignore error here ?
		} else {
			if header, _ := s.backend.HeaderByNumber(context.Background(),
				rpc.BlockNumber(number)); header != nil {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to ethstats", "first",
			history[0].Number, "last", history[len(history)-1].Number)
	} else {

...
func (a *Account) getState(ctx context.Context) (*state.StateDB, error) {
	state, _, err := a.r.backend.StateAndHeaderByNumberOrHash(ctx,
		a.blockNrOrHash)
	return state, err
}
...
func (a *Account) TransactionCount(ctx context.Context) (hexutil.Uint64,
	error) {
...
func (a *Account) Storage(ctx context.Context,
	args struct{ Slot common.Hash }) (common.Hash, error) {
	state, err := a.getState(ctx)
	if err != nil {
	...
	case types.DynamicFeeTxType:
		if block != nil {
			if baseFee, _ := block.BaseFeePerGas(ctx); baseFee != nil {
				...
				return (hexutil.Big)(*math.BigMin(new(big.Int).Add(tx.GasTipCap(),
					baseFee.ToInt()), tx.GasFeeCap()))
			}
		}
...
func (t *Transaction) EffectiveGasPrice(ctx context.Context) (*hexutil.Big,
	error) {
	tx, block := t.resolve(ctx)
	if tx == nil {
		return nil, nil
	...
	return (*hexutil.Big)(math.BigMin(new(big.Int).Add(tx.GasTipCap(),
		header.BaseFee), tx.GasFeeCap())), nil
}
...
func (t *Transaction) CumulativeGasUsed(ctx context.Context) (*hexutil.Uint64,
	error) {
	receipt, err := t.getReceipt(ctx)
	if err != nil || receipt == nil {
		return nil, err
...
func (t *Transaction) BlobGasUsed(ctx context.Context) (*hexutil.Uint64,
	error) {
	tx, _ := t.resolve(ctx)
	if tx == nil {
		return nil, nil
...
func (t *Transaction) CreatedContract(ctx context.Context,
	args BlockNumberArgs) (*Account, error) {
	receipt, err := t.getReceipt(ctx)
...
func (t *Transaction) getLogs(ctx context.Context, hash common.Hash) (*[]*Log,
	error) {
	var (
	...
	ix := sort.Search(len(logs),
		func(i int) bool { return uint64(logs[i].TxIndex) >= t.index })
...
func (b *Block) Miner(ctx context.Context, args BlockNumberArgs) (*Account,
	error) {
	header, err := b.resolveHeader(ctx)
	if err != nil {
		return nil, err
...
func (b *Block) TransactionAt(ctx context.Context,
	args struct{ Index Long }) (*Transaction, error) {
	block, err := b.resolve(ctx)
	if err != nil || block == nil {
		return nil, err
	}
...
func (b *Block) OmmerAt(ctx context.Context, args struct{ Index Long }) (*Block,
	error) {
	block, err := b.resolve(ctx)
	if err != nil || block == nil {
		return nil, err
...
// `Log` objects.
func runFilter(ctx context.Context, r *Resolver,
	filter *filters.Filter) ([]*Log, error) {
	logs, err := filter.Logs(ctx)
	if err != nil || logs == nil {
		return nil, err
	}
...
func (b *Block) Logs(ctx context.Context,
	args struct{ Filter BlockFilterCriteria }) ([]*Log, error) {
...
func (b *Block) Call(ctx context.Context, args struct {
	...
	result, err := ethapi.DoCall(ctx, b.r.backend, args.Data, *b.numberOrHash,
		nil, nil, b.r.backend.RPCEVMTimeout(), b.r.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
...
func (b *Block) EstimateGas(ctx context.Context, args struct {
	...
	return ethapi.DoEstimateGas(ctx, b.r.backend, args.Data, *b.numberOrHash,
		nil, b.r.backend.RPCGasCap())
}
...
func (p *Pending) TransactionCount(ctx context.Context) (hexutil.Uint64,
	error) {
...
func (p *Pending) Call(ctx context.Context, args struct {
	...
	pendingBlockNr := rpc.BlockNumberOrHashWithNumber(rpc.PendingBlockNumber)
	result, err := ethapi.DoCall(ctx, p.r.backend, args.Data, pendingBlockNr,
		nil, nil, p.r.backend.RPCEVMTimeout(), p.r.backend.RPCGasCap())
	if err != nil {
		return nil, err
	}
	status := hexutil.Uint64(1)
...
func (p *Pending) EstimateGas(ctx context.Context, args struct {
	...
	return ethapi.DoEstimateGas(ctx, p.r.backend, args.Data, latestBlockNr, nil,
		p.r.backend.RPCGasCap())
}
...
func (r *Resolver) Transaction(ctx context.Context,
	args struct{ Hash common.Hash }) *Transaction {
	tx := &Transaction{
		r:    r,
		hash: args.Hash,
	}
...
func (r *Resolver) SendRawTransaction(ctx context.Context,
	args struct{ Data hexutil.Bytes }) (common.Hash, error) {
...
func (r *Resolver) Logs(ctx context.Context,
	args struct{ Filter FilterCriteria }) ([]*Log, error) {
...
func (r *Resolver) MaxPriorityFeePerGas(ctx context.Context) (hexutil.Big,
	error) {
	tipcap, err := r.backend.SuggestGasTipCap(ctx)
	if err != nil {
		return hexutil.Big{}, err

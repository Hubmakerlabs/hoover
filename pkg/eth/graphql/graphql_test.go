...
func TestBuildSchema(t *testing.T) {
	...
	if _, err := newHandler(stack, nil, nil, []string{},
		[]string{}); err != nil {
...
func TestGraphQLBlockSerialization(t *testing.T) {
	...
	newGQLService(t, stack, false, genesis, 10,
	...
	for i, tt := range []struct {
		...
		resp, err := http.Post(fmt.Sprintf("%s/graphql", stack.HTTPEndpoint()),
		...
		if tt.code != resp.StatusCode {
			t.Errorf("testcase %d %s,\nwrong statuscode, have: %v, want: %v", i,
				tt.body, resp.StatusCode, tt.code)
		}
		if ctype := resp.Header.Get("Content-Type"); ctype != "application/json" {
			t.Errorf("testcase %d \nwrong Content-Type, have: %v, want: %v", i,
...
func TestGraphQLBlockSerializationEIP2718(t *testing.T) {
	...
	genesis := &core.Genesis{
		...
		Alloc: core.GenesisAlloc{
			...
			dad: {
				Code: []byte{byte(vm.PC), byte(vm.PC), byte(vm.SLOAD),
					byte(vm.SLOAD)},
				Nonce:   0,
...
func TestGraphQLConcurrentResolvers(t *testing.T) {
	...
	handler, chain := newGQLService(t, stack, false, genesis, 1,
		func(i int, gen *core.BlockGen) {
			tx, _ = types.SignNewTx(key, signer,
				&types.LegacyTx{To: &dad, Gas: 100000,
					GasPrice: big.NewInt(params.InitialBaseFee)})
			gen.AddTx(tx)
			tx, _ = types.SignNewTx(key, signer,
				&types.LegacyTx{To: &dad, Nonce: 1, Gas: 100000,
					GasPrice: big.NewInt(params.InitialBaseFee)})
			gen.AddTx(tx)
			tx, _ = types.SignNewTx(key, signer,
				&types.LegacyTx{To: &dad, Nonce: 2, Gas: 100000,
					GasPrice: big.NewInt(params.InitialBaseFee)})
			gen.AddTx(tx)
		})
	// start node
	...
	for i, tt := range []struct {
		...
		// Multiple txes race to get/set the block hash.
		{
			...
			want: fmt.Sprintf(`{"block":{"transactions":[{"logs":[{"account":{"address":"%s"}},{"account":{"address":"%s"}}]},{"logs":[{"account":{"address":"%s"}},{"account":{"address":"%s"}}]},{"logs":[{"account":{"address":"%s"}},{"account":{"address":"%s"}}]}]}}`,
		...
		// Multiple fields of block race to resolve header and body.
		{
			...
			want: fmt.Sprintf(`{"block":{"number":"0x1","hash":"%s","gasLimit":"0xaf79e0","ommerCount":"0x0","transactionCount":"0x3","totalDifficulty":"0x200000"}}`,
				chain[len(chain)-1].Hash()),
		},
		// Multiple fields of a block race to resolve the header and body.
		{
			body: fmt.Sprintf(`{ transaction(hash: "%s") { block { number hash gasLimit ommerCount transactionCount } } }`,
				tx.Hash()),
			want: fmt.Sprintf(`{"transaction":{"block":{"number":"0x1","hash":"%s","gasLimit":"0xaf79e0","ommerCount":"0x0","transactionCount":"0x3"}}}`,
				chain[len(chain)-1].Hash()),
		},
		// Account fields race the resolve the state object.
		{
			body: fmt.Sprintf(`{ block { account(address: "%s") { balance transactionCount code } } }`,
				dadStr),
		...
		res := handler.Schema.Exec(context.Background(), tt.body, "",
			map[string]interface{}{})
		if res.Errors != nil {
			t.Fatalf("failed to execute query for testcase #%d: %v", i,
				res.Errors)
		}
		have, err := json.Marshal(res.Data)
		if err != nil {
			t.Fatalf("failed to encode graphql response for testcase #%d: %s",
				i, err)
		}
...
func TestWithdrawals(t *testing.T) {
	...
	handler, _ := newGQLService(t, stack, true, genesis, 1,
		func(i int, gen *core.BlockGen) {
			tx, _ := types.SignNewTx(key, signer,
				&types.LegacyTx{To: &common.Address{}, Gas: 100000,
					GasPrice: big.NewInt(params.InitialBaseFee)})
			gen.AddTx(tx)
...
func newGQLService(t *testing.T, stack *node.Node, shanghai bool,
	gspec *core.Genesis, genBlocks int,
	genfunc func(i int, gen *core.BlockGen)) (*handler, []*types.Block) {
	...
	// Set up handler
	filterSystem := filters.NewFilterSystem(ethBackend.APIBackend,
		filters.Config{})
	handler, err := newHandler(stack, ethBackend.APIBackend, filterSystem,
		[]string{}, []string{})
	if err != nil {

...
func Render(templateFile, outputFile string, outputPerm os.FileMode,
	x interface{}) {
...
func RenderString(templateContent, outputFile string, outputPerm os.FileMode,
	x interface{}) {
	tpl := template.Must(template.New("").Parse(templateContent))
	render(tpl, outputFile, outputPerm, x)
}

func render(tpl *template.Template, outputFile string, outputPerm os.FileMode,
	x interface{}) {
	if err := os.MkdirAll(filepath.Dir(outputFile), 0755); err != nil {
		log.Fatal(err)
	}
	out, err := os.OpenFile(outputFile, os.O_CREATE|os.O_WRONLY|os.O_EXCL,
		outputPerm)
	if err != nil {
		log.Fatal(err)
	}
...
func FindMainPackages(dir string) []string {
	...
	for _, cmd := range cmds {
		...
		pkgs, err := parser.ParseDir(token.NewFileSet(), pkgdir, nil,

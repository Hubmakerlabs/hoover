...
func Setup(ctx *cli.Context) error {
	...
	default:
		...
		return fmt.Errorf("unknown log format: %v",
			ctx.String(logFormatFlag.Name))
	}
	var (
	...
	if rotation {
		...
		if len(logFile) > 0 {
			...
			context = append(context, "location",
		...
		ostream = log.StreamHandler(io.MultiWriter(output, lumberWriter),
			logfmt)
	} else if logFile != "" {
...
func StartPProf(address string, withMetrics bool) {
	...
	log.Info("Starting pprof server", "addr",
		fmt.Sprintf("http://%s/debug/pprof", address))
	go func() {

...
func (args *TransactionArgs) setDefaults(ctx context.Context, b Backend) error {
	...
	if args.Data != nil && args.Input != nil && !bytes.Equal(*args.Data,
		*args.Input) {
	...
	if args.ChainID != nil {
		if have := (*big.Int)(args.ChainID); have.Cmp(want) != 0 {
			return fmt.Errorf("chainId does not match node's (have=%v, want=%v)",
				have, want)
		}
	} else {
...
func (args *TransactionArgs) setFeeDefaults(ctx context.Context,
	b Backend) error {
	...
	// https://github.com/Hubmakerlabs/hoover/pkg/eth/pull/23274 for more information.
	...
	if (args.GasPrice != nil && !eip1559ParamsSet) || (args.GasPrice == nil && eip1559ParamsSet) {
		...
		if args.GasPrice == nil && args.MaxFeePerGas.ToInt().Cmp(args.MaxPriorityFeePerGas.ToInt()) < 0 {
			return fmt.Errorf("maxFeePerGas (%v) < maxPriorityFeePerGas (%v)",
...
func (args *TransactionArgs) setLondonFeeDefaults(ctx context.Context,
	head *types.Header, b Backend) error {
...
// live transaction.
func (args *TransactionArgs) ToMessage(globalGasCap uint64,
	...
	if globalGasCap != 0 && globalGasCap < gas {
		log.Warn("Caller gas above allowance, capping", "requested", gas, "cap",
			globalGasCap)
		gas = globalGasCap
	}
	var (
	...
	if baseFee == nil {
		...
		if args.GasPrice != nil {
			...
			if gasFeeCap.BitLen() > 0 || gasTipCap.BitLen() > 0 {
				gasPrice = math.BigMin(new(big.Int).Add(gasTipCap, baseFee),
					gasFeeCap)
			}
		}
	}

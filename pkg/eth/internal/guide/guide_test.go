...
func TestAccountManagement(t *testing.T) {
	...
	ks := keystore.NewKeyStore(filepath.Join(workdir, "keystore"),
	...
	if err := ks.Update(newAcc, "Creation password",
		"Update password"); err != nil {
	...
	if _, err := ks.Import(jsonAcc, "Export password",
		"Import password"); err != nil {
	...
	tx := types.NewTransaction(0, common.Address{}, big.NewInt(0), 0,
		big.NewInt(0), nil)
	chain := big.NewInt(1)

	// Sign a transaction with a single authorization
	if _, err := ks.SignTxWithPassphrase(signer, "Signer password", tx,
		chain); err != nil {
	...
	if err := ks.TimedUnlock(signer, "Signer password",
		time.Second); err != nil {

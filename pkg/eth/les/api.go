...
func (api *LightServerAPI) ClientInfo(nodes []string) map[enode.ID]map[string]interface{} {
	...
	for _, id := range ids {
		if peer := api.server.peers.peer(id); peer != nil {
			...
			api.server.clientPool.BalanceOperation(id, "",
				func(balance vfs.AtomicBalanceOperator) {
...
// assigned to it.
func (api *LightServerAPI) PriorityClientInfo(start, stop enode.ID,
	maxCount int) map[enode.ID]map[string]interface{} {
	res := make(map[enode.ID]map[string]interface{})
...
func (api *LightServerAPI) clientInfo(peer *clientPeer,
	balance vfs.ReadOnlyBalance) map[string]interface{} {
...
func (api *LightServerAPI) setParams(params map[string]interface{},
	client *clientPeer,
	posFactors, negFactors *vfs.PriceFactors) (updateFactors bool, err error) {
...
func (api *LightServerAPI) SetClientParams(nodes []string,
...
func (api *LightServerAPI) SetDefaultParams(params map[string]interface{}) error {
	update, err := api.setParams(params, nil, &api.defaultPosFactors,
		&api.defaultNegFactors)
	if update {
		api.server.clientPool.SetDefaultFactors(api.defaultPosFactors,
			api.defaultNegFactors)
	}
	return err
}
...
func (api *LightServerAPI) AddBalance(node string,
	amount int64) (balance [2]uint64, err error) {
	var id enode.ID
	if id, err = parseNode(node); err != nil {
		return
	}
	api.server.clientPool.BalanceOperation(id, "",
		func(nb vfs.AtomicBalanceOperator) {
...
func (api *LightServerAPI) Benchmark(setups []map[string]interface{},
	passCount, length int) ([]map[string]interface{}, error) {
	...
	rs := api.server.handler.runBenchmark(benchmarks, passCount,
		time.Millisecond*time.Duration(length))

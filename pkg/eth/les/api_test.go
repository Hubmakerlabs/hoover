...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/simulations"
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/simulations/adapters"
...
var (
	...
	simAdapter = flag.String("adapter", "exec",
		"type of simulation: sim|socket|exec|docker")
)

func TestMain(m *testing.M) {
	flag.Parse()
	log.PrintOrigins(true)
	log.Root().SetHandler(log.LvlFilterHandler(log.Lvl(*loglevel),
...
func testCapacityAPI(t *testing.T, clientCount int) {
	...
	for !testSim(t, 1, clientCount, []string{testServerDataDir}, nil,
		func(ctx context.Context, net *simulations.Network,
			servers []*simulations.Node, clients []*simulations.Node) bool {
			if len(servers) != 1 {
			...
			if minCap > reqMinCap {
				t.Fatalf("Minimum client capacity (%d) bigger than required minimum for this test (%d)",
					minCap, reqMinCap)
			}
			...
			for i, client := range clients {
				...
				if i != freeIdx {
					setCapacity(ctx, t, serverRpcClient, client.ID(),
						testCap/uint64(len(clients)))
				}
			...
			for c := 0; c < 5; c++ {
				setCapacity(ctx, t, serverRpcClient, clients[freeIdx].ID(),
					minCap)
				freeIdx = rand.Intn(len(clients))
				var sum float64
				for i := range clients {
				...
				for i, client := range clients {
					...
					if i != freeIdx && capacity < getCapacity(ctx, t,
						serverRpcClient, client.ID()) {
						setCapacity(ctx, t, serverRpcClient, client.ID(),
							capacity)
					}
				}
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64,
	common.Hash) {
	res := make(map[string]interface{})
	if err := client.CallContext(ctx, &res, "eth_getBlockByNumber", "latest",
		false); err != nil {
...
func freezeClient(ctx context.Context, t *testing.T, server *rpc.Client,
	clientID enode.ID) {
	if err := server.CallContext(ctx, nil, "debug_freezeClient",
		clientID); err != nil {
		t.Fatalf("Failed to freeze client: %v", err)
	}
}

func setCapacity(ctx context.Context, t *testing.T, server *rpc.Client,
	clientID enode.ID, cap uint64) {
	params := make(map[string]interface{})
	params["capacity"] = cap
	if err := server.CallContext(ctx, nil, "les_setClientParams",
		[]enode.ID{clientID}, []string{}, params); err != nil {
		t.Fatalf("Failed to set client capacity: %v", err)
	}
}

func getCapacity(ctx context.Context, t *testing.T, server *rpc.Client,
	clientID enode.ID) uint64 {
	var res map[enode.ID]map[string]interface{}
	if err := server.CallContext(ctx, &res, "les_clientInfo",
		[]enode.ID{clientID}, []string{}); err != nil {
...
func getCapacityInfo(ctx context.Context, t *testing.T,
	server *rpc.Client) (minCap, totalCap uint64) {
...
func NewAdapter(adapterType string,
	services adapters.LifecycleConstructors) (adapter adapters.NodeAdapter,
...
func testSim(t *testing.T, serverCount, clientCount int,
	serverDir, clientDir []string,
	test func(ctx context.Context, net *simulations.Network,
		servers []*simulations.Node, clients []*simulations.Node) bool) bool {
...
func newLesClientService(ctx *adapters.ServiceContext,
	stack *node.Node) (node.Lifecycle, error) {
	config := ethconfig.Defaults
	config.SyncMode = downloader.LightSync
	return New(stack, &config)
}

func newLesServerService(ctx *adapters.ServiceContext,
	stack *node.Node) (node.Lifecycle, error) {
	config := ethconfig.Defaults

...
func (b *benchmarkBlockHeaders) init(h *serverHandler, count int) error {
	...
	if b.byHash {
		...
		for i := range b.hashes {
			b.hashes[i] = rawdb.ReadCanonicalHash(h.chainDb,
				uint64(b.offset+rand.Int63n(b.randMax)))
		}
	}
	return nil
}
...
func (b *benchmarkBlockHeaders) request(peer *serverPeer, index int) error {
	if b.byHash {
		return peer.requestHeadersByHash(0, b.hashes[index], b.amount, b.skip,
			b.reverse)
	}
	return peer.requestHeadersByNumber(0,
		uint64(b.offset+rand.Int63n(b.randMax)), b.amount, b.skip, b.reverse)
}
...
func (b *benchmarkProofsOrCode) request(peer *serverPeer, index int) error {
	...
	crand.Read(key)
	if b.code {
		return peer.requestCode(0,
			[]CodeReq{{BHash: b.headHash, AccountAddress: key}})
	}
	...
	if b.bloom {
		...
		for i := range reqs {
			...
			binary.BigEndian.PutUint64(key[2:],
				uint64(rand.Int63n(int64(b.sectionCount))))
			reqs[i] = HelperTrieReq{Type: htBloomBits,
				TrieIdx: b.sectionCount - 1, Key: key}
		}
	} else {
		for i := range reqs {
			key := make([]byte, 8)
			binary.BigEndian.PutUint64(key[:],
				uint64(rand.Int63n(int64(b.headNum))))
			reqs[i] = HelperTrieReq{Type: htCanonical,
				TrieIdx: b.sectionCount - 1, Key: key, AuxReq: htAuxHeader}
		}
	}
...
func (b *benchmarkTxSend) init(h *serverHandler, count int) error {
	...
	for i := range b.txs {
		...
		crand.Read(data)
		tx, err := types.SignTx(types.NewTransaction(0, addr, new(big.Int), 0,
			new(big.Int), data), signer, key)
		if err != nil {
			panic(err)
		}
		b.txs[i] = tx
...
// number of passes
func (h *serverHandler) runBenchmark(benchmarks []requestBenchmark,
	passCount int, targetTime time.Duration) []*benchmarkSetup {
...
func (h *serverHandler) measure(setup *benchmarkSetup, count int) error {
	...
	peer1 := newServerPeer(lpv2, NetworkId, false,
		p2p.NewPeer(id, "client", nil), clientMeteredPipe)
	peer2 := newClientPeer(lpv2, NetworkId, p2p.NewPeer(id, "server", nil),
		serverMeteredPipe)
	...
	peer2.fcClient = flowcontrol.NewClientNode(h.server.fcManager,
		peer2.fcParams)

...
func (h *clientHandler) runPeer(version uint, p *p2p.Peer,
	rw p2p.MsgReadWriter) error {
	peer := newServerPeer(int(version), h.backend.config.NetworkId, false, p,
...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	forkid := forkid.NewID(h.backend.blockchain.Config(),
		h.backend.BlockChain().Genesis(),
		h.backend.blockchain.CurrentHeader().Number.Uint64(),
		h.backend.blockchain.CurrentHeader().Time)
	if err := p.Handshake(h.backend.blockchain.Genesis().Hash(), forkid,
...
func (h *clientHandler) handleMsg(p *serverPeer) error {
	...
	case msg.Code == AnnounceMsg:
		...
		if req.Hash != (common.Hash{}) {
			...
			p.Log().Trace("Announce message content", "number", req.Number,
				"hash", req.Hash, "td", req.Td, "reorg", req.ReorgDepth)

...
func newCostTracker(db ethdb.Database, config *ethconfig.Config) (*costTracker,
	uint64) {
...
func (ct *costTracker) makeCostList(globalFactor float64) RequestCostList {
	...
	for code, data := range reqAvgTimeCost {
		baseCost := maxCost(data.baseCost, reqMaxInSize[code].baseCost,
			reqMaxOutSize[code].baseCost)
		reqCost := maxCost(data.reqCost, reqMaxInSize[code].reqCost,
			reqMaxOutSize[code].reqCost)
		if ct.minBufLimit != 0 {
			...
			case r := <-ct.reqInfoCh:
				...
				if gfCorr != 0 {
					...
					if time.Duration(now-lastUpdate) > time.Second {
						...
						log.Debug("global cost factor updated", "factor",
							factor)
					}
				}
...
func (ct *costTracker) realCost(servingTime uint64,
	inSize, outSize uint32) uint64 {
...
func (ct *costTracker) printStats() {
	...
	for code, arr := range ct.stats {
		log.Info("Request cost statistics", "code", code, "1/16", arr[0].Load(),
			"1/8", arr[1].Load(), "1/4", arr[2].Load(), "1/2", arr[3].Load(),
			"1", arr[4].Load(), "2", arr[5].Load(), "4", arr[6].Load(), "8",
			arr[7].Load(), "16", arr[8].Load(), ">16", arr[9].Load())
	}
}

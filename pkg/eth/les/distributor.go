...
func newRequestDistributor(peers *serverPeerSet,
	clock mclock.Clock) *requestDistributor {
		...
		for peer := range d.peers {
			if _, ok := checkedPeers[peer]; !ok && peer.canQueue() && req.canSend(peer) {
				...
				if wait == 0 {
					...
					sel.Update(selectPeerItem{peer: peer, req: req,
						weight: uint64(bufRemain*1000000) + 1})
				} else {
					if bestWait == 0 || wait < bestWait {
						bestWait = wait
					}

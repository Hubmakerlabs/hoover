...
func (node *ClientNode) recalcBV(now mclock.AbsTime) {
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("updated  bv=%d  MRR=%d  BufLimit=%d", node.bufValue,
				node.params.MinRecharge, node.params.BufLimit))
	}
	node.lastTime = now
}
...
func (node *ClientNode) UpdateParams(params ServerParams) {
	...
	if params.MinRecharge >= node.params.MinRecharge {
		...
		node.updateSchedule = append(node.updateSchedule,
			scheduledUpdate{time: now.Add(DecParamDelay), params: params})
	}
}
...
func (node *ClientNode) AcceptRequest(reqID, index, maxCost uint64) (accepted bool,
	bufShort uint64, priority int64) {
	node.lock.Lock()
	defer node.lock.Unlock()
	...
	if int64(maxCost) > node.bufValue {
		if node.log != nil {
			node.log.add(now,
				fmt.Sprintf("rejected  reqID=%d  bv=%d  maxCost=%d", reqID,
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("accepted  reqID=%d  bv=%d  maxCost=%d  sumCost=%d",
				reqID, node.bufValue, maxCost, node.sumCost))
	}
...
func (node *ClientNode) RequestProcessed(reqID, index, maxCost, realCost uint64) uint64 {
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("processed  reqID=%d  bv=%d  maxCost=%d  realCost=%d  sumCost=%d  oldSumCost=%d  reportedBV=%d",
				reqID, node.bufValue, maxCost, realCost, node.sumCost,
				node.accepted[index], bv))
	}
...
func (node *ServerNode) recalcBLE(now mclock.AbsTime) {
	...
	node.lastTime = now
	if node.log != nil {
		node.log.add(now, fmt.Sprintf("updated  bufEst=%d  MRR=%d  BufLimit=%d",
			node.bufEstimate, node.params.MinRecharge, node.params.BufLimit))
	}
}
...
func (node *ServerNode) CanSend(maxCost uint64) (time.Duration, float64) {
	...
	if node.bufEstimate >= maxCost {
		...
		if node.log != nil {
			node.log.add(now,
				fmt.Sprintf("canSend  bufEst=%d  maxCost=%d  true  relBuf=%f",
					node.bufEstimate, maxCost, relBuf))
		}
		return 0, relBuf
	}
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("canSend  bufEst=%d  maxCost=%d  false  timeLeft=%v",
				node.bufEstimate, maxCost, timeLeft))
	}
	return timeLeft, 0
}
...
func (node *ServerNode) QueuedRequest(reqID, maxCost uint64) {
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("queued  reqID=%d  bufEst=%d  maxCost=%d  sumCost=%d",
				reqID, node.bufEstimate, maxCost, node.sumCost))
	}
}
...
func (node *ServerNode) ReceivedReply(reqID, bv uint64) {
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("received  reqID=%d  bufEst=%d  reportedBv=%d  sumCost=%d  oldSumCost=%d",
				reqID, node.bufEstimate, bv, node.sumCost, sc))
	}
}
...
func (node *ServerNode) ResumeFreeze(bv uint64) {
	...
	if node.log != nil {
		node.log.add(now,
			fmt.Sprintf("unfreeze  bv=%d  sumCost=%d", bv, node.sumCost))
	}
}

...
// any moment.
func NewClientManager(curve PieceWiseLinear,
	clock mclock.Clock) *ClientManager {
	cm := &ClientManager{
		clock: clock,
		rcQueue: prque.New[int64, *ClientNode](func(a *ClientNode,
			i int) {
			a.queueIndex = i
		}),
...
func (cm *ClientManager) accepted(node *ClientNode, maxCost uint64,
	now mclock.AbsTime) (priority int64) {
	cm.lock.Lock()
	defer cm.lock.Unlock()
...
func (cm *ClientManager) processed(node *ClientNode, maxCost, realCost uint64,
	now mclock.AbsTime) {
	if realCost > maxCost {
		realCost = maxCost
	}
...
func (cm *ClientManager) updateBuffer(node *ClientNode, add int64,
	now mclock.AbsTime) {
	cm.lock.Lock()
	defer cm.lock.Unlock()
	...
	if node.corrBufValue > node.bufValue {
		if node.log != nil {
			node.log.add(now,
				fmt.Sprintf("corrected  bv=%d  oldBv=%d", node.corrBufValue,
					node.bufValue))
		}
...
func (cm *ClientManager) updateParams(node *ClientNode, params ServerParams,
	now mclock.AbsTime) {
	cm.lock.Lock()
	defer cm.lock.Unlock()
...
func (cm *ClientManager) addToQueue(node *ClientNode) {
	if cm.priorityOffset-node.rcFullIntValue < -0x4000000000000000 {
		...
		newRcQueue := prque.New[int64, *ClientNode](func(a *ClientNode,
			i int) {
			a.queueIndex = i
		})
...
func (cm *ClientManager) updateNodeRc(node *ClientNode, bvc int64,
	params *ServerParams, now mclock.AbsTime) {
	cm.updateRecharge(now)
	wasFull := true

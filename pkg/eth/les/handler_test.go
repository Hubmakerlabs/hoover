...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/light"
...
func expectResponse(r p2p.MsgReader, msgcode, reqID, bv uint64,
...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		// A single random block should be retrievable by hash and number too
		{
			...
			&GetBlockHeadersData{Origin: hashOrNumber{Number: limit / 2},
				Amount: 1},
			[]common.Hash{bc.GetBlockByNumber(limit / 2).Hash()},
		},
		...
		// The chain endpoints should be retrievable
		{
			...
			&GetBlockHeadersData{Origin: hashOrNumber{Number: bc.CurrentBlock().Number.Uint64()},
				Amount: 1},
		...
		{
			&GetBlockHeadersData{Origin: hashOrNumber{Number: bc.CurrentBlock().Number.Uint64() - 4},
				Skip: 3, Amount: 3},
			[]common.Hash{
			...
			&GetBlockHeadersData{Origin: hashOrNumber{Number: 4}, Skip: 3,
				Amount: 3, Reverse: true},
			[]common.Hash{
		...
		// Check that requesting more than available is handled gracefully, even if mid skip
		{
			...
			&GetBlockHeadersData{Origin: hashOrNumber{Number: 4}, Skip: 2,
				Amount: 3, Reverse: true},
			[]common.Hash{
				bc.GetBlockByNumber(4).Hash(),
		...
		{
			&GetBlockHeadersData{Origin: hashOrNumber{Hash: unknown},
				Amount: 1},
			[]common.Hash{},
		}, {
			&GetBlockHeadersData{Origin: hashOrNumber{Number: bc.CurrentBlock().Number.Uint64() + 1},
				Amount: 1},
			[]common.Hash{},
		},
	}
	...
	for i, tt := range tests {
		...
		if err := expectResponse(rawPeer.app, BlockHeadersMsg, reqID,
			testBufLimit, headers); err != nil {
...
func testGetBlockBodies(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		{10, nil, nil,
			10}, // Multiple random blocks should be retrievable
		{limit, nil, nil,
			limit}, // The maximum possible blocks should be retrievable
		// {limit + 1, nil, nil, limit},                                  // No more than the possible block count should be returned
		{0, []common.Hash{bc.Genesis().Hash()}, []bool{true},
			1}, // The genesis block should be retrievable
		{0, []common.Hash{bc.CurrentBlock().Hash()}, []bool{true},
			1}, // The chains head block should be retrievable
		{0, []common.Hash{{}}, []bool{false},
			0}, // A non existent block should not be returned
	...
	for i, tt := range tests {
		...
		if err := expectResponse(rawPeer.app, BlockBodiesMsg, reqID,
			testBufLimit, bodies); err != nil {
...
func testGetCode(t *testing.T, protocol int) {
	...
	if err := expectResponse(rawPeer.app, CodeMsg, 42, testBufLimit,
		codes); err != nil {
...
func testGetStaleCode(t *testing.T, protocol int) {
	...
	check(bc.CurrentHeader().Number.Uint64(),
		[][]byte{testContractCodeDeployed}) // Fresh contract
}
...
func testGetReceipt(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().Number.Uint64(); i++ {
		...
		receipts = append(receipts,
			rawdb.ReadReceipts(server.db, block.Hash(), block.NumberU64(),
				block.Time(), bc.Config()))
	}
	// Send the hash request and verify the response
	sendRequest(rawPeer.app, GetReceiptsMsg, 42, hashes)
	if err := expectResponse(rawPeer.app, ReceiptsMsg, 42, testBufLimit,
		receipts); err != nil {
...
func testGetProofs(t *testing.T, protocol int) {
	...
	for i := uint64(0); i <= bc.CurrentBlock().Number.Uint64(); i++ {
		header := bc.GetHeaderByNumber(i)
		trie, _ := trie.New(trie.StateTrieID(header.Root),
			server.backend.Blockchain().TrieDB())
	...
	if err := expectResponse(rawPeer.app, ProofsV2Msg, 42, testBufLimit,
		proofsV2.List()); err != nil {
...
func testGetStaleProof(t *testing.T, protocol int) {
	...
	check := func(number uint64, wantOK bool) {
		...
		if wantOK {
			proofsV2 := trienode.NewProofSet()
			t, _ := trie.New(trie.StateTrieID(header.Root),
				server.backend.Blockchain().TrieDB())
...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	root := light.GetChtRoot(server.db, 0,
		bc.GetHeaderByNumber(config.ChtSize-1).Hash())
	trie, _ := trie.New(trie.TrieID(root),
		trie.NewDatabase(rawdb.NewTable(server.db,
			string(rawdb.ChtTablePrefix)), trie.HashDefaults))
	...
	if err := expectResponse(rawPeer.app, HelperTrieProofsMsg, 42, testBufLimit,
		proofsV2); err != nil {
		t.Errorf("proofs mismatch: %v", err)
	}
}
...
func testGetBloombitsProofs(t *testing.T, protocol int) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		root := light.GetBloomTrieRoot(server.db, 0,
			bc.GetHeaderByNumber(config.BloomTrieSize-1).Hash())
		trie, _ := trie.New(trie.TrieID(root),
			trie.NewDatabase(rawdb.NewTable(server.db,
				string(rawdb.BloomTrieTablePrefix)), trie.HashDefaults))
		trie.Prove(key, &proofs.Proofs)

		// Send the proof request and verify the response
		sendRequest(rawPeer.app, GetHelperTrieProofsMsg, 42, requests)
		if err := expectResponse(rawPeer.app, HelperTrieProofsMsg, 42,
			testBufLimit, proofs); err != nil {
...
func testTransactionStatus(t *testing.T, protocol int) {
	...
	test := func(tx *types.Transaction, send bool, expStatus light.TxStatus) {
		...
		if send {
			...
			sendRequest(rawPeer.app, GetTxStatusMsg, reqID,
				[]common.Hash{tx.Hash()})
		}
		if err := expectResponse(rawPeer.app, TxStatusMsg, reqID, testBufLimit,
			[]light.TxStatus{expStatus}); err != nil {
	...
	tx0, _ := types.SignTx(types.NewTransaction(0, userAddr1, big.NewInt(10000),
		params.TxGas, nil, nil), signer, bankKey)
	test(tx0, true, light.TxStatus{Status: txpool.TxStatusUnknown,
		Error: "transaction underpriced: tip needed 1, tip permitted 0"})

	tx1, _ := types.SignTx(types.NewTransaction(0, userAddr1, big.NewInt(10000),
		params.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	test(tx1, false,
		light.TxStatus{Status: txpool.TxStatusUnknown}) // query before sending, should be unknown
	test(tx1, true,
		light.TxStatus{Status: txpool.TxStatusPending}) // send valid processable tx, should return pending
	test(tx1, true,
		light.TxStatus{Status: txpool.TxStatusPending}) // adding it again should not return an error

	tx2, _ := types.SignTx(types.NewTransaction(1, userAddr1, big.NewInt(10000),
		params.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	tx3, _ := types.SignTx(types.NewTransaction(2, userAddr1, big.NewInt(10000),
		params.TxGas, big.NewInt(100000000000), nil), signer, bankKey)
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig,
		chain.GetBlockByNumber(0), ethash.NewFaker(), server.db, 1,
		func(i int, block *core.BlockGen) {
			block.AddTx(tx1)
			block.AddTx(tx2)
		})
	...
	test(tx1, false, light.TxStatus{Status: txpool.TxStatusIncluded,
		Lookup: &rawdb.LegacyTxLookupEntry{BlockHash: block1hash, BlockIndex: 1,
			Index: 0}})

	test(tx2, false, light.TxStatus{Status: txpool.TxStatusIncluded,
		Lookup: &rawdb.LegacyTxLookupEntry{BlockHash: block1hash, BlockIndex: 1,
			Index: 1}})

	// create a reorg that rolls them back
	gchain, _ = core.GenerateChain(params.TestChainConfig,
		chain.GetBlockByNumber(0), ethash.NewFaker(), server.db, 2,
		func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
...
func testStopResume(t *testing.T, protocol int) {
	...
	req := func() {
		reqID++
		sendRequest(rawPeer.app, GetBlockHeadersMsg, reqID,
			&GetBlockHeadersData{Origin: hashOrNumber{Hash: header.Hash()},
				Amount: 1})
	}
	for i := 1; i <= 5; i++ {
		...
		for expBuf >= testCost {
			...
			expBuf -= testCost
			if err := expectResponse(rawPeer.app, BlockHeadersMsg, reqID,
				expBuf, []*types.Header{header}); err != nil {

...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/les/utils"
	vfc "github.com/Hubmakerlabs/hoover/pkg/eth/les/vflux/client"
...
func (p *peerCommons) sendReceiveHandshake(sendList keyValueList) (keyValueList,
	error) {
	var (
	...
	go func() {
		...
		if msg.Code != StatusMsg {
			errc <- errResp(ErrNoStatusMsg, "first msg has code %x (!= %x)",
				msg.Code, StatusMsg)
			return
		}
		if msg.Size > ProtocolMaxMsgSize {
			errc <- errResp(ErrMsgTooLarge, "%v > %v", msg.Size,
				ProtocolMaxMsgSize)
			return
		}
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64,
	genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter,
	sendCallback func(*keyValueList),
	recvCallback func(keyValueMap) error) error {
	p.lock.Lock()
	defer p.lock.Unlock()
	...
	if rGenesis != genesis {
		return errResp(ErrGenesisBlockMismatch, "%x (!= %x)", rGenesis[:8],
			genesis[:8])
	}
	...
	if int(rVersion) != p.version {
		return errResp(ErrProtocolVersionMismatch, "%d (!= %d)", rVersion,
			p.version)
	}
...
type serverPeer struct {
	...
	// Test callback hooks
	hasBlockHook func(common.Hash, uint64,
		bool) bool // Used to determine whether the server has the specified block.
}

func newServerPeer(version int, network uint64, trusted bool, p *p2p.Peer,
	rw p2p.MsgReadWriter) *serverPeer {
	return &serverPeer{
...
// and content.
func sendRequest(w p2p.MsgWriter, msgcode, reqID uint64,
...
func (p *serverPeer) sendRequest(msgcode, reqID uint64, data interface{},
	amount int) error {
...
func (p *serverPeer) requestHeadersByHash(reqID uint64, origin common.Hash,
	amount int, skip int, reverse bool) error {
	p.Log().Debug("Fetching batch of headers", "count", amount, "fromhash",
		origin, "skip", skip, "reverse", reverse)
	return p.sendRequest(GetBlockHeadersMsg, reqID,
		&GetBlockHeadersData{Origin: hashOrNumber{Hash: origin},
...
func (p *serverPeer) requestHeadersByNumber(reqID, origin uint64, amount int,
	skip int, reverse bool) error {
	p.Log().Debug("Fetching batch of headers", "count", amount, "fromnum",
		origin, "skip", skip, "reverse", reverse)
	return p.sendRequest(GetBlockHeadersMsg, reqID,
		&GetBlockHeadersData{Origin: hashOrNumber{Number: origin},
			Amount: uint64(amount), Skip: uint64(skip), Reverse: reverse},
		amount)
}
...
func (p *serverPeer) requestHelperTrieProofs(reqID uint64,
	reqs []HelperTrieReq) error {
	p.Log().Debug("Fetching batch of HelperTrie proofs", "count", len(reqs))
	return p.sendRequest(GetHelperTrieProofsMsg, reqID, reqs, len(reqs))
}

// requestTxStatus fetches a batch of transaction status records from a remote node.
func (p *serverPeer) requestTxStatus(reqID uint64,
	txHashes []common.Hash) error {
...
func (p *serverPeer) sendTxs(reqID uint64, amount int, txs rlp.RawValue) error {
	p.Log().Debug("Sending batch of transactions", "amount", amount, "size",
		len(txs))
...
func (p *serverPeer) HasBlock(hash common.Hash, number uint64,
	hasState bool) bool {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *serverPeer) updateFlowControl(update keyValueMap) {
	...
	if update.get("flowControl/BL",
		&params.BufLimit) == nil && update.get("flowControl/MRR",
		&params.MinRecharge) == nil {
...
func (p *serverPeer) Handshake(genesis common.Hash, forkid forkid.ID,
	...
	return p.handshake(common.Big0, common.Hash{}, 0, genesis, forkid,
		forkFilter, func(lists *keyValueList) {
			...
			*lists = (*lists).add("announceType", p.announceType)
			...
			if err := recv.get("flowControl/BL",
				&sParams.BufLimit); err != nil {
				return err
			}
			if err := recv.get("flowControl/MRR",
				&sParams.MinRecharge); err != nil {
				return err
			}
			var MRC RequestCostList
			...
			if !p.onlyAnnounce {
				for msgCode := range reqAvgTimeCost {
					if p.fcCosts[msgCode] == nil {
						return errResp(ErrUselessPeer,
							"peer does not support message %d", msgCode)
					}
				}
			}
			return nil
...
func (p *serverPeer) answeredRequest(id uint64) {
	...
	if m.rest == -1 || e.amount <= 1 {
		reqCount = 1
		vtReqs[0] = vfc.ServedRequest{ReqType: uint32(m.first),
			Amount: e.amount}
	} else {
		reqCount = 2
		vtReqs[0] = vfc.ServedRequest{ReqType: uint32(m.first), Amount: 1}
		vtReqs[1] = vfc.ServedRequest{ReqType: uint32(m.rest),
			Amount: e.amount - 1}
	}
...
func newClientPeer(version int, network uint64, p *p2p.Peer,
	rw p2p.MsgReadWriter) *clientPeer {
	return &clientPeer{
		peerCommons: peerCommons{
			Peer:      p,
			rw:        rw,
...
func (p *clientPeer) replyBlockHeaders(reqID uint64,
	headers []*types.Header) *reply {
...
func (p *clientPeer) replyBlockBodiesRLP(reqID uint64,
	bodies []rlp.RawValue) *reply {
...
func (p *clientPeer) replyReceiptsRLP(reqID uint64,
	receipts []rlp.RawValue) *reply {
	data, _ := rlp.EncodeToBytes(receipts)
	return &reply{p.rw, ReceiptsMsg, reqID, data}
}

// replyProofsV2 creates a reply with a batch of merkle proofs, corresponding to the ones requested.
func (p *clientPeer) replyProofsV2(reqID uint64,
	proofs trienode.ProofList) *reply {
...
func (p *clientPeer) replyHelperTrieProofs(reqID uint64,
	resp HelperTrieResps) *reply {
...
func (p *clientPeer) replyTxStatus(reqID uint64,
	stats []light.TxStatus) *reply {
...
func (p *clientPeer) UpdateCapacity(newCap uint64, requested bool) {
	...
	if newCap != p.fcParams.MinRecharge {
		p.fcParams = flowcontrol.ServerParams{MinRecharge: newCap,
			BufLimit: newCap * bufLimitRatio}
...
func (p *clientPeer) sendLastAnnounce() {
	...
	if p.headInfo.Td == nil || p.lastAnnounce.Td.Cmp(p.headInfo.Td) > 0 {
		if !p.queueSend(func() { p.sendAnnounce(p.lastAnnounce) }) {
			p.Log().Debug("Dropped announcement because queue is full",
				"number", p.lastAnnounce.Number, "hash", p.lastAnnounce.Hash)
		} else {
			p.Log().Debug("Sent announcement", "number", p.lastAnnounce.Number,
				"hash", p.lastAnnounce.Hash)
		}
		p.headInfo = blockInfo{Hash: p.lastAnnounce.Hash,
			Number: p.lastAnnounce.Number, Td: p.lastAnnounce.Td}
	}
}

// Handshake executes the les protocol handshake, negotiating version number,
// network IDs, difficulties, head and genesis blocks.
func (p *clientPeer) Handshake(td *big.Int, head common.Hash, headNum uint64,
	genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter,
	server *LesServer) error {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter,
		func(lists *keyValueList) {
			...
			*lists = (*lists).add("flowControl/MRR",
				server.defParams.MinRecharge)

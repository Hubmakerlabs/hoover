...
func TestPeerSubscription(t *testing.T) {
	...
	var id enode.ID
	rand.Read(id[:])
	peer := newServerPeer(2, NetworkId, false, p2p.NewPeer(id, "name", nil),
		nil)
	peers.register(peer)
...
func TestHandshake(t *testing.T) {
	...
	peer2 := newServerPeer(2, NetworkId, true, p2p.NewPeer(id, "name", nil),
		app)
	...
	var (
		...
		forkID1        = forkid.NewID(chain1.Config(), chain1.Genesis(),
			chain1.CurrentHeader().Number.Uint64(), chain1.CurrentHeader().Time)
		forkID2 = forkid.NewID(chain2.Config(), chain2.Genesis(),
			chain2.CurrentHeader().Number.Uint64(), chain2.CurrentHeader().Time)
		filter1, filter2 = forkid.NewFilter(chain1), forkid.NewFilter(chain2)
	)

	go func() {
		errCh1 <- peer1.handshake(td, head, headNum, genesis, forkID1, filter1,
			func(list *keyValueList) {
				var announceType uint64 = announceTypeSigned
				*list = (*list).add("announceType", announceType)
			}, nil)
	}()
	go func() {
		errCh2 <- peer2.handshake(td, head, headNum, genesis, forkID2, filter2,
			nil, func(recv keyValueMap) error {
				var reqType uint64

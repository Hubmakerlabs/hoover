...
func newRetrieveManager(peers *serverPeerSet, dist *requestDistributor,
	srto func() time.Duration) *retrieveManager {
...
// cancelled.
func (rm *retrieveManager) retrieve(ctx context.Context, reqID uint64,
	req *distReq, val validatorFunc, shutdown chan struct{}) error {
...
func (rm *retrieveManager) sendReq(reqID uint64, req *distReq,
	val validatorFunc) *sentReq {
	r := &sentReq{
		rm:       rm,
		req:      req,
		id:       reqID,
	...
	req.request = func(p distPeer) func() {
		...
		r.lock.Lock()
		r.sentTo[p] = sentReqToPeer{delivered: false, frozen: false,
...
func (r *sentReq) deliver(peer distPeer, msg *Msg) error {
	...
	r.sentTo[peer] = sentReqToPeer{delivered: true, frozen: false,
		event: s.event}
	if valid {
...
func (r *sentReq) frozen(peer distPeer) {
	...
	if ok && !s.delivered && !s.frozen {
		r.sentTo[peer] = sentReqToPeer{delivered: false, frozen: true,
			event: s.event}

...
func newServerHandler(server *LesServer, blockchain *core.BlockChain,
	chainDb ethdb.Database, txpool *txpool.TxPool,
	synced func() bool) *serverHandler {
...
func (h *serverHandler) runPeer(version uint, p *p2p.Peer,
	rw p2p.MsgReadWriter) error {
	peer := newClientPeer(int(version), h.server.config.NetworkId, p,
		newMeteredMsgWriter(rw, int(version)))
	defer peer.close()
	h.wg.Add(1)
	defer h.wg.Done()
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	var (
		...
		forkID = forkid.NewID(h.blockchain.Config(), h.blockchain.Genesis(),
			number, head.Time)
	)
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(),
		forkID, h.forkFilter, h.server); err != nil {
...
func (h *serverHandler) beforeHandle(p *clientPeer, reqID, responseCount uint64,
	msg p2p.Msg, reqCnt uint64, maxCount uint64) (*servingTask, uint64) {
	...
	accepted, bufShort, priority := p.fcClient.AcceptRequest(reqID,
		responseCount, maxCost)
	if !accepted {
		p.freeze()
		p.Log().Error("Request came too early", "remaining",
			common.PrettyDuration(time.Duration(bufShort*1000000/p.fcParams.MinRecharge)))
		p.fcClient.OneTimeCost(inSizeCost)
		return nil, 0
	}
...
func (h *serverHandler) afterHandle(p *clientPeer, reqID, responseCount uint64,
	msg p2p.Msg, maxCost uint64, reqCnt uint64, task *servingTask,
	reply *reply) {
	if reply != nil {
		task.done()
	}
	...
	if h.server.costTracker.testing {
		...
		realCost = h.server.costTracker.realCost(task.servingTime, msg.Size,
			replySize)
		if realCost > maxCost {
			realCost = maxCost
		}
	...
	if reply != nil {
		...
		h.server.costTracker.updateStats(msg.Code, reqCnt, task.servingTime,
			realCost)
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light Ethereum message arrived", "code", msg.Code, "bytes",
		msg.Size)
	...
	task, maxCost := h.beforeHandle(p, reqID, responseCount, msg, reqCnt,
		req.MaxCount)
	if task == nil {
		return nil
	}
	...
	go func() {
		...
		h.afterHandle(p, reqID, responseCount, msg, maxCost, reqCnt, task,
			reply)
...
func getAccount(triedb *trie.Database, root common.Hash,
	addr common.Address) (types.StateAccount, error) {
	...
	if acc == nil {
		return types.StateAccount{}, fmt.Errorf("account %#x is not present",
			addr)
	}
	return *acc, nil
}
...
func (h *serverHandler) GetHelperTrie(typ uint, index uint64) *trie.Trie {
	...
	switch typ {
	case htCanonical:
		sectionHead := rawdb.ReadCanonicalHash(h.chainDb,
			(index+1)*h.server.iConfig.ChtSize-1)
		root, prefix = light.GetChtRoot(h.chainDb, index,
			sectionHead), string(rawdb.ChtTablePrefix)
	case htBloomBits:
		sectionHead := rawdb.ReadCanonicalHash(h.chainDb,
			(index+1)*h.server.iConfig.BloomTrieSize-1)
		root, prefix = light.GetBloomTrieRoot(h.chainDb, index,
			sectionHead), string(rawdb.BloomTrieTablePrefix)
	}
	if root == (common.Hash{}) {
		return nil
	}
	triedb := trie.NewDatabase(rawdb.NewTable(h.chainDb, prefix),
		trie.HashDefaults)
	...
	for {
		...
		case ev := <-headCh:
			...
			if lastHead != nil {
				...
				if ancestor := rawdb.FindCommonAncestor(h.chainDb, header,
					lastHead); ancestor != nil {
					reorg = lastHead.Number.Uint64() - ancestor.Number.Uint64()
				}
			}
			lastHead, lastTd = header, td
			log.Debug("Announcing block to peers", "number", number, "hash",
				hash, "td", td, "reorg", reorg)
			h.server.peers.broadcast(announceData{Hash: hash, Number: number,
				Td: td, ReorgDepth: reorg})
		case <-h.closeCh:
			return
		}
	}

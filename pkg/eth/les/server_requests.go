...
type RequestType struct {
	...
	Handle                                                           func(msg Decoder) (serve serveRequestFn,
		reqID, amount uint64, err error)
}
...
type serveRequestFn func(backend serverBackend, peer *clientPeer,
	waitOrStop func() bool) *reply
...
func handleGetBlockHeaders(msg Decoder) (serveRequestFn, uint64, uint64,
	error) {
	var r GetBlockHeadersPacket
	if err := msg.Decode(&r); err != nil {
		return nil, 0, 0, err
	}
	return func(backend serverBackend, p *clientPeer,
		waitOrStop func() bool) *reply {
		...
		for !unknown && len(headers) < int(r.Query.Amount) && bytes < softResponseLimit {
			...
			case hashMode && r.Query.Reverse:
				...
				if ancestor == 0 {
					...
					r.Query.Origin.Hash, r.Query.Origin.Number = bc.GetAncestor(r.Query.Origin.Hash,
						r.Query.Origin.Number, ancestor, &maxNonCanonical)
			...
			case hashMode && !r.Query.Reverse:
				...
				if next <= current {
					...
					p.Log().Warn("GetBlockHeaders skip overflow attack",
						"current", current, "skip", r.Query.Skip, "next", next,
						"attacker", string(infos))
					unknown = true
				} else {
					if header := bc.GetHeaderByNumber(next); header != nil {
						nextHash := header.Hash()
						expOldHash, _ := bc.GetAncestor(nextHash, next,
							r.Query.Skip+1, &maxNonCanonical)
...
func handleGetCode(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer,
		...
		for i, request := range r.Reqs {
			...
			if header == nil {
				p.Log().Warn("Failed to retrieve associate header for code",
					"hash", request.BHash)
				p.bumpInvalid()
				continue
			}
			...
			if !backend.ArchiveMode() && header.Number.Uint64()+core.TriesInMemory <= local {
				p.Log().Debug("Reject stale code request", "number",
					header.Number.Uint64(), "head", local)
				p.bumpInvalid()
				continue
			}
			...
			if err != nil {
				p.Log().Warn("Failed to retrieve account for code", "block",
					header.Number, "hash", header.Hash(), "account", address,
					"err", err)
				p.bumpInvalid()
				continue
			}
			code, err := bc.StateCache().ContractCode(address,
				common.BytesToHash(account.CodeHash))
			if err != nil {
				p.Log().Warn("Failed to retrieve account code", "block",
					header.Number, "hash", header.Hash(), "account", address,
					"codehash", common.BytesToHash(account.CodeHash), "err",
					err)
				continue
			}
		...
		for i, request := range r.Reqs {
			...
			if request.BHash != lastBHash {
				...
				if header = bc.GetHeaderByHash(request.BHash); header == nil {
					p.Log().Warn("Failed to retrieve header for proof", "hash",
						request.BHash)
					p.bumpInvalid()
					continue
				}
				...
				local := bc.CurrentHeader().Number.Uint64()
				if !backend.ArchiveMode() && header.Number.Uint64()+core.TriesInMemory <= local {
					p.Log().Debug("Reject stale trie request", "number",
						header.Number.Uint64(), "head", local)
					p.bumpInvalid()
					continue
				}
			...
			case 0:
				...
				if trie == nil || err != nil {
					p.Log().Warn("Failed to open storage trie for proof",
						"block", header.Number, "hash", header.Hash(), "root",
						root, "err", err)
					continue
				}
			default:
				...
				if err != nil {
					p.Log().Warn("Failed to retrieve account for proof",
				...
				if trie == nil || err != nil {
					p.Log().Warn("Failed to open storage trie for proof",
						"block", header.Number, "hash", header.Hash(),
						"account", address, "root", account.Root, "err", err)
					continue
				}
			}
			// Prove the user's request from the account or storage trie
			if err := trie.Prove(request.Key, nodes); err != nil {
				p.Log().Warn("Failed to prove state request", "block",
					header.Number, "hash", header.Hash(), "err", err)
				continue
			}
...
func handleGetHelperTrieProofs(msg Decoder) (serveRequestFn, uint64, uint64,
	error) {
	...
	return func(backend serverBackend, p *clientPeer,
		...
		return p.replyHelperTrieProofs(r.ReqID,
			HelperTrieResps{Proofs: nodes.List(), AuxData: auxData})
	}, r.ReqID, uint64(len(r.Reqs)), nil
}
	...
	return func(backend serverBackend, p *clientPeer,
		...
		for i, tx := range r.Txs {
			...
			if stats[i].Status == txpool.TxStatusUnknown {
				if errs := backend.TxPool().Add([]*types.Transaction{tx}, false,
					backend.AddTxsSync()); errs[0] != nil {

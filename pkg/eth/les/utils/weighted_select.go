...
func NewWeightedRandomSelect(wfn WeightFn) *WeightedRandomSelect {
	return &WeightedRandomSelect{root: &wrsNode{maxItems: wrsBranches},
		idx: make(map[WrsItem]int), wfn: wfn}
}
...
func (w *WeightedRandomSelect) setWeight(item WrsItem, weight uint64) {
	if weight > math.MaxInt64-w.root.sumCost {
		...
		w.setWeight(item, 0)
		if weight > math.MaxInt64-w.root.sumCost {
			log.Error("WeightedRandomSelect overflow", "sumCost",
				w.root.sumCost, "new weight", weight)
	...
	if ok {
		...
		if weight != 0 {
			if w.root.itemCnt == w.root.maxItems {
				// add a new level
				newRoot := &wrsNode{sumCost: w.root.sumCost,
					itemCnt: w.root.itemCnt, level: w.root.level + 1,
...
func (n *wrsNode) insert(item WrsItem, weight uint64) int {
	...
	if n.items[branch] == nil {
		subNode = &wrsNode{maxItems: n.maxItems / wrsBranches,
			level: n.level - 1}

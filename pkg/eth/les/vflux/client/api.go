...
func (api *PrivateClientAPI) Distribution(nodeStr string,
	normalized bool) (RtDistribution, error) {
	...
	if id, err := parseNodeStr(nodeStr); err == nil {
		return api.vt.GetNode(id).RtStats().Distribution(normalized,
			expFactor), nil
	} else {
...
func (api *PrivateClientAPI) Timeout(nodeStr string, failRate float64) (float64,
	error) {
	if nodeStr == "" {
...
func (api *PrivateClientAPI) Value(nodeStr string, timeout float64) (float64,
	error) {

...
func checkU64(t *testing.T, name string, value, exp uint64) {
	if value != exp {
		t.Errorf("Incorrect value for %s: got %d, expected %d", name, value,
			exp)
	}
}
...
func checkF64(t *testing.T, name string, value, exp, tol float64) {
	if value < exp-tol || value > exp+tol {
		t.Errorf("Incorrect value for %s: got %f, expected %f", name, value,
			exp)
	}
}
...
func TestServerBasket(t *testing.T) {
	...
	checkU64(t, "s.basket[0].amount", s.basket.items[0].amount,
		5000*basketFactor)
	checkU64(t, "s.basket[0].value", s.basket.items[0].value, 50000)
	checkU64(t, "s.basket[1].amount", s.basket.items[1].amount,
		5000*basketFactor)
	...
	checkU64(t, "transfer1[0].amount", transfer1.items[0].amount,
		2500*basketFactor)
	checkU64(t, "transfer1[0].value", transfer1.items[0].value, 25000)
	checkU64(t, "transfer1[1].amount", transfer1.items[1].amount,
		2500*basketFactor)
	...
	checkU64(t, "transfer2[0].amount", transfer2.items[0].amount,
		(2500+1000)/4*basketFactor)
	checkU64(t, "transfer2[0].value", transfer2.items[0].value, (25000+10000)/4)
	checkU64(t, "transfer2[1].amount", transfer2.items[1].amount,
		2500/4*basketFactor)
...
func TestConvertMapping(t *testing.T) {
	...
	checkU64(t, "bc[3].amount", bc.items[3].amount,
		4) // 8 should be scaled down to 4
}
...
func TestNormalize(t *testing.T) {
	for cycle := 0; cycle < 100; cycle += 1 {
		...
		ref := referenceBasket{basket: requestBasket{items: make([]basketItem,
			10)}}
		for i := 0; i < 10; i++ {
		...
		if float64(sumAmount)*(1+epsilon) < float64(sumValue) || float64(sumAmount)*(1-epsilon) > float64(sumValue) {
			t.Fatalf("Failed to normalize sumAmount: %d sumValue: %d",
				sumAmount, sumValue)
		}
	}
}

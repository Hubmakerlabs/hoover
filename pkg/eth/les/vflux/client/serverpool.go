...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/enr"
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/nodestate"
...
var (
	...
	sfDialProcess     = nodestate.MergeFlags(sfQuery, sfCanDial, sfDialing,
		sfConnected, sfRedialWait)

	sfiNodeHistory = clientSetup.NewPersistentField("nodeHistory",
		reflect.TypeOf(nodeHistory{}),
	...
	sfiNodeWeight = clientSetup.NewField("nodeWeight",
		reflect.TypeOf(uint64(0)))
	sfiConnectedStats = clientSetup.NewField("connectedStats",
		reflect.TypeOf(ResponseTimeStats{}))
	sfiLocalAddress = clientSetup.NewPersistentField("localAddress",
		reflect.TypeOf(&enr.Record{}),
		func(field interface{}) ([]byte, error) {
...
func NewServerPool(db ethdb.KeyValueStore, dbKey []byte,
	mixTimeout time.Duration, query QueryFunc, clock mclock.Clock,
	trustedURLs []string, requestList []RequestInfo) (*ServerPool,
	enode.Iterator) {
	s := &ServerPool{
		db:           db,
		clock:        clock,
		...
		vt: NewValueTracker(db, &mclock.System{}, requestList,
			time.Minute, 1/float64(time.Hour), 1/float64(time.Hour*100),
			1/float64(time.Hour*1000)),
		ns: nodestate.NewNodeStateMachine(db,
			[]byte(string(dbKey)+"ns:"), clock, clientSetup),
	}
	s.recalTimeout()
	s.mixer = enode.NewFairMix(mixTimeout)
	knownSelector := NewWrsIterator(s.ns, sfHasValue, sfDialProcess,
		sfiNodeWeight)
	alwaysConnect := NewQueueIterator(s.ns, sfAlwaysConnect, sfDialProcess,
		true, nil)
	...
	s.ns.SubscribeState(nodestate.MergeFlags(sfWaitDialTimeout, sfConnected),
		func(n *enode.Node, oldState, newState nodestate.Flags) {
	...
	return s, &serverPoolIterator{
		...
		nextFn: func(node *enode.Node) {
			s.ns.Operation(func() {
				...
				s.ns.SetStateSub(node, sfWaitDialTimeout, nodestate.Flags{},
					time.Second*10)
			})
		},
		nodeFn: s.DialNode,
...
func (s *ServerPool) AddMetrics(
	...
	if serverSelectableGauge != nil {
		s.ns.AddLogMetrics(sfHasValue, sfDialProcess, "selectable", nil, nil,
			serverSelectableGauge)
	}
	if serverDialedMeter != nil {
		s.ns.AddLogMetrics(sfDialing, nodestate.Flags{}, "dialed",
			serverDialedMeter, nil, nil)
	}
	if serverConnectedGauge != nil {
		s.ns.AddLogMetrics(sfConnected, nodestate.Flags{}, "connected", nil,
			nil, serverConnectedGauge)
	}
}
...
func (s *ServerPool) addPreNegFilter(input enode.Iterator,
	query QueryFunc) enode.Iterator {
	s.fillSet = NewFillSet(s.ns, input, sfQuery)
	s.ns.SubscribeState(sfDialProcess,
		func(n *enode.Node, oldState, newState nodestate.Flags) {
			...
			if rand.Intn(maxQueryFails*2) < int(failMax) {
				...
				s.ns.SetStateSub(n, sfCanDial, nodestate.Flags{},
					time.Second*10)
			...
			go func() {
				...
				if q == -1 {
					...
					if fails%warnQueryFails == 0 {
						...
						log.Warn("UDP connection queries failed", "count",
							fails)
					}
				} else {
	...
	return NewQueueIterator(s.ns, sfCanDial, nodestate.Flags{}, false,
		func(waiting bool) {
			if waiting {
...
func (s *ServerPool) Start() {
	...
	unixTime := s.unixTime()
	s.ns.Operation(func() {
		s.ns.ForEach(sfHasValue, nodestate.Flags{},
			func(node *enode.Node, state nodestate.Flags) {
				s.calculateWeight(node)
				if n, ok := s.ns.GetField(node,
					sfiNodeHistory).(nodeHistory); ok && n.redialWaitEnd > unixTime {
					...
					s.ns.SetStateSub(node, sfRedialWait, nodestate.Flags{},
...
func (s *ServerPool) Stop() {
	...
	s.ns.Operation(func() {
		s.ns.ForEach(sfConnected, nodestate.Flags{},
			func(n *enode.Node, state nodestate.Flags) {
...
func (s *ServerPool) recalTimeout() {
	...
	if s.timeout != timeout {
		...
		if s.totalValueGauge != nil {
			s.totalValueGauge.Update(int64(rts.Value(s.timeWeights,
				s.vt.StatsExpFactor())))
		}
	}
...
func (s *ServerPool) getTimeoutAndWeight() (time.Duration,
	ResponseTimeWeights) {
	s.recalTimeout()
	s.timeoutLock.RLock()
	defer s.timeoutLock.RUnlock()
...
func (s *ServerPool) serviceValue(node *enode.Node) (sessionValue, totalValue float64) {
	...
	if connStats, ok := s.ns.GetField(node,
		sfiConnectedStats).(ResponseTimeStats); ok {
		diff := currentStats
...
func (s *ServerPool) updateWeight(node *enode.Node, totalValue float64,
	totalDialCost uint64) {
...
func (s *ServerPool) setRedialWait(node *enode.Node, addDialCost int64,
	waitStep float64) {

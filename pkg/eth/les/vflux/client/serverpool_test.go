...
func (s *ServerPoolTest) addTrusted(i int) {
	s.trusted = append(s.trusted,
		enode.SignNull(&enr.Record{}, testNodeID(i)).String())
}
...
func (s *ServerPoolTest) start() {
	...
	s.sp, s.spi = NewServerPool(s.db, []byte("sp:"), 0, testQuery, s.clock,
		s.trusted, requestList)
...
func (s *ServerPoolTest) setNodes(count, conn, wait int,
	service, trusted bool) (res []int) {
	for ; count > 0; count-- {
...
func (s *ServerPoolTest) checkNodes(t *testing.T, nodes []int) {
	...
	if sum < spMinTotal || sum > spMaxTotal {
		t.Errorf("Total connection amount %d outside expected range %d to %d",
			sum, spMinTotal, spMaxTotal)
	}
}

func TestServerPool(t *testing.T) {
	testServerPool(t, false,
		false)
}
func TestServerPoolWithPreNeg(t *testing.T) {
	testServerPool(t, true,
		false)
}
...
func TestServerPoolRestartNoDiscovery(t *testing.T) {
	testServerPoolRestartNoDiscovery(t,
		false)
}
...
func TestServerPoolTrustedNoDiscovery(t *testing.T) {
	testServerPoolTrustedNoDiscovery(t,
		false)
}

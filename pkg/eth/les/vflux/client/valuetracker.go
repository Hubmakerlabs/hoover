...
func (nv *NodeValueTracker) UpdateCosts(reqCosts []uint64) {
	...
	nv.updateCosts(reqCosts, nv.vt.refBasket.reqValues,
		nv.vt.refBasket.reqValueFactor(reqCosts))
}
...
func (nv *NodeValueTracker) updateCosts(reqCosts []uint64, reqValues []float64,
	rvFactor float64) {
	nv.lock.Lock()
...
func (nv *NodeValueTracker) transferStats(now mclock.AbsTime,
	transferRate float64) (requestBasket, ResponseTimeStats) {
	nv.lock.Lock()
	defer nv.lock.Unlock()
...
func (nv *NodeValueTracker) Served(reqs []ServedRequest,
	...
	var value float64
	for _, r := range reqs {
		nv.basket.add(r.ReqType, r.Amount,
			nv.reqCosts[r.ReqType]*uint64(r.Amount), expFactor)
...
func NewValueTracker(db ethdb.KeyValueStore, clock mclock.Clock,
	reqInfo []RequestInfo, updatePeriod time.Duration,
	transferRate, statsExpRate, offlineExpRate float64) *ValueTracker {
	now := clock.Now()
...
func (vt *ValueTracker) loadFromDb(mapping []string) error {
	...
	if version != vtVersion {
		log.Error("Unknown ValueTracker version", "stored", version, "current",
			nvtVersion)
		return fmt.Errorf("Unknown ValueTracker version %d (current version is %d)",
			version, vtVersion)
	}
...
loop:
	...
	if int(vte.RefBasketMapping) == vt.currentMapping {
		...
		if vte.RefBasketMapping >= uint(len(vt.mappings)) {
			log.Error("Unknown request basket mapping", "stored",
				vte.RefBasketMapping, "current", vt.currentMapping)
			return fmt.Errorf("Unknown request basket mapping %d (current version is %d)",
				vte.RefBasketMapping, vt.currentMapping)
		}
		vt.refBasket.basket = vte.RefBasket.convertMapping(vt.mappings[vte.RefBasketMapping],
			mapping, vt.initRefBasket)
	}
	return nil
}
...
func (vt *ValueTracker) loadOrNewNode(id enode.ID) *NodeValueTracker {
	...
	if version != nvtVersion {
		log.Error("Unknown NodeValueTracker version", "stored", version,
			"current", nvtVersion)
		return nv
	}
	...
	if int(nve.ServerBasketMapping) == vt.currentMapping {
		...
		if nve.ServerBasketMapping >= uint(len(vt.mappings)) {
			log.Error("Unknown request basket mapping", "stored",
				nve.ServerBasketMapping, "current", vt.currentMapping)
			return nv
		}
		nv.basket.basket = nve.ServerBasket.convertMapping(vt.mappings[nve.ServerBasketMapping],
			vt.mappings[vt.currentMapping], vt.initRefBasket)
	}
	return nv
}
...
func (vt *ValueTracker) saveNode(id enode.ID, nv *NodeValueTracker) {
	...
	if err != nil {
		log.Error("Failed to encode service value information", "id", id, "err",
			err)
		return
	}
	...
	if err := vt.db.Put(append(vtNodeKey, id[:]...),
		append(enc1, enc2...)); err != nil {
		log.Error("Failed to save service value information", "id", id, "err",
			err)
	}
}
...
func (vt *ValueTracker) periodicUpdate() {
	...
	for _, nv := range vt.connected {
		nv.updateCosts(nv.reqCosts, vt.refBasket.reqValues,
			vt.refBasket.reqValueFactor(nv.reqCosts))
	}
	vt.saveToDb()
}
...
func (vt *ValueTracker) RequestStats() []RequestStatsItem {
	...
	for i, item := range vt.refBasket.basket.items {
		...
		res[i].ReqAmount = expFactor.Value(float64(item.amount)/basketFactor,
			vt.refBasket.basket.exp)

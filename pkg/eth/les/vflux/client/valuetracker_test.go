...
func TestValueTracker(t *testing.T) {
	...
	for i := range requestList {
		requestList[i] = RequestInfo{Name: "testreq" + strconv.Itoa(i),
			InitAmount: 1, InitValue: 1}
		totalAmount[i] = 1
	...
	for round := 0; round < testRounds; round++ {
		...
		vt := NewValueTracker(db, clock, requestList, time.Minute,
			1/float64(time.Hour), expRate, expRate)
		...
		if makeRequests {
			for i := 0; i < testReqCount; i++ {
				...
				nodes[node].Served([]ServedRequest{{uint32(reqType),
					uint32(reqAmount)}},
					respTime)
		...
		for i, rp := range relPrices {
			...
			if ratio < 0.99 || ratio > 1.01 {
				t.Errorf("reqValues (%v) does not match relPrices (%v)",
					vt.refBasket.reqValues, relPrices)
				break
			}
		}
		...
		for i, bi := range vt.refBasket.basket.items {
			basketAmount[i] += uint64(exp.Value(float64(bi.amount),
		...
		for i, a := range basketAmount {
			...
			if amount+10 < totalAmount[i] || amount > totalAmount[i]+10 {
				t.Errorf("totalAmount[%d] mismatch in round %d (expected %d, got %d)",
					i, round, totalAmount[i], amount)
			}
		}
		...
		if sumValue < mainValue-10 || sumValue > mainValue+10 {
			t.Errorf("Main rtStats value does not match sum of node rtStats values in round %d (main %v, sum %v)",
				round, mainValue, sumValue)
		}
	}
}

...
type (
	...
	nodePriority interface {
		...
		estimatePriority(cap uint64, addBalance int64,
			future, bias time.Duration, update bool) int64
	}
...
func (b *balance) addValue(now mclock.AbsTime, amount int64, pos bool,
	force bool) (uint64, uint64, int64, error) {
	var (
...
func (n *nodeBalance) AddBalance(amount int64) (uint64, uint64, error) {
	...
	n.bt.updateTotalBalance(n, func() bool {
		n.updateBalance(now)
		if old, new, _, err = n.balance.addValue(now, amount, true,
			false); err != nil {
			return false
		}
	...
	if n.setFlags {
		if setPriority {
			n.bt.ns.SetStateSub(n.node, n.bt.setup.priorityFlag,
				nodestate.Flags{}, 0)
		}
...
func (n *nodeBalance) RequestServed(cost uint64) (newBalance uint64) {
	...
	if !n.balance.pos.IsZero() {
		...
		if posCost == 0 {
			...
			var net int64
			_, newBalance, net, _ = n.balance.addValue(now, posCost, true,
				false)
			if posCost == net {
				fcost = 0
			} else {
	...
	if fcost > 0 && n.negFactor.RequestFactor != 0 {
		n.balance.addValue(now, int64(fcost*n.negFactor.RequestFactor), false,
			false)
		check = true
	}
	n.sumReqCost += cost
...
func (n *nodeBalance) estimatePriority(capacity uint64, addBalance int64,
	future, bias time.Duration, update bool) int64 {
	n.lock.Lock()
	defer n.lock.Unlock()
...
func (n *nodeBalance) updateBalance(now mclock.AbsTime) {
	if n.active && now > n.lastUpdate {
		n.balance = n.reducedBalance(n.balance, n.lastUpdate,
			time.Duration(now-n.lastUpdate), n.capacity, 0)
		n.lastUpdate = now
	}
}
...
func (n *nodeBalance) balanceExhausted() {
	...
	n.lock.Unlock()
	if n.setFlags {
		n.bt.ns.SetStateSub(n.node, nodestate.Flags{}, n.bt.setup.priorityFlag,
			0)
	}
}
...
func (n *nodeBalance) balanceToPriority(now mclock.AbsTime, b balance,
	capacity uint64) int64 {
	pos := b.posValue(now)
	if pos > 0 {
...
func (n *nodeBalance) priorityToBalance(priority int64,
	capacity uint64) (uint64, uint64) {
	if priority > 0 {
...
func (n *nodeBalance) reducedBalance(b balance, start mclock.AbsTime,
	dt time.Duration, capacity uint64, avgReqCost float64) balance {

...
func (z zeroExpirer) SetLogOffset(now mclock.AbsTime,
...
func newBalanceTestSetup(db ethdb.KeyValueStore,
	posExp, negExp utils.ValueExpirer) *balanceTestSetup {
	// Initialize and customize the setup for the balance testing
	clock := &mclock.Simulated{}
	setup := newServerSetup()
	setup.clientField = setup.setup.NewField("balanceTestClient",
		reflect.TypeOf(balanceTestClient{}))
...
func (b *balanceTestSetup) setBalance(node *nodeBalance,
	pos, neg uint64) (err error) {
	b.bt.BalanceOperation(node.node.ID(), node.connAddress,
		func(balance AtomicBalanceOperator) {
			err = balance.SetBalance(pos, neg)
		})
	return
}

func (b *balanceTestSetup) addBalance(node *nodeBalance,
	add int64) (old, new uint64, err error) {
	b.bt.BalanceOperation(node.node.ID(), node.connAddress,
		func(balance AtomicBalanceOperator) {
...
func TestAddBalance(t *testing.T) {
	...
	for _, i := range inputs {
		...
		if b.bt.TotalTokenAmount() != i.total {
			t.Fatalf("Total positive balance mismatch, want %v, got %v",
				i.total, b.bt.TotalTokenAmount())
		}
	}
}
...
func TestBalanceTimeCost(t *testing.T) {
	...
	for _, i := range inputs {
		...
		if pos, _ := node.GetBalance(); pos != i.expPos {
			t.Fatalf("Positive balance mismatch, want %v, got %v", i.expPos,
				pos)
		}
		if _, neg := node.GetBalance(); neg != i.expNeg {
			t.Fatalf("Negative balance mismatch, want %v, got %v", i.expNeg,
				neg)
		}
	}
...
func TestBalanceReqCost(t *testing.T) {
	...
	b.setBalance(node, uint64(time.Minute),
		0) // 1 minute time serving time allowance
	var inputs = []struct {
		reqCost uint64
		expPos  uint64
		expNeg  uint64
...
func TestEstimatedPriority(t *testing.T) {
	...
	for _, i := range inputs {
		...
		if priority != i.priority {
			t.Fatalf("Estimated priority mismatch, want %v, got %v", i.priority,
				priority)
		}
	}
}
...
func TestCallbackChecking(t *testing.T) {
	...
	for _, i := range inputs {
		...
		if diff != i.expDiff {
			t.Fatalf("Time difference mismatch, want %v, got %v", i.expDiff,
				diff)
		}
	}
}
...
func TestBalancePersistence(t *testing.T) {
	...
	posExp.SetRate(0,
		math.Log(2)/float64(time.Hour*2)) // halves every two hours
	...
	expTotal := func(expTotal uint64) {
		...
		if total != expTotal {
			t.Fatalf("Total token amount incorrect, want %v, got %v", expTotal,
				total)
		}
	}

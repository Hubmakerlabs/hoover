...
func newBalanceTracker(ns *nodestate.NodeStateMachine, setup *serverSetup,
	db ethdb.KeyValueStore, clock mclock.Clock,
	posExp, negExp utils.ValueExpirer) *balanceTracker {
	...
	bt.ndb.forEachBalance(false,
		func(id enode.ID, balance utils.ExpiredValue) bool {
			bt.inactive.AddExp(balance)
			return true
		})

	ns.SubscribeField(bt.setup.capacityField,
	...
	ns.SubscribeField(bt.setup.clientField,
		func(node *enode.Node, state nodestate.Flags,
			oldValue, newValue interface{}) {
			type peer interface {
				FreeClientId() string
			...
			if newValue != nil {
				n := bt.newNodeBalance(node, newValue.(peer).FreeClientId(),
					true)
				bt.lock.Lock()
				...
				ns.SetStateSub(node, nodestate.Flags{}, bt.setup.priorityFlag,
					0)
				if b, _ := ns.GetField(node,
					bt.setup.balanceField).(*nodeBalance); b != nil {
					b.deactivate()
				}
	...
	go func() {
		for {
			...
			case <-clock.After(persistExpirationRefresh):
				now := clock.Now()
				bt.ndb.setExpiration(posExp.LogOffset(now),
					negExp.LogOffset(now))
			case <-bt.quit:
				return
			}
		}
...
func (bt *balanceTracker) stop() {
	...
	close(bt.quit)
	bt.ns.ForEach(nodestate.Flags{}, nodestate.Flags{},
		func(node *enode.Node, state nodestate.Flags) {
			if n, ok := bt.ns.GetField(node,
				bt.setup.balanceField).(*nodeBalance); ok {
				n.lock.Lock()
...
func (bt *balanceTracker) TotalTokenAmount() uint64 {
	...
	bt.balanceTimer.Update(func(_ time.Duration) bool {
		...
		bt.ns.ForEach(nodestate.Flags{}, nodestate.Flags{},
			func(node *enode.Node, state nodestate.Flags) {
				if n, ok := bt.ns.GetField(node,
					bt.setup.balanceField).(*nodeBalance); ok && n.active {
...
func (bt *balanceTracker) GetPosBalanceIDs(start, stop enode.ID,
	maxCount int) (result []enode.ID) {
...
func (bt *balanceTracker) BalanceOperation(id enode.ID, connAddress string,
	cb func(AtomicBalanceOperator)) {
...
func (bt *balanceTracker) newNodeBalance(node *enode.Node, connAddress string,
	setFlags bool) *nodeBalance {
	...
	n := &nodeBalance{
		...
		balance: balance{pos: pb, neg: nb, posExp: bt.posExp,
			negExp: bt.negExp},
	...
	if setFlags && n.checkPriorityStatus() {
		n.bt.ns.SetStateSub(n.node, n.bt.setup.priorityFlag, nodestate.Flags{},
			0)
	}
	return n
}
...
func (bt *balanceTracker) storeBalance(id []byte, neg bool,
...
func (bt *balanceTracker) canDropBalance(now mclock.AbsTime, neg bool,
	b utils.ExpiredValue) bool {
	if neg {
...
func (bt *balanceTracker) updateTotalBalance(n *nodeBalance,
	callback func() bool) {
	bt.lock.Lock()
	defer bt.lock.Unlock()

...
type nodeDB struct {
	...
	evictCallBack func(mclock.AbsTime, bool,
		utils.ExpiredValue) bool // Callback to determine whether the balance can be evicted.
	clock       mclock.Clock
	closeCh     chan struct{}
...
func (db *nodeDB) key(id []byte, neg bool) []byte {
	...
	if len(prefix)+len(db.verbuf)+len(id) > len(db.auxbuf) {
		db.auxbuf = append(db.auxbuf,
			make([]byte, len(prefix)+len(db.verbuf)+len(id)-len(db.auxbuf))...)
	}
	copy(db.auxbuf[:len(db.verbuf)], db.verbuf[:])
	copy(db.auxbuf[len(db.verbuf):len(db.verbuf)+len(prefix)], prefix)
	copy(db.auxbuf[len(prefix)+len(db.verbuf):len(prefix)+len(db.verbuf)+len(id)],
		id)
...
func (db *nodeDB) getPosBalanceIDs(start, stop enode.ID,
...
func (db *nodeDB) forEachBalance(neg bool,
	callback func(id enode.ID, balance utils.ExpiredValue) bool) {
...
func (db *nodeDB) expireNodes() {
	...
	for _, neg := range []bool{false, true} {
		...
		for iter.Next() {
			...
			if db.evictCallBack != nil && db.evictCallBack(db.clock.Now(), neg,
				balance) {
				deleted++
	...
	log.Debug("Expire nodes", "visited", visited, "deleted", deleted, "elapsed",

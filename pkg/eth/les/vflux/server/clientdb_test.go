...
func TestNodeDB(t *testing.T) {
	...
	for _, c := range cases {
		if c.positive {
			...
			if pb := ndb.getOrNewBalance(c.id.Bytes(),
				false); !reflect.DeepEqual(pb, c.balance) {
				t.Fatalf("Positive balance mismatch, want %v, got %v",
					c.balance, pb)
			}
		} else {
			ndb.setBalance([]byte(c.ip), true, c.balance)
			if nb := ndb.getOrNewBalance([]byte(c.ip),
				true); !reflect.DeepEqual(nb, c.balance) {
				t.Fatalf("Negative balance mismatch, want %v, got %v",
					c.balance, nb)
			}
		}
	}
	for _, c := range cases {
		if c.positive {
			...
			if pb := ndb.getOrNewBalance(c.id.Bytes(),
				false); !reflect.DeepEqual(pb, utils.ExpiredValue{}) {
				t.Fatalf("Positive balance mismatch, want %v, got %v",
					utils.ExpiredValue{}, pb)
			}
		} else {
			ndb.delBalance([]byte(c.ip), true)
			if nb := ndb.getOrNewBalance([]byte(c.ip),
				true); !reflect.DeepEqual(nb, utils.ExpiredValue{}) {
				t.Fatalf("Negative balance mismatch, want %v, got %v",
					utils.ExpiredValue{}, nb)
			}
		}
	}
	...
	if pos, neg := ndb.getExpiration(); pos != posExp || neg != negExp {
		t.Fatalf("Expiration mismatch, want %v / %v, got %v / %v", posExp,
			negExp, pos, neg)
	}
...
func TestNodeDBExpiration(t *testing.T) {
	...
	if iterated != 8 {
		t.Fatalf("Failed to evict useless balances, want %v, got %d", 8,
			iterated)
	}
	...
	if iterated != 16 {
		t.Fatalf("Failed to evict useless balances, want %v, got %d", 16,
			iterated)
	}
}

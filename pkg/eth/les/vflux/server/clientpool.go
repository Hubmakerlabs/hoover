...
type clientPeer interface {
	...
	UpdateCapacity(newCap uint64,
		requested bool) // signals a capacity update (requested is true if it is a result of a SetCapacity call on the given peer
	Disconnect() // initiates disconnection (Unregister should always be called)
}

// NewClientPool creates a new client pool
func NewClientPool(balanceDb ethdb.KeyValueStore, minCap uint64,
	connectedBias time.Duration, clock mclock.Clock,
	synced func() bool) *ClientPool {
	setup := newServerSetup()
	ns := nodestate.NewNodeStateMachine(nil, nil, clock, setup.setup)
	cp := &ClientPool{
		priorityPool: newPriorityPool(ns, setup, clock, minCap, connectedBias,
			4, 100),
		balanceTracker: newBalanceTracker(ns, setup, balanceDb, clock,
			&utils.Expirer{}, &utils.Expirer{}),
		setup:         setup,
		ns:            ns,
		clock:         clock,
		minCap:        minCap,
	...
	ns.SubscribeState(nodestate.MergeFlags(setup.activeFlag, setup.inactiveFlag,
		setup.priorityFlag),
		func(node *enode.Node, oldState, newState nodestate.Flags) {
			if newState.Equals(setup.inactiveFlag) {
				// set timeout for non-priority inactive client
				var timeout time.Duration
				if c, ok := ns.GetField(node,
					setup.clientField).(clientPeer); ok {
			...
			if oldState.Equals(setup.inactiveFlag) && newState.Equals(setup.inactiveFlag.Or(setup.priorityFlag)) {
				ns.SetStateSub(node, setup.inactiveFlag, nodestate.Flags{},
					0) // priority gained; remove timeout
			}
	...
	ns.SubscribeField(setup.capacityField,
	...
	// add metrics
	cp.ns.SubscribeState(nodestate.MergeFlags(cp.setup.activeFlag,
		cp.setup.inactiveFlag),
		func(node *enode.Node, oldState, newState nodestate.Flags) {
...
func (cp *ClientPool) Register(peer clientPeer) ConnectedBalance {
	...
	balance, _ := cp.ns.GetField(peer.Node(),
		cp.setup.balanceField).(*nodeBalance)
	return balance
}
...
func (cp *ClientPool) SetCapacity(node *enode.Node, reqCap uint64,
	bias time.Duration, requested bool) (capacity uint64, err error) {
	cp.lock.RLock()
	...
	cp.ns.Operation(func() {
		...
		if newCap := cp.requestCapacity(node, minTarget, maxTarget,
			bias); newCap >= minTarget && newCap <= maxTarget {
			capacity = newCap
			return
		}
...
func (cp *ClientPool) serveCapQuery(id enode.ID, freeID string,
	data []byte) []byte {
	...
	cp.BalanceOperation(id, freeID, func(balance AtomicBalanceOperator) {
		...
		for i, addTokens := range req.AddTokens {
			...
			result[i] = curve.maxCapacity(func(capacity uint64) int64 {
				return balance.estimatePriority(capacity, add, 0, bias,
					false) / int64(capacity)
			})
...
func (cp *ClientPool) Handle(id enode.ID, address string, name string,
	data []byte) []byte {
	switch name {

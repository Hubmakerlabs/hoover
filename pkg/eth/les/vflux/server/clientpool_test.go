...
func newPoolTestPeer(i int, disconnCh chan int) *poolTestPeer {
	return &poolTestPeer{
		index:     i,
		disconnCh: disconnCh,
		node: enode.SignNull(&enr.Record{},
			enode.ID{byte(i % 256), byte(i >> 8)}),
	}
}
...
func getBalance(pool *ClientPool, p *poolTestPeer) (pos, neg uint64) {
	pool.BalanceOperation(p.node.ID(), p.FreeClientId(),
		func(nb AtomicBalanceOperator) {
...
func testClientPool(t *testing.T, activeLimit, clientCount, paidCount int,
	randomDisconnect bool) {
	var (
	...
	pool.SetDefaultFactors(PriceFactors{TimeFactor: 1, CapacityFactor: 0,
		RequestFactor: 1},
		PriceFactors{TimeFactor: 1, CapacityFactor: 0, RequestFactor: 1})
	...
	for tickCounter := 0; tickCounter < testClientPoolTicks; tickCounter++ {
		...
		if tickCounter == testClientPoolTicks/4 {
			...
			for i := 0; i < paidCount; i++ {
				addBalance(pool, newPoolTestPeer(i, disconnCh).node.ID(),
					amount)
			}
		}
	...
	for i, c := range connected {
		...
		if connTicks[i] < min || connTicks[i] > max {
			t.Errorf("Total connected time of test node #%d (%d) outside expected range (%d to %d)",
				i, connTicks[i], min, max)
		}
	}
	pool.Stop()
}

func testPriorityConnect(t *testing.T, pool *ClientPool, p *poolTestPeer,
	cap uint64, expSuccess bool) {
	...
	if newCap, _ := pool.SetCapacity(p.node, cap, defaultConnectedBias,
		true); newCap != cap {
		if expSuccess {
...
func TestConnectPaidClientToFullPool(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		addBalance(pool, newPoolTestPeer(i, nil).node.ID(),
			int64(time.Second*20))
		connect(pool, newPoolTestPeer(i, nil))
	}
	addBalance(pool, newPoolTestPeer(11, nil).node.ID(),
		int64(time.Second*2)) // Add low balance to new paid client
	if cap := connect(pool, newPoolTestPeer(11, nil)); cap != 0 {
		t.Fatalf("Low balance paid client should be rejected")
	}
	clock.Run(time.Second)
	addBalance(pool, newPoolTestPeer(12, nil).node.ID(),
		int64(time.Minute*5)) // Add high balance to new paid client
...
func TestPaidClientKickedOut(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		addBalance(pool, newPoolTestPeer(i, kickedCh).node.ID(),
			10000000000) // 10 second allowance
...
func TestPositiveBalanceCalculation(t *testing.T) {
	...
	if checkDiff(pb, uint64(time.Minute*2)) {
		t.Fatalf("Positive balance mismatch, want %v, got %v",
			uint64(time.Minute*2), pb)
	}
}
...
func TestDowngradePriorityClient(t *testing.T) {
	...
	if p.cap != 10 {
		t.Fatalf("The capacity of priority peer hasn't been updated, got: %d",
			p.cap)
	}
...
func TestNegativeBalanceCalculation(t *testing.T) {
	...
	pool.SetLimits(10, uint64(10)) // Total capacity limit is 10
	pool.SetDefaultFactors(PriceFactors{TimeFactor: 1e-3, CapacityFactor: 0,
		RequestFactor: 1},
		PriceFactors{TimeFactor: 1e-3, CapacityFactor: 0, RequestFactor: 1})
...
func TestInactiveClient(t *testing.T) {
	...
	pool.SetDefaultFactors(PriceFactors{TimeFactor: 1, CapacityFactor: 0,
		RequestFactor: 0},
		PriceFactors{TimeFactor: 1, CapacityFactor: 0, RequestFactor: 0})

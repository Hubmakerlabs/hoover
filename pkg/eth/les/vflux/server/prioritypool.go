...
func newPriorityPool(ns *nodestate.NodeStateMachine, setup *serverSetup,
	clock mclock.Clock, minCap uint64, activeBias time.Duration,
	capacityStepDiv, fineStepDiv uint64) *priorityPool {
	pp := &priorityPool{
		setup:           setup,
		ns:              ns,
		clock:           clock,
	...
	pp.activeQueue = prque.NewLazyQueue(activeSetIndex, activePriority,
		pp.activeMaxPriority, clock, lazyQueueRefresh)

	ns.SubscribeField(pp.setup.balanceField,
		func(node *enode.Node, state nodestate.Flags,
			...
			if newValue != nil {
				...
				ns.SetStateSub(node, nodestate.Flags{},
					pp.setup.activeFlag.Or(pp.setup.inactiveFlag), 0)
				if n, _ := pp.ns.GetField(node,
					pp.setup.queueField).(*ppNodeInfo); n != nil {
					pp.disconnectNode(n)
				}
	...
	ns.SubscribeState(pp.setup.activeFlag.Or(pp.setup.inactiveFlag),
		func(node *enode.Node, oldState, newState nodestate.Flags) {
			if c, _ := pp.ns.GetField(node,
				pp.setup.queueField).(*ppNodeInfo); c != nil {
				if oldState.IsEmpty() {
					pp.connectNode(c)
				}
				if newState.IsEmpty() {
	...
	ns.SubscribeState(pp.setup.updateFlag,
		func(node *enode.Node, oldState, newState nodestate.Flags) {
			if !newState.IsEmpty() {
...
func (pp *priorityPool) requestCapacity(node *enode.Node,
	minTarget, maxTarget uint64, bias time.Duration) uint64 {
	pp.lock.Lock()
...
func activePriority(c *ppNodeInfo) int64 {
	if c.bias == 0 {
		...
		return invertPriority(c.nodePriority.estimatePriority(c.tempCapacity, 0,
			0, c.bias, true))
	}
}

// activeMaxPriority callback returns estimated maximum priority of ppNodeInfo item in activeQueue
func (pp *priorityPool) activeMaxPriority(c *ppNodeInfo,
	until mclock.AbsTime) int64 {
		...
		if c.connected {
			...
			if c.capacity != oldCapacity {
				updates = append(updates,
					capUpdate{c.node, oldCapacity, c.capacity})
			}
		}
	}
	pp.tempState = nil
...
func (pp *priorityPool) updateFlags(updates []capUpdate) {
	for _, f := range updates {
		if f.oldCap == 0 {
			pp.ns.SetStateSub(f.node, pp.setup.activeFlag,
				pp.setup.inactiveFlag, 0)
		}
		if f.newCap == 0 {
			pp.ns.SetStateSub(f.node, pp.setup.inactiveFlag,
				pp.setup.activeFlag, 0)
...
func (pp *priorityPool) getCapacityCurve() *capacityCurve {
	...
	for pp.activeCap > 0 {
		...
		if pp.activeCap > pp.maxCap {
			log.Error("Active capacity is greater than allowed maximum",
				"active", pp.activeCap, "maximum", pp.maxCap)
		} else {

...
func (c *ppTestClient) estimatePriority(cap uint64, addBalance int64,
...
func TestPriorityPool(t *testing.T) {
	...
	setup.balanceField = setup.setup.NewField("ppTestClient",
		reflect.TypeOf(&ppTestClient{}))
...
func TestCapacityCurve(t *testing.T) {
	...
	check := func(balance, expCap uint64) {
		...
		if fail {
			t.Errorf("Incorrect capacity for %d balance (got %d, expected %d)",
				balance, cap, expCap)
		}
	}

...
type (
	...
	Service interface {
		Handle(id enode.ID, address string, name string,
			data []byte) []byte // never called concurrently
	}
...
func (s *Server) Serve(id enode.ID, address string,
	requests vflux.Requests) vflux.Replies {
	...
	for i, req := range requests {
		if service := s.services[req.Service]; service != nil {
			results[i] = service.backend.Handle(id, address, req.Name,
				req.Params)
		}
	}
	s.lock.Unlock()
...
func (s *Server) ServeEncoded(id enode.ID, addr *net.UDPAddr,
	req []byte) []byte {

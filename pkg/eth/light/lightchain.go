...
// validator.
func NewLightChain(odr OdrBackend, config *params.ChainConfig,
	engine consensus.Engine) (*LightChain, error) {
	bc := &LightChain{
...
func (lc *LightChain) loadLastState() error {
	...
	log.Info("Loaded most recent local header", "number", header.Number, "hash",
		header.Hash(), "td", headerTd, "age",
		common.PrettyAge(time.Unix(int64(header.Time), 0)))
	return nil
}
...
func (lc *LightChain) GetBody(ctx context.Context,
	hash common.Hash) (*types.Body, error) {
...
func (lc *LightChain) GetBodyRLP(ctx context.Context,
	hash common.Hash) (rlp.RawValue, error) {
	// Short circuit if the body's already in the cache, retrieve otherwise
...
// caching it if found.
func (lc *LightChain) GetBlock(ctx context.Context, hash common.Hash,
...
// caching it if found.
func (lc *LightChain) GetBlockByHash(ctx context.Context,
...
func (lc *LightChain) GetBlockByNumber(ctx context.Context,
	number uint64) (*types.Block, error) {
...
func (lc *LightChain) Rollback(chain []common.Hash) {
	...
	for i := len(chain) - 1; i >= 0; i-- {
		...
		if head := lc.hc.CurrentHeader(); head.Hash() == hash {
			...
			lc.hc.SetCurrentHeader(lc.GetHeader(head.ParentHash,
...
func (lc *LightChain) SetCanonical(header *types.Header) error {
	...
	log.Info("Set the chain head", "number", block.Number(), "hash",
		block.Hash())
	return nil
}
...
func (lc *LightChain) GetTdOdr(ctx context.Context, hash common.Hash,
	number uint64) *big.Int {
...
func (lc *LightChain) GetAncestor(hash common.Hash, number, ancestor uint64,
...
func (lc *LightChain) GetHeaderByNumberOdr(ctx context.Context,

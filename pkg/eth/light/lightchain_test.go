...
func makeHeaderChain(parent *types.Header, n int, db ethdb.Database,
	seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig,
		types.NewBlockWithHeader(parent), ethash.NewFaker(), db, n,
...
func newCanonical(n int) (ethdb.Database, *LightChain, error) {
	...
	blockchain, _ := NewLightChain(&dummyOdr{db: db,
		indexerConfig: TestClientIndexerConfig}, gspec.Config,
		ethash.NewFaker())
...
func testFork(t *testing.T, LightChain *LightChain, i, n int,
	comparator func(td1, td2 *big.Int)) {
	...
	headerChainB := makeHeaderChain(LightChain2.CurrentHeader(), n, db,
		forkSeed)
...
func testHeaderChainImport(chain []*types.Header,
	lightchain *LightChain) error {
	for _, header := range chain {
		// Try and validate the header
		if err := lightchain.engine.VerifyHeader(lightchain.hc,
			header); err != nil {
			return err
		}
		...
		rawdb.WriteTd(lightchain.chainDb, header.Hash(), header.Number.Uint64(),
			new(big.Int).Add(header.Difficulty,
				lightchain.GetTd(header.ParentHash, header.Number.Uint64()-1)))
...
func makeHeaderChainWithDiff(genesis *types.Block, d []int,
	seed byte) []*types.Header {
...
func testReorg(t *testing.T, first, second []int, td int64) {
	...
	if have := bc.GetTd(bc.CurrentHeader().Hash(),
		bc.CurrentHeader().Number.Uint64()); have.Cmp(want) != 0 {
...
func TestBadHeaderHashes(t *testing.T) {
	...
	if _, err = bc.InsertHeaderChain(headers); !errors.Is(err,
		core.ErrBannedHash) {
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	if bc.CurrentHeader().Hash() != headers[3].Hash() {
		t.Errorf("last header hash mismatch: have: %x, want %x",
			bc.CurrentHeader().Hash(), headers[3].Hash())
	}

...
func (odr *testOdr) Retrieve(ctx context.Context, req OdrRequest) error {
	...
	case *TrieRequest:
		...
		if len(req.Id.AccountAddress) > 0 {
			t, err = odr.serverState.OpenStorageTrie(req.Id.StateRoot,
				common.BytesToAddress(req.Id.AccountAddress), req.Id.Root)
		} else {
...
type odrTestFn func(ctx context.Context, db ethdb.Database, bc *core.BlockChain,
	lc *LightChain, bhash common.Hash) ([]byte, error)

func TestOdrGetBlockLes2(t *testing.T) { testChainOdr(t, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db ethdb.Database, bc *core.BlockChain,
	lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrGetReceipts(ctx context.Context, db ethdb.Database, bc *core.BlockChain,
	lc *LightChain, bhash common.Hash) ([]byte, error) {
	var receipts types.Receipts
	if bc != nil {
		if number := rawdb.ReadHeaderNumber(db, bhash); number != nil {
			if header := rawdb.ReadHeader(db, bhash, *number); header != nil {
				receipts = rawdb.ReadReceipts(db, bhash, *number, header.Time,
					bc.Config())
			}
		}
	} else {
...
func odrAccounts(ctx context.Context, db ethdb.Database, bc *core.BlockChain,
	lc *LightChain, bhash common.Hash) ([]byte, error) {
...
func odrContractCall(ctx context.Context, db ethdb.Database,
	bc *core.BlockChain, lc *LightChain, bhash common.Hash) ([]byte, error) {
	...
	for i := 0; i < 3; i++ {
		...
		vmenv := vm.NewEVM(context, txContext, st, config,
...
func testChainGen(i int, block *core.BlockGen) {
	...
	case 0:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress),
			...
			block.BaseFee(), nil), signer, testBankKey)
		block.AddTx(tx)
	case 1:
		...
		tx2, _ := types.SignTx(types.NewTransaction(nonce, acc2Addr,
			big.NewInt(1_000_000_000_000_000), params.TxGas, block.BaseFee(),
			nil), signer, acc1Key)
		nonce++
		tx3, _ := types.SignTx(types.NewContractCreation(nonce, big.NewInt(0),
			1000000, block.BaseFee(), testContractCode), signer, acc1Key)
	...
	case 2:
		...
		tx, _ := types.SignTx(types.NewTransaction(block.TxNonce(testBankAddress),
			testContractAddr, big.NewInt(0), 100000, block.BaseFee(), data),
			signer, testBankKey)
		block.AddTx(tx)
	case 3:
...
func testChainOdr(t *testing.T, protocol int, fn odrTestFn) {
	...
	blockchain, _ := core.NewBlockChain(sdb, nil, gspec, nil,
		ethash.NewFullFaker(), vm.Config{}, nil, nil)
	_, gchain, _ := core.GenerateChainWithGenesis(gspec, ethash.NewFaker(), 4,
		testChainGen)
	...
	odr := &testOdr{sdb: sdb, ldb: ldb, serverState: blockchain.StateCache(),
		indexerConfig: TestClientIndexerConfig}
	...
	test := func(expFail int) {
		for i := uint64(0); i <= blockchain.CurrentHeader().Number.Uint64(); i++ {
			...
			if exp && !eq {
				t.Errorf("ODR test output for block %d doesn't match full node",
					i)
			}
		}
	}

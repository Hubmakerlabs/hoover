...
func NewState(ctx context.Context, head *types.Header,
	odr OdrBackend) *state.StateDB {
	state, _ := state.New(head.Root, NewStateDatabase(ctx, head, odr), nil)
	return state
}

func NewStateDatabase(ctx context.Context, head *types.Header,
	odr OdrBackend) state.Database {
...
func (db *odrDatabase) OpenStorageTrie(stateRoot common.Hash,
	address common.Address, root common.Hash) (state.Trie, error) {
...
func (db *odrDatabase) ContractCode(addr common.Address,
	codeHash common.Hash) ([]byte, error) {
	if codeHash == sha3Nil {
		return nil, nil
	}
...
func (db *odrDatabase) ContractCodeSize(addr common.Address,
	codeHash common.Hash) (int, error) {
...
func (t *odrTrie) GetAccount(address common.Address) (*types.StateAccount,
	error) {
	var (
		enc []byte
...
func (t *odrTrie) UpdateAccount(address common.Address,
	acc *types.StateAccount) error {
	key := crypto.Keccak256(address.Bytes())
...
func (t *odrTrie) UpdateContractCode(_ common.Address, _ common.Hash,
	_ []byte) error {
	return nil
}
	...
	for {
		...
		if t.trie == nil {
			...
			if len(t.id.AccountAddress) > 0 {
				id = trie.StorageTrieID(t.id.StateRoot,
					crypto.Keccak256Hash(t.id.AccountAddress), t.id.Root)
			} else {
				id = trie.StateTrieID(t.id.StateRoot)
			}
			triedb := trie.NewDatabase(t.db.backend.Database(),
				trie.HashDefaults)
...
func (it *nodeIterator) do(fn func() error) {
	...
	for {
		...
		if missing.NodeHash == lasthash {
			it.err = fmt.Errorf("retrieve loop for trie node %x",
				missing.NodeHash)
			return
		}

...
func NewTxPool(config *params.ChainConfig, chain *LightChain,
	relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		config:      config,
...
func (pool *TxPool) GetNonce(ctx context.Context, addr common.Address) (uint64,
	error) {
...
func (txc txStateChanges) getLists() (mined []common.Hash,
...
func (pool *TxPool) checkMinedTxs(ctx context.Context, hash common.Hash,
	number uint64, txc txStateChanges) error {
	...
	if list != nil {
		...
		if _, err := GetBlockReceipts(ctx, pool.odr, hash,
			number); err != nil { // ODR caches, ignore results
			return err
		}
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context,
	newHeader *types.Header) (txStateChanges, error) {
	...
	for i := len(newHashes) - 1; i >= 0; i-- {
		hash := newHashes[i]
		if err := pool.checkMinedTxs(ctx, hash,
			newHeader.Number.Uint64()-uint64(i), txc); err != nil {
			return txc, err
		}
		pool.head = hash
	}
...
func (pool *TxPool) validateTx(ctx context.Context,
	...
	gas, err := core.IntrinsicGas(tx.Data(), tx.AccessList(), tx.To() == nil,
		true, pool.istanbul, pool.shanghai)
	if err != nil {
		return err
	}
	if tx.Gas() < gas {
...
func (pool *TxPool) add(ctx context.Context, tx *types.Transaction) error {
	...
	log.Debug("Pooled new transaction", "hash", hash, "from",
		log.Lazy{Fn: func() common.Address {
			from, _ := types.Sender(pool.signer,
				tx)
			return from
		}}, "to", tx.To())
	return nil
...
func (pool *TxPool) Content() (map[common.Address][]*types.Transaction,
...
func (pool *TxPool) ContentFrom(addr common.Address) ([]*types.Transaction,
	[]*types.Transaction) {
	pool.mu.RLock()
	defer pool.mu.RUnlock()

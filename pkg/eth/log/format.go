...
func TerminalFormat(usecolor bool) Format {
	return FormatFunc(func(r *Record) []byte {
		...
		if locationEnabled.Load() {
			...
			if color > 0 {
				fmt.Fprintf(b, "\x1b[%dm%s\x1b[0m[%s|%s]%s %s ", color, lvl,
					r.Time.Format(termTimeFormat), location, padding, msg)
			} else {
				fmt.Fprintf(b, "%s[%s|%s]%s %s ", lvl,
					r.Time.Format(termTimeFormat), location, padding, msg)
			}
		} else {
			if color > 0 {
				fmt.Fprintf(b, "\x1b[%dm%s\x1b[0m[%s] %s ", color, lvl,
					r.Time.Format(termTimeFormat), msg)
			} else {
				fmt.Fprintf(b, "%s[%s] %s ", lvl, r.Time.Format(termTimeFormat),
					msg)
			}
		}
	...
	return FormatFunc(func(r *Record) []byte {
		...
		for i := 0; i < len(r.Ctx); i += 2 {
			if k, ok := r.Ctx[i].(string); ok {
				...
				props[errorKey] = fmt.Sprintf("%+T is not a string key,",
					r.Ctx[i])
			}
		}

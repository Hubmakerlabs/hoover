...
func InfluxDB(r metrics.Registry, d time.Duration,
	url, database, username, password, namespace string) {
...
func InfluxDBWithTagsOnce(r metrics.Registry,
	url, database, username, password, namespace string,
	tags map[string]string) error {
	u, err := uurl.Parse(url)
	if err != nil {
		return fmt.Errorf("unable to parse InfluxDB. url: %s, err: %v", url,
			err)
	}
...
func (r *reporter) run() {
	...
	for {
		...
		case <-pingTicker.C:
			...
			if err != nil {
				log.Warn("Got error while sending a ping to InfluxDB, trying to recreate client",
					"err", err)
...
func (r *reporter) send(tstamp int64) error {
	...
	r.reg.Each(func(name string, i interface{}) {
		...
		if p, err := client.NewPoint(measurement, r.tags, fields,
			now); err == nil {
			bps.AddPoint(p)
		}
	})

...
func InfluxDBV2WithTags(r metrics.Registry, d time.Duration, endpoint string,
	token string, bucket string, organization string, namespace string,
	tags map[string]string) {
	rep := &v2Reporter{
		reg:          r,
		interval:     d,
...
func (r *v2Reporter) run() {
	...
	for {
		...
		case <-pingTicker.C:
			...
			if err != nil {
				log.Warn("Got error from influxdb client health check", "err",
					err.Error())
			}
		}
	}

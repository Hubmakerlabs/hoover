...
func NewReporter(r metrics.Registry, d time.Duration, e string, t string,
	s string, p []float64, u time.Duration) *Reporter {
...
func (rep *Reporter) BuildRequest(now time.Time,
	r metrics.Registry) (snapshot Batch, err error) {
	snapshot = Batch{
	...
	r.Each(func(name string, metric interface{}) {
		...
		case metrics.Histogram:
			...
			if ms.Count() > 0 {
				...
				measurement[SumSquares] = sumSquares(ms.Count(), ms.Mean(),
					ms.StdDev())

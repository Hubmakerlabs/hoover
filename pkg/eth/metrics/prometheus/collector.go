...
func (c *collector) addCounterFloat64(name string,
	m metrics.CounterFloat64Snapshot) {
	c.writeGaugeCounter(name, m.Count())
}
...
func (c *collector) addGaugeFloat64(name string,
	m metrics.GaugeFloat64Snapshot) {
	c.writeGaugeCounter(name, m.Value())
}
...
func (c *collector) addHistogram(name string, m metrics.HistogramSnapshot) {
	...
	for i := range pv {
		c.writeSummaryPercentile(name, strconv.FormatFloat(pv[i], 'f', -1, 64),
			ps[i])
	}
	c.buff.WriteRune('\n')
}
...
func (c *collector) addResettingTimer(name string,
	m metrics.ResettingTimerSnapshot) {
	if m.Count() <= 0 {
		return
	}

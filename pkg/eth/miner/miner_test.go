...
func (m *mockBackend) StateAtBlock(block *types.Block, reexec uint64,
	base *state.StateDB, checkLive bool,
	preferDisk bool) (statedb *state.StateDB, err error) {
...
func minerTestGenesisBlock(period uint64, gasLimit uint64,
	faucet common.Address) *core.Genesis {
	...
	return &core.Genesis{
		...
		Alloc: map[common.Address]core.GenesisAccount{
			...
			faucet: {Balance: new(big.Int).Sub(new(big.Int).Lsh(big.NewInt(1),
				256), big.NewInt(9))},
		},
	}
}
func createMiner(t *testing.T) (*Miner, *event.TypeMux, func(skipMiner bool)) {
	...
	genesis := minerTestGenesisBlock(15, 11_500_000,
		common.HexToAddress("12345"))
	...
	bc, err := core.NewBlockChain(chainDB, nil, genesis, nil, engine,
		vm.Config{}, nil, nil)
	if err != nil {
		t.Fatalf("can't create new chain %v", err)
	}
	statedb, _ := state.New(bc.Genesis().Root(), bc.StateCache(), nil)
	blockchain := &testBlockChain{bc.Genesis().Root(), chainConfig, statedb,
		10000000, new(event.Feed)}

	pool := legacypool.New(testTxPoolConfig, blockchain)
	txpool, _ := txpool.New(new(big.Int).SetUint64(testTxPoolConfig.PriceLimit),
		blockchain, []txpool.SubPool{pool})

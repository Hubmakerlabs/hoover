...
func DefaultIPCEndpoint(clientIdentifier string) string {
	...
	config := &Config{DataDir: DefaultDataDir(),
	...
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'geth' subdirectory of datadir.",
					"file", oldpath)
			}
			return oldpath
		}
...
func (c *Config) NodeKey() *ecdsa.PrivateKey {
	...
	if c.DataDir == "" {
		...
		if err != nil {
			log.Crit(fmt.Sprintf("Failed to generate ephemeral node key: %v",
				err))
		}
		return key
	}

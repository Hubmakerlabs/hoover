...
func TestNodeCloseMultipleTimes(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		if err := stack.Close(); err != ErrNodeStopped {
			t.Fatalf("iter %d: stop failure mismatch: have %v, want %v", i, err,
				ErrNodeStopped)
		}
	}
}
...
func TestNodeStartMultipleTimes(t *testing.T) {
	...
	if err := stack.Start(); err != ErrNodeRunning {
		t.Fatalf("start failure mismatch: have %v, want %v ", err,
			ErrNodeRunning)
	}
	...
	if err := stack.Close(); err != ErrNodeStopped {
		t.Fatalf("stop failure mismatch: have %v, want %v ", err,
			ErrNodeStopped)
	}
}
...
func TestNodeUsedDataDir(t *testing.T) {
	...
	if err != ErrDatadirUsed {
		t.Fatalf("duplicate datadir failure mismatch: have %v, want %v", err,
			ErrDatadirUsed)
	}
}
...
func TestLifecycleStartupError(t *testing.T) {
	...
	if err := stack.Start(); err != failure {
		t.Fatalf("stack startup failure mismatch: have %v, want %v", err,
			failure)
	}
	for id := range lifecycles {
...
func TestLifecycleTerminationGuarantee(t *testing.T) {
	...
	if err, ok := err.(*StopError); !ok {
		...
		if err.Services[failer] != failure {
			t.Fatalf("failer termination failure mismatch: have %v, want %v",
				err.Services[failer], failure)
		}
		if len(err.Services) != 1 {
			t.Fatalf("failure count mismatch: have %d, want %d",
				len(err.Services), 1)
		}
	}
	for id := range lifecycles {
...
func TestWebsocketHTTPOnSeparatePort_WSRequest(t *testing.T) {
	...
	if node.WSEndpoint() != ws {
		t.Fatalf("ws endpoint is incorrect: expected %s, got %s", ws,
			node.WSEndpoint())
	}
...
func TestNodeRPCPrefix(t *testing.T) {
	...
	tests := []rpcPrefixTest{
		...
		// only http prefix
		{
			...
			wantHTTP: []string{"/testprefix", "/testprefix?p=1",
				"/testprefix/x", "/testprefix/x?p=1"},
			wantNoHTTP: []string{"/", "/?p=1", "/test", "/test?p=1"},
			wantWS:     []string{"/", "/?p=1"},
			wantNoWS: []string{"/testprefix", "/testprefix?p=1", "/test",
				"/test?p=1"},
		},
		// only ws prefix
		{
			...
			wantHTTP: []string{"/", "/?p=1"},
			wantNoHTTP: []string{"/testprefix", "/testprefix?p=1", "/test",
				"/test?p=1"},
			wantWS: []string{"/testprefix", "/testprefix?p=1",
				"/testprefix/x", "/testprefix/x?p=1"},
...
func (test rpcPrefixTest) check(t *testing.T, node *Node) {
	...
	for _, path := range test.wantHTTP {
		...
		if resp.StatusCode != 200 {
			t.Errorf("Error: %s: bad status code %d, want 200", path,
				resp.StatusCode)
		}
	}
	for _, path := range test.wantNoHTTP {
		...
		if resp.StatusCode != 404 {
			t.Errorf("Error: %s: bad status code %d, want 404", path,
				resp.StatusCode)
		}
	}
	...
	for _, path := range test.wantNoWS {
		err := wsRequest(t, wsBase+path)
		if err == nil {
			t.Errorf("Error: %s: WebSocket connection succeeded for path in wantNoWS",
				path)
		}
	}
}
...
func containsProtocol(stackProtocols []p2p.Protocol,
	protocol p2p.Protocol) bool {

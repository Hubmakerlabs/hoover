	...
	runDialTest(t, config, []dialTestRound{
		...
		// 9 nodes are discovered, but only 2 are dialed.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x00),
					"127.0.0.1:30303"), // not dialed because already connected as static peer
				newNode(uintID(0x02), "127.0.0.1:30303"), // ...
				newNode(uintID(0x03), "127.0.0.1:30303"),
				newNode(uintID(0x04), "127.0.0.1:30303"),
				newNode(uintID(0x05),
					"127.0.0.1:30303"), // not dialed because there are only two slots
		...
		// Dial to 0x03 completes, filling the last remaining peer slot.
		{
			...
			discovered: []*enode.Node{
				newNode(uintID(0x09),
					"127.0.0.1:30303"), // not dialed because there are no free slots
			},
		},
	...
	runDialTest(t, config, []dialTestRound{
		...
		// aren't yet connected.
		{
			peersAdded: []*conn{
				...
				{flags: dynDialedConn,
					node: newNode(uintID(0x02), "127.0.0.2:30303")},
			},
		...
		// Only 0x01 is dialed.
		{
			peersAdded: []*conn{
				{flags: inboundConn,
					node: newNode(uintID(0x07), "127.0.0.7:30303")},
			},
			peersRemoved: []enode.ID{
				uintID(0x01),
			},
...
func runDialTest(t *testing.T, config dialConfig, rounds []dialTestRound) {
	...
	for i, round := range rounds {
		...
		if err := dialer.completeDials(round.failed,
			errors.New("oops")); err != nil {
			t.Fatalf("round %d: %v", i, err)
		}
...
func (d *dialTestDialer) Dial(ctx context.Context, n *enode.Node) (net.Conn,
	error) {
...
func (d *dialTestDialer) waitForDials(nodes []*enode.Node) error {
	...
	for len(waitset) > 0 {
		...
		case req := <-d.init:
			...
			if !ok {
				return fmt.Errorf("attempt to dial unexpected node %v",
					req.n.ID())
			}
			if !reflect.DeepEqual(req.n, want) {
				return fmt.Errorf("ENR of dialed node %v does not match test",
					req.n.ID())
			}

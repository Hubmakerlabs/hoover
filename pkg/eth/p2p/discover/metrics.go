...
func init() {
	for i := 0; i < nBuckets; i++ {
		bucketsCounter = append(bucketsCounter,
			metrics.NewRegisteredCounter(fmt.Sprintf("%s/bucket/%d/count",
				moduleName, i), nil))
	}
}
...
func (c *meteredUdpConn) ReadFromUDP(b []byte) (n int, addr *net.UDPAddr,
	err error) {
...
func (c *meteredUdpConn) WriteToUDP(b []byte, addr *net.UDPAddr) (n int,
	err error) {

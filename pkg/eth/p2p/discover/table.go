...
func newTable(t transport, db *enode.DB, cfg Config) (*Table, error) {
	...
	tab := &Table{
		...
		ips: netutil.DistinctNetSet{Subnet: tableSubnet,
			Limit: tableIPLimit},
	}
	...
	for i := range tab.buckets {
		tab.buckets[i] = &bucket{
			index: i,
			ips: netutil.DistinctNetSet{Subnet: bucketSubnet,
				Limit: bucketIPLimit},
		}
	}
	tab.seedRand()
...
func (tab *Table) setFallbackNodes(nodes []*enode.Node) error {
	...
	for _, n := range nodes {
		...
		if tab.cfg.NetRestrict != nil && !tab.cfg.NetRestrict.Contains(n.IP()) {
			tab.log.Error("Bootstrap node filtered by netrestrict", "id",
				n.ID(), "ip", n.IP())
			continue
		}
...
func (tab *Table) loadSeedNodes() {
	...
	for i := range seeds {
		...
		age := log.Lazy{Fn: func() interface{} {
			return time.Since(tab.db.LastPongReceived(seed.ID(),
				seed.IP()))
		}}
		tab.log.Trace("Found seed node in database", "id", seed.ID(), "addr",
			seed.addr(), "age", age)
...
func (tab *Table) doRevalidate(done chan<- struct{}) {
	...
	if last.Seq() < remoteSeq {
		...
		if err != nil {
			tab.log.Debug("ENR request failed", "id", last.ID(), "addr",
				last.addr(), "err", err)
		} else {
			last = &node{Node: *n, addedAt: last.addedAt,
				livenessChecks: last.livenessChecks}
		}
	}
	...
	if err == nil {
		...
		last.livenessChecks++
		tab.log.Debug("Revalidated node", "b", bi, "id", last.ID(), "checks",
			last.livenessChecks)
	...
	if r := tab.replace(b, last); r != nil {
		tab.log.Debug("Replaced dead node", "b", bi, "id", last.ID(), "ip",
			last.IP(), "checks", last.livenessChecks, "r", r.ID(), "rip",
			r.IP())
	} else {
		tab.log.Debug("Removed dead node", "b", bi, "id", last.ID(), "ip",
			last.IP(), "checks", last.livenessChecks)
	}
}
...
func (tab *Table) findnodeByID(target enode.ID, nresults int,
	preferLive bool) *nodesByDistance {
	tab.mutex.Lock()
	defer tab.mutex.Unlock()

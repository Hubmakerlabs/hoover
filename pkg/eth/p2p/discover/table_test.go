...
func TestTable_pingReplace(t *testing.T) {
	run := func(newNodeResponding, lastInBucketResponding bool) {
		name := fmt.Sprintf("newNodeResponding=%t/lastInBucketResponding=%t",
			newNodeResponding, lastInBucketResponding)
...
func testPingReplace(t *testing.T,
	newNodeIsResponding, lastInBucketIsResponding bool) {
	...
	pingSender := wrapNode(enode.NewV4(&pingKey.PublicKey, net.IP{127, 0, 0, 1},
		99, 99))
	...
	if found := contains(tab.bucket(pingSender.ID()).entries,
		last.ID()); found != lastInBucketIsResponding {
		t.Errorf("last entry found: %t, want: %t", found,
			lastInBucketIsResponding)
	}
	wantNewEntry := newNodeIsResponding && !lastInBucketIsResponding
	if found := contains(tab.bucket(pingSender.ID()).entries,
		pingSender.ID()); found != wantNewEntry {
	...
	prop := func(nodes []*node, bumps []int) (ok bool) {
		...
		for i, pos := range bumps {
			...
			if hasDuplicates(b.entries) {
				t.Logf("bucket has duplicates after %d/%d bumps:", i+1,
					len(bumps))
...
func TestTable_findnodeByID(t *testing.T) {
	...
	test := func(test *closeTest) bool {
		...
		if len(result) != wantN {
			t.Errorf("wrong number of nodes: got %d, want %d", len(result),
				wantN)
			return false
...
func TestTable_addVerifiedNode(t *testing.T) {
	...
	if !reflect.DeepEqual(tab.bucket(n1.ID()).entries, newBcontent) {
		t.Fatalf("wrong bucket content after update: %v",
			tab.bucket(n1.ID()).entries)
	}
	checkIPLimitInvariant(t, tab)
}
...
func TestTable_revalidateSyncRecord(t *testing.T) {
	...
	if !reflect.DeepEqual(intable, n2) {
		t.Fatalf("table contains old record with seq %d, want seq %d",
			intable.Seq(), n2.Seq())
	}
}

...
func TestUDPv4_Lookup(t *testing.T) {
	...
	for _, e := range results {
		t.Logf("  ld=%d, %x", enode.LogDist(lookupTestnet.target.id(), e.ID()),
			e.ID().Bytes())
	}
	if len(results) != bucketSize {
		t.Errorf("wrong number of results: got %d, want %d", len(results),
			bucketSize)
	}
...
func TestUDPv4_LookupIteratorClose(t *testing.T) {
	...
	if n := it.Node(); n != nil {
		t.Errorf("iterator returned non-nil node after close and %d more calls",
			ncalls)
	}
}
...
func serveTestnet(test *udpTest, testnet *preminedTestnet) {
	for done := false; !done; {
		done = test.waitPacketOut(func(p v4wire.Packet, to *net.UDPAddr,
			hash []byte) {
			n, key := testnet.nodeByAddr(to)
			switch p.(type) {
			case *v4wire.Ping:
				test.packetInFrom(nil, key, to,
					&v4wire.Pong{Expiration: futureExp, ReplyTok: hash})
			case *v4wire.Findnode:
				dist := enode.LogDist(n.ID(), testnet.target.id())
				nodes := testnet.nodesAtDistance(dist - 1)
				test.packetInFrom(nil, key, to,
					&v4wire.Neighbors{Expiration: futureExp, Nodes: nodes})
			}
		})
	}
}
...
func checkLookupResults(t *testing.T, tn *preminedTestnet,
	results []*enode.Node) {
	t.Helper()
	t.Logf("results:")
	for _, e := range results {
		t.Logf("  ld=%d, %x", enode.LogDist(tn.target.id(), e.ID()),
			e.ID().Bytes())
	}
...
func (tn *preminedTestnet) nodeByAddr(addr *net.UDPAddr) (*enode.Node,
	*ecdsa.PrivateKey) {
...
func (tn *preminedTestnet) neighborsAtDistances(base *enode.Node,
	distances []uint, elems int) []*enode.Node {

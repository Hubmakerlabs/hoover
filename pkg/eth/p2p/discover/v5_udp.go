...
type codecV5 interface {
	...
	Encode(enode.ID, string, v5wire.Packet, *v5wire.Whoareyou) ([]byte,
		v5wire.Nonce, error)
...
// data or nil.
func (t *UDPv5) RegisterTalkHandler(protocol string,
	handler TalkRequestHandler) {
	t.talk.register(protocol, handler)
}

// TalkRequest sends a talk request to a node and waits for a response.
func (t *UDPv5) TalkRequest(n *enode.Node, protocol string,
...
func (t *UDPv5) TalkRequestToID(id enode.ID, addr *net.UDPAddr, protocol string,
	request []byte) ([]byte, error) {
	req := &v5wire.TalkRequest{Protocol: protocol, Message: request}
...
func (t *UDPv5) RequestENR(n *enode.Node) (*enode.Node, error) {
	...
	if len(nodes) != 1 {
		return nil, fmt.Errorf("%d nodes in response for distance zero",
			len(nodes))
	}
	return nodes[0], nil
}
...
func (t *UDPv5) findnode(n *enode.Node, distances []uint) ([]*enode.Node,
	error) {
	resp := t.callToNode(n, v5wire.NodesMsg,
		&v5wire.Findnode{Distances: distances})
	return t.waitForNodes(resp, distances)
}

// waitForNodes waits for NODES responses to the given call.
func (t *UDPv5) waitForNodes(c *callV5, distances []uint) ([]*enode.Node,
	error) {
	defer t.callDone(c)
	...
	for {
		...
		case responseP := <-c.ch:
			...
			for _, record := range response.Nodes {
				...
				if err != nil {
					t.log.Debug("Invalid record in "+response.Name(), "id",
						c.node.ID(), "err", err)
					continue
				}
...
func (t *UDPv5) verifyResponseNode(c *callV5, r *enr.Record, distances []uint,
	seen map[enode.ID]struct{}) (*enode.Node, error) {
...
func (t *UDPv5) callToNode(n *enode.Node, responseType byte,
	req v5wire.Packet) *callV5 {
...
func (t *UDPv5) callToID(id enode.ID, addr *net.UDPAddr, responseType byte,
	req v5wire.Packet) *callV5 {
...
func (t *UDPv5) sendResponse(toID enode.ID, toAddr *net.UDPAddr,
	packet v5wire.Packet) error {
	_, err := t.send(toID, toAddr, packet, nil)
	return err
}

func (t *UDPv5) sendFromAnotherThread(toID enode.ID, toAddr *net.UDPAddr,
	packet v5wire.Packet) {
	select {
...
func (t *UDPv5) send(toID enode.ID, toAddr *net.UDPAddr, packet v5wire.Packet,
	c *v5wire.Whoareyou) (v5wire.Nonce, error) {
	addr := toAddr.String()
...
func (t *UDPv5) handleCallResponse(fromID enode.ID, fromAddr *net.UDPAddr,
	p v5wire.Packet) bool {
	ac := t.activeCallByNode[fromID]
	if ac == nil || !bytes.Equal(p.RequestID(), ac.reqid) {
		t.log.Debug(fmt.Sprintf("Unsolicited/late %s response", p.Name()), "id",
			fromID, "addr", fromAddr)
		return false
	}
	if !fromAddr.IP.Equal(ac.addr.IP) || fromAddr.Port != ac.addr.Port {
		t.log.Debug(fmt.Sprintf("%s from wrong endpoint", p.Name()), "id",
			fromID, "addr", fromAddr)
		return false
	}
	if p.Kind() != ac.responseType {
		t.log.Debug(fmt.Sprintf("Wrong discv5 response type %s", p.Name()),
			"id", fromID, "addr", fromAddr)
		return false
	}
...
func (t *UDPv5) handle(p v5wire.Packet, fromID enode.ID,
	...
	case *v5wire.Pong:
		if t.handleCallResponse(fromID, fromAddr, p) {
			t.localNode.UDPEndpointStatement(fromAddr,
				&net.UDPAddr{IP: p.ToIP, Port: int(p.ToPort)})
		}
...
func (t *UDPv5) handleUnknown(p *v5wire.Unknown, fromID enode.ID,
	fromAddr *net.UDPAddr) {
...
func (t *UDPv5) handleWhoareyou(p *v5wire.Whoareyou, fromID enode.ID,
	fromAddr *net.UDPAddr) {
	...
	if c.node == nil {
		...
		t.log.Debug("Can't handle "+p.Name(), "addr", fromAddr, "err",
			"call has no ENR")
...
func (t *UDPv5) matchWithCall(fromID enode.ID, nonce v5wire.Nonce) (*callV5,
	error) {
...
func (t *UDPv5) handlePing(p *v5wire.Ping, fromID enode.ID,
	fromAddr *net.UDPAddr) {
...
func (t *UDPv5) handleFindnode(p *v5wire.Findnode, fromID enode.ID,
	fromAddr *net.UDPAddr) {
...
func (t *UDPv5) collectTableNodes(rip net.IP, distances []uint,
	limit int) []*enode.Node {

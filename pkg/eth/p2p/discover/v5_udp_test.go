...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/discover/v5wire"
...
func TestUDPv5_unknownPacket(t *testing.T) {
	...
	check := func(p *v5wire.Whoareyou, wantSeq uint64) {
		...
		if p.RecordSeq != wantSeq {
			t.Errorf("wrong record seq %d in WHOAREYOU, want %d", p.RecordSeq,
				wantSeq)
		}
	}
	...
	test.waitPacketOut(func(p *v5wire.Whoareyou, addr *net.UDPAddr,
		_ v5wire.Nonce) {
		check(p, 0)
	})
...
func TestUDPv5_findnodeHandling(t *testing.T) {
	...
	test.packetIn(&v5wire.Findnode{ReqID: []byte{1},
		Distances: []uint{4234098}})
	...
	test.packetIn(&v5wire.Findnode{ReqID: []byte{5},
		Distances: []uint{249, 248}})
...
func (test *udpV5Test) expectNodes(wantReqID []byte, wantTotal uint8,
	wantNodes []*enode.Node) {
	...
	for {
		test.waitPacketOut(func(p *v5wire.Nodes, addr *net.UDPAddr,
			_ v5wire.Nonce) {
			if !bytes.Equal(p.ReqID, wantReqID) {
				test.t.Fatalf("wrong request ID %v in response, want %v",
					p.ReqID, wantReqID)
			}
			if p.RespCount != wantTotal {
				test.t.Fatalf("wrong total response count %d, want %d",
					p.RespCount, wantTotal)
			}
...
func TestUDPv5_pingCall(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.Ping, addr *net.UDPAddr,
		_ v5wire.Nonce) {
	})
	...
	test.waitPacketOut(func(p *v5wire.Ping, addr *net.UDPAddr, _ v5wire.Nonce) {
		test.packetInFrom(test.remotekey, test.remoteaddr,
			&v5wire.Pong{ReqID: p.ReqID})
	})
	if err := <-done; err != nil {
		t.Fatal(err)
	}
	...
	test.waitPacketOut(func(p *v5wire.Ping, addr *net.UDPAddr, _ v5wire.Nonce) {
		...
		test.packetInFrom(test.remotekey, wrongAddr,
			&v5wire.Pong{ReqID: p.ReqID})
	})
	if err := <-done; err != errTimeout {
...
func TestUDPv5_findnodeCall(t *testing.T) {
	...
	// Serve the responses:
	test.waitPacketOut(func(p *v5wire.Findnode, addr *net.UDPAddr,
		_ v5wire.Nonce) {
...
func TestUDPv5_talkHandling(t *testing.T) {
	...
	test.udp.RegisterTalkHandler("test",
		func(id enode.ID, addr *net.UDPAddr, message []byte) []byte {
			recvMessage = message
	...
	test.waitPacketOut(func(p *v5wire.TalkResponse, addr *net.UDPAddr,
		_ v5wire.Nonce) {
		if !bytes.Equal(p.ReqID, []byte("foo")) {
			t.Error("wrong request ID in response:", p.ReqID)
		}
...
func TestUDPv5_talkRequest(t *testing.T) {
	...
	test.waitPacketOut(func(p *v5wire.TalkRequest, addr *net.UDPAddr,
		_ v5wire.Nonce) {
	})
	if err := <-done; err != errTimeout {
		t.Fatalf("want errTimeout, got %q", err)
	...
	go func() {
		_, err := test.udp.TalkRequestToID(remote.ID(), test.remoteaddr, "test",
			[]byte("test request 2"))
		done <- err
	}()
	test.waitPacketOut(func(p *v5wire.TalkRequest, addr *net.UDPAddr,
...
func TestUDPv5_lookup(t *testing.T) {
	...
	if results := test.udp.Lookup(lookupTestnet.target.id()); len(results) > 0 {
		t.Fatalf("lookup on empty table returned %d results: %#v", len(results),
			results)
	}
	...
	for done := false; !done; {
		done = test.waitPacketOut(func(p v5wire.Packet, to *net.UDPAddr,
			_ v5wire.Nonce) {
			...
			case *v5wire.Findnode:
				...
				nodes := lookupTestnet.neighborsAtDistances(recipient,
					p.Distances, 16)
				t.Logf("Got FINDNODE for %v, returning %d nodes", p.Distances,
					len(nodes))
...
func TestUDPv5_LocalNode(t *testing.T) {
	...
	if testVal != outputVal {
		t.Errorf("Wanted %#x to be retrieved from the record but instead got %#x",
			testVal, outputVal)
	}
}
...
func TestUDPv5_PingWithIPV4MappedAddress(t *testing.T) {
	...
	go func() {
		test.udp.handlePing(&v5wire.Ping{ENRSeq: 1}, remote.ID(),
			test.remoteaddr)
		done <- struct{}{}
	}()
	test.waitPacketOut(func(p *v5wire.Pong, addr *net.UDPAddr, _ v5wire.Nonce) {
		...
		if len(p.ToIP) != net.IPv4len {
			t.Errorf("Received ip address with incorrect length: %d",
				len(p.ToIP))
		}
		if !p.ToIP.Equal(rawIP) {
			t.Errorf("Received incorrect ip address: wanted %s but received %s",
				rawIP.String(), p.ToIP.String())
		}
	})
	<-done
}
...
func (c *testCodec) Encode(toID enode.ID, addr string, p v5wire.Packet,
	_ *v5wire.Whoareyou) ([]byte, v5wire.Nonce, error) {
	c.ctr++
	var authTag v5wire.Nonce
	binary.BigEndian.PutUint64(authTag[:], c.ctr)

	penc, _ := rlp.EncodeToBytes(p)
	frame, err := rlp.EncodeToBytes(testCodecFrame{c.id, authTag, p.Kind(),
		penc})
	return frame, authTag, err
}

func (c *testCodec) Decode(input []byte, addr string) (enode.ID, *enode.Node,
	v5wire.Packet, error) {
...
func (c *testCodec) decodeFrame(input []byte) (frame testCodecFrame,
	p v5wire.Packet, err error) {
...
func (test *udpV5Test) packetInFrom(key *ecdsa.PrivateKey, addr *net.UDPAddr,
	packet v5wire.Packet) {
	test.t.Helper()

	ln := test.getNode(key, addr)
	codec := &testCodec{test: test, id: ln.ID()}
	enc, _, err := codec.Encode(test.udp.Self().ID(), addr.String(), packet,
		nil)
	if err != nil {
...
func (test *udpV5Test) getNode(key *ecdsa.PrivateKey,
	addr *net.UDPAddr) *enode.LocalNode {
...
func (test *udpV5Test) waitPacketOut(validate interface{}) (closed bool) {
	...
	if !reflect.TypeOf(p).AssignableTo(exptype) {
		test.t.Errorf("sent packet type mismatch, got: %v, want: %v",
			reflect.TypeOf(p), exptype)
		return false
	}
	fn.Call([]reflect.Value{reflect.ValueOf(p), reflect.ValueOf(&dgram.to),
		reflect.ValueOf(frame.AuthTag)})
	return false
}

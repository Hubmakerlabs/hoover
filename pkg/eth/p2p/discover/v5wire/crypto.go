...
func DecodePubkey(curve elliptic.Curve, e []byte) (*ecdsa.PublicKey, error) {
	...
	default:
		return nil, fmt.Errorf("unsupported curve %s in DecodePubkey",
			curve.Params().Name)
	}
}

// idNonceHash computes the ID signature hash used in the handshake.
func idNonceHash(h hash.Hash, challenge, ephkey []byte,
	destID enode.ID) []byte {
	h.Reset()
...
func makeIDSignature(hash hash.Hash, key *ecdsa.PrivateKey,
	challenge, ephkey []byte, destID enode.ID) ([]byte, error) {
...
func verifyIDSignature(hash hash.Hash, sig []byte, n *enode.Node,
	challenge, ephkey []byte, destID enode.ID) error {
	...
	case "v4":
		...
		return nil
	default:
		return fmt.Errorf("can't verify ID nonce signature against scheme %q",
			idscheme)
	}
}
...
func deriveKeys(hash hashFn, priv *ecdsa.PrivateKey, pub *ecdsa.PublicKey,
	...
	sec := session{writeKey: make([]byte, aesKeySize),

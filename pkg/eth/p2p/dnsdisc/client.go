...
func (c *Client) resolveRoot(ctx context.Context, loc *linkEntry) (rootEntry,
	error) {
	e, err, _ := c.singleflight.Do(loc.str, func() (interface{}, error) {
		txts, err := c.cfg.Resolver.LookupTXT(ctx, loc.domain)
		c.cfg.Logger.Trace("Updating DNS discovery root", "tree", loc.domain,
			"err", err)
		if err != nil {
...
// if it isn't cached.
func (c *Client) resolveEntry(ctx context.Context, domain, hash string) (entry,
	error) {
...
func (c *Client) doResolveEntry(ctx context.Context,
	domain, hash string) (entry, error) {
...
func (it *randomIterator) nextNode() *enode.Node {
	for {
		...
		if err != nil {
			...
			it.c.cfg.Logger.Debug("Error in DNS random node sync", "tree",
				ct.loc.domain, "err", err)
			continue
		}
		if n != nil {
			return n
...
func (it *randomIterator) waitForRootUpdates(trees []*clientTree) bool {
	...
	it.c.cfg.Logger.Debug("DNS iterator waiting for root updates", "sleep",
		sleep, "tree", minTree.loc.domain)

...
func TestClientSyncTree(t *testing.T) {
	...
	if !reflect.DeepEqual(sortByID(stree.Nodes()), wantNodes) {
		t.Errorf("wrong nodes in synced tree:\nhave %v\nwant %v",
			spew.Sdump(stree.Nodes()), spew.Sdump(wantNodes))
	}
...
func TestClientSyncTreeBadNode(t *testing.T) {
	...
	wantErr := nameError{name: "INDMVBZEEQ4ESVYAKGIYU74EAA.n",
...
func checkIterator(t *testing.T, it enode.Iterator, wantNodes []*enode.Node) {
	...
	t.Logf("checkIterator called Next %d times to find %d nodes", calls,
		len(wantNodes))
	for _, n := range want {
...
func makeTestTree(domain string, nodes []*enode.Node, links []string) (*Tree,
	string) {
...
func (mr mapResolver) LookupTXT(ctx context.Context, name string) ([]string,
	error) {

...
func (t *Tree) Sign(key *ecdsa.PrivateKey, domain string) (url string,
	err error) {
	root := *t.root
...
func MakeTree(seq uint, nodes []*enode.Node, links []string) (*Tree, error) {
	...
	for _, n := range records {
		if len(n.Record().Signature()) == 0 {
			return nil, fmt.Errorf("can't add node %v: unsigned node record",
				n.ID())
		}
	}
	...
	t.root = &rootEntry{seq: seq, eroot: subdomain(eroot),
		lroot: subdomain(lroot)}
	return t, nil
}
...
func (e *rootEntry) String() string {
	return fmt.Sprintf(rootPrefix+" e=%s l=%s seq=%d sig=%s", e.eroot, e.lroot,
		e.seq, b64format.EncodeToString(e.sig))
}
...
func parseRoot(e string) (rootEntry, error) {
	...
	var seq uint
	if _, err := fmt.Sscanf(e, rootPrefix+" e=%s l=%s seq=%d sig=%s", &eroot,
		&lroot, &seq, &sig); err != nil {

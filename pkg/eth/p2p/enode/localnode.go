...
func NewLocalNode(db *DB, key *ecdsa.PrivateKey) *LocalNode {
	ln := &LocalNode{
		...
		endpoint4: lnEndpoint{
			track: netutil.NewIPTracker(iptrackWindow, iptrackContactWindow,
				iptrackMinStatements),
		},
		endpoint6: lnEndpoint{
			track: netutil.NewIPTracker(iptrackWindow, iptrackContactWindow,
				iptrackMinStatements),
...
func (ln *LocalNode) UDPEndpointStatement(fromaddr, endpoint *net.UDPAddr) {
	...
	ln.endpointForIP(endpoint.IP).track.AddStatement(fromaddr.String(),
		endpoint.String())
	ln.updateEndpoints()
}
...
func (ln *LocalNode) sign() {
	...
	ln.cur.Store(n)
	log.Info("New local node record", "seq", ln.seq, "id", n.ID(), "ip", n.IP(),
		"udp", n.UDP(), "tcp", n.TCP())
}

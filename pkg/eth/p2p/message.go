...
func (msg Msg) Decode(val interface{}) error {
	...
	if err := s.Decode(val); err != nil {
		return newPeerError(errInvalidMsg, "(code %x) (size %d) %v", msg.Code,
			msg.Size, err)
	}
	return nil
}
...
func ExpectMsg(r MsgReader, code uint64, content interface{}) error {
	...
	if msg.Code != code {
		return fmt.Errorf("message code mismatch: got %d, expected %d",
			msg.Code, code)
	}
	if content == nil {
		return msg.Discard()
	...
	if int(msg.Size) != len(contentEnc) {
		return fmt.Errorf("message size mismatch: got %d, want %d", msg.Size,
			len(contentEnc))
	}
...
// feed
func newMsgEventer(rw MsgReadWriter, feed *event.Feed, peerID enode.ID,
	proto, remote, local string) *msgEventer {
	return &msgEventer{
		MsgReadWriter: rw,
		feed:          feed,
		peerID:        peerID,

...
type Interface interface {
	...
	AddMapping(protocol string, extport, intport int, name string,
...
func Map(m Interface, c <-chan struct{}, protocol string, extport, intport int,
	name string) {
	log := log.New("proto", protocol, "extport", extport, "intport", intport,
		"interface", m)
	...
	if _, err := m.AddMapping(protocol, extport, intport, name,
		DefaultMapTimeout); err != nil {
...
func (n ExtIP) String() string {
	return fmt.Sprintf("ExtIP(%v)",
		net.IP(n))
}

// These do nothing.

func (ExtIP) AddMapping(string, int, int, string, time.Duration) (uint16,
	error) {
	return 0, nil
}
...
func (n *autodisc) AddMapping(protocol string, extport, intport int,

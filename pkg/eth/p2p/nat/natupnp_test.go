...
// 		} else {
// 			t.Skipf("UPnP not discovered (known issue, see https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/21476)")
// 		}
// 	}
// 	upnp, _ := discovered.(*upnp)
// 	if upnp.service != "IGDv1-IP1" {
// 		t.Errorf("upnp.service mismatch: got %q, want %q", upnp.service,
// 			"IGDv1-IP1")
// 	}
// 	wantURL := "http://" + dev.listener.Addr().String() + "/InternetGatewayDevice.xml"
// 	if upnp.dev.URLBaseStr != wantURL {
// 		t.Errorf("upnp.dev.URLBaseStr mismatch: got %q, want %q",
// 			upnp.dev.URLBaseStr, wantURL)
// 	}
// }
...
func (dev *fakeIGD) replaceListenAddr(resp string) string {
	return strings.ReplaceAll(resp, "{{listenAddr}}",
		dev.listener.Addr().String())
}
...
func (dev *fakeIGD) listen() (err error) {
	...
	if dev.mcastListener, err = net.ListenMulticastUDP("udp", nil,
		laddr); err != nil {
		dev.listener.Close()
		return err
	}

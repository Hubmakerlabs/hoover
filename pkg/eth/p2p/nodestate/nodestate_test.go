...
func testSetup(flagPersist []bool, fieldType []reflect.Type) (*Setup, []Flags,
	[]Field) {
	setup := &Setup{}
	...
	for i, ftype := range fieldType {
		...
		case reflect.TypeOf(uint64(0)):
			fields[i] = setup.NewPersistentField(fmt.Sprintf("field-%d", i),
				ftype, uint64FieldEnc, uint64FieldDec)
		case reflect.TypeOf(""):
			fields[i] = setup.NewPersistentField(fmt.Sprintf("field-%d", i),
				ftype, stringFieldEnc, stringFieldDec)
		default:
...
func TestCallback(t *testing.T) {
	...
	ns.SubscribeState(flags[0],
		func(n *enode.Node, oldState, newState Flags) { set0 <- struct{}{} })
	ns.SubscribeState(flags[1],
		func(n *enode.Node, oldState, newState Flags) { set1 <- struct{}{} })
	ns.SubscribeState(flags[2],
		func(n *enode.Node, oldState, newState Flags) { set2 <- struct{}{} })
...
func TestPersistentFlags(t *testing.T) {
	...
	ns.SetState(testNode(1), flags[0], Flags{},
		time.Second) // state with timeout should not be saved
...
func TestSetField(t *testing.T) {
	...
	s, flags, fields := testSetup([]bool{true},
...
func TestSetState(t *testing.T) {
	...
	ns.SubscribeState(flags[0].Or(flags[1]),
...
func TestPersistentFields(t *testing.T) {
	...
	s, flags, fields := testSetup([]bool{true},
		[]reflect.Type{reflect.TypeOf(uint64(0)), reflect.TypeOf("")})
	ns := NewNodeStateMachine(mdb, []byte("-ns"), clock, s)
...
func TestFieldSub(t *testing.T) {
	...
	ns.SubscribeField(fields[0],
		func(n *enode.Node, state Flags, oldValue, newValue interface{}) {
	...
	check := func(state Flags, oldValue, newValue interface{}) {
		if !lastState.Equals(state) || lastOldValue != oldValue || lastNewValue != newValue {
			t.Fatalf("Incorrect field sub callback (expected [%v %v %v], got [%v %v %v])",
				state, oldValue, newValue, lastState, lastOldValue,
				lastNewValue)
		}
	}
	ns.Start()
...
func TestDuplicatedFlags(t *testing.T) {
	...
	check(Flags{}, flags[0], true)
	ns.SetState(testNode(1), flags[0], Flags{},
		2*time.Second) // extend the timeout to 2s
...
func TestCallbackOrder(t *testing.T) {
	...
	lastState := Flags{}
	ns.SubscribeState(MergeFlags(flags[1], flags[2], flags[3]),
		func(n *enode.Node, oldState, newState Flags) {

...
func newHashMAC(cipher cipher.Block, h hash.Hash) hashMAC {
	...
	if cipher.BlockSize() != len(m.aesBuffer) {
		panic(fmt.Errorf("invalid MAC cipher block size %d",
			cipher.BlockSize()))
	}
...
func (c *Conn) Write(code uint64, data []byte) (uint32, error) {
	...
	if c.snappyWriteBuffer != nil {
		...
		c.snappyWriteBuffer = growslice(c.snappyWriteBuffer,
			snappy.MaxEncodedLen(len(data)))
...
func (h *sessionState) writeFrame(conn io.Writer, code uint64,
	data []byte) error {
	h.wbuf.reset()
...
func (h *handshakeState) runRecipient(conn io.ReadWriter,
	prv *ecdsa.PrivateKey) (s Secrets, err error) {
...
func (h *handshakeState) handleAuthMsg(msg *authMsgV4,
	prv *ecdsa.PrivateKey) error {
...
func (h *handshakeState) secrets(auth, authResp []byte) (Secrets, error) {
	ecdheSecret, err := h.randomPrivKey.GenerateShared(h.remoteRandomPub,
		sskLen, sskLen)
	if err != nil {
		return Secrets{}, err
	}
	...
	sharedSecret := crypto.Keccak256(ecdheSecret,
		crypto.Keccak256(h.respNonce, h.initNonce))
...
func (h *handshakeState) staticSharedSecret(prv *ecdsa.PrivateKey) ([]byte,
	error) {
...
func (h *handshakeState) runInitiator(conn io.ReadWriter, prv *ecdsa.PrivateKey,
	remote *ecdsa.PublicKey) (s Secrets, err error) {
	h.initiator = true
...
func (h *handshakeState) makeAuthMsg(prv *ecdsa.PrivateKey) (*authMsgV4,
	error) {
...
func (h *handshakeState) readMsg(msg interface{}, prv *ecdsa.PrivateKey,
...
func importPublicKey(pubKey []byte) (*ecies.PublicKey, error) {
	...
	case 65:
		pubKey65 = pubKey
	default:
		return nil, fmt.Errorf("invalid public key length %v (expect 64/65)",
			len(pubKey))
	}

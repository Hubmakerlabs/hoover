...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/p2p/discover"
...
func (c *conn) set(f connFlag, val bool) {
	for {
		...
		if atomic.CompareAndSwapInt32((*int32)(&c.flags), int32(oldFlags),
			int32(flags)) {
			return
		}
	}
...
func (s *sharedUDPConn) ReadFromUDP(b []byte) (n int, addr *net.UDPAddr,
	err error) {
	...
	for {
		...
		case c := <-srv.checkpointAddPeer:
			...
			if err == nil {
				...
				p := srv.launchPeer(c)
				peers[c.node.ID()] = p
				srv.log.Debug("Adding p2p peer", "peercount", len(peers), "id",
					p.ID(), "conn", c.flags, "addr", p.RemoteAddr(), "name",
					p.Name())
		...
		case pd := <-srv.delpeer:
			...
			delete(peers, pd.ID())
			srv.log.Debug("Removing p2p peer", "peercount", len(peers), "id",
				pd.ID(), "duration", d, "req", pd.requested, "err", pd.err)
...
func (srv *Server) postHandshakeChecks(peers map[enode.ID]*Peer,
	inboundCount int, c *conn) error {
	switch {
...
func (srv *Server) addPeerChecks(peers map[enode.ID]*Peer, inboundCount int,
	c *conn) error {
	// Drop connections with no matching protocols.
	if len(srv.Protocols) > 0 && countMatchingProtocols(srv.Protocols,
		c.caps) == 0 {
...
func (srv *Server) listenLoop() {
	...
	for {
		...
		if err := srv.checkInboundConn(remoteIP); err != nil {
			srv.log.Debug("Rejected inbound connection", "addr",
...
func (srv *Server) SetupConn(fd net.Conn, flags connFlag,
	dialDest *enode.Node) error {
...
func (srv *Server) setupConn(c *conn, flags connFlag,
	...
	if dialDest != nil {
		...
		if err := dialDest.Load((*enode.Secp256k1)(dialPubkey)); err != nil {
			err = fmt.Errorf("%w: dial destination doesn't have a secp256k1 public key",
				errEncHandshakeError)
			srv.log.Trace("Setting up connection failed", "addr",
				c.fd.RemoteAddr(), "conn", c.flags, "err", err)
			return err
		}
	}
	...
	if err != nil {
		srv.log.Trace("Failed RLPx handshake", "addr", c.fd.RemoteAddr(),
			"conn", c.flags, "err", err)
	...
	clog := srv.log.New("id", c.node.ID(), "addr", c.fd.RemoteAddr(), "conn",
		c.flags)
	...
	if id := c.node.ID(); !bytes.Equal(crypto.Keccak256(phs.ID), id[:]) {
		clog.Trace("Wrong devp2p handshake identity", "phsid",
			hex.EncodeToString(phs.ID))

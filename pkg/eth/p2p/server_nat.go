...
func (srv *Server) portMappingLoop() {
	...
	newLogger := func(p string, e int, i int) log.Logger {
		return log.New("proto", p, "extport", e, "intport", i, "interface",
			srv.NAT)
	}
	...
	for {
		...
		case <-extip.C():
			...
			if err != nil {
				log.Debug("Couldn't get external IP", "err", err, "interface",
					srv.NAT)
			} else if !ip.Equal(lastExtIP) {
				log.Debug("External IP changed", "ip", extip, "interface",
					srv.NAT)
			} else {
				return
			}
		...
		case <-refresh.C():
			for _, m := range mappings {
				...
				p, err := srv.NAT.AddMapping(m.protocol, external, m.port,
					m.name, portMapDuration)
				if err != nil {

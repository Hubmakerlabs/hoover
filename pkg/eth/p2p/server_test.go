...
func newTestTransport(rpub *ecdsa.PublicKey, fd net.Conn,
	dialDest *ecdsa.PublicKey) transport {
...
func (c *testTransport) doEncHandshake(prv *ecdsa.PrivateKey) (*ecdsa.PublicKey,
	error) {
	return c.rpub, nil
}

func (c *testTransport) doProtoHandshake(our *protoHandshake) (*protoHandshake,
	error) {
...
func startTestServer(t *testing.T, remoteKey *ecdsa.PublicKey,
	pf func(*Peer)) *Server {
	config := Config{
		Name:        "test",
		MaxPeers:    10,
...
func TestServerDial(t *testing.T) {
	...
	case conn := <-accepted:
		...
		case peer := <-connected:
			...
			peers := srv.Peers()
			if !reflect.DeepEqual(peers, []*Peer{peer}) {
				t.Errorf("Peers mismatch: got %v, want %v", peers,
					[]*Peer{peer})
			}
			...
			go func() {
				...
				if peer := srv.Peers()[0]; !peer.Info().Network.Trusted {
					t.Errorf("peer is not trusted after AddTrustedPeer: %v",
						peer)
				}
				srv.RemoveTrustedPeer(node)
				if peer := srv.Peers()[0]; peer.Info().Network.Trusted {
					t.Errorf("peer is trusted after RemoveTrustedPeer: %v",
						peer)
				}
				done <- true
			}()
...
func TestServerAtCap(t *testing.T) {
	...
	newconn := func(id enode.ID) *conn {
		...
		return &conn{fd: fd, transport: tx, flags: inboundConn, node: node,
	...
	anotherID := randomID()
	c := newconn(anotherID)
	if err := srv.checkpoint(c,
		srv.checkpointPostHandshake); err != DiscTooManyPeers {
...
func TestServerSetupConn(t *testing.T) {
	...
	tests := []struct {
		...
		{
			tt: &setupTransport{pubkey: clientpub,
				encHandshakeErr: errEncHandshakeError},
			flags:        inboundConn,
		...
		{
			tt: &setupTransport{pubkey: clientpub,
				phs: protoHandshake{ID: randomID().Bytes()}},
		...
		{
			tt: &setupTransport{pubkey: clientpub,
				protoHandshakeErr: errProtoHandshakeError},
			dialDest:     enode.NewV4(clientpub, nil, 0, 0),
			flags:        dynDialedConn,
			wantCalls:    "doEncHandshake,doProtoHandshake,close,",
		...
		{
			tt: &setupTransport{pubkey: srvpub,
				phs: protoHandshake{ID: crypto.FromECDSAPub(srvpub)[1:]}},
			flags:        inboundConn,
			wantCalls:    "doEncHandshake,close,",
	...
	for i, test := range tests {
		t.Run(test.wantCalls, func(t *testing.T) {
			...
			if !errors.Is(test.tt.closeErr, test.wantCloseErr) {
				t.Errorf("test %d: close error mismatch: got %q, want %q", i,
					test.tt.closeErr, test.wantCloseErr)
			}
			if test.tt.calls != test.wantCalls {
				t.Errorf("test %d: calls mismatch: got %q, want %q", i,
					test.tt.calls, test.wantCalls)
			}
		})
	}
}
...
func (c *setupTransport) doEncHandshake(prv *ecdsa.PrivateKey) (*ecdsa.PublicKey,
	error) {
	c.calls += "doEncHandshake,"
	return c.pubkey, c.encHandshakeErr
}

func (c *setupTransport) doProtoHandshake(our *protoHandshake) (*protoHandshake,
	error) {
...
func TestServerInboundThrottle(t *testing.T) {
	...
	go func() {
		...
		if n, err := conn.Read(buf); err != io.EOF || n != 0 {
			t.Errorf("expected io.EOF and n == 0, got error %q and n == %d",
				err, n)
		}
		connClosed <- struct{}{}
	}()
...
func listenFakeAddr(network, laddr string, remoteAddr net.Addr) (net.Listener,
	error) {

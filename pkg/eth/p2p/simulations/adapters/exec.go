...
func (n *ExecNode) waitForStartupJSON(ctx context.Context) (string,
	chan nodeStartupJSON) {
	var (
	...
	srv.Handler = http.HandlerFunc(func(w http.ResponseWriter,
		r *http.Request) {
...
func (n *ExecNode) execCommand() *exec.Cmd {
	return &exec.Cmd{
		Path: reexec.Self(),
		Args: []string{"p2p-node", strings.Join(n.Config.Node.Lifecycles, ","),
			n.ID.String()},
	}
}
...
func initLogging() {
	...
	glogger = log.NewGlogHandler(log.StreamHandler(writer,
...
func execP2PNode() {
	...
	resp, err := http.Post(statusURL, "application/json",

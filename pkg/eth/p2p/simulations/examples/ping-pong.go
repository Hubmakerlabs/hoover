...
var adapterType = flag.String("adapter", "sim",
	`node adapter to use (one of "sim", "exec" or "docker")`)
...
func main() {
	...
	services := map[string]adapters.LifecycleConstructor{
		"ping-pong": func(ctx *adapters.ServiceContext,
	...
	if err := http.ListenAndServe(":8888",
		simulations.NewServer(network)); err != nil {
...
func (p *pingPongService) Run(peer *p2p.Peer, rw p2p.MsgReadWriter) error {
	...
	go func() {
		for {
			...
			log.Info("received message", "msg.code", msg.Code, "msg.payload",
				string(payload))
			p.received.Add(1)

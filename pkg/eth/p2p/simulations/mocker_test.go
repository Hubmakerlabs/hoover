...
func TestMocker(t *testing.T) {
	...
	if resp.StatusCode != 200 {
		t.Fatalf("Invalid Status Code received, expected 200, got %d",
			resp.StatusCode)
	}
	...
	resp, err = http.PostForm(s.URL+"/mocker/start",
		url.Values{"mocker-type": {mockertype},
	...
	resp.Body.Close()
	if resp.StatusCode != 200 {
		t.Fatalf("Invalid Status Code received for starting mocker, expected 200, got %d",
			resp.StatusCode)
	}
	...
	if len(nodesInfo) != nodeCount {
		t.Fatalf("Expected %d number of nodes, got: %d", nodeCount,
			len(nodesInfo))
	}
	...
	resp.Body.Close()
	if resp.StatusCode != 200 {
		t.Fatalf("Invalid Status Code received for stopping mocker, expected 200, got %d",
			resp.StatusCode)
	}

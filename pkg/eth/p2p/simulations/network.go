...
func NewNetwork(nodeAdapter adapters.NodeAdapter,
	conf *NetworkConfig) *Network {
	return &Network{
		NetworkConfig: *conf,
...
func (net *Network) NewNodeWithConfig(conf *adapters.NodeConfig) (*Node,
	error) {
	net.lock.Lock()
	...
	if conf.Reachable == nil {
		conf.Reachable = func(otherID enode.ID) bool {
			...
			if err != nil && bytes.Compare(conf.ID.Bytes(),
				otherID.Bytes()) < 0 {
				return false
			}
			return true
...
// snapshots
func (net *Network) startWithSnapshots(id enode.ID,
	...
	sub, err := client.Subscribe(context.Background(), "admin", events,
		"peerEvents")
	if err != nil {
...
func (net *Network) watchPeerEvents(id enode.ID, events chan *p2p.PeerEvent,
	...
	for {
		...
		case err := <-sub.Err():
			if err != nil {
				log.Error("Error in peer event subscription", "id", id, "err",
					err)
			}
			return
		}
...
func (net *Network) Disconnect(oneID, otherID enode.ID) error {
	...
	if conn == nil {
		return fmt.Errorf("connection between %v and %v does not exist", oneID,
			otherID)
	}
	if !conn.Up {
...
func (net *Network) DidConnect(one, other enode.ID) error {
	...
	if err != nil {
		return fmt.Errorf("connection between %v and %v does not exist", one,
			other)
	}
	if conn.Up {
...
func (net *Network) DidSend(sender, receiver enode.ID, proto string,
	code uint64) error {
	msg := &Msg{
		One:      sender,
		Other:    receiver,
...
func (net *Network) DidReceive(sender, receiver enode.ID, proto string,
	code uint64) error {
	msg := &Msg{
		One:      sender,
		Other:    receiver,
...
func (net *Network) GetRandomNode(excludeIDs ...enode.ID) *Node {
	net.lock.RLock()
	defer net.lock.RUnlock()
	return net.getRandomNode(net.getNodeIDs(nil),
		excludeIDs) // no need to exclude twice
}
...
func (net *Network) initConn(oneID, otherID enode.ID) (*Conn, error) {
	...
	if time.Since(conn.initiated) < DialBanTimeout {
		return nil, fmt.Errorf("connection between %v and %v recently attempted",
			oneID, otherID)
	}
...
func (c *Conn) String() string {
	return fmt.Sprintf("Conn %v->%v", c.One.TerminalString(),
		c.Other.TerminalString())
}
...
func (m *Msg) String() string {
	return fmt.Sprintf("Msg(%d) %v->%v", m.Code, m.One.TerminalString(),
		m.Other.TerminalString())
}
...
func (net *Network) SnapshotWithServices(addServices []string,
	removeServices []string) (*Snapshot, error) {
	return net.snapshot(addServices, removeServices)
}

func (net *Network) snapshot(addServices []string,
	removeServices []string) (*Snapshot, error) {
	net.lock.Lock()
	defer net.lock.Unlock()
	snap := &Snapshot{
	...
	for i, node := range net.Nodes {
		...
		for _, addSvc := range addServices {
			...
			if !haveSvc {
				snap.Nodes[i].Node.Config.Lifecycles = append(snap.Nodes[i].Node.Config.Lifecycles,
					addSvc)
			}
		}
...
func (net *Network) Load(snap *Snapshot) error {
	...
	for _, n := range snap.Nodes {
		...
		if err := net.startWithSnapshots(n.Node.Config.ID,
			n.Snapshots); err != nil {
			return err
		}
	}
		...
		for {
			...
			case e := <-events:
				...
				if !e.Conn.Up {
					...
					log.Warn("load snapshot: unexpected disconnection", "one",
						e.Conn.One, "other", e.Conn.Other)
					continue
				}

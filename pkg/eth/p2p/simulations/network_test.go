...
func TestSnapshot(t *testing.T) {
	...
	adapter := adapters.NewSimAdapter(adapters.LifecycleConstructors{
		"noopwoop": func(ctx *adapters.ServiceContext,
...
OUTER:
	for {
		...
		case ev := <-evC:
			if ev.Type == EventTypeConn && !ev.Control {
				...
				if !ev.Conn.Up {
					t.Fatalf("unexpected disconnect: %v -> %v", ev.Conn.One,
						ev.Conn.Other)
				}
				checkIds[ev.Conn.One] = append(checkIds[ev.Conn.One],
					ev.Conn.Other)
				checkIds[ev.Conn.Other] = append(checkIds[ev.Conn.Other],
					ev.Conn.One)
				connEventCount--
	...
	log.Debug("snapshot taken", "nodes", len(snap.Nodes), "conns",
		len(snap.Conns), "json", string(j))

	// verify that the snap element numbers check out
	if len(checkIds) != len(snap.Conns) || len(checkIds) != len(snap.Nodes) {
		t.Fatalf("snapshot wrong node,conn counts %d,%d != %d", len(snap.Nodes),
			len(snap.Conns), len(checkIds))
	}
...
OuterTwo:
	...
	for _, snapConn := range snap.Conns {
		...
		if !match {
			t.Fatalf("network missing conn %v -> %v", snapConn.One,
				snapConn.Other)
		}
	}
	...
	case ev := <-evC:
		if ev.Type == EventTypeConn {
			t.Fatalf("Superfluous conn found %v -> %v", ev.Conn.One,
				ev.Conn.Other)
		}
	}
...
func TestNetworkSimulation(t *testing.T) {
	...
	check := func(ctx context.Context, id enode.ID) (bool, error) {
		...
		var peerCount int64
		if err := client.CallContext(ctx, &peerCount,
			"test_peerCount"); err != nil {
			return false, err
		}
		switch {
		case peerCount < 2:
	...
	if len(snap.Nodes) != nodeCount {
		t.Fatalf("expected snapshot to contain %d nodes, got %d", nodeCount,
			len(snap.Nodes))
	}
	if len(snap.Conns) != nodeCount {
		t.Fatalf("expected snapshot to contain %d connections, got %d",
			nodeCount, len(snap.Conns))
	}
	for i, id := range ids {
		conn := snap.Conns[i]
		if conn.One != id {
		...
		if conn.Other != peerID {
			t.Fatalf("expected conn[%d].Other to be %s, got %s", i, peerID,
				conn.Other)
		}
	}
}
...
func createTestNodesWithProperty(property string, count int,
	network *Network) (propertyNodes []*Node, err error) {
	for i := 0; i < count; i++ {
		nodeConf := adapters.RandomNodeConfig()
...
func TestGetNodeIDs(t *testing.T) {
	...
	for _, node1 := range nodes {
		...
		if !match {
			t.Fatalf("A created node was not returned by GetNodes(), ID: %s",
				node1.ID().String())
		}
	}
	...
	for _, nodeID := range gotNodeIDsExcl {
		if bytes.Equal(excludeNodeID.Bytes(), nodeID.Bytes()) {
			t.Fatalf("GetNodeIDs returned the node ID we excluded, ID: %s",
				nodeID.String())
		}
	}
}
...
func TestGetNodes(t *testing.T) {
	...
	for _, node := range gotNodesExcl {
		if bytes.Equal(excludeNodeID.Bytes(), node.ID().Bytes()) {
			t.Fatalf("GetNodes returned the node we excluded, ID: %s",
				node.ID().String())
		}
	}
}
...
func TestGetNodesByID(t *testing.T) {
	...
	for _, node1 := range subsetNodes {
		...
		if !match {
			t.Fatalf("A created node was not returned by GetNodesByID(), ID: %s",
				node1.ID().String())
		}
	}
}
...
func TestGetNodesByProperty(t *testing.T) {
	...
	numPropertyNodes := 3
	propertyTest := "test"
	propertyNodes, err := createTestNodesWithProperty(propertyTest,
		numPropertyNodes, network)
	if err != nil {
	...
	if len(gotNodesByProperty) != numPropertyNodes {
		t.Fatalf("Expected %d nodes with a property, got %d", numPropertyNodes,
			len(gotNodesByProperty))
	}
	...
	for _, node1 := range propertyNodes {
		...
		if !match {
			t.Fatalf("A created node with property was not returned by GetNodesByProperty(), ID: %s",
				node1.ID().String())
		}
	}
}
...
func TestGetNodeIDsByProperty(t *testing.T) {
	...
	for _, node1 := range propertyNodes {
		...
		if !match {
			t.Fatalf("Not all nodes IDs were returned by GetNodeIDsByProperty(), ID: %s",
				id1.String())
		}
	}
}

func triggerChecks(ctx context.Context, ids []enode.ID, trigger chan enode.ID,
...
func casesNodeUnmarshalJSONConfigField() []nodeUnmarshalTestCase {
	...
	return []nodeUnmarshalTestCase{
		...
		},
		{
			...
			want: newNode(nil,
				&adapters.NodeConfig{Name: "node_ecdd0", Port: 44665}, false),
		},
	}
}

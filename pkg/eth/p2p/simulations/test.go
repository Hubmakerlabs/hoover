...
func VerifyRing(t *testing.T, net *Network, ids []enode.ID) {
	...
	for i := 0; i < n; i++ {
		for j := i + 1; j < n; j++ {
			...
			if i == j-1 || (i == 0 && j == n-1) {
				if c == nil {
					t.Errorf("nodes %v and %v are not connected, but they should be",
						i, j)
				}
			} else {
				if c != nil {
					t.Errorf("nodes %v and %v are connected, but they should not be",
						i, j)
				}
			}
		}
...
func VerifyFull(t *testing.T, net *Network, ids []enode.ID) {
	...
	want := n * (n - 1) / 2
	if connections != want {
		t.Errorf("wrong number of connections, got: %v, want: %v", connections,
			want)
	}
}

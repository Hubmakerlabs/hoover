...
func (t *Tracker) Track(peer string, version uint, reqCode uint64,
	resCode uint64, id uint64) {
	if !metrics.Enabled {
		return
	}
	t.lock.Lock()
	...
	if _, ok := t.pending[id]; ok {
		log.Error("Network request id collision", "protocol", t.protocol,
			"version", version, "code", reqCode, "id", id)
		return
	}
	...
	if pending := len(t.pending); pending >= maxTrackedPackets {
		log.Error("Request tracker exceeded allowance", "pending", pending,
			"peer", peer, "protocol", t.protocol, "version", version, "code",
			reqCode)
		return
	}
	...
	g := fmt.Sprintf("%s/%s/%d/%#02x", trackedGaugeName, t.protocol, version,
		reqCode)
...
func (t *Tracker) clean() {
	...
	for t.expire.Len() > 0 {
		...
		g := fmt.Sprintf("%s/%s/%d/%#02x", trackedGaugeName, t.protocol,
			req.version, req.reqCode)
		metrics.GetOrRegisterGauge(g, nil).Dec(1)

		m := fmt.Sprintf("%s/%s/%d/%#02x", lostMeterName, t.protocol,
			req.version, req.reqCode)
...
func (t *Tracker) schedule() {
	...
	t.wake = time.AfterFunc(time.Until(t.pending[t.expire.Front().Value.(uint64)].time.Add(t.timeout)),
		t.clean)
}
...
func (t *Tracker) Fulfil(peer string, version uint, code uint64, id uint64) {
	...
	req, ok := t.pending[id]
	if !ok {
		m := fmt.Sprintf("%s/%s/%d/%#02x", staleMeterName, t.protocol, version,
			code)
		metrics.GetOrRegisterMeter(m, nil).Mark(1)
		return
	}
	...
	if req.peer != peer || req.version != version || req.resCode != code {
		log.Warn("Network response id collision",
			...
			"want", fmt.Sprintf("%s:%s/%d:%d", peer, t.protocol, req.version,
				req.resCode),
		)
		return
	}
	...
	h := fmt.Sprintf("%s/%s/%d/%#02x", waitHistName, t.protocol, req.version,
		req.reqCode)
	...
	metrics.GetOrRegisterHistogramLazy(h, nil,
		sampler).Update(time.Since(req.time).Microseconds())
}

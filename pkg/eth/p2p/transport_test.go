...
func TestProtocolHandshake(t *testing.T) {
	var (
		...
		hs0     = &protoHandshake{Version: 3, ID: pub0,
			Caps: []Cap{{"a", 0}, {"b", 2}}}

		prv1, _ = crypto.GenerateKey()
		pub1    = crypto.FromECDSAPub(&prv1.PublicKey)[1:]
		hs1     = &protoHandshake{Version: 3, ID: pub1,
			Caps: []Cap{{"c", 1}, {"d", 3}}}
	...
	go func() {
		...
		if !reflect.DeepEqual(rpubkey, &prv1.PublicKey) {
			t.Errorf("dial side remote pubkey mismatch: got %v, want %v",
				rpubkey, &prv1.PublicKey)
			return
		}
	...
	go func() {
		...
		if !reflect.DeepEqual(rpubkey, &prv0.PublicKey) {
			t.Errorf("listen side remote pubkey mismatch: got %v, want %v",
				rpubkey, &prv0.PublicKey)
			return
		}
		...
		if err := ExpectMsg(rlpx, discMsg,
			[]DiscReason{DiscQuitting}); err != nil {
...
func TestProtocolHandshakeErrors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			code: handshakeMsg,
			msg:  []byte{1, 2, 3},
			err: newPeerError(errInvalidMsg,

...
func wrapStreamError(err error, typ reflect.Type) error {
	switch err {
	case ErrCanonInt:
		return &decodeError{msg: "non-canonical integer (leading zero bytes)",
			typ: typ}
	case ErrCanonSize:
...
func makeDecoder(typ reflect.Type, tags rlpstruct.Tags) (dec decoder,
	err error) {
	kind := typ.Kind()
	switch {
...
func decodeListArray(s *Stream, val reflect.Value, elemdec decoder) error {
	...
	if i < vlen {
		return &decodeError{msg: "input list has too few elements",
			typ: val.Type()}
	}
...
func makeNilPtrDecoder(etype reflect.Type, etypeinfo *typeinfo,
	ts rlpstruct.Tags) decoder {
	...
	return func(s *Stream, val reflect.Value) (err error) {
		...
		if kind != Byte && size == 0 {
			if kind != nilKind {
				return &decodeError{
					msg: fmt.Sprintf("wrong kind of empty value (got %v, want %v)",
						kind, nilKind),
					typ: typ,
				}
			}
...
func (s *Stream) ReadBytes(b []byte) error {
	...
	case String:
		if uint64(len(b)) != size {
			return fmt.Errorf("input value has wrong size %d, want %d", size,
				len(b))
		}
		if err = s.readFull(b); err != nil {
			return err

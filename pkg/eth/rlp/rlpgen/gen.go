...
import (
	...
	"github.com/Hubmakerlabs/hoover/pkg/eth/rlp/internal/rlpstruct"
)
...
func (op basicOp) genDecode(ctx *genContext) (string, string) {
	...
	if op.decodeNeedsConversion() {
		conv := ctx.temp()
		fmt.Fprintf(&b, "%s := %s(%s)\n", conv,
			types.TypeString(op.typ, ctx.qualify), resultV)
		result = conv
	}
...
func (bctx *buildContext) makeByteArrayOp(name *types.Named,
	typ *types.Array) byteArrayOp {
	nt := types.Type(name)
	if name == nil {
		nt = typ
	}
...
func (op byteArrayOp) genDecode(ctx *genContext) (string, string) {
	...
	fmt.Fprintf(&b, "var %s %s\n", resultV,
		types.TypeString(op.name, ctx.qualify))
	fmt.Fprintf(&b,
		"if err := dec.ReadBytes(%s[:]); err != nil { return err }\n", resultV)
...
func (op uint256Op) genDecode(ctx *genContext) (string, string) {
	...
	fmt.Fprintf(&b,
		"if err := dec.ReadUint256(&%s); err != nil { return err }\n", resultV)
...
func (op encoderDecoderOp) genDecode(ctx *genContext) (string, string) {
	...
	fmt.Fprintf(&b, "%s := new(%s)\n", resultV,
		types.TypeString(etyp, ctx.qualify))
...
func (bctx *buildContext) makePtrOp(elemTyp types.Type,
	tags rlpstruct.Tags) (op, error) {
...
func (op ptrOp) genDecode(ctx *genContext) (string, string) {
	...
	var b bytes.Buffer
	fmt.Fprintf(&b, "var %s %s\n", resultV,
		types.TypeString(types.NewPointer(op.elemTyp), ctx.qualify))
...
func (bctx *buildContext) makeStructOp(named *types.Named,
	typ *types.Struct) (op, error) {
...
func (op structOp) writeOptionalFields(b *bytes.Buffer, ctx *genContext,
	v string) {
	...
	for i, field := range op.optionalFields {
		...
		zeroV[i] = ctx.temp()
		fmt.Fprintf(b, "%s := %s\n", zeroV[i],
			nonZeroCheck(selector, field.typ, ctx.qualify))
	}
...
func (op structOp) decodeOptionalFields(b *bytes.Buffer, ctx *genContext,
	resultV string) {
...
func (op sliceOp) genDecode(ctx *genContext) (string, string) {
	...
	var b bytes.Buffer
	fmt.Fprintf(&b, "var %s %s\n", sliceV,
		types.TypeString(op.typ, ctx.qualify))
	fmt.Fprintf(&b, "if _, err := dec.List(); err != nil { return err }\n")
...
func (bctx *buildContext) makeOp(name *types.Named, typ types.Type,
	tags rlpstruct.Tags) (op, error) {
	switch typ := typ.(type) {
	case *types.Named:
		if isBigInt(typ) {
		...
		if bctx.isDecoder(typ) {
			return nil, fmt.Errorf("type %v implements rlp.Decoder with non-pointer receiver",
				typ)
		}
	...
	case *types.Pointer:
		...
		if bctx.isEncoder(typ) {
			...
			return nil, fmt.Errorf("type %v implements rlp.Encoder but not rlp.Decoder",
				typ)
		}
		if bctx.isDecoder(typ) {
			return nil, fmt.Errorf("type %v implements rlp.Decoder but not rlp.Encoder",
				typ)
		}
...
func (bctx *buildContext) generate(typ *types.Named,
	encoder, decoder bool) ([]byte, error) {
	bctx.topType = typ

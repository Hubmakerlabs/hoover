...
func TestClientRequest(t *testing.T) {
	...
	var resp echoResult
	if err := client.Call(&resp, "test_echo", "hello", 10,
		&echoArgs{"world"}); err != nil {
		t.Fatal(err)
	}
...
func TestClientResponseType(t *testing.T) {
	...
	if err := client.Call(nil, "test_echo", "hello", 10,
		&echoArgs{"world"}); err != nil {
		t.Errorf("Passing nil as result should be fine, but got an error: %v",
			err)
	}
...
func TestClientErrorData(t *testing.T) {
	...
	if e, ok := err.(DataError); !ok {
		...
		t.Fatalf("wrong error data %#v, want %#v", e.ErrorData(),
			testError{}.ErrorData())
	}
}
...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error: &jsonError{Code: -32601,
				Message: "the method no_such_method does not exist/is not available"},
		},
	}
...
func TestClientBatchRequest_len(t *testing.T) {
	b, err := json.Marshal([]jsonrpcMessage{
		...
		{Version: "2.0", ID: json.RawMessage("2"),
			Result: json.RawMessage(`"0x2"`)},
	})
	if err != nil {
		t.Fatal("failed to encode jsonrpc message:", err)
	}
	s := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter,
		r *http.Request) {
		_, err := rw.Write(b)
		if err != nil {
	...
	t.Run("too-few", func(t *testing.T) {
		...
		for i, elem := range batch[:2] {
			if elem.Error != nil {
				t.Errorf("expected no error for batch element %d, got %q", i,
					elem.Error)
			}
		}
		for i, elem := range batch[2:] {
...
func TestClientNotify(t *testing.T) {
	...
	if err := client.Notify(context.Background(), "test_echo", "hello", 10,
		&echoArgs{"world"}); err != nil {
		t.Fatal(err)
	}
}
	...
	caller := func(index int) {
		...
		for i := 0; i < nreqs; i++ {
			...
			case err == nil:
				...
				t.Errorf("no error for call with %v wait time (deadline: %v)",
					timeout, hasDeadline)
				// default:
...
func TestClientSubscribe(t *testing.T) {
	...
	nc := make(chan int)
	count := 10
	sub, err := client.Subscribe(context.Background(), "nftest", nc,
		"someSubscription", count, 0)
	if err != nil {
	...
	case err := <-sub.Err():
		if err != nil {
			t.Fatalf("Err returned a non-nil error after explicit unsubscribe: %q",
				err)
		}
...
func TestClientSubscribeClose(t *testing.T) {
	...
	go func() {
		sub, err = client.Subscribe(context.Background(), "nftest2", nc,
			"hangSubscription", 999)
		errc <- err
	}()
...
// This test reproduces https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/17837 where the
...
func TestClientSubscriptionUnsubscribeServer(t *testing.T) {
	...
	client, _ := newClient(context.Background(), cfg,
		func(context.Context) (ServerCodec, error) {
	...
	ch := make(chan int)
	sub, err := client.Subscribe(context.Background(), "nftest", ch,
		"someSubscription", 1, 1)
	if err != nil {
		t.Fatal(err)
	}
...
// It is the reproducer for https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/22322
func TestClientSubscriptionChannelClose(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		sub, err := client.Subscribe(context.Background(), "nftest", ch,
			"someSubscription", maxClientSubscriptionBuffer-1, 1)
		if err != nil {
			t.Fatal(err)
		}
		sub.Unsubscribe()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.Subscribe(ctx, "nftest", nc, "someSubscription",
			count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
		...
		for i := 0; i < count; i++ {
			...
			case err := <-sub.Err():
				if wantError && err != ErrSubscriptionQueueOverflow {
					t.Fatalf("(%d/%d) got error %q, want %q", i, count, err,
...
func TestClientSetHeader(t *testing.T) {
	var gotHeader bool
	srv := newTestServer()
	httpsrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,
		r *http.Request) {
...
func TestClientHTTP(t *testing.T) {
	...
	for i := range results {
		...
		go func() {
			errc <- client.Call(&results[i], "test_echo", wantResult.String,
				wantResult.Int, wantResult.Args)
		}()
	}
	...
	for i := range results {
		if !reflect.DeepEqual(results[i], wantResult) {
			t.Errorf("result %d mismatch: got %#v, want %#v", i, results[i],
				wantResult)
		}
	}
}
...
func TestClientReconnect(t *testing.T) {
	...
	var resp echoResult
	if err := client.CallContext(ctx, &resp, "test_echo", "", 1,
		nil); err != nil {
		t.Fatal(err)
	}
	...
	if err := client.CallContext(ctx, &resp, "test_echo", "", 2,
		nil); err == nil {
...
func httpTestClient(srv *Server, transport string, fl *flakeyListener) (*Client,
	*httptest.Server) {
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-ethereum-test-ipc-%d-%d", os.Getpid(),
		rand.Int63())
...
func (l *flakeyListener) Accept() (net.Conn, error) {
	...
	if err == nil {
		...
		time.AfterFunc(timeout, func() {
			log.Debug(fmt.Sprintf("killing conn %v after %v", c.LocalAddr(),
				timeout))
			c.Close()
		})
	}

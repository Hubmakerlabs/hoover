...
func newHandler(connCtx context.Context, conn jsonWriter, idgen func() ID,
	reg *serviceRegistry,
	batchRequestLimit, batchResponseMaxSize int) *handler {
	...
	h.unsubscribeCb = newCallback(reflect.Value{},
		reflect.ValueOf(h.unsubscribe))
	return h
}
...
func (b *batchCallBuffer) respondWithError(ctx context.Context, conn jsonWriter,
	err error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
...
func (h *handler) handleBatch(msgs []*jsonrpcMessage) {
	...
	h.startCallProc(func(cp *callProc) {
		var (
			...
			callBuffer = &batchCallBuffer{calls: calls,
		...
		for {
			...
			if resp != nil && h.batchResponseMaxSize != 0 {
				...
				if responseBytes > h.batchResponseMaxSize {
					err := &internalServerError{errcodeResponseTooLarge,
...
func (h *handler) respondWithBatchTooLarge(cp *callProc,
...
func (h *handler) handleNonBatchCall(cp *callProc, msg *jsonrpcMessage) {
	...
	if timeout, ok := ContextRequestTimeout(cp.ctx); ok {
		timer = time.AfterFunc(timeout, func() {
			cancel()
			responded.Do(func() {
				resp := msg.errorResponse(&internalServerError{errcodeTimeout,
					errMsgTimeout})
				h.conn.writeJSON(cp.ctx, resp, true)
			})
		})
...
func (h *handler) handleResponses(batch []*jsonrpcMessage,
	handleCall func(*jsonrpcMessage)) {
	...
	for _, msg := range batch {
		...
		case msg.isResponse():
			handleResp(msg)
			h.log.Trace("Handled RPC response", "reqid", idForLog{msg.ID},
...
func (h *handler) handleCallMsg(ctx *callProc,
	...
	case msg.isCall():
		...
		var ctx []interface{}
		ctx = append(ctx, "reqid", idForLog{msg.ID}, "duration",
			time.Since(start))
		if resp.Error != nil {
...
func (h *handler) handleCall(cp *callProc,
	msg *jsonrpcMessage) *jsonrpcMessage {
	if msg.isSubscribe() {
	...
	if callb != h.unsubscribeCb {
		...
		updateServeTimeHistogram(msg.Method, answer.Error == nil,
			time.Since(start))
	}
...
func (h *handler) handleSubscribe(cp *callProc,
	msg *jsonrpcMessage) *jsonrpcMessage {
	if !h.allowSubscribe {
...
func (h *handler) runMethod(ctx context.Context, msg *jsonrpcMessage,
	callb *callback, args []reflect.Value) *jsonrpcMessage {

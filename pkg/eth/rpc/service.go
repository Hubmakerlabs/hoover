...
func (r *serviceRegistry) registerName(name string, rcvr interface{}) error {
	...
	if len(callbacks) == 0 {
		return fmt.Errorf("service %T doesn't have any suitable methods/subscriptions to expose",
			rcvr)
	}
...
func (c *callback) call(ctx context.Context, method string,
	args []reflect.Value) (res interface{}, errRes error) {
	...
	defer func() {
		if err := recover(); err != nil {
			...
			log.Error("RPC method " + method + " crashed: " + fmt.Sprintf("%v\n%s",
				err, buf))
			errRes = &internalServerError{errcodePanic,

...
func originIsAllowed(allowedOrigins mapset.Set[string],
	browserOrigin string) bool {
...
func ruleAllowsOrigin(allowedOrigin string, browserOrigin string) bool {
	...
	if err != nil {
		log.Warn("Error parsing allowed origin specification", "spec",
			allowedOrigin, "error", err)
		return false
	}
	browserScheme, browserHostname, browserPort, err = parseOriginURL(browserOrigin)
	if err != nil {
		log.Warn("Error parsing browser 'Origin' field", "Origin",
			browserOrigin, "error", err)
		return false
	}
...
func DialWebsocketWithDialer(ctx context.Context, endpoint, origin string,
...
// The context is used for the initial connection establishment. It does not
// affect subsequent interactions with the client.
func DialWebsocket(ctx context.Context, endpoint, origin string) (*Client,
	error) {
	cfg := new(clientConfig)
	if origin != "" {
		cfg.setHeader("origin", origin)
...
func newClientTransportWS(endpoint string, cfg *clientConfig) (reconnectFunc,
	error) {
	dialer := cfg.wsDialer
	if dialer == nil {
...
func newWebsocketCodec(conn *websocket.Conn, host string, req http.Header,
	readLimit int64) ServerCodec {
...
func (wc *websocketCodec) writeJSON(ctx context.Context, v interface{},
	isError bool) error {

	...
	testLimit := func(limit *int64) {
		...
		if overLimit > 0 {
			...
			if err == nil || err != websocket.ErrReadLimit {
				t.Fatalf("wrong error with limit %d: %v expecting %v", expLimit,
...
func TestClientWebsocketPing(t *testing.T) {
	...
	// This is important for reproducing https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn,
	shutdown, sendPing <-chan struct{}) {

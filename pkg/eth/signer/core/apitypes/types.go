...
type SendTxArgs struct {
	...
	// Issue detail: https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/15628
...
func TypedDataAndHash(typedData TypedData) ([]byte, string, error) {
	domainSeparator, err := typedData.HashStruct("EIP712Domain",
		typedData.Domain.Map())
	if err != nil {
		return nil, "", err
	}
	typedDataHash, err := typedData.HashStruct(typedData.PrimaryType,
		typedData.Message)
	if err != nil {
		return nil, "", err
	}
	rawData := fmt.Sprintf("\x19\x01%s%s", string(domainSeparator),
		string(typedDataHash))
	return crypto.Keccak256([]byte(rawData)), rawData, nil
}

// HashStruct generates a keccak256 hash of the encoding of the provided data
func (typedData *TypedData) HashStruct(primaryType string,
	data TypedDataMessage) (hexutil.Bytes, error) {
...
func (typedData *TypedData) Dependencies(primaryType string,
	found []string) []string {
...
func (typedData *TypedData) EncodeData(primaryType string,
	data map[string]interface{}, depth int) (hexutil.Bytes, error) {
	...
	if exp, got := len(typedData.Types[primaryType]), len(data); exp < got {
		return nil, fmt.Errorf("there is extra data provided in the message (%d < %d)",
			exp, got)
	}
		...
		if encType[len(encType)-1:] == "]" {
			...
			for _, item := range arrayValue {
				if typedData.Types[parsedType] != nil {
					...
					encodedData, err := typedData.EncodeData(parsedType,
						mapValue, depth+1)
					if err != nil {
						return nil, err
					}
					...
					bytesValue, err := typedData.EncodePrimitiveValue(parsedType,
						item, depth)
					if err != nil {
						return nil, err
					}
			...
			byteValue, err := typedData.EncodePrimitiveValue(encType, encValue,
				depth)
			if err != nil {
				return nil, err
			}
...
func parseInteger(encType string, encValue interface{}) (*big.Int, error) {
	...
	case float64:
		...
		if float64(int64(v)) == v {
			...
			return nil, fmt.Errorf("invalid float value %v for type %v", v,
				encType)
		}
	}
	if b == nil {
		return nil, fmt.Errorf("invalid integer value %v/%v for type %v",
			encValue, reflect.TypeOf(encValue), encType)
	}
	if b.BitLen() > length {
		return nil, fmt.Errorf("integer larger than '%v'", encType)
	}
	if !signed && b.Sign() == -1 {
		return nil, fmt.Errorf("invalid negative value for unsigned type %v",
			encType)
	}
	return b, nil
}
...
func (typedData *TypedData) EncodePrimitiveValue(encType string,
	encValue interface{}, depth int) ([]byte, error) {
	switch encType {
	case "address":
		retval := make([]byte, 32)
...
func dataMismatchError(encType string, encValue interface{}) error {
	return fmt.Errorf("provided data '%v' doesn't match type '%s'", encValue,
		encType)
}
...
func convertDataToSlice(encValue interface{}) ([]interface{}, error) {
	...
	if rv.Kind() == reflect.Slice {
		...
		return outEncValue, fmt.Errorf("provided data '%v' is not slice",
			encValue)
	}
...
func (typedData *TypedData) formatData(primaryType string,
	data map[string]interface{}) ([]*NameValueType, error) {
		...
		if field.isArray() {
			...
			for _, v := range arrayValue {
				if typedData.Types[parsedType] != nil {
					...
					primitiveOutput, err := formatPrimitiveValue(field.Type,
						encValue)
					if err != nil {
						return nil, err
					}
...
func formatPrimitiveValue(encType string, encValue interface{}) (string,
	...
	case "address":
		if stringValue, ok := encValue.(string); !ok {
			return "", fmt.Errorf("could not format value %v as address",
				encValue)
		} else {
	...
	for typeKey, typeArr := range t {
		...
		for i, typeObj := range typeArr {
			...
			if typeKey == typeObj.Type {
				return fmt.Errorf("type %q cannot reference itself",
					typeObj.Type)
			}
			...
			if _, exist := t[typeObj.typeName()]; !exist {
				return fmt.Errorf("reference type %q is undefined",
					typeObj.Type)
			}
...
func isPrimitiveTypeValid(primitiveType string) bool {
	...
	for n := 1; n <= 32; n++ {
		...
		if primitiveType == fmt.Sprintf("bytes%d",
			n) || primitiveType == fmt.Sprintf("bytes%d[]", n) {
			return true
		}
	}
	// For 'intN','intN[]' and 'uintN','uintN[]' we allow N in increments of 8, from 8 up to 256
	for n := 8; n <= 256; n += 8 {
		if primitiveType == fmt.Sprintf("int%d",
			n) || primitiveType == fmt.Sprintf("int%d[]", n) {
			return true
		}
		if primitiveType == fmt.Sprintf("uint%d",
			n) || primitiveType == fmt.Sprintf("uint%d[]", n) {
			return true
		}
	}
	return false

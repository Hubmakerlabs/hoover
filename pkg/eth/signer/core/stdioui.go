...
func (ui *StdIOUI) dispatch(serviceMethod string, args interface{},
...
func (ui *StdIOUI) ApproveSignData(request *SignDataRequest) (SignDataResponse,
	error) {
...
func (ui *StdIOUI) ApproveNewAccount(request *NewAccountRequest) (NewAccountResponse,
	error) {
...
func (ui *StdIOUI) ShowError(message string) {
	...
	if err != nil {
		log.Info("Error calling 'ui_showError'", "exc", err.Error(), "msg",
			message)
	}
}
...
func (ui *StdIOUI) ShowInfo(message string) {
	...
	if err != nil {
		log.Info("Error calling 'ui_showInfo'", "exc", err.Error(), "msg",
			message)
	}
}
func (ui *StdIOUI) OnApprovedTx(tx ethapi.SignTransactionResult) {
	...
	if err != nil {
		log.Info("Error calling 'ui_onApprovedTx'", "exc", err.Error(), "tx",
			tx)
	}
}
...
func (ui *StdIOUI) OnSignerStartup(info StartupInfo) {
	...
	if err != nil {
		log.Info("Error calling 'ui_onSignerStartup'", "exc", err.Error(),
			"info", info)
	}
}
func (ui *StdIOUI) OnInputRequired(info UserInputRequest) (UserInputResponse,
	error) {
	var result UserInputResponse
	err := ui.dispatch("ui_onInputRequired", info, &result)
	if err != nil {
		log.Info("Error calling 'ui_onInputRequired'", "exc", err.Error(),
			"info", info)
	}
	return result, err
}

...
func (s *UIServerAPI) ListAccounts(ctx context.Context) ([]accounts.Account,
	error) {
...
func (s *UIServerAPI) DeriveAccount(url string, path string,
...
func (s *UIServerAPI) ImportRawKey(privkey string,
	...
	if err := ValidatePasswordFormat(password); err != nil {
		return accounts.Account{}, fmt.Errorf("password requirements not met: %v",
			err)
	}
	// No error
...
func (s *UIServerAPI) Export(ctx context.Context,
	addr common.Address) (json.RawMessage, error) {
...
func (api *UIServerAPI) Import(ctx context.Context, keyJSON json.RawMessage,
	oldPassphrase, newPassphrase string) (accounts.Account, error) {
	...
	return be[0].(*keystore.KeyStore).Import(keyJSON, oldPassphrase,
		newPassphrase)
}

...
func (db *Database) ValidateTransaction(selector *string,
	...
	if tx.To == nil {
		...
		// e.g. https://github.com/Hubmakerlabs/hoover/pkg/eth/issues/16106.
		if len(data) == 0 {
			...
			messages.Warn(fmt.Sprintf("Transaction will create a contract, but the payload is suspiciously small (%d bytes)",
				len(data)))
		}
...
func (db *Database) ValidateCallData(selector *string, data []byte,
	...
	if n := len(data) - 4; n%32 != 0 {
		messages.Warn(fmt.Sprintf("Transaction data is not valid ABI (length should be a multiple of 32 (was %d))",
			n))
	}
	// If a custom method selector was provided, validate with that
	if selector != nil {
		if info, err := verifySelector(*selector, data); err != nil {
			messages.Warn(fmt.Sprintf("Transaction contains data, but provided ABI signature could not be matched: %v",
				err))
		} else {
			messages.Info(fmt.Sprintf("Transaction invokes the following method: %q",
				info.String()))
	...
	if err != nil {
		messages.Warn(fmt.Sprintf("Transaction contains data, but the ABI signature could not be found: %v",
			err))
		return
	}
	if info, err := verifySelector(embedded, data); err != nil {
		messages.Warn(fmt.Sprintf("Transaction contains data, but provided ABI signature could not be verified: %v",
			err))
	} else {
		messages.Info(fmt.Sprintf("Transaction invokes the following method: %q",
			info.String()))

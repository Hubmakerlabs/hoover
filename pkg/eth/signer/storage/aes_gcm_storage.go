...
func (s *AESEncryptedStorage) Put(key, value string) {
	...
	if err != nil {
		log.Warn("Failed to read encrypted storage", "err", err, "file",
			s.filename)
		return
	}
...
func (s *AESEncryptedStorage) readEncryptedStorage() (map[string]storedCredential,
	error) {
	...
	if err = json.Unmarshal(raw, &creds); err != nil {
		log.Warn("Failed to unmarshal encrypted storage", "err", err, "file",
			s.filename)
		return nil, err
	}
	return creds, nil
...
func encrypt(key []byte, plaintext []byte, additionalData []byte) ([]byte,
	[]byte, error) {
...
func decrypt(key []byte, nonce []byte, ciphertext []byte,

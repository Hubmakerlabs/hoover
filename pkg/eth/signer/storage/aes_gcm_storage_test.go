...
func TestEncryption(t *testing.T) {
	...
	if !bytes.Equal(plaintext, p) {
		t.Errorf("Failed: expected plaintext recovery, got %v expected %v",
			string(plaintext), string(p))
	}
}
	...
	for k, v := range a {
		if v2, exist := creds[k]; !exist {
			...
			if !bytes.Equal(v.CipherText, v2.CipherText) {
				t.Errorf("Wrong ciphertext, expected %x got %x", v.CipherText,
					v2.CipherText)
			}
...
func TestEnd2End(t *testing.T) {
	log.Root().SetHandler(log.LvlFilterHandler(log.Lvl(3),
	...
	if v, err := s2.Get("bazonk"); v != "foobar" || err != nil {
		t.Errorf("Expected bazonk->foobar (nil error), got '%v' (%v error)", v,
			err)
	}
}

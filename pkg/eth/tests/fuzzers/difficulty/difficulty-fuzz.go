...
func (f *fuzzer) fuzz() int {
	...
	// Number can range between 0 and up to 32 bytes (but not so that the child exceeds it)
	{
		...
		number := new(big.Int).SetBytes(f.readSlice(0,
			4)) // 4 bytes: 32 bits: block num max 4 billion
	...
	for i, pair := range []struct {
		...
		{ethash.FrontierDifficultyCalculator,
			ethash.CalcDifficultyFrontierU256},
		{ethash.HomesteadDifficultyCalculator,
			ethash.CalcDifficultyHomesteadU256},
		{ethash.DynamicDifficultyCalculator(bombDelay),
			ethash.MakeDifficultyCalculatorU256(bombDelay)},
	} {

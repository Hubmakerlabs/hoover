...
import (
	...
	l "github.com/Hubmakerlabs/hoover/pkg/eth/les"
...
func makechain() (bc *core.BlockChain, addresses []common.Address,
	txHashes []common.Hash) {
	gspec := &core.Genesis{
	...
	_, blocks, _ := core.GenerateChainWithGenesis(gspec, ethash.NewFaker(),
		testChainLen,
		func(i int, gen *core.BlockGen) {
			...
			nonce := uint64(i)
			if i%4 == 0 {
				tx, _ = types.SignTx(types.NewContractCreation(nonce,
					big.NewInt(0), 200000, big.NewInt(params.GWei),
					testContractCode), signer, bankKey)
				addr = crypto.CreateAddress(bankAddr, nonce)
			} else {
				addr = common.BigToAddress(big.NewInt(int64(i)))
				tx, _ = types.SignTx(types.NewTransaction(nonce, addr,
					big.NewInt(10000), params.TxGas, big.NewInt(params.GWei),
					nil), signer, bankKey)
			}
			gen.AddTx(tx)
	...
	bc, _ = core.NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil,
...
func makeTries() (chtTrie *trie.Trie, bloomTrie *trie.Trie,
	chtKeys, bloomKeys [][]byte) {
...
func newFuzzer(input []byte) *fuzzer {
	...
	txpool, _ := txpool.New(new(big.Int).SetUint64(legacypool.DefaultConfig.PriceLimit),
...
func fuzz(input []byte) int {
	...
	for !f.exhausted {
		...
		case 1:
			req := &l.GetBlockBodiesPacket{Hashes: make([]common.Hash,
				f.randomInt(l.MaxBodyFetch+1))}
		...
		case 2:
			req := &l.GetCodePacket{Reqs: make([]l.CodeReq,
				f.randomInt(l.MaxCodeFetch+1))}
		...
		case 3:
			req := &l.GetReceiptsPacket{Hashes: make([]common.Hash,
				f.randomInt(l.MaxReceiptFetch+1))}
			for i := range req.Hashes {
				req.Hashes[i] = f.randomBlockHash()
			}
		...
		case 4:
			req := &l.GetProofsPacket{Reqs: make([]l.ProofReq,
				f.randomInt(l.MaxProofsFetch+1))}
			for i := range req.Reqs {
				if f.randomBool() {
		...
		case 5:
			req := &l.GetHelperTrieProofsPacket{Reqs: make([]l.HelperTrieReq,
				f.randomInt(l.MaxHelperTrieProofsFetch+1))}
			for i := range req.Reqs {
		...
		case 6:
			req := &l.SendTxPacket{Txs: make([]*types.Transaction,
				f.randomInt(l.MaxTxSend+1))}
			signer := types.HomesteadSigner{}
			for i := range req.Txs {
				var nonce uint64
				if f.randomBool() {
				...
				req.Txs[i], _ = types.SignTx(types.NewTransaction(nonce,
					common.Address{}, big.NewInt(10000), params.TxGas,
					big.NewInt(1000000000*int64(f.randomByte())), nil), signer,
					bankKey)
			}
			f.doFuzz(l.SendTxV2Msg, req)

		case 7:
			req := &l.GetTxStatusPacket{Hashes: make([]common.Hash,
				f.randomInt(l.MaxTxStatus+1))}
			for i := range req.Hashes {

...
func (f *fuzzer) fuzz() int {
	...
	var (
		...
		options = trie.NewStackTrieOptions().WithWriter(func(path []byte,
			hash common.Hash, blob []byte) {
			rawdb.WriteTrieNode(spongeB, common.Hash{}, path, hash, blob,
				dbB.Scheme())
		})
	...
	if nodes != nil {
		dbA.Update(rootA, types.EmptyRootHash, 0,
	...
	trieB.Commit()
	if rootA != rootB {
		panic(fmt.Sprintf("roots differ: (trie) %x != %x (stacktrie)", rootA,
			rootB))
	}
	sumA := spongeA.sponge.Sum(nil)
	sumB := spongeB.sponge.Sum(nil)
	if !bytes.Equal(sumA, sumB) {
		panic(fmt.Sprintf("sequence differ: (trie) %x != %x (stacktrie)", sumA,
			sumB))
	}
	...
	var (
		...
		optionsC = trie.NewStackTrieOptions().WithWriter(func(path []byte,
			hash common.Hash, blob []byte) {

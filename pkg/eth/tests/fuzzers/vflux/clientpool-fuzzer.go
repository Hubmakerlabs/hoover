...
func (p *clientPeer) UpdateCapacity(newCap uint64, requested bool) {
	...
	doLog("Update capacity", "peer", p.node.ID(), "origin", origin, "cap",
		newCap, "origintotal", originTotal, "total", p.fuzzer.activeCap,
		"requested", requested)
}
...
func (p *clientPeer) Disconnect() {
	...
	p.capacity = 0
	p.balance = nil
	doLog("Disconnect", "peer", p.node.ID(), "origin", origin, "origintotal",
		originTotal, "total", p.fuzzer.activeCap)
}
...
func fuzzClientPool(input []byte) int {
	...
	for !f.exhausted && count < 1000 {
		...
		case 2:
			...
			doLog("Set limits", "maxcount", f.maxCount, "maxcap", f.maxCap,
				"origincount", count, "oricap", cap)
		case 3:
			bias := f.randomDelay()
		...
		case 6:
			...
			doLog("Set capacity", "id", f.peers[index].node.ID(), "reqcap",
				reqCap, "bias", bias, "requested", requested)
		case 7:
			index := f.randomByte()
		...
		case 8:
			index := f.randomByte()
			pool.BalanceOperation(f.peers[index].node.ID(),
				f.peers[index].freeID, func(balance vfs.AtomicBalanceOperator) {
					count := f.randomInt(4)
					for i := 0; i < count; i++ {
		...
		case 9:
			...
			pool.Active()
			pool.Limits()
			pool.GetPosBalanceIDs(f.peers[f.randomByte()].node.ID(),
				f.peers[f.randomByte()].node.ID(), f.randomInt(100))
		case 10:
			req := vflux.CapacityQueryReq{
				Bias: uint64(f.randomByte()),
				AddTokens: make([]vflux.IntOrInf,
					f.randomInt(vflux.CapacityQueryMaxLen+1)),
			}
			...
			pool.Handle(f.peers[f.randomByte()].node.ID(),
				f.peers[f.randomByte()].freeID, vflux.CapacityQueryName, reqEnc)
		}
		...
		doLog("Clientpool stats in fuzzer", "count", f.activeCap, "maxcount",
			f.maxCount, "cap", f.activeCap, "maxcap", f.maxCap)
		activeCount, activeCap := pool.Active()
		doLog("Clientpool stats in pool", "count", activeCount, "cap",
			activeCap)

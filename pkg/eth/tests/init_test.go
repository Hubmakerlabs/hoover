...
var (
	...
	legacyStateTestDir = filepath.Join(baseDir, "LegacyTests", "Constantinople",
		"GeneralStateTests")
...
func (tm *testMatcher) fails(pattern string, reason string) {
	...
	tm.failpat = append(tm.failpat,
		testFailure{regexp.MustCompile(pattern), reason})
}
...
func (tm *testMatcher) config(pattern string, cfg params.ChainConfig) {
	tm.configpat = append(tm.configpat,
		testConfig{regexp.MustCompile(pattern), cfg})
}
...
func (tm *testMatcher) walk(t *testing.T, dir string, runTest interface{}) {
	...
	if os.IsNotExist(err) || !dirinfo.IsDir() {
		fmt.Fprintf(os.Stderr,
			"can't find test files in %s, did you clone the tests submodule?\n",
			dir)
		t.Skip("missing test files")
	}
	err = filepath.Walk(dir,
		func(path string, info os.FileInfo, err error) error {
			name := filepath.ToSlash(strings.TrimPrefix(path,
...
func (tm *testMatcher) runTestFile(t *testing.T, path, name string,
	runTest interface{}) {
...
func makeMapFromTestFunc(f interface{}) reflect.Value {
	...
	if ftyp.Kind() != reflect.Func || ftyp.NumIn() != 3 || ftyp.NumOut() != 0 || ftyp.In(0) != testingT || ftyp.In(1) != stringT {
		panic(fmt.Sprintf("bad test function type: want func(*testing.T, string, <TestType>), have %s",
			ftyp))
	}
	testType := ftyp.In(2)
...
func runTestFunc(runTest interface{}, t *testing.T, name string,
	m reflect.Value, key string) {

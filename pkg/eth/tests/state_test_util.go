...
func GetChainConfig(forkString string) (baseConfig *params.ChainConfig,
	eips []int, err error) {
	var (
	...
	for _, eip := range eipsStrings {
		if eipNum, err := strconv.Atoi(eip); err != nil {
			return nil, nil, fmt.Errorf("syntax error, invalid eip number %v",
				eipNum)
		} else {
...
func (t *StateTest) Run(subtest StateSubtest, vmconfig vm.Config,
	snapshotter bool, scheme string,
	postCheck func(err error, snaps *snapshot.Tree,
		state *state.StateDB)) (result error) {
	triedb, snaps, statedb, root, err := t.RunNoVerify(subtest, vmconfig,
		snapshotter, scheme)
	...
	if root != common.Hash(post.Root) {
		return fmt.Errorf("post state root mismatch: got %x, want %x", root,
			post.Root)
	}
	if logs := rlpHash(statedb.Logs()); logs != common.Hash(post.Logs) {
		return fmt.Errorf("post state logs hash mismatch: got %x, want %x",
			logs, post.Logs)
	}
...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config,
	snapshotter bool, scheme string) (*trie.Database, *snapshot.Tree,
	*state.StateDB, common.Hash, error) {
	...
	triedb, snaps, statedb := MakePreState(rawdb.NewMemoryDatabase(),
		t.json.Pre, snapshotter, scheme)
	...
	if len(post.TxBytes) != 0 {
		...
		if _, err := types.Sender(types.LatestSigner(config),
			&ttx); err != nil {
			triedb.Close()
			return nil, nil, nil, common.Hash{}, err
		}
	...
	root, _ := statedb.Commit(block.NumberU64(),
...
func MakePreState(db ethdb.Database, accounts core.GenesisAlloc,
	snapshotter bool, scheme string) (*trie.Database, *snapshot.Tree,
	*state.StateDB) {
...
func (tx *stTransaction) toMessage(ps stPostState,
	baseFee *big.Int) (*core.Message, error) {
	...
	if ps.Indexes.Data > len(tx.Data) {
		return nil, fmt.Errorf("tx data index %d out of bounds",
			ps.Indexes.Data)
	}
	if ps.Indexes.Value > len(tx.Value) {
		return nil, fmt.Errorf("tx value index %d out of bounds",
			ps.Indexes.Value)
	}
	if ps.Indexes.Gas > len(tx.GasLimit) {
		return nil, fmt.Errorf("tx gas limit index %d out of bounds",
			ps.Indexes.Gas)
	}
	...
	if baseFee != nil {
		...
		gasPrice = math.BigMin(new(big.Int).Add(tx.MaxPriorityFeePerGas,
			baseFee),
			tx.MaxFeePerGas)
	}
	if gasPrice == nil {

...
func TestIterator(t *testing.T) {
	...
	for k, v := range all {
		if found[k] != v {
			t.Errorf("iterator value mismatch for %s: got %q want %q", k,
				found[k], v)
		}
	}
}
...
func testNodeIteratorCoverage(t *testing.T, scheme string) {
	...
	for _, element := range elements {
		if blob, err := reader.Node(common.Hash{}, element.path,
			element.hash); err != nil {
			t.Errorf("failed to retrieve reported node %x: %v", element.hash,
				err)
		} else if !bytes.Equal(blob, element.blob) {
			t.Errorf("node blob is different, want %v got %v", element.blob,
				blob)
		}
	}
	var (
	...
	for it.Next() {
		...
		if elem, ok := elements[crypto.Keccak256Hash(it.Value())]; !ok {
			...
			t.Errorf("node blob is different, want %v got %v", elem.blob,
				it.Value())
		}
	}
	it.Release()
...
func checkIteratorOrder(want []kvs, it *Iterator) error {
	for it.Next() {
		if len(want) == 0 {
			return fmt.Errorf("didn't expect any more values, got key %q",
				it.Key)
		}
...
func TestDifferenceIterator(t *testing.T) {
	...
	found := make(map[string]string)
	di, _ := NewDifferenceIterator(triea.MustNodeIterator(nil),
		trieb.MustNodeIterator(nil))
	it := NewIterator(di)
	for it.Next() {
		found[string(it.Key)] = string(it.Value)
	}
	...
	for _, item := range all {
		if found[item.k] != item.v {
			t.Errorf("iterator value mismatch for %s: got %v want %v", item.k,
				found[item.k], item.v)
		}
	}
	if len(found) != len(all) {
		t.Errorf("iterator count mismatch: got %d values, want %d", len(found),
			len(all))
	}
}
...
func TestUnionIterator(t *testing.T) {
	...
	di, _ := NewUnionIterator([]NodeIterator{triea.MustNodeIterator(nil),
		trieb.MustNodeIterator(nil)})
	it := NewIterator(di)
	...
	for i, kv := range all {
		...
		if kv.k != string(it.Key) {
			t.Errorf("iterator value mismatch for element %d: got key %s want %s",
				i, it.Key, kv.k)
		}
		if kv.v != string(it.Value) {
			t.Errorf("iterator value mismatch for element %d: got value %s want %s",
				i, it.Value, kv.v)
		}
	}
	if it.Next() {
...
func testIteratorContinueAfterError(t *testing.T, memonly bool, scheme string) {
	...
	for i := 0; i < 20; i++ {
		...
		if memonly {
			...
			rval = rawdb.ReadTrieNode(diskdb, common.Hash{}, rpath, rhash,
				tdb.Scheme())
			rawdb.DeleteTrieNode(diskdb, common.Hash{}, rpath, rhash,
				tdb.Scheme())
		}
		...
		if memonly {
			...
			rawdb.WriteTrieNode(diskdb, common.Hash{}, rpath, rhash, rval,
				tdb.Scheme())
		}
		checkIteratorNoDups(t, it, seen)
		if it.Error() != nil {
		...
		if len(seen) != wantNodeCount {
			t.Fatal("wrong node iteration count, got", len(seen), "want",
				wantNodeCount)
		}
	}
}
...
func testIteratorContinueAfterSeekError(t *testing.T, memonly bool,
	...
	if memonly {
		...
		barNodeBlob = rawdb.ReadTrieNode(diskdb, common.Hash{}, barNodePath,
			barNodeHash, triedb.Scheme())
		rawdb.DeleteTrieNode(diskdb, common.Hash{}, barNodePath, barNodeHash,
			triedb.Scheme())
	}
	...
	if memonly {
		...
		rawdb.WriteTrieNode(diskdb, common.Hash{}, barNodePath, barNodeHash,
			barNodeBlob, triedb.Scheme())
	}
...
func checkIteratorNoDups(t *testing.T, it NodeIterator,

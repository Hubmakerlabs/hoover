...
func makeProvers(trie *Trie) []func(key []byte) *memorydb.Database {
	...
	provers = append(provers, func(key []byte) *memorydb.Database {
		proof := memorydb.New()
		if it := NewIterator(trie.MustNodeIterator(key)); it.Next() && bytes.Equal(key,
			it.Key) {
...
func TestProof(t *testing.T) {
	...
	for i, prover := range makeProvers(trie) {
		for _, kv := range vals {
			proof := prover(kv.k)
			if proof == nil {
				t.Fatalf("prover %d: missing key %x while constructing proof",
					i, kv.k)
			}
			...
			if !bytes.Equal(val, kv.v) {
				t.Fatalf("prover %d: verified value mismatch for key %x: have %x, want %x",
					i, kv.k, val, kv.v)
			}
		}
	}
...
func TestOneElementProof(t *testing.T) {
	...
	for i, prover := range makeProvers(trie) {
		...
		if !bytes.Equal(val, []byte("v")) {
			t.Fatalf("prover %d: verified value mismatch: have %x, want 'k'", i,
				val)
		}
	}
}
	...
	for i, prover := range makeProvers(trie) {
		for _, kv := range vals {
			...
			if _, err := VerifyProof(root, kv.k, proof); err == nil {
				t.Fatalf("prover %d: expected proof to fail for key %x", i,
					kv.k)
			}
		}
	}
...
func TestMissingKeyProof(t *testing.T) {
	...
	for i, key := range []string{"a", "j", "l", "z"} {
		...
		if val != nil {
			t.Fatalf("test %d: verified value mismatch: have %x, want nil", i,
				val)
		}
	}
}
...
func TestRangeProof(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		...
		if err != nil {
			t.Fatalf("Case %d(%d->%d) expect no error, got %v", i, start, end-1,
				err)
		}
	}
}
...
func TestOneElementRangeProof(t *testing.T) {
	...
	_, err := VerifyRangeProof(trie.Hash(), entries[start].k,
		[][]byte{entries[start].k}, [][]byte{entries[start].v}, proof)
	if err != nil {
	...
	_, err = VerifyRangeProof(trie.Hash(), first, [][]byte{entries[start].k},
		[][]byte{entries[start].v}, proof)
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
	...
	_, err = VerifyRangeProof(tinyTrie.Hash(), first, [][]byte{entry.k},
		[][]byte{entry.v}, proof)
	if err != nil {
		t.Fatalf("Expected no error, got %v", err)
	}
...
func TestSingleSideRangeProof(t *testing.T) {
	for i := 0; i < 64; i++ {
		...
		for _, pos := range cases {
			...
			_, err := VerifyRangeProof(trie.Hash(), common.Hash{}.Bytes(), k, v,
				proof)
			if err != nil {
				t.Fatalf("Expected no error, got %v", err)
			}
...
func TestBadRangeProof(t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		...
		_, err := VerifyRangeProof(trie.Hash(), first, keys, vals, proof)
		if err == nil {
			t.Fatalf("%d Case %d index %d range: (%d->%d) expect error, got nil",
				i, testcase, index, start, end-1)
		}
	}
}
...
func TestSameSideProofs(t *testing.T) {
	...
	_, err := VerifyRangeProof(trie.Hash(), first, [][]byte{entries[pos].k},
		[][]byte{entries[pos].v}, proof)
	if err == nil {
...
func TestHasRightElement(t *testing.T) {
	...
	var cases = []struct {
		...
		{len(entries) - 1, len(entries),
			false}, // Single last element with two existent proofs(point to same key)
		{0, len(entries),
			false}, // The whole set with existent left proof
		{-1, len(entries),
			false}, // The whole set with non-existent left proof
	}
	for _, c := range cases {
		var (
			firstKey []byte
		...
		if hasMore != c.hasMore {
			t.Fatalf("Wrong hasMore indicator, want %t, got %t", c.hasMore,
				hasMore)
		}
	}
}
...
func TestBloatedProof(t *testing.T) {
	...
	if _, err := VerifyRangeProof(trie.Hash(), keys[0], keys, vals,
		proof); err != nil {
...
func TestEmptyValueRangeProof(t *testing.T) {
	...
	entries = append(append(append([]*kv{}, entries[:mid]...), noop),
		entries[mid:]...)
...
func BenchmarkVerifyProof(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		im := i % len(keys)
		if _, err := VerifyProof(root, []byte(keys[im]),
			proofs[im]); err != nil {
...
func BenchmarkVerifyRangeProof10(b *testing.B) {
	benchmarkVerifyRangeProof(b,
		10)
}
func BenchmarkVerifyRangeProof100(b *testing.B) {
	benchmarkVerifyRangeProof(b,
		100)
}
func BenchmarkVerifyRangeProof1000(b *testing.B) {
	benchmarkVerifyRangeProof(b,
		1000)
}
func BenchmarkVerifyRangeProof5000(b *testing.B) {
	benchmarkVerifyRangeProof(b,
		5000)
}
...
func benchmarkVerifyRangeProof(b *testing.B, size int) {
	...
	for i := 0; i < b.N; i++ {
		...
		if err != nil {
			b.Fatalf("Case %d(%d->%d) expect no error, got %v", i, start, end-1,
				err)
		}
	}
}

func BenchmarkVerifyRangeNoProof10(b *testing.B) {
	benchmarkVerifyRangeNoProof(b,
		100)
}
func BenchmarkVerifyRangeNoProof500(b *testing.B) {
	benchmarkVerifyRangeNoProof(b,
		500)
}
func BenchmarkVerifyRangeNoProof1000(b *testing.B) {
	benchmarkVerifyRangeNoProof(b,
		1000)
}

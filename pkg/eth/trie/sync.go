...
var (
	...
	accountNodeSyncedGauge = metrics.NewRegisteredGauge("trie/sync/nodes/account",
		nil)

	// storageNodeSyncedGauge is the metric to track how many account trie
	// node are written during the sync.
	storageNodeSyncedGauge = metrics.NewRegisteredGauge("trie/sync/nodes/storage",
		nil)
...
type LeafCallback func(keys [][]byte, path []byte, leaf []byte,
	parent common.Hash, parentPath []byte) error
...
func NewSync(root common.Hash, database ethdb.KeyValueReader,
	callback LeafCallback, scheme string) *Sync {
	ts := &Sync{
		scheme:   scheme,
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash,
	parentPath []byte, callback LeafCallback) {
...
func (s *Sync) Commit(dbw ethdb.Batch) error {
	...
	for path, value := range s.membatch.nodes {
		...
		rawdb.WriteTrieNode(dbw, owner, inner, s.membatch.hashes[path], value,
			s.scheme)
	}
	...
	for path := range s.membatch.deletes {
		owner, inner := ResolvePath([]byte(path))
		rawdb.DeleteTrieNode(dbw, owner, inner, common.Hash{}, /* unused */
			s.scheme)
	}
	...
	case *shortNode:
		...
		if _, ok := node.Val.(hashNode); ok && s.scheme == rawdb.PathScheme {
			...
			for i := 1; i < len(key); i++ {
				...
				var exists bool
				if owner == (common.Hash{}) {
					exists = rawdb.ExistsAccountTrieNode(s.database,
						append(inner, key[:i]...))
				} else {
					exists = rawdb.ExistsStorageTrieNode(s.database, owner,
						append(inner, key[:i]...))
				}
				if exists {
					req.deletes = append(req.deletes, key[:i])
					deletionGauge.Inc(1)
					log.Debug("Detected dangling node", "owner", owner, "path",
						append(inner, key[:i]...))
				}
			}
	...
	for _, child := range children {
		...
		if req.callback != nil {
			if node, ok := (child.node).(valueNode); ok {
				...
				if len(child.path) == 2*common.HashLength {
					...
					paths = append(paths,
						hexToKeybytes(child.path[:2*common.HashLength]))
					paths = append(paths,
						hexToKeybytes(child.path[2*common.HashLength:]))
				}
				if err := req.callback(paths, child.path, node, req.hash,
					req.path); err != nil {
					return nil, err
				}
			}
		...
		if node, ok := (child.node).(hashNode); ok {
			...
			go func(child childNode) {
				...
				if rawdb.HasTrieNode(s.database, owner, inner, chash,
					s.scheme) {
					return
				}

...
func makeTestTrie(scheme string) (ethdb.Database, *Database, *StateTrie,
	map[string][]byte) {
	...
	if err := triedb.Update(root, types.EmptyRootHash, 0,
...
// content map.
func checkTrieContents(t *testing.T, db ethdb.Database, scheme string,
	root []byte, content map[string][]byte, rawTrie bool) {
	// Check root availability and trie contents
	ndb := newTestDatabase(db, scheme)
	if err := checkTrieConsistency(db, scheme, common.BytesToHash(root),
		rawTrie); err != nil {
	...
	for key, val := range content {
		if have := r.MustGet([]byte(key)); !bytes.Equal(have, val) {
			t.Errorf("entry %x: content mismatch: have %x, want %x", key, have,
				val)
		}
	}
}
...
func checkTrieConsistency(db ethdb.Database, scheme string, root common.Hash,
	rawTrie bool) error {
	ndb := newTestDatabase(db, scheme)
	var it NodeIterator
	if rawTrie {
...
func TestEmptySync(t *testing.T) {
	...
	for i, trie := range []*Trie{emptyA, emptyB, emptyC, emptyD} {
		...
		if paths, nodes, codes := sync.Missing(1); len(paths) != 0 || len(nodes) != 0 || len(codes) != 0 {
			t.Errorf("test %d: content requested for empty trie: %v, %v, %v", i,
				paths, nodes, codes)
		}
	}
}
...
func testIterativeSync(t *testing.T, count int, bypath bool, scheme string) {
	...
	for len(elements) > 0 {
		...
		if !bypath {
			for i, element := range elements {
				...
				if err != nil {
					t.Fatalf("failed to retrieve node data for hash %x: %v",
						element.hash, err)
				}
			...
			for i, element := range elements {
				...
				if err != nil {
					t.Fatalf("failed to retrieve node data for path %x: %v",
						element.path, err)
				}
				results[i] = NodeSyncResult{element.path, data}
			}
	...
	checkTrieContents(t, diskdb, srcDb.Scheme(), srcTrie.Hash().Bytes(),
		srcData, false)
}
...
func testIterativeDelayedSync(t *testing.T, scheme string) {
	...
	for len(elements) > 0 {
		...
		for i, element := range elements[:len(results)] {
			...
			data, err := reader.Node(owner, inner, element.hash)
			if err != nil {
				t.Fatalf("failed to retrieve node data for %x: %v",
					element.hash, err)
			}
			results[i] = NodeSyncResult{element.path, data}
		}
...
func testIncompleteSync(t *testing.T, scheme string) {
	...
	for len(elements) > 0 {
		...
		for _, result := range results {
			...
			if hash != root {
				...
				addedHashes = append(addedHashes,
					crypto.Keccak256Hash(result.Data))
			}
		}
	...
	for i, path := range addedKeys {
		...
		if err := checkTrieConsistency(diskdb, srcDb.Scheme(), root,
			false); err == nil {
...
func testSyncOrdering(t *testing.T, scheme string) {
	...
	for i := 0; i < len(reqs)-1; i++ {
		if len(reqs[i]) > 1 || len(reqs[i+1]) > 1 {
			...
			t.Errorf("Invalid request tuples: len(%v) or len(%v) > 1", reqs[i],
				reqs[i+1])
		}
		if bytes.Compare(compactToHex(reqs[i][0]),
			compactToHex(reqs[i+1][0])) > 0 {
			t.Errorf("Invalid request order: %v before %v",
				compactToHex(reqs[i][0]), compactToHex(reqs[i+1][0]))
		}
	}
}
...
func syncWith(t *testing.T, root common.Hash, db ethdb.Database,
	...
	for len(elements) > 0 {
		...
		for index, result := range results {
			if err := sched.ProcessNode(result); err != nil {
				t.Fatalf("failed to process result[%d][%v] data %v %v", index,
					[]byte(result.Path), result.Data, err)
			}
		}
		batch := db.NewBatch()
		if err := sched.Commit(batch); err != nil {
...
func testSyncMovingTarget(t *testing.T, scheme string) {
	...
	if err := srcDb.Update(root, preRoot, 0, trienode.NewWithNodeSet(nodes),
		nil); err != nil {
		panic(err)
	}
...
func testPivotMove(t *testing.T, scheme string, tiny bool) {
	var (
		...
		writeFn = func(key []byte, val []byte, tr *Trie,
	...
	if err := srcTrieDB.Update(rootA, types.EmptyRootHash, 0,
		trienode.NewWithNodeSet(nodesA), nil); err != nil {
		panic(err)
	}
	...
	if err := srcTrieDB.Update(rootB, rootA, 0, trienode.NewWithNodeSet(nodesB),
		nil); err != nil {
		panic(err)
	}
	...
	writeFn([]byte{0x01, 0x24}, stateA[string([]byte{0x01, 0x24})], srcTrie,
		stateC)
	writeFn([]byte{0x02, 0x34}, nil, srcTrie, stateC)
	writeFn([]byte{0x13, 0x44}, nil, srcTrie, stateC)

	rootC, nodesC, _ := srcTrie.Commit(false)
	if err := srcTrieDB.Update(rootC, rootB, 0, trienode.NewWithNodeSet(nodesC),
		nil); err != nil {
		panic(err)
	}

...
func newTrieReader(stateRoot, owner common.Hash, db *Database) (*trieReader,
	error) {
	...
	if err != nil {
		return nil, &MissingNodeError{Owner: owner, NodeHash: stateRoot,
			err: err}
	}
...
func (r *trieReader) node(path []byte, hash common.Hash) ([]byte, error) {
	...
	if r.banned != nil {
		if _, ok := r.banned[string(path)]; ok {
			return nil, &MissingNodeError{Owner: r.owner, NodeHash: hash,
				Path: path}
		}
	}
	if r.reader == nil {
...
func (l *trieLoader) OpenStorageTrie(stateRoot common.Hash,

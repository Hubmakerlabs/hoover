...
type layer interface {
	...
	update(root common.Hash, id uint64, block uint64,
		nodes map[common.Hash]map[string]*trienode.Node,
		states *triestate.Set) *diffLayer
...
func (c *Config) sanitize() *Config {
	...
	if conf.DirtyCacheSize > maxBufferSize {
		log.Warn("Sanitizing invalid node buffer size", "provided",
			common.StorageSize(conf.DirtyCacheSize), "updated",
...
func New(diskdb ethdb.Database, config *Config) *Database {
	...
	if ancient, err := diskdb.AncientDatadir(); err == nil && ancient != "" && !db.readOnly {
		...
		pruned, err := truncateFromHead(db.diskdb, freezer,
			db.tree.bottom().stateID())
		if err != nil {
	...
	if rawdb.ReadSnapSyncStatusFlag(diskdb) == rawdb.StateSyncRunning && !db.readOnly {
		if err := db.Disable(); err != nil {
			log.Crit("Failed to disable database", "err",
...
func (db *Database) Update(root common.Hash, parentRoot common.Hash,
	block uint64, nodes *trienode.MergedNodeSet, states *triestate.Set) error {
...
func (db *Database) Enable(root common.Hash) error {
	...
	if stored != root {
		return fmt.Errorf("state root mismatch: stored %x, synced %x", stored,
			root)
	}
	...
	db.tree.reset(newDiskLayer(root, 0, db, nil,
		newNodeBuffer(db.bufferSize, nil, 0)))
...
func (db *Database) Recover(root common.Hash,
	loader triestate.TrieLoader) error {
	db.lock.Lock()
	defer db.lock.Unlock()
	...
	log.Debug("Recovered state", "root", root, "elapsed",
...
func (db *Database) Recoverable(root common.Hash) bool {
	...
	parent := root
	return checkHistories(db.freezer, *id+1, dl.stateID()-*id,
		func(m *meta) error {
			if m.parent != parent {
...
func (db *Database) Size() (diffs common.StorageSize,
	nodes common.StorageSize) {
...
func (db *Database) SetBufferSize(size int) error {
	...
	if size > maxBufferSize {
		log.Info("Capped node buffer size", "provided",
			common.StorageSize(size), "adjusted",
			common.StorageSize(maxBufferSize))
		size = maxBufferSize
	}
	db.bufferSize = size

...
func updateTrie(addrHash common.Hash, root common.Hash,
	dirties, cleans map[common.Hash][]byte) (common.Hash, *trienode.NodeSet) {
...
func newTester(t *testing.T, historyLimit uint64) *tester {
	var (
		disk, _ = rawdb.NewDatabaseWithFreezer(rawdb.NewMemoryDatabase(),
			t.TempDir(), "", false)
		db = New(disk, &Config{
	...
	for i := 0; i < 2*128; i++ {
		...
		if err := db.Update(root, parent, uint64(i), nodes,
			states); err != nil {
...
func (t *tester) mutateStorage(ctx *genctx, addr common.Address,
	root common.Hash) common.Hash {
	var (
		addrHash = crypto.Keccak256Hash(addr.Bytes())
		storage  = make(map[common.Hash][]byte)
		origin   = make(map[common.Hash][]byte)
	...
	root, set := updateTrie(crypto.Keccak256Hash(addr.Bytes()), root, storage,
		t.storages[addrHash])
...
func (t *tester) clearStorage(ctx *genctx, addr common.Address,
	root common.Hash) common.Hash {
	var (
		addrHash = crypto.Keccak256Hash(addr.Bytes())
		storage  = make(map[common.Hash][]byte)
...
func (t *tester) generate(parent common.Hash) (common.Hash,
	*trienode.MergedNodeSet, *triestate.Set) {
	var (
		ctx     = newCtx()
...
func (t *tester) verifyState(root common.Hash) error {
	...
	for addrHash, account := range t.snapAccounts[root] {
		blob, err := reader.Node(common.Hash{}, addrHash.Bytes(),
			crypto.Keccak256Hash(account))
	...
	for addrHash, slots := range t.snapStorages[root] {
		for hash, slot := range slots {
			blob, err := reader.Node(addrHash, hash.Bytes(),
				crypto.Keccak256Hash(slot))
...
func (t *tester) verifyHistory() error {
	...
	for i, root := range t.roots {
		...
		if obj.meta.parent != parent {
			return fmt.Errorf("unexpected parent, want: %x, got: %x", parent,
				obj.meta.parent)
		}
		if obj.meta.root != root {
			return fmt.Errorf("unexpected root, want: %x, got: %x", root,
				obj.meta.root)
		}
	}
	return nil
...
func TestDatabaseRollback(t *testing.T) {
	...
	for i := tester.bottomIndex(); i >= 0; i-- {
		...
		loader := newHashLoader(tester.snapAccounts[root],
			tester.snapStorages[root])
...
func TestDatabaseRecoverable(t *testing.T) {
	...
	for i, c := range cases {
		...
		if result != c.expect {
			t.Fatalf("case: %d, unexpected result, want %t, got %t", i,
				c.expect, result)
		}
	}
}
...
func TestDisable(t *testing.T) {
	...
	if tester.db.tree.bottom().rootHash() != stored {
		t.Fatalf("Root hash is not matched exp %x got %x", stored,
			tester.db.tree.bottom().rootHash())
	}
}
...
func TestTailTruncateHistory(t *testing.T) {
	...
	if head != stored {
		t.Fatalf("Failed to truncate excess history object above, stored: %d, head: %d",
			stored, head)
	}
}

...
func newDiffLayer(parent layer, root common.Hash, id uint64, block uint64,
	...
	log.Debug("Created new diff layer", "id", id, "block", block, "nodes",
		count, "size", common.StorageSize(dl.memory))
	return dl
}
...
func (dl *diffLayer) node(owner common.Hash, path []byte, hash common.Hash,
	...
	if ok {
		...
		if ok {
			...
			if n.Hash != hash {
				...
				log.Error("Unexpected trie node in diff layer", "owner", owner,
					"path", path, "expect", hash, "got", n.Hash)
				return nil, newUnexpectedNodeError("diff", hash, n.Hash, owner,
					path, n.Blob)
			}
...
func (dl *diffLayer) Node(owner common.Hash, path []byte,
...
func (dl *diffLayer) update(root common.Hash, id uint64, block uint64,
	nodes map[common.Hash]map[string]*trienode.Node,
	states *triestate.Set) *diffLayer {

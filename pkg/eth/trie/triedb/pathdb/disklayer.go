...
func newDiskLayer(root common.Hash, id uint64, db *Database,
	cleans *fastcache.Cache, buffer *nodebuffer) *diskLayer {
...
func (dl *diskLayer) Node(owner common.Hash, path []byte,
	...
	if dl.cleans != nil {
		if blob := dl.cleans.Get(nil, key); len(blob) > 0 {
			...
			log.Error("Unexpected trie node in clean cache", "owner", owner,
				"path", path, "expect", hash, "got", got)
		}
	...
	if nHash != hash {
		...
		log.Error("Unexpected trie node in disk", "owner", owner, "path", path,
			"expect", hash, "got", nHash)
...
func (dl *diskLayer) update(root common.Hash, id uint64, block uint64,
...
func (dl *diskLayer) commit(bottom *diffLayer, force bool) (*diskLayer, error) {
	...
	ndl := newDiskLayer(bottom.root, bottom.stateID(), dl.db, dl.cleans,
	...
	if err := ndl.buffer.flush(ndl.db.diskdb, ndl.cleans, ndl.id,
		force); err != nil {
		return nil, err
	}
...
func (dl *diskLayer) revert(h *history,
	loader triestate.TrieLoader) (*diskLayer, error) {
	...
	// layer.
	nodes, err := triestate.Apply(h.meta.parent, h.meta.root, h.accounts,
		h.storages, loader)
	if err != nil {
		return nil, err
	}
	...
	return newDiskLayer(h.meta.parent, dl.id-1, dl.db, dl.cleans,
		dl.buffer), nil
}

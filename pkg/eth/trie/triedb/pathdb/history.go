...
func (m *meta) encode() []byte {
	...
	binary.BigEndian.PutUint64(buf[1+2*common.HashLength:historyMetaSize],
		m.block)
...
func (m *meta) decode(blob []byte) error {
	...
	case stateHistoryVersion:
		...
		if (len(blob)-historyMetaSize)%common.AddressLength != 0 {
			return fmt.Errorf("corrupted state history meta, len: %d",
				len(blob))
		}
		m.version = blob[0]
		...
		for pos := historyMetaSize; pos < len(blob); {
			m.incomplete = append(m.incomplete,
				common.BytesToAddress(blob[pos:pos+common.AddressLength]))
...
func newHistory(root common.Hash, parent common.Hash, block uint64,
	states *triestate.Set) *history {
	var (
...
func (r *decoder) verify() error {
	if len(r.accountIndexes)%accountIndexSize != 0 || len(r.accountIndexes) == 0 {
		return fmt.Errorf("invalid account index, len: %d",
			len(r.accountIndexes))
	}
	if len(r.storageIndexes)%slotIndexSize != 0 {
		return fmt.Errorf("invalid storage index, len: %d",
			len(r.storageIndexes))
	}
	return nil
}
...
func (r *decoder) readStorage(accIndex accountIndex) ([]common.Hash,
	map[common.Hash][]byte, error) {
	var (
		last    common.Hash
		list    []common.Hash
...
func readHistory(freezer *rawdb.ResettableFreezer, id uint64) (*history,
	error) {
	...
	if err := dec.decode(accountData, storageData, accountIndexes,
		storageIndexes); err != nil {
		return nil, err
	}
	return &dec, nil
}
...
func writeHistory(freezer *rawdb.ResettableFreezer, dl *diffLayer) error {
	...
	var (
		start   = time.Now()
		history = newHistory(dl.rootHash(), dl.parentLayer().rootHash(),
			dl.block, dl.states)
	)
	...
	rawdb.WriteStateHistory(freezer, dl.stateID(), history.meta.encode(),
		accountIndex, storageIndex, accountData, storageData)

	historyDataBytesMeter.Mark(int64(dataSize))
	historyIndexBytesMeter.Mark(int64(indexSize))
	historyBuildTimeMeter.UpdateSince(start)
	log.Debug("Stored state history", "id", dl.stateID(), "block", dl.block,
		"data", dataSize, "index", indexSize, "elapsed",
...
func checkHistories(freezer *rawdb.ResettableFreezer, start, count uint64,
	check func(*meta) error) error {
	for count > 0 {
		number := count
		if number > 10000 {
...
func truncateFromHead(db ethdb.Batcher, freezer *rawdb.ResettableFreezer,
	nhead uint64) (int, error) {
	...
	if ohead < nhead || nhead < otail {
		return 0, fmt.Errorf("out of range, tail: %d, head: %d, target: %d",
			otail, ohead, nhead)
	}
...
func truncateFromTail(db ethdb.Batcher, freezer *rawdb.ResettableFreezer,
	ntail uint64) (int, error) {
	ohead, err := freezer.Ancients()
	if err != nil {
		return 0, err
	}

...
func makeHistory() *history {
	return newHistory(testutil.RandomHash(), types.EmptyRootHash, 0,
		randomStateSet(3))
}
...
func checkHistory(t *testing.T, db ethdb.KeyValueReader,
	freezer *rawdb.ResettableFreezer, id uint64, root common.Hash, exist bool) {
...
func checkHistoriesInRange(t *testing.T, db ethdb.KeyValueReader,
	freezer *rawdb.ResettableFreezer, from, to uint64, roots []common.Hash,
	exist bool) {
...
func TestTruncateHeadHistory(t *testing.T) {
	...
	for i := 0; i < len(hs); i++ {
		...
		rawdb.WriteStateHistory(freezer, uint64(i+1), hs[i].meta.encode(),
	...
	for size := len(hs); size > 0; size-- {
		...
		checkHistoriesInRange(t, db, freezer, uint64(size), uint64(10),
			roots[size-1:], false)
		checkHistoriesInRange(t, db, freezer, uint64(1), uint64(size-1),
			roots[:size-1], true)
	}
}
...
func TestTruncateTailHistory(t *testing.T) {
	...
	for newTail := 1; newTail < len(hs); newTail++ {
		...
		checkHistoriesInRange(t, db, freezer, uint64(1), uint64(newTail),
			roots[:newTail], false)
		checkHistoriesInRange(t, db, freezer, uint64(newTail+1), uint64(10),
			roots[newTail:], true)
	}
}
...
func TestTruncateTailHistories(t *testing.T) {
	...
	for i, c := range cases {
		...
		if pruned != c.expPruned {
			t.Error("Unexpected pruned items", "want", c.expPruned, "got",
				pruned)
		}
		if c.empty {
			checkHistoriesInRange(t, db, freezer, uint64(1), uint64(10), roots,
				false)
		} else {
			tail := 10 - int(c.limit)
			checkHistoriesInRange(t, db, freezer, uint64(1), c.maxPruned,
				roots[:tail], false)
			checkHistoriesInRange(t, db, freezer, c.minUnpruned, uint64(10),
				roots[tail:], true)
		}
	}
}
...
func TestTruncateOutOfRange(t *testing.T) {
	...
	cases := []struct {
		...
		{0, head + 1,
			fmt.Errorf("out of range, tail: %d, head: %d, target: %d", tail,
				head, head+1)},
		{0, tail - 1,
			fmt.Errorf("out of range, tail: %d, head: %d, target: %d", tail,
				head, tail-1)},
...
func openFreezer(datadir string, readOnly bool) (*rawdb.ResettableFreezer,
	error) {

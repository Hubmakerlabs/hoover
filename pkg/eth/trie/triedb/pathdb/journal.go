...
func (db *Database) loadJournal(diskRoot common.Hash) (layer, error) {
	...
	if version != journalVersion {
		return nil, fmt.Errorf("%w want %d got %d", errUnexpectedVersion,
	...
	if !bytes.Equal(root.Bytes(), diskRoot.Bytes()) {
		return nil, fmt.Errorf("%w want %x got %x", errUnmatchedJournal, root,
			diskRoot)
	}
	...
	log.Debug("Loaded layer journal", "diskroot", diskRoot, "diffhead",
		head.rootHash())
	return head, nil
}
...
func (db *Database) loadDiskLayer(r *rlp.Stream) (layer, error) {
	...
	if stored > id {
		return nil, fmt.Errorf("invalid state id: stored %d resolved %d",
			stored, id)
	}
	...
	for _, entry := range encoded {
		...
		for _, n := range entry.Nodes {
			if len(n.Blob) > 0 {
				subset[string(n.Path)] = trienode.New(crypto.Keccak256Hash(n.Blob),
					n.Blob)
			} else {
	...
	base := newDiskLayer(root, id, db, nil,
		newNodeBuffer(db.bufferSize, nodes, id-stored))
	return base, nil
}
...
func (db *Database) loadDiffLayer(parent layer, r *rlp.Stream) (layer, error) {
	...
	return db.loadDiffLayer(newDiffLayer(parent, root, parent.stateID()+1,
		block, nodes, triestate.New(accounts, storages, incomplete)), r)
}
...
func (dl *diskLayer) journal(w io.Writer) error {
	...
	for owner, subset := range dl.buffer.nodes {
		...
		for path, node := range subset {
			entry.Nodes = append(entry.Nodes,
				journalNode{Path: []byte(path), Blob: node.Blob})
		}
	...
	log.Debug("Journaled pathdb disk layer", "root", dl.root, "nodes",
		len(dl.buffer.nodes))
	return nil
}
...
func (dl *diffLayer) journal(w io.Writer) error {
	...
	log.Debug("Journaled pathdb diff layer", "root", dl.root, "parent",
		dl.parent.rootHash(), "id", dl.stateID(), "block", dl.block, "nodes",
		len(dl.nodes))
	return nil
}
...
func (db *Database) Journal(root common.Hash) error {
	...
	if l, ok := l.(*diffLayer); ok {
		log.Info("Persisting dirty state to disk", "head", l.block, "root",
			root, "layers", l.id-disk.id+disk.buffer.layers)
	...
	db.readOnly = true
	log.Info("Persisted dirty state to disk", "size",
		common.StorageSize(journal.Len()), "elapsed",

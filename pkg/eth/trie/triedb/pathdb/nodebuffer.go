...
func (b *nodebuffer) node(owner common.Hash, path []byte,
	hash common.Hash) (*trienode.Node, error) {
	...
	if n.Hash != hash {
		...
		log.Error("Unexpected trie node in node buffer", "owner", owner, "path",
			path, "expect", hash, "got", n.Hash)
...
func (b *nodebuffer) revert(db ethdb.KeyValueReader,
	nodes map[common.Hash]map[string]*trienode.Node) error {
		...
		for path, n := range subset {
			...
			if !ok {
				...
				if owner == (common.Hash{}) {
					...
					_, nhash = rawdb.ReadStorageTrieNode(db, owner,
						[]byte(path))
				}
				...
				panic(fmt.Sprintf("non-existent node (%x %v) blob: %v", owner,
					path, crypto.Keccak256Hash(n.Blob).Hex()))
			}
			current[path] = n
...
func (b *nodebuffer) updateSize(delta int64) {
	...
	s := b.size
	b.size = 0
	log.Error("Invalid pathdb buffer size", "prev", common.StorageSize(s),
		"delta", common.StorageSize(delta))
}
...
func (b *nodebuffer) setSize(size int, db ethdb.KeyValueStore,
	clean *fastcache.Cache, id uint64) error {
	b.limit = uint64(size)
...
func (b *nodebuffer) flush(db ethdb.KeyValueStore, clean *fastcache.Cache,
	id uint64, force bool) error {
	...
	if head+b.layers != id {
		return fmt.Errorf("buffer layers (%d) cannot be applied on top of persisted state id (%d) to reach requested state id (%d)",
			b.layers, head, id)
	}
	var (
		start = time.Now()
	...
	log.Debug("Persisted pathdb nodes", "nodes", len(b.nodes), "bytes",
		common.StorageSize(size), "elapsed",
...
// into clean cache.
func writeNodes(batch ethdb.Batch,
		...
		for path, n := range subset {
			if n.IsDeleted() {
				...
				if owner == (common.Hash{}) {
					...
					rawdb.WriteStorageTrieNode(batch, owner, []byte(path),
						n.Blob)
				}
				if clean != nil {

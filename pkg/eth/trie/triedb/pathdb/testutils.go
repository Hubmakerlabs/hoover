...
func newTestHasher(owner common.Hash, root common.Hash,
	cleans map[common.Hash][]byte) (*testHasher, error) {
	if cleans == nil {
		cleans = make(map[common.Hash][]byte)
	}
	if got, _ := hash(cleans); got != root {
		return nil, fmt.Errorf("state root mismatched, want: %x, got: %x", root,
			got)
	}
	return &testHasher{
		owner:   owner,
...
func (h *testHasher) Commit(collectLeaf bool) (common.Hash, *trienode.NodeSet,
	...
	for hash, val := range h.dirties {
		...
		if len(val) == 0 {
			...
			set.AddNode(hash.Bytes(),
				trienode.New(crypto.Keccak256Hash(val), val))
		}
	}
...
func newHashLoader(accounts map[common.Hash][]byte,
	storages map[common.Hash]map[common.Hash][]byte) *hashLoader {
	return &hashLoader{
		accounts: accounts,
		storages: storages,
	}
...
func (l *hashLoader) OpenStorageTrie(stateRoot common.Hash,
	addrHash, root common.Hash) (triestate.Trie, error) {

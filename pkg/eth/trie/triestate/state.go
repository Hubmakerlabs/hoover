...
type TrieLoader interface {
	...
	OpenStorageTrie(stateRoot common.Hash, addrHash, root common.Hash) (Trie,
		error)
}
...
func New(accounts map[common.Address][]byte,
	storages map[common.Address]map[common.Hash][]byte,
	incomplete map[common.Address]struct{}) *Set {
	return &Set{
		Accounts:   accounts,
		Storages:   storages,
...
func Apply(prevRoot common.Hash, postRoot common.Hash,
	accounts map[common.Address][]byte,
	storages map[common.Address]map[common.Hash][]byte,
	loader TrieLoader) (map[common.Hash]map[string]*trienode.Node, error) {
	...
	if root != prevRoot {
		return nil, fmt.Errorf("failed to revert state, want %#x, got %#x",
			prevRoot, root)
	}
